...
public class Console
{
        ...
        try (TableNameCompleter tableNameCompleter = new TableNameCompleter(queryRunner);
            ...
            while (!exiting.get()) {
                ...
                for (Statement split : splitter.getCompleteStatements()) {
                    ...
                    process(queryRunner, split.statement(), outputFormat, tableNameCompleter::populateCache, true, System.out, System.out);
    ...
    private static boolean executeCommand(QueryRunner queryRunner, String query, OutputFormat outputFormat, boolean ignoreErrors, boolean showProgress)
    {
        ...
        for (Statement split : splitter.getCompleteStatements()) {
            if (!isEmptyStatement(split.statement())) {
                if (!process(queryRunner, split.statement(), outputFormat, () -> {}, showProgress, System.out, System.err)) {
                    if (!ignoreErrors) {
                        return false;
                    }
                    success = false;
    ...
    private static boolean process(QueryRunner queryRunner, String sql, OutputFormat outputFormat, Runnable schemaChanged, boolean interactive, PrintStream out, PrintStream errorChannel)
    {
        String finalSql;
        try {
        ...
        try (Query query = queryRunner.startQuery(finalSql)) {
            boolean success = query.renderOutput(out, errorChannel, outputFormat, interactive);
