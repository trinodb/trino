...
- A product test is a JUnit test that verifies Trino against a packaged server and external services.
- An environment is the Testcontainers-backed runtime needed by a test, such as Trino plus MySQL, Hive, or Kerberos.
- A lane is a logical test path with a specific environment and configuration. For example, the MySQL lane runs tests
  against a Trino cluster configured with the MySQL connector.
- Tags (`@TestGroup.*`) describe which product-test suites should run a test. Tags can be applied to classes or methods.
- A suite selects tagged tests, starts the required environment, and is what CI invokes for a product-test bucket.
...
- Add explicit `@TestGroup.*` tags used by suites. Tags can be applied to test classes or individual test methods.

Use `@TestGroup.ProfileSpecificTests` for tests that must only run in a suite that explicitly selects their
