...
class TestKafkaProtobufReadsSchemaRegistrySmokeTest
{
            ...
            throws Exception
    {
        ...
        try (Connection conn = env.createTrinoConnection();
                ...
                ResultSet rs = stmt.executeQuery(
                        "SELECT * FROM " + KAFKA_CATALOG + "." + KAFKA_SCHEMA + ".\"" + topicName + "\"")) {
            assertThat(rs.getMetaData().getColumnCount()).isEqualTo(8);
            assertThat(rs.next()).isTrue();
            assertThat(rs.getString(1)).isEqualTo("foobar");
            ...
            assertThat(rs.getFloat(5)).isEqualTo(3.14f);
            assertThat(rs.getBoolean(6)).isTrue();
            assertThat(rs.getString(7)).isEqualTo("ZERO");
            assertThat(rs.getTimestamp(8)).isEqualTo(java.sql.Timestamp.valueOf(timestamp));
            assertThat(rs.next()).isFalse();
        }
    }
