...
import io.trino.tests.product.deltalake.DeltaLakeDatabricks164Environment;
import io.trino.tests.product.suite.SuiteRunner.TestRunResult;

import java.util.ArrayList;
import java.util.List;

public final class SuiteDeltaLakeDatabricks164
{
    // Parity note: legacy launcher keeps a dedicated 164 lane filtered by DELTA_LAKE_DATABRICKS_164.
    // There are currently no tests tagged for that group, so this lane intentionally discovers 0 tests.
    private SuiteDeltaLakeDatabricks164() {}
    ...
    public static void main(String[] args)
    {
        ...
        results.add(SuiteRunner.forEnvironment(DeltaLakeDatabricks164Environment.class)
                .includeTag(TestGroup.ConfiguredFeatures.class)
                .includeTag(TestGroup.DeltaLakeDatabricks164.class)
                .run());
