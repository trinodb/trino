...
{
    private List<NotificationRecords.Event> notifications;
    ...
    private void assertEventCountEventually(EventType eventType, String objectName, int expectedCount)
    {
        ...
        while (System.nanoTime() < deadlineNanos) {
            long actual = notifications.stream()
                    .filter(event -> event.eventName().equals(eventType.toString()))
                    .filter(event -> normalizedObjectName(event.object().key()).equals(objectName))
                    .count();
