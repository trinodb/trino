...
{
    @Test
    void testRefreshTheFilesCacheWhenTableIsRecreated(DeltaLakeOssEnvironment env)
    {
        String tableName = "test_dl_cached_table_files_refresh_" + randomNameSuffix();
        String tableDirectory = "databricks-compatibility-test-" + tableName;
        // Disable delta.load_metadata_from_checksum_file. When it is enabled, the stale cache
        // issue from https://github.com/trinodb/trino/issues/13737 does not reproduce.
        ...
        assertThat(env.executeTrino(selectWithChecksumFilesDisabled)).containsOnly(row(1));
        ...
        env.deleteObjects(tableDirectory);
        ...
        assertThatThrownBy(() -> env.executeTrino(selectWithChecksumFilesDisabled))
        ...
        assertThat(env.executeTrino(selectWithChecksumFilesDisabled)).containsOnly(row(2));
