...
{
    @Test
    void testUnregisterTable(DeltaLakeOssEnvironment env)
    {
        ...
        try {
            ...
            assertThatThrownBy(() -> env.executeTrino("SELECT * FROM delta.default." + tableName))
                    .hasMessageMatching("(?s).*Table '.*' does not exist.*");
            assertThatThrownBy(() -> env.executeSpark("SELECT * FROM default." + tableName))
                    .satisfies(TestDeltaLakeProceduresCompatibility::assertSparkTableNotFound);
        }
        finally {
    ...
    private static void assertSparkTableNotFound(Throwable failure)
    {
        Assertions.assertThat(Throwables.getStackTraceAsString(failure))
                .matches("(?s).*(Table or view not found|The table or view .* cannot be found).*");
    }
}
