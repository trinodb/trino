...
class TestDeltaLakeUpdateCompatibility
{
    ...
    void testUpdatesFromDatabricks(DeltaLakeDatabricksEnvironment env)
    {
        ...
        assertThat(env.executeTrinoSql(
                """
                CREATE TABLE delta.default."%1$s" (id, value)
                WITH (location = 's3://%2$s/databricks-compatibility-test-%1$s') AS
                VALUES (1, 'Poland'), (2, 'Germany'), (3, 'Romania')
