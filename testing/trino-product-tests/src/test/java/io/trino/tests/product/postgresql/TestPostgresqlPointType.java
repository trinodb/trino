...
@ProductTest
@RequiresEnvironment(PostgresqlBasicEnvironment.class)
@TestGroup.Postgresql
@TestGroup.ProfileSpecificTests
class TestPostgresqlPointType
{
    private static final String TABLE_NAME = "public.areas";

    @BeforeEach
    @AfterEach
    void dropTestTable(PostgresqlBasicEnvironment env)
    {
        env.executePostgresqlUpdate("DROP TABLE IF EXISTS " + TABLE_NAME);
    }

    @Test
    void testReadPointFromPostgres(PostgresqlBasicEnvironment env)
    {
    ...
    @Test
    void testWritePointFromTrino(PostgresqlBasicEnvironment env)
    {
        env.executePostgresqlUpdate("CREATE TABLE " + TABLE_NAME + " (id SERIAL PRIMARY KEY, loc point)");
