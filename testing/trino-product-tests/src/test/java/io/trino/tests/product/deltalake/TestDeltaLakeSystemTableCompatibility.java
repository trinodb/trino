...
{
    @Test
    void testTablePropertiesCaseSensitivity(DeltaLakeOssEnvironment env)
    {
        ...
        try {
            ...
            QueryResult trinoResult = env.executeTrino("SELECT * FROM delta.default.\"" + tableName + "$properties\" WHERE key != 'location'");
    ...
    @Test
    void testTablePropertiesWithTableFeatures(DeltaLakeOssEnvironment env)
    {
        ...
        try {
            QueryResult deltaResult = env.executeSpark("SHOW TBLPROPERTIES default." + tableName);
            QueryResult trinoResult = env.executeTrino("SELECT * FROM delta.default.\"" + tableName + "$properties\" WHERE key LIKE 'delta%'");
            assertThat(deltaResult).contains(expectedRows);
            assertThat(trinoResult).contains(expectedRows);
            assertThat(trinoResult.rows()).containsExactlyInAnyOrderElementsOf(deltaResult.rows());
        }
    ...
    @Test
    void testTablePartitionsWithDeletionVectors(DeltaLakeOssEnvironment env)
    {
        ...
        List<Row> expectedBeforeDelete = List.of(
                row("{\"a\":1,\"c\":\"varchar_1\"}", 3L, 2931L, "{\"b\":{\"min\":11,\"max\":19,\"null_count\":0}}"),
                row("{\"a\":1,\"c\":\"varchar_2\"}", 1L, 977L, "{\"b\":{\"min\":13,\"max\":13,\"null_count\":0}}"));

        List<Row> expectedAfterFirstDelete = List.of(
                row("{\"a\":1,\"c\":\"varchar_1\"}", 2L, 1954L, "{\"b\":{\"min\":11,\"max\":17,\"null_count\":0}}"),
                row("{\"a\":1,\"c\":\"varchar_2\"}", 1L, 977L, "{\"b\":{\"min\":13,\"max\":13,\"null_count\":0}}"));
    ...
    @Test
    void testTablePartitionsWithNoColumnStats(DeltaLakeOssEnvironment env)
    {
        ...
        List<Row> expected = List.of(
                row("{\"a\":1,\"c\":\"varchar_1\"}", 3L, 2931L, "{\"b\":{\"min\":null,\"max\":null,\"null_count\":null}}"),
                row("{\"a\":1,\"c\":\"varchar_2\"}", 1L, 977L, "{\"b\":{\"min\":null,\"max\":null,\"null_count\":null}}"),
                row("{\"a\":1,\"c\":\"varchar_3\"}", 1L, 941L, "{\"b\":{\"min\":null,\"max\":null,\"null_count\":null}}"));
