        ...
        implements TableFormatsTestEnvironment
{
            ...
            throws Exception
    {
        ...
        trinoCluster = MultiNodeTrinoCluster.builder()
                ...
                .withCatalog("hive", hiveCatalog(metastoreUri)
                        ...
                        .put("azure.access-key", "${ENV:ABFS_ACCESS_KEY}")
                ...
                .withCoordinatorCustomizer(container -> {
                    container.withEnv("ABFS_ACCESS_KEY", abfsAccessKey);
                    container.withEnv("ABFS_ACCOUNT", abfsAccount);
                })
                .withWorkerCustomizer(container -> {
                    container.withEnv("ABFS_ACCESS_KEY", abfsAccessKey);
                    container.withEnv("ABFS_ACCOUNT", abfsAccount);
                })
                .build();
    ...
    public void executeHadoopFsCommand(String command)
    {
        try {
            ...
            throw new UncheckedIOException("Failed to execute Hadoop fs command: " + command, e);
        }
        catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Interrupted while executing Hadoop fs command: " + command, e);
        }
    }
    ...
    private void configureHadoop(HadoopContainer container, String abfsContainer, String account, String accessKey, String schema)
    {
        ...
        container.withEnv("ABFS_CONTAINER", abfsContainer);
