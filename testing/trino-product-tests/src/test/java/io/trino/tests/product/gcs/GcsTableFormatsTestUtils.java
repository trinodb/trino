...
public final class GcsTableFormatsTestUtils
{
    ...
    public static void testCreateAndSelectNationTable(GcsEnvironment env, String catalog)
    {
        String tableName = "nation_" + randomNameSuffix();
        String tableLocation = env.getWarehouseDirectory() + "/" + tableName;
        try {
    ...
    public static void testBasicWriteOperations(GcsEnvironment env, String catalog)
    {
    ...
    public static void testCreateAndInsertTable(GcsEnvironment env, String catalog)
    {
        String tableName = "table_write_operations_" + randomNameSuffix();
        env.executeTrinoUpdate(format("CREATE SCHEMA %s.test WITH (location = '%s')", catalog, env.getWarehouseDirectory()));
        try {
    ...
    public static void testPathContainsSpecialCharacter(GcsEnvironment env, String catalog, String partitioningPropertyName, boolean verifyWithSpark, String sparkCatalog)
    {
        String tableName = "test_path_special_character_" + randomNameSuffix();
        try {
            env.executeTrinoUpdate(format("CREATE SCHEMA %1$s.test WITH (location = '%2$s')", catalog, env.getWarehouseDirectory()));
            env.executeTrinoUpdate(format(
    ...
    public static void testLocationContainsDiscouragedCharacter(GcsEnvironment env, String catalog)
    {
        ...
        String locationWithDiscouragedChars = env.getWarehouseDirectory() + "/[*]#?";

        try {
            env.executeTrinoUpdate("CREATE SCHEMA " + schemaName + " WITH (location = '" + locationWithDiscouragedChars + "')");
            env.executeTrinoUpdate("CREATE TABLE " + tableName + " (id bigint, someValue varchar)");
            env.executeTrinoUpdate("INSERT INTO " + tableName + " VALUES (1, 'someValue')");
            ...
            env.executeTrinoUpdate("DROP SCHEMA IF EXISTS " + schemaName);
        }
    }

    public static void testSparkCompatibilityOnTrinoCreatedTable(GcsEnvironment env, String catalog, String sparkCatalog)
    {
        ...
        try {
            env.executeTrinoUpdate(format("CREATE SCHEMA %s.test_compat WITH (location = '%s')", catalog, env.getWarehouseDirectory()));
