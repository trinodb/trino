...
class TestMariaDb
{
            ...
            throws Exception
    {
        try (Connection conn = environment.createTrinoConnection();
            ...
            stmt.execute("DROP TABLE IF EXISTS mariadb.test.nation");
            int count = stmt.executeUpdate(
                    "CREATE TABLE mariadb.test.nation AS SELECT * FROM tpch.tiny.nation");
            assertThat(count).isEqualTo(TpchTableResults.NATION_ROW_COUNT);

            // Verify the data was created with full row validation
            try (ResultSet rs = stmt.executeQuery("SELECT * FROM mariadb.test.nation")) {
