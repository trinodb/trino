...
@ProductTest
@RequiresEnvironment(DeltaLakeFlociCachingEnvironment.class)
...
{
    @Test
    void testConfiguredConnectors(DeltaLakeFlociCachingEnvironment env)
    {
        assertDefaultConnectors(env, "delta_lake");
    }

    @Test
    void testReadFromCache(DeltaLakeFlociCachingEnvironment env)
    {
        testReadFromTable(env, "table1");
        testReadFromTable(env, "table2");
    }

    private void testReadFromTable(DeltaLakeFlociCachingEnvironment env, String tableNameSuffix)
    {
    ...
    private void createTestTable(
            DeltaLakeFlociCachingEnvironment env,
            String cachedSchema,
    ...
    private long getTableSize(DeltaLakeFlociCachingEnvironment env, String tableName)
    {
    ...
    private CacheStats getCacheStats(DeltaLakeFlociCachingEnvironment env)
    {
        var result = env.executeTrino(
                "SELECT " +
                        "  sum(\"cachereads.alltime.count\") as cacheReads, " +
                        "  sum(\"externalreads.alltime.count\") as externalReads " +
                        "FROM jmx.current.\"io.trino.blob.cache.alluxio:name=delta,type=alluxiocachestats\"");
