...
class TestHiveTableStatistics
{
    ...
    private static final String ALL_TYPES_DATA_ROW_1 =
            ...
            (TINYINT '121', SMALLINT '32761', INTEGER '2147483641', BIGINT '9223372036854775801',
             REAL '123.341', DOUBLE '234.561', CAST(344.671 AS DECIMAL(10,0)), CAST(345.671 AS DECIMAL(10,5)),
    ...
    private static final String ALL_TYPES_DATA_ROW_2 =
            ...
            (TINYINT '127', SMALLINT '32767', INTEGER '2147483647', BIGINT '9223372036854775807',
             REAL '123.345', DOUBLE '235.567', CAST(345.678 AS DECIMAL(10,0)), CAST(345.678 AS DECIMAL(10,5)),
    ...
    private static void createAllTypesTable(MultinodeHive4Environment env, String tableName)
    {
        env.executeTrinoUpdate(format(
                "CREATE TABLE hive.default.%s (%s) WITH (format = 'TEXTFILE')",
                tableName,
                ALL_TYPES_TABLE_DDL));
    }

    private static void insertAllTypesRowsWithoutStatistics(MultinodeHive4Environment env, String tableName, String... rows)
    {
        env.executeTrinoInSession(session -> {
            session.executeUpdate("SET SESSION hive.collect_column_statistics_on_write = false");
            try {
                session.executeUpdate(format("INSERT INTO hive.default.%s VALUES %s", tableName, String.join(", ", rows)));
            }
            finally {
                session.executeUpdate("RESET SESSION hive.collect_column_statistics_on_write");
            }
        });
    }
    ...
    private static void assertStatsContainsOnly(MultinodeHive4Environment env, String showStatsQuery, List<Object[]> expectedRows)
    {
        ...
        var actualRows = result.getRows().stream()
                .sorted(Comparator.comparing(row -> (String) row.getValue(0), Comparator.nullsLast(Comparator.naturalOrder())))
                .toList();
        expectedRows = expectedRows.stream()
                .sorted(Comparator.comparing(row -> (String) row[0], Comparator.nullsLast(Comparator.naturalOrder())))
                .toList();
    ...
    private static void assertStatsContainsOnly(MultinodeHive4Environment env, String showStatsQuery, Row... expectedRows)
    {
        assertStatsContainsOnly(
                env,
                showStatsQuery,
                Arrays.stream(expectedRows)
                        .map(row -> row.getValues().toArray())
                        .toList());
    }
    ...
    void testStatisticsForTablePartitionedByBigint(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            assertThat(env.executeTrino(showStatsWholeTable)).containsOnly(
                    ...
                    row("p_regionkey", null, 3.0, 0.0, null, "1", "3"),
                    row("p_comment", 1500.0, 5.0, 0.0, null, null, null),
                    row(null, null, null, null, 15.0, null, null));
            ...
            assertStatsContainsOnly(env, showStatsPartitionTwo, ImmutableList.of(
                    ...
                    new Object[] {"p_comment", 352.0, 5.0, 0.0, null, null, null},
            ...
            assertThat(env.executeTrino(showStatsWholeTable)).containsOnly(
                    ...
                    row("p_regionkey", null, 3.0, 0.0, null, "1", "3"),
                    row("p_comment", 1199.0, 5.0, 0.0, null, null, null),
                    row(null, null, null, null, 15.0, null, null));
        }
        finally {
            env.executeTrinoUpdate("DROP TABLE IF EXISTS hive.default." + tableName);
    ...
    void testStatisticsForAllDataTypes(MultinodeHive4Environment env)
    {
        ...
        try {
            createAllTypesTable(env, tableName);
            insertAllTypesRowsWithoutStatistics(env, tableName, ALL_TYPES_DATA_ROW_1, ALL_TYPES_DATA_ROW_2);
            ...
            // Hive 4 omits timestamp bounds, producing statistics that Trino cannot consume.
            env.executeHiveUpdate("ANALYZE TABLE default." + tableName + " COMPUTE STATISTICS FOR COLUMNS " +
                    "c_tinyint, c_smallint, c_int, c_bigint, c_float, c_double, c_decimal, c_decimal_w_params, " +
                    "c_date, c_string, c_varchar, c_char, c_boolean, c_binary");
            env.executeTrinoUpdate("CALL system.flush_metadata_cache()");
    ...
    void testAnalyzeForTablePartitionedByBigint(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            assertThat(env.executeTrino(showStatsPartitionTwo)).containsOnly(
                    ...
                    row("p_regionkey", null, 1.0, 0.0, null, "2", "2"),
                    row("p_comment", 352.0, 5.0, 0.0, null, null, null),
                    row(null, null, null, null, 5.0, null, null));
        }
        finally {
            env.executeTrinoUpdate("DROP TABLE IF EXISTS hive.default." + tableName);
    ...
    void testAnalyzeForAllDataTypes(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            // Hive 4 omits timestamp bounds when persisting the statistics written by Trino.
            assertThat(env.executeTrino("ANALYZE hive.default." + tableName + " WITH (columns = ARRAY[" +
                    "'c_tinyint', 'c_smallint', 'c_int', 'c_bigint', 'c_float', 'c_double', 'c_decimal', " +
                    "'c_decimal_w_params', 'c_date', 'c_string', 'c_varchar', 'c_char', 'c_boolean', 'c_binary'])"))
    ...
    void testAnalyzeForAllDataTypesNoData(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            assertThat(env.executeTrino("SHOW STATS FOR hive.default." + tableName)).containsOnly(getAllTypesEmptyTableStatistics());
        }
        finally {
            env.executeTrinoUpdate("DROP TABLE IF EXISTS hive.default." + tableName);
        }
    ...
    void testComputeTableStatisticsOnCreateTable(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            // Create empty table
            createAllTypesTable(env, emptyTableName);

            // Create table with only nulls
            createAllTypesTable(env, allNullTableName);
            insertAllTypesRowsWithoutStatistics(env, allNullTableName, ALL_TYPES_ALL_NULL_ROW);
    ...
    void testComputeFloatingPointStatistics(String dataType, MultinodeHive4Environment env)
    {
        ...
        try {
            env.executeTrinoUpdate(format(
                    "CREATE TABLE hive.default.%s(c_basic %s, c_minmax %s, c_inf %s, c_ninf %s, c_nan %s, c_nzero %s)",
                    tableName,
                    dataType,
                    dataType,
                    dataType,
            ...
            throws SQLException
    {
        ...
        try {
            ...
            try (Connection connection = env.createHiveConnection();
                ...
                try {
                    statement.execute(format("INSERT INTO TABLE default.%s PARTITION (p=1) VALUES (11),(12),(13),(14)", tableName));
                    statement.execute(format("INSERT INTO TABLE default.%s PARTITION (p=2) VALUES (21),(22),(23)", tableName));
                }
                finally {
    ...
    void testComputeTableStatisticsOnInsert(MultinodeHive4Environment env)
    {
        ...
        try {
            ...
            createAllTypesTable(env, allTypesTable);
            insertAllTypesRowsWithoutStatistics(env, allTypesTable, ALL_TYPES_DATA_ROW_1, ALL_TYPES_DATA_ROW_2);

            createAllTypesTable(env, emptyAllTypesTable);

            createAllTypesTable(env, allTypesAllNullTable);
            insertAllTypesRowsWithoutStatistics(env, allTypesAllNullTable, ALL_TYPES_ALL_NULL_ROW);
            ...
            try {
                ...
                assertThat(env.executeTrino("SHOW STATS FOR hive.default." + tableName)).containsOnly(
                        ...
                        row("c_timestamp", null, 2.0, 0.5, null, "2015-05-10 12:15:31.000", "2015-05-10 12:15:36.000"),
                ...
                assertStatsContainsOnly(
                        env,
                        "SHOW STATS FOR hive.default." + tableName,
                        ...
                        row("c_timestamp", null, 2.0, 0.4, null, "2015-05-10 12:15:30.000", "2015-05-10 12:15:36.000"),
