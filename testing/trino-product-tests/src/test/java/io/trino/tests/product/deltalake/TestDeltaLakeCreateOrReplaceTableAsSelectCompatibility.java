...
class TestDeltaLakeCreateOrReplaceTableAsSelectCompatibility
{
    ...
    @Test
    void testCreateOrReplaceTableOnTrinoWithSchemaChange(DeltaLakeOssEnvironment env)
    {
        String tableName = "test_replace_table_with_schema_change_" + randomNameSuffix();
    ...
    @Test
    void testCreateOrReplaceTableAndInsertOnTrinoWithSchemaChange(DeltaLakeOssEnvironment env)
    {
    ...
    @Test
    void testCreateOrReplaceTableWithSchemaChangeOnCheckpoint(DeltaLakeOssEnvironment env)
    {
        String tableName = "test_replace_table_with_schema_change_" + randomNameSuffix();
        env.executeTrinoUpdate("CREATE TABLE delta.default." + tableName + " (ts VARCHAR) " +
                "with (location = 's3://" + env.getBucketName() + "/databricks-compatibility-test-" + tableName + "', checkpoint_interval = 10)");
    ...
    @Test
    void testCreateOrReplaceTableWithOnlyFeaturesChangedOnTrino(DeltaLakeOssEnvironment env)
    {
    ...
    private static void assertTransactionLogVersion(DeltaLakeOssEnvironment env, String tableName, int versionNumber)
    {
    ...
    private static void assertLastEntryIsCheckpointed(DeltaLakeOssEnvironment env, String tableName)
    {
        List<String> jsonEntries = listJsonLogEntries(env, tableName);
        assertThat(jsonEntries).isNotEmpty();
        String lastJsonEntry = jsonEntries.stream().max(String::compareTo).orElseThrow();
    ...
    private static List<String> listJsonLogEntries(DeltaLakeOssEnvironment env, String tableName)
    {
        return listLogEntries(env, tableName, ".json");
    }

    private static List<String> listCheckpointEntries(DeltaLakeOssEnvironment env, String tableName)
    {
        return listLogEntries(env, tableName, ".checkpoint.parquet");
    }

    private static List<String> listLogEntries(DeltaLakeOssEnvironment env, String tableName, String suffix)
    {
        String prefix = "databricks-compatibility-test-" + tableName + "/_delta_log/";
        return env.listObjects(prefix).stream()
