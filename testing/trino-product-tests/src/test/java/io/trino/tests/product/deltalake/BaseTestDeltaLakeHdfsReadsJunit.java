...
abstract class BaseTestDeltaLakeHdfsReadsJunit
{
    ...
    @BeforeEach
    void setUp(DeltaLakeKerberosHdfsEnvironment env)
            throws IOException
    {
        env.deleteHdfsPath(TARGET_PATH);
        env.createHdfsDirectory(TARGET_PATH);
        ...
        for (ClassPath.ResourceInfo resourceInfo : resources) {
            try (InputStream resourceInputStream = resourceInfo.asByteSource().openBufferedStream()) {
                ...
                env.saveHdfsFile(fileName, resourceInputStream.readAllBytes());
            }
        }
    }
    ...
    void testReads(DeltaLakeKerberosHdfsEnvironment env)
    {
        try {
            env.executeTrinoUpdate("DROP TABLE IF EXISTS delta.default.region");
            env.executeTrinoUpdate("CALL delta.system.register_table('default', 'region', 'hdfs://hadoop-master:9000/tmp/region')");
            assertThat(env.executeTrino("SELECT count(*) FROM delta.default.region")).containsOnly(row(5L));
        }
        finally {
            env.executeTrinoUpdate("DROP TABLE IF EXISTS delta.default.region");
        }
