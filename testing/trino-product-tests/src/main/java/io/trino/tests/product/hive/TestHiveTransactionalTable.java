        ...
        extends HiveProductTest
{
    ...
    {
        return result.rows().stream().map(Row::new).collect(toImmutableList());
    }
    ...
    public void testMergeWithUnpredictablePredicates()
    {
        withTemporaryTable("test_without_aliases_target", true, false, NONE, targetTable -> {
            ...
            withTemporaryTable("test_without_aliases_source", true, false, NONE, sourceTable -> {
                ...
                query(format("INSERT INTO %s (customer, purchases, address) VALUES('Fred', 30, 'Franklin')", targetTable));
                verifySelectForTrinoAndHive("SELECT * FROM " + targetTable, "TRUE",
    ...
    public void testMergeWithSimplifiedUnpredictablePredicates()
    {
        withTemporaryTable("test_without_aliases_target", true, false, NONE, targetTable -> {
            ...
            log.info("Inserting into target");
            query(format("INSERT INTO %s (customer, purchases, address)" +
            ...
            withTemporaryTable("test_without_aliases_source", true, false, NONE, sourceTable -> {
                ...
                log.info("Inserting into source");
                query(format("INSERT INTO %s (customer, purchases, address) VALUES ('Dave', 11, 'Darbyshire')", sourceTable));
    ...
    public void testMergeSubqueries()
    {
        withTemporaryTable("merge_nation_target", true, false, NONE, targetTable -> {
            ...
            log.info("Inserting into target");
            query(format("INSERT INTO %s (nation_name, region_name) VALUES ('FRANCE', 'EUROPE'), ('ALGERIA', 'AFRICA'), ('GERMANY', 'EUROPE')", targetTable));

            withTemporaryTable("merge_nation_source", true, false, NONE, sourceTable -> {
                query(format("CREATE TABLE %s (nation_name VARCHAR, region_name VARCHAR) WITH (transactional = true)", sourceTable));

                query(format("INSERT INTO %s VALUES ('ALGERIA', 'AFRICA'), ('FRANCE', 'EUROPE'), ('EGYPT', 'MIDDLE EAST'), ('RUSSIA', 'EUROPE')", sourceTable));
    ...
    public void testMergeOriginalFilesTarget()
    {
        withTemporaryTable("region", true, false, NONE, targetTable -> {
            query(format("CREATE TABLE %s WITH (transactional=true) AS TABLE tpch.tiny.region", targetTable));

            // This merge is illegal, because many nations have the same region
            log.info("About to run merge that fails because one target rows matching multiple source rows");
            assertThat(() -> query(format("MERGE INTO %s r USING tpch.tiny.nation n", targetTable) +
                    "    ON r.regionkey = n.regionkey" +
                    "    WHEN MATCHED" +
                    "        THEN UPDATE SET comment = n.comment"))
                    .failsWithMessage("One MERGE target table row matched more than one source row");

            log.info("About to run a legal merge");
            query(format("MERGE INTO %s r USING tpch.tiny.nation n", targetTable) +
    ...
    private static void verifySelectForTrinoAndHive(String select, String whereClause, Row... rows)
    {
