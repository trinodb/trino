        ...
        extends ProductTest
{
    ...
    private void cleanSchema(String schema, long startTime, GlueClient glueClient)
    {
        Database database;
        try {
            database = glueClient.getDatabase(GetDatabaseRequest.builder().name(schema).build()).database();
        }
        ...
        for (String tableName : allTestTableNames) {
            try {
                Table table = glueClient.getTable(GetTableRequest.builder().databaseName(schema).name(tableName).build()).table();
