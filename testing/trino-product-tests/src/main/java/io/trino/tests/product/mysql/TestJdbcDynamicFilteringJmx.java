        ...
        extends ProductTest
{
    ...
    public void testDynamicFilteringStats()
    {
        ...
        assertThat(onTrino().executeQuery("SELECT \"completeddynamicfilters.totalcount\" FROM jmx.current.\"io.trino.plugin.jdbc:name=mysql,type=dynamicfilteringstats\" WHERE node = 'trino-coordinator'"))
                .containsOnly(row(1));
        assertThat(onTrino().executeQuery("SELECT \"totaldynamicfilters.totalcount\" FROM jmx.current.\"io.trino.plugin.jdbc:name=mysql,type=dynamicfilteringstats\" WHERE node = 'trino-coordinator'"))
                .containsOnly(row(1));
    }
}
