...
@TestsEnvironment
public final class EnvMultinodeWarpSpeedMinio
        extends EnvironmentProvider
{
    ...
    private final ResourceProvider envConfigDir;
    private final ResourceProvider envGeneralConfigDir;
                                  ...
                                  Minio minio)
    {
        ...
        envConfigDir = dockerFiles.getDockerFilesHostDirectory("conf/environment/multinode-warp-speed-minio");
        envGeneralConfigDir = dockerFiles.getDockerFilesHostDirectory("warp/trino/");
    }
    ...
    private void configureTrinoContainer(DockerContainer container)
    {
        container.withPrivilegedMode(true)
                .withCopyFileToContainer(forHostPath(envConfigDir.getPath("trino/etc")), CONTAINER_TRINO_ETC)
    ...
    private void configureWarpStorage(DockerContainer container)
    {
        container.withCopyFileToContainer(
                forHostPath(envGeneralConfigDir.getPath("setup-warp-speed.sh"), 0755),
    ...
    private DockerContainer createSparkContainer()
    {
        DockerContainer container = new DockerContainer("ghcr.io/trinodb/testing/spark3-hudi:" + hadoopImagesVersion, SPARK_CONTAINER_NAME)
                .withCopyFileToContainer(forHostPath(envConfigDir.getPath("spark-defaults.conf")), "/spark/conf/spark-defaults.conf")
