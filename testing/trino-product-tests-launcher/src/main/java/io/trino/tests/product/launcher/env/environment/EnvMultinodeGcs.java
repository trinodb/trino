        ...
        extends EnvironmentProvider
{
    ...
    {
        String base64EncodedCredentials = requireEnv("GCP_CREDENTIALS_KEY");
        ...
        try {
            ...
            Files.write(gcpCredentialsFile.toPath(), Base64.getDecoder().decode(base64EncodedCredentials));
        }
        catch (IOException e) {
    ...
    private Path getCoreSiteOverrideXml(String containerGcpCredentialsFilePath)
    {
        try {
            String coreSite = Files.readString(dockerFiles.getDockerFilesHostDirectory("conf/environment/multinode-gcs").getPath("core-site-overrides-template.xml"))
                    .replace("%GCP_PROJECT_NAME%", requireEnv("GCP_PROJECT_NAME"))
