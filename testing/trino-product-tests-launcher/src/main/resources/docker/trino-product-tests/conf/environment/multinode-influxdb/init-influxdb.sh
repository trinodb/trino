...
if ( [ ! -z "$INIT_USERS" ] || [ ! -z "$INFLUXDB_DB" ] || [ "$(ls -A /docker-entrypoint-initdb.d 2> /dev/null)" ] ) && [ ! "$(ls -d "$META_DIR" 2>/dev/null)" ]; then
  ...
  #custom initialized for trino product tests
  if [ ! -z "$INFLUXDB_DB" ] && [ ! -z "$INFLUXDB_INIT_DATAFILE" ]; then
    INFLUX_CMD="influx -database $INFLUXDB_DB -host 127.0.0.1 -port $INFLUXDB_INIT_PORT -execute "
    echo "init data test.."
    while read -r line
    do
      $INFLUX_CMD "INSERT $line" &> /dev/null;
    done < "$INFLUXDB_INIT_DATAFILE"
  fi
