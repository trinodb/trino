...
final class ServerArchive
{
            ...
            throws IOException
    {
        ...
        // a streaming assembly splits its output into fixed size gzip members, all of which have to be decoded
        try (InputStream input = new BufferedInputStream(newInputStream(archive));
                InputStream gzipStream = GzipCompressorInputStream.builder().setInputStream(input).setDecompressConcatenated(true).get();
                TarArchiveInputStream archiveInput = new TarArchiveInputStream(gzipStream, true)) {
