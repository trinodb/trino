...
public class TestTpchDistributedStats
{
    ...
    {
        // Estimates are significantly off as they are generated by DEFAULT_FILTER_FACTOR
    ...
    public void testTablesample()
    {
        statisticsAssertion.check("SELECT * FROM orders TABLESAMPLE BERNOULLI (42)",
                // BERNOULLI sample gets converted to a `rand() < 0.42` filter and gets estimated as (0.9 * row count) due to DEFAULT_FILTER_FACTOR
                checks -> checks.estimate(OUTPUT_ROW_COUNT, relativeError(1.0, 1.2)));
