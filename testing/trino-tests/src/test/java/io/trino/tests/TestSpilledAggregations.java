...
import static io.trino.SystemSessionProperties.AGGREGATION_OPERATOR_UNSPILL_MEMORY_LIMIT;
import static io.trino.SystemSessionProperties.SPILL_DISTINCTING_AGGREGATIONS_ENABLED;
import static io.trino.SystemSessionProperties.SPILL_ORDERING_AGGREGATIONS_ENABLED;
        ...
        extends AbstractTestAggregations
{
    ...
    public void testDoesNotSpillOrderByWhenDisabled()
    {
        Session session = Session.builder(getSession())
                .setSystemProperty(SPILL_ORDERING_AGGREGATIONS_ENABLED, "false")
    ...
    {
        Session session = Session.builder(getSession())
                .setSystemProperty(SPILL_DISTINCTING_AGGREGATIONS_ENABLED, "false")
                // set this low so that if we ran with spill the query would fail
                .setSystemProperty(AGGREGATION_OPERATOR_UNSPILL_MEMORY_LIMIT, "1B")
                .build();
        // the sum() is necessary so that the aggregation isn't optimized into multiple aggregation nodes
        assertQuery(session,
                "SELECT custkey, sum(custkey), count(DISTINCT orderpriority) FROM orders GROUP BY custkey ORDER BY 1");
    }
