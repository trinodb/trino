...
public class TestClusterMemoryLeakDetector
{
    ...
    public void testLeakDetector()
    {
        ...
        leakDetector.checkForMemoryLeaks(_ -> Optional.empty(), ImmutableMap.of());
        ...
        leakDetector.checkForMemoryLeaks(queryId -> Optional.of(createQueryExecution(queryId, testQuery, RUNNING)), ImmutableMap.of(testQuery, 1L));
        assertThat(leakDetector.getNumberOfLeakedQueries()).isEqualTo(0);

        // the leak detector should report exactly one leaked query since the query is finished, and its end time is way in the past
        leakDetector.checkForMemoryLeaks(queryId -> Optional.of(createQueryExecution(queryId, testQuery, FINISHED)), ImmutableMap.of(testQuery, 1L));
        ...
        leakDetector.checkForMemoryLeaks(queryId -> Optional.of(createQueryExecution(queryId, testQuery, FINISHED)), ImmutableMap.of(testQuery, 0L));
        assertThat(leakDetector.getNumberOfLeakedQueries()).isEqualTo(0);

        // the leak detector should report exactly one leaked query since the coordinator doesn't know of any query
        leakDetector.checkForMemoryLeaks(_ -> Optional.empty(), ImmutableMap.of(testQuery, 1L));
