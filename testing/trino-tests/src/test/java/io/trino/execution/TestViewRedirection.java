...
final class TestViewRedirection
        extends AbstractTestQueryFramework
{
    ...
    private static final List<String> REDIRECTION_CHAIN = IntStream.range(0, MAX_VIEW_REDIRECTIONS + 2).boxed()
    ...
    {
        // Redirect to a view that doesn't exist in the target. The redirection chain resolves to a concrete
        // target, so resolution must fail deterministically at the redirection boundary rather than surfacing
        // a downstream error, mirroring getRedirectionAwareTableHandle.
        assertThat(query(format("SELECT * FROM %s.%s.%s", CATALOG_NAME, SCHEMA_ONE, BAD_REDIRECTION_SRC)))
                .failure()
                .hasMessageContaining(format(
                        "but the target view '%s.%s.%s' does not exist",
                        CATALOG_NAME,
                        SCHEMA_TWO,
                        NON_EXISTENT_VIEW));
    }
    ...
    @Test
    public void testViewRedirectionAtMaxHops()
    {
        assertThat(query(format("SELECT * FROM %s.%s.%s", CATALOG_NAME, SCHEMA_THREE, REDIRECTION_CHAIN.get(1))))
                .matches("VALUES BIGINT '1'");
    }
