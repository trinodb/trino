        ...
        implements ITestListener
{
    ...
    public ReportResourceHungryTests()
    {
        ...
        isInstance("io.trino.transaction.TransactionManager", true).ifPresent(rules::add);
                        ...
                        if (rule.isExpensiveResource(field, value, stage)) {
                            reportListenerFailure(
                                    ...
                                    """
                                            ...
                                            \tof %s [%s]
                                            \tis set to [%s]
                                            \tin %s stage, matching [%s] rule.
                                            \tResources must not be allocated in field initializer or test constructor,
    ...
    {
        return isInstance(className, false);
    }

    private static Optional<Rule> isInstance(String className, boolean exceptStaticFields)
    {
        return tryLoadClass(className)
                .map(clazz -> isInstance(clazz, exceptStaticFields));
    }

    private static Rule isInstance(Class<?> clazz)
    {
        return isInstance(clazz, false);
    }

    private static Rule isInstance(Class<?> clazz, boolean exceptStaticFields)
    {
        return named(
                "%s::isInstance%s".formatted(clazz.getName(), exceptStaticFields ? " unless static" : ""),
                (field, value, stage) -> {
                    if (exceptStaticFields && isStatic(field.getModifiers())) {
                        return false;
                    }
