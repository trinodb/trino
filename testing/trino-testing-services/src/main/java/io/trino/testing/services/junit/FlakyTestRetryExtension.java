        ...
        for (int attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
            try {
                ...
                if (!matchPattern.matcher(stackTrace).find()) {
                    log.warn(throwable,
                            "Test %s failed but stacktrace does not match pattern '%s', not retrying",
                            formatTestName(extensionContext),
                            flaky.match());
                    throw throwable;
                }
                ...
                if (attempt <= MAX_RETRIES) {
                    log.warn(throwable,
                            "Test %s attempt %d failed (issue: %s), retrying...",
                            formatTestName(extensionContext),
                            attempt,
                            flaky.issue());
                }
                    ...
                    log.error(
                            throwable,
                            "Test %s failed after %d attempts (issue: %s)",
                            formatTestName(extensionContext),
                            attempt,
                            flaky.issue());
                }
