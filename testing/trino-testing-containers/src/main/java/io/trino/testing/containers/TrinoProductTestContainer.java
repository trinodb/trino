 ...
 * mysql.start();
 *
 * TrinoContainer trino = TrinoProductTestContainer.builder()
 *         .withNetwork(network)
 ...
 * </pre>
 * <p>
 * Product-test environments intentionally use explicit lifecycle management instead
 * of JUnit's {@code @Container} lifecycle. A suite starts one environment, runs all
 * selected tests against it, and then shuts it down before moving to the next
 * environment. This keeps ownership with the product-test environment manager
 * instead of individual JUnit test classes.
 */
public final class TrinoProductTestContainer
{
    ...
    /**
     * Starts the container and waits until it can run queries.
     * <p>
     * Use this for product-test environments that use the standard
     * {@link TrinoProductTestContainer} readiness check. Environments with custom
     * authentication, TLS, access control, or multi-node readiness requirements
     * should start the container directly and use their own readiness check.
     */
    public static void startAndWait(TrinoContainer container)
    {
        container.start();
        try {
            waitForClusterReady(container);
        }
        catch (SQLException e) {
