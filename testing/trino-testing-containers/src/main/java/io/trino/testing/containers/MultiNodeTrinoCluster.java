 ...
 * The cluster ensures:
 * <ul>
 *   <li>Coordinator and workers start together so workers can satisfy coordinator readiness checks</li>
...
{
    private static final String BASE_IMAGE = "trinodb/trino";
    ...
    private MultiNodeTrinoCluster(TrinoContainer coordinator, List<TrinoWorkerContainer> workers)
    {
        ...
        this.expectedNodeCount = workers.size() + 1; // workers + coordinator
    }

    /**
     * Starts the coordinator and workers together.
     */
    public void start()
    {
        Startables.deepStart(Stream.concat(Stream.of(coordinator), workers.stream())).join();
    }
            ...
            throws SQLException, InterruptedException
    {
        ...
        throw new RuntimeException("Trino cluster not ready: found %d nodes, expected %d".formatted(activeNodeCount, expectedNodeCount));
    }
    ...
    public static class Builder
    {
        ...
        {
            this.imageName = BASE_IMAGE + ":" + requireNonNull(version, "version is null");
            return this;
        }
