...
import io.trino.hadoop.$internal.com.microsoft.azure.storage.RetryExponentialRetry;
import io.trino.hadoop.$internal.com.microsoft.azure.storage.RetryNoRetry;
import io.trino.hadoop.$internal.com.microsoft.azure.storage.SendingRequestEvent;
import io.trino.hadoop.$internal.com.microsoft.azure.storage.StorageCredentials;
import io.trino.hadoop.$internal.com.microsoft.azure.storage.StorageCredentialsAccountAndKey;
import io.trino.hadoop.$internal.com.microsoft.azure.storage.StorageCredentialsSharedAccessSignature;
...
import io.trino.hadoop.$internal.com.microsoft.azure.storage.blob.BlobType;
...
@VisibleForTesting
public class DecryptingAzureNativeFileSystemStore
        implements NativeFileSystemStore
{
            ...
            implements JSON.Convertor
    {
        ...
        public void toJSON(Object obj, JSON.Output out)
        {
            ...
            // Don't store group as null, just store it as empty string (which is FileStatus behavior).
        ...
        private static PermissionStatus fromJSONMap(@SuppressWarnings("rawtypes") Map object)
        {
            return new PermissionStatus((String) object.get(OWNER_TAG),
                ...
                // The initial - below is the Unix file type, which FsPermission needs there but ignores.
                FsPermission.valueOf("-" + object.get(PERMISSIONS_TAG)));
        }
    }
            ...
            throws IllegalArgumentException, IOException
    {
        ...
        boolean useLocalSasKeyMode = conf.getBoolean(KEY_USE_LOCAL_SAS_KEY_MODE, DEFAULT_USE_LOCAL_SAS_KEY_MODE);
     ...
     * @param uri
     *          -- WASB blob URI
     * @return accountName -- the account name for the URI.
     * @throws URISyntaxException
     *           if the URI does not have an authority it is badly formed.
     */
    private String getAccountFromAuthority(URI uri)
            throws URISyntaxException
    {
        ...
        // Check if authority container the delimiter separating the account name from the the container.
     ...
     * @param uri
     *          -- WASB blob URI
     * @return containerName -- the container name for the URI. May be null.
     * @throws URISyntaxException
     *           if the uri does not have an authority it is badly formed.
     */
    private String getContainerFromAuthority(URI uri)
            throws URISyntaxException
    {
        // Check to make sure that the authority is valid for the URI.
        String authority = uri.getRawAuthority();
        ...
        // The URI has a valid authority. Extract the container name. It is the second component of the WASB URI authority.
        if (!authority.contains(WASB_AUTHORITY_DELIMITER)) {
            // The authority does not have a container name. Use the default container by setting the container name to the default Azure root container.
    ...
    private void configureAzureStorageSession()
    {
        ...
        if (storageInteractionLayer == null) {
            throw new AssertionError(String.format("Cannot configure storage session for URI '%s' if storage session has not been established.", sessionUri));
        }
    ...
    private void connectUsingCredentials(String accountName, StorageCredentials credentials, String containerName)
     ...
     */
    private void connectToAzureStorageInSecureMode(String accountName, String containerName)
              throws StorageException, URISyntaxException
    {
        ...
        if (!(this.storageInteractionLayer instanceof SecureStorageInterfaceImpl)) {
            throw new AssertionError("connectToAzureStorageInSASKeyMode() should be called only for SecureStorageInterfaceImpl instances");
        }
     ...
     */
    private void connectUsingConnectionStringCredentials(final String accountName, final String containerName, final String accountKey)
            throws StorageException, URISyntaxException
    {
        // If the account name is "acc.blob.core.windows.net", then the rawAccountName is just "acc"
     ...
     */
    private void connectUsingSASCredentials(final String accountName, final String containerName, final String sas)
            throws StorageException, URISyntaxException
    {
            ...
            throws KeyProviderException
    {
        ...
        if (keyProviderClass == null) {
            ...
            // create an instance of the key provider class and verify it implements KeyProvider
     ...
     *
     * @throws AzureException on any exception that isn't IOException
     * @throws IOException otherwise
     */
    private void createAzureStorageSession()
            throws AzureException, IOException
    {
        // Make sure this object was properly initialized with references to the sessionUri and sessionConfiguration.
        ...
        // File system object initialized, attempt to establish a session with the Azure storage service for the target URI string.
        try {
            ...
            if (null == accountName) {
                // Account name is not specified as part of the URI. Throw indicating an invalid account name.
                throw new AzureException(String.format("Cannot load WASB file system account name not specified in URI: %s.", sessionUri.toString()));
            }
            ...
            if (useSecureMode) {
                connectToAzureStorageInSecureMode(accountName, containerName);
                return;
            }
            ...
            if (StringUtils.isNotEmpty(propertyValue)) {
                // Account key was found. Create the Azure storage session using the account key and container.
            ...
            // Caught exception while attempting to initialize the Azure File System store, re-throw the exception.
     ...
     */
    private String verifyAndConvertToStandardFormat(String rawDir)
            throws URISyntaxException
    {
    ...
    public boolean isKeyForDirectorySet(String key, Set<String> dirSet)
    {
        ...
        for (String dir : dirSet) {
            ...
            // Allow for blob directories with paths relative to the default file system.
            try {
                URI uriPageBlobDir = new URI(dir);
                if (null == uriPageBlobDir.getAuthority()) {
                    // Concatenate the default file system prefix with the relative page blob directory path.
            ...
            throws StorageException, AzureException
    {
        synchronized (containerStateLock) {
            ...
            try {
                ...
                if (StorageErrorCode.RESOURCE_NOT_FOUND.toString().equals(ex.getErrorCode())) {
            ...
            if (currentKnownContainerState == ContainerState.DoesntExist) {
                // If the container doesn't exist and we intend to write to it create it now.
    ...
    @Override
    public DataOutputStream storefile(String keyEncoded, PermissionStatus permissionStatus, String key)
            throws AzureException
    {
        try {
            ...
            if (isAnonymousAccess()) {
                // Preemptively raise an exception indicating no uploads are allowed to anonymous accounts.
            ...
            // Check that no attempt is made to write to blobs on default $root containers.
            if (AZURE_ROOT_CONTAINER.equals(getContainerFromAuthority(sessionUri))) {
                ...
                final String errMsg = String.format("Writes to '%s' container for URI '%s' are prohibited, only updates on non-root containers permitted.",
                        AZURE_ROOT_CONTAINER, sessionUri.toString());
                throw new AzureException(errMsg);
            }

            // Get the blob reference from the store's container and return it.
            ...
            if (isBlockBlobWithCompactionKey(key)) {
                outputStream = new BlockBlobAppendStream((CloudBlockBlobWrapper) blob, keyEncoded, this.uploadBlockSizeBytes, true, getInstrumentedContext());
            }
            else {
            ...
            throws StorageException
    {
        if (blob instanceof CloudPageBlobWrapper) {
            ...
            // Handle both ClouldBlockBlobWrapperImpl and (only for the test code path) MockCloudBlockBlobWrapper.
    ...
    private static void storeLinkAttribute(CloudBlobWrapper blob, String linkTarget)
    {
        ...
        if (linkTarget != null) {
            encodedLinkTarget = URLEncoder.encode(linkTarget, StandardCharsets.UTF_8);
        }
    ...
    private static String getLinkAttributeValue(CloudBlobWrapper blob)
    {
        ...
        if (encodedLinkTarget != null) {
            return URLDecoder.decode(encodedLinkTarget, StandardCharsets.UTF_8);
        }
        return null;
    }
    ...
    private static String retrieveVersionAttribute(CloudBlobContainerWrapper container)
    {
        ...
        if (metadata == null) {
            ...
            return metadata.getOrDefault(OLD_VERSION_METADATA_KEY, null);
        }
    }
            ...
            throws AzureException
    {
        ...
        try {
            ...
            // Caught exception while attempting upload. Re-throw as an Azure storage exception.
            throw new AzureException(e);
        }
        catch (IOException e) {
     ...
     */
    @Override
    public void storeEmptyLinkFile(String key, String tempBlobKey, PermissionStatus permissionStatus)
            throws AzureException
    {
        if (null == storageInteractionLayer) {
            final String errMsg = String.format("Storage session expected for URI '%s' but does not exist.", sessionUri);
     ...
     */
    @Override
    public String getLinkInFileMetadata(String key)
            throws AzureException
    {
        if (null == storageInteractionLayer) {
            final String errMsg = String.format("Storage session expected for URI '%s' but does not exist.", sessionUri);
     ...
     *
     */
    private Iterable<ListBlobItem> listRootBlobs(boolean includeMetadata, boolean useFlatBlobListing)
            throws StorageException, URISyntaxException
    {
    ...
    private OperationContext getInstrumentedContext(boolean bindConcurrentOOBIo)
    {
        ...
        // Set User-Agent
        operationContext.getSendingRequestEventHandler().addListener(new StorageEvent<>() {
            @Override
            ...
            throws IOException
    {
        ...
        try {
            if (checkContainer(ContainerAccessType.PureRead) == ContainerState.DoesntExist) {
                // The container doesn't exist, so spare some service calls and just return null now.
                return null;
            }

            // Handle the degenerate cases where the key does not exist or the key is a container.
            if (key.equals("/")) {
            ...
            // Query the underlying folder/container to list the blobs stored there under that key.
            ...
            for (ListBlobItem blobItem : objects) {
                if (blobItem instanceof CloudBlockBlobWrapper || blobItem instanceof CloudPageBlobWrapper) {
                    ...
                    // The key specifies a directory. Create a FileMetadata object which specifies as such.
                    BlobProperties properties = blob.getProperties();
    ...
    @Override
    public FileMetadata[] list(String prefix, final int maxListingCount, final int maxListingDepth)
            throws IOException
    {
        return listInternal(prefix, maxListingCount, maxListingDepth);
    }

    private FileMetadata[] listInternal(String prefix, final int maxListingCount, final int maxListingDepth)
            throws IOException
    {
        try {
            checkContainer(ContainerAccessType.PureRead);
            ...
            // Enable flat listing option only if depth is unbounded and config KEY_ENABLE_FLAT_LISTING is enabled.
            boolean enableFlatListing = (maxListingDepth < 0 && sessionConfiguration.getBoolean(KEY_ENABLE_FLAT_LISTING, DEFAULT_ENABLE_FLAT_LISTING));
            ...
            for (ListBlobItem blobItem : objects) {
                ...
                if (blobItem instanceof CloudBlockBlobWrapper || blobItem instanceof CloudPageBlobWrapper) {
                    ...
                    if (!enableFlatListing) {
                        // Currently at a depth of one, decrement the listing depth for sub-directories.
     ...
     */
    private void buildUpList(CloudBlobDirectoryWrapper aCloudBlobDirectory, HashMap<String, FileMetadata> metadataHashMap, final int maxListingCount, final int maxListingDepth)
            throws Exception
    {
        // Push the blob directory onto the stack.
        AzureLinkedStack<Iterator<ListBlobItem>> dirIteratorStack = new AzureLinkedStack<>();
        ...
        if (0 == maxListingDepth || 0 == maxListingCount) {
            // Recurrence depth and listing count are already exhausted. Return immediately.
            return;
        }

        // The directory listing depth is unbounded if the maximum listing depth is negative.
        ...
        while (null != blobItemIterator && (maxListingCount <= 0 || metadataHashMap.size() < maxListingCount)) {
            while (blobItemIterator.hasNext()) {
                ...
                // Add the file metadata to the list if this is not a blob directory item.
                if (blobItem instanceof CloudBlockBlobWrapper || blobItem instanceof CloudPageBlobWrapper) {
                    CloudBlobWrapper blob = (CloudBlobWrapper) blobItem;
                    BlobProperties properties = blob.getProperties();
            ...
            if (dirIteratorStack.isEmpty()) {
                ...
                // Pop the next directory item from the stack and decrement the depth.
     ...
     */
    @Override
    public boolean delete(String key, SelfRenewingLease lease)
            throws IOException
    {
        try {
    ...
    @Override
    public void rename(String srcKey, String dstKey, boolean acquireLease, SelfRenewingLease existingLease)
            throws IOException
    {
    ...
    @Override
    public void rename(String srcKey, String dstKey, boolean acquireLease, SelfRenewingLease existingLease, boolean overwriteDestination)
            throws IOException
    {
     ...
     */
    @Override
    public boolean explicitFileExists(String key)
            throws AzureException
    {
        CloudBlobWrapper blob;
        try {
            blob = getBlobReference(key);
            return null != blob && blob.exists(getInstrumentedContext());
        }
        catch (StorageException | URISyntaxException e) {
            throw new AzureException(e);
        }
     ...
     */
    @Override
    public SelfRenewingLease acquireLease(String key)
            throws AzureException
    {
        ...
        try {
            ...
            // Caught exception while attempting to get lease. Re-throw as an Azure storage exception.
            throw new AzureException(e);
        }
    }

    @Override
    public void updateFolderLastModifiedTime(String key, Date lastModified, SelfRenewingLease folderLease)
            throws AzureException
    {
        try {
            checkContainer(ContainerAccessType.ReadThenWrite);
            CloudBlobWrapper blob = getBlobReference(key);
            // setLastModified function is not available in 2.0.0 version. blob.uploadProperties automatically updates last modified timestamp to current time
            blob.uploadProperties(getInstrumentedContext(), folderLease);
        }
        catch (Exception e) {
            // Caught exception while attempting to update the properties. Re-throw as an Azure storage exception.
            throw new AzureException(e);
        }
    }

    @Override
    public void updateFolderLastModifiedTime(String key, SelfRenewingLease folderLease)
            throws AzureException
    {
    ...
    @Override
    public DataOutputStream retrieveAppendStream(String key, int bufferSize)
            throws IOException
    {
        try {
            ...
            OutputStream outputStream = new BlockBlobAppendStream(
