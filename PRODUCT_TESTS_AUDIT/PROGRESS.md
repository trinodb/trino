...
> **Historical snapshot:** This tracker records the state of the March 2026 audit. It is not the current completion
> status of the rebased stack. A later commit-by-commit review reopened substantial fidelity questions involving
> TPCH/TPC-DS SQL coverage, suite-selection semantics, Hive and Iceberg compatibility variants, environment topology,
> and `TestSqlCancel`. Existing `complete` rows below describe the earlier pass and must not be treated as proof that
...
- Known unresolved fidelity gaps in the later review: see the historical-snapshot notice above.
...
| `SuiteHiveKerberos` | `hive` | `complete` | The legacy Kerberos ticket refresh product test was already replaced by direct HDFS/Kerberos tests before this migration stack, so no migrated product-test class is expected here. |
