        ...
        implements SystemAccessControl
{
    ...
    public void checkCanRevokeTablePrivilege(Identity identity, Privilege privilege, CatalogSchemaTableName table,
            PrestoPrincipal revokee, boolean grantOptionFor)
    {
    ...
    public void checkCanSelectFromColumns(Identity identity, CatalogSchemaTableName table, Set<String> columns)
    {
        for (RangerPrestoResource rangerPrestoResource : createResource(table, columns)) {
            if (!authorizer.canSelectResource(rangerPrestoResource, identity)) {
    ...
    {
        for (RangerPrestoResource rangerPrestoResource : createResource(table, columns)) {
            if (!authorizer.canCreateResource(rangerPrestoResource, identity)) {
                denySelectColumns(table.getSchemaTableName().getTableName(), columns);
            }
        }
    }
    ...
    public void checkCanGrantTablePrivilege(Identity identity, Privilege privilege,
            ...
            PrestoPrincipal grantee, boolean withGrantOption)
    {
    ...
    {
        if (!authorizer.canSeeResource(createResource(catalogSchemaName.getCatalogName(),
                catalogSchemaName.getSchemaName()), identity)) {
