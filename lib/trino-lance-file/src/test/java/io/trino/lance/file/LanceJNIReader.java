        ...
        implements Closeable
{
            ...
            throws IOException
    {
        ...
        this.arrowReader = lanceReader.readAll(columnNames, null, READ_BATCH_SIZE);
    ...
    public void convert(VectorSchemaRoot vectorSchemaRoot)
    {
        pageBuilder.declarePositions(vectorSchemaRoot.getRowCount());
        List<FieldVector> fieldVectors = vectorSchemaRoot.getFieldVectors();

        for (int column = 0; column < columnTypes.size(); column++) {
            convertType(pageBuilder.getBlockBuilder(column), columnTypes.get(column), fieldVectors.get(column), 0, fieldVectors.get(column).getValueCount());
        }
