...
/// `coercionPlan` must honour a rule's `NumericRelation` guards for ground
...
class CoercionGuardTest
{
    ...
    @Test
    void testIntegralToDecimalAtPrecisionCap()
    {
        for (String integral : List.of("tinyint", "smallint", "integer", "bigint")) {
            for (int scale : new int[] {0, 20, 35, 36, 38}) {
                assertThat(typeSystem.coercionPlan(symbol(integral), apply("decimal", literal(38), literal(scale))))
                        .as("%s -> decimal(38,%s)", integral, scale)
                        .isPresent();
            }
            for (int scale : new int[] {35, 36, 37}) {
                assertThat(typeSystem.coercionPlan(symbol(integral), apply("decimal", literal(37), literal(scale))))
                        .as("%s -> decimal(37,%s)", integral, scale)
                        .isEmpty();
            }
        }
    }
