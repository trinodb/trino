...
public class ExoticSignatureTest
{
    ...
    void testUnregisteredParametricTypeResolvesByExactMatch()
    {
        ...
        CompiledSignature bigintClassify = SignatureCompiler.compile(Signature.builder()
                .argumentTypes(mapType(type("bigint"), type("double")), type("Classifier", type("bigint")))
                .returnType(type("bigint"))
                .build());
        CompiledSignature varcharClassify = SignatureCompiler.compile(Signature.builder()
    ...
    @Test
    void testResolverRetainsAuthoritativeSignature()
    {
        ...
        FunctionResolver resolver = new FunctionResolver(LIBRARY.typeSystem());
        assertThat(resolver.resolveSignatures(List.of(signature), List.of(symbol("bigint"))))
                .isInstanceOfSatisfying(FunctionResolver.Resolved.class, resolved -> {
                    assertThat(resolved.resolution().signature()).contains(signature);
