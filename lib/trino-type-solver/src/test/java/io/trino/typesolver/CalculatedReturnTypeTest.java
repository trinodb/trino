...
import static io.trino.typesolver.Expression.BinaryOperator.GREATER_THAN;
...
public class CalculatedReturnTypeTest
{
    ...
    {
        CompiledSignature addDecimal = new CompiledSignature(
                List.of(
                        variable("p1"),
                        variable("s1"),
                        variable("p2"),
        ...
        CompiledSignature.MatchOutcome outcome = addDecimal.matchFunctionCallOutcome(
                List.of(
                        apply("decimal", literal(10), literal(2)),
                        apply("decimal", literal(5), literal(3))),
                TrinoPreset.typeSystem());

        assertThat(outcome).isInstanceOfSatisfying(CompiledSignature.Satisfied.class, satisfied -> {
    ...
    {
        CompiledSignature multiplyDecimal = new CompiledSignature(
                List.of(
                        variable("p1"),
                        variable("s1"),
                        variable("p2"),
        ...
        CompiledSignature.MatchOutcome outcome = multiplyDecimal.matchFunctionCallOutcome(
                List.of(
                        apply("decimal", literal(10), literal(2)),
                        apply("decimal", literal(5), literal(3))),
                TrinoPreset.typeSystem());
    ...
    {
        CompiledSignature concatVarchar = new CompiledSignature(
        ...
        CompiledSignature.MatchOutcome outcome = concatVarchar.matchFunctionCallOutcome(
                List.of(
    ...
    /// [Integer#MAX_VALUE] rather than wrapping to a negative length, mirroring Trino's
    /// 64-bit `TypeCalculation`. Trino's growth formulas clamp with `min(2147483647, ...)`,
    /// so the saturated fold reaches the same (unbounded) length.
    @Test
    void testCalculatedLengthSaturatesOnOverflow()
    {
        CompiledSignature grow = new CompiledSignature(
                List.of(variable("n1"), variable("n2")),
                List.of(),
                function(
        ...
        CompiledSignature.MatchOutcome outcome = grow.matchFunctionCallOutcome(
    ...
    /// Exercises [Expression.Conditional] through return-type materialization on both branches.
    @Test
    void testConditionalReturnTypeEvaluatesChosenBranch()
    {
        ...
        assertThat(multiplyDecimal.matchFunctionCallOutcome(
                ...
                TrinoPreset.typeSystem()))
                .isInstanceOfSatisfying(CompiledSignature.Satisfied.class, satisfied ->
