...
/// Prunes dominated alternatives inside a [Choice].
///
/// Two forms of dominance:
///
/// - **Guard subsumption** — if two alternatives share the same witness, the one
///   with a larger guard set is weaker and drops out (whoever has fewer guards is
///   satisfiable at least as often).
/// - **Witness subtype comparison** — if two alternatives have the same guard set and
///   one witness is a subtype of the other, the more specific one wins (the more
///   general witness is dominated).
///
...
final class ChoiceSimplifier
{
    ...
    {
        alternatives = Alternative.normalize(alternatives);
