...
///
/// Handles [NumericRelation] constraints: evaluates relations against current
...
final class NumericPropagator
{
    ...
    static void process(BinaryOperation operation, Solver.SolverState state)
    {
        ...
        state.pendingConstraints().park(new NumericRelation(operation));
    }

    /// Try to evaluate a relation against current bounds.
    ///
    /// @return [Optional#empty()] if undetermined, otherwise the truth value
    ...
    private static void applyNumericState(String variable, OptionalInt min, OptionalInt max, Solver.SolverState state)
    {
        ...
        state.putVariableState(variable, new NumericVariableState(min, max));
    }
    ...
    private static Optional<Range> evaluateRange(Expression expression, Solver.SolverState state)
    {
        return switch (expression) {
            ...
            case BinaryOperation(BinaryOperator operator, Expression left, Expression right) -> {
                ...
                yield switch (operator) {
                    case ADD -> Optional.of(new Range(Expression.saturateToInt((long) leftValue.min() + rightValue.min()), Expression.saturateToInt((long) leftValue.max() + rightValue.max())));
                    case SUBTRACT -> Optional.of(new Range(Expression.saturateToInt((long) leftValue.min() - rightValue.max()), Expression.saturateToInt((long) leftValue.max() - rightValue.min())));
    ...
    private static void updateNumericBounds(String variable, BinaryOperator operator, int literal, boolean variableOnLeft, Solver.SolverState state)
    {
        ...
        switch (operator) {
            ...
            case NOT_EQUAL, ADD, SUBTRACT, MULTIPLY, DIVIDE, MIN, MAX -> {
                state.pendingConstraints().park(new NumericRelation(new BinaryOperation(
                        operator,
