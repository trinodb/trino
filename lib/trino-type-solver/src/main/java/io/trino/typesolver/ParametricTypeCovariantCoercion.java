...
///
/// Expresses: `C(T1, ..., Tn) ≤ C(U1, ..., Un) iff Ti ≤ Ui for all i`. Reduces a
...
///
/// Used by the Trino preset for `array`, `map`, and `row`. All three are
/// treated identically — arity matches, then each argument pair is queued as a new subtype
/// question.
///
/// Covariance is an oversimplification for `map` keys (they are really invariant in
