...
///
/// Implementations define a partial order over [FunctionResolver.Resolution]s. The
/// [FunctionResolver] consults this partial order to pick a unique "most specific"
...
///
/// - Reversibility: `compare(a, b).reverse() == compare(b, a)` for all `a, b`.
/// - Reflexivity: `compare(a, a) == EQUIVALENT`.
///
/// Implementations are NOT required to be transitive — [Order#INCOMPARABLE] is a
/// first-class outcome, and transitivity would promote it to equivalence, which is wrong.
///
/// **Scope.** `Specificity` only orders *already-applicable* resolutions. Picking
/// which candidates apply is the [Solver]'s job; picking among them is this interface's.
///
/// **Composition.** Use [#then] to fall through to a secondary comparator whenever the
/// primary returns [Order#INCOMPARABLE] or [Order#EQUIVALENT]. This is how dialect
...
public interface Specificity
{
    ...
    /// Fall through to `next` when `this` can't distinguish two candidates.
    ...
    /// [Order#INCOMPARABLE]; dialects with richer rules should layer their own
    /// comparator via [#then] to break such ties.
