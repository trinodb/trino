...
/// Asserts that `subtype` can be coerced to `supertype`.
///
/// This is the solver's primary way of expressing "a value of one type flowing into a
/// position expecting another." Examples:
///
/// - `Subtype(integer, bigint)` — concrete widening; checked via the coercion rule list.
/// - `Subtype(T, bigint)` — upper bound on `T`; recorded and later materialized.
/// - `Subtype(integer, T)` — lower bound on `T`.
/// - `Subtype(array(T), array(integer))` — structural decomposition into element subtyping.
///
/// When neither side has a variable, the solver looks for a matching coercion rule; if
/// multiple rules apply, the result is deferred as a [Choice].
