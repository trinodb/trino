...
/// 2. **Reconcile** — once the worklist is empty, iteratively refine restricted
///    [Domain]s and revisit pending constraints. Each pass may reintroduce
...
public class Solver
{
    ...
    private void bindTypeVariable(String variable, Expression expression, SolverState state)
    {
        ...
        if (variableState.binding().isPresent()) {
            ...
            Expression bound = Expression.substitute(variableState.binding().orElseThrow(), currentSubstitutions(state.variableStates()));
