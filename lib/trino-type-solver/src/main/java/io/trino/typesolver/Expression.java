...
///
/// The same interface covers both type-level expressions (`bigint`,
/// `array(integer)`, `row(varchar, T)`, etc.) and the numeric-parameter
/// sublanguage used inside parametric types and guard relations (`p - s`,
/// `38`, `n1 + n2`). This is deliberate — a decimal's precision and
...
///
/// - **Type-level**: [Symbol], [Application], [Row],
///   [AnyRow], [FunctionType]
/// - **Value-level (integer)**: [Literal], [BinaryOperation]
/// - **Cross-cutting**: [Variable] (can stand for either kind)
///
/// [RowField] is a helper for [Row] and isn't an `Expression` itself.
///
/// This interface is constructor-agnostic: nothing in here knows about `integer`,
/// `decimal`, or any SQL-specific type name. Those only appear at registration
/// time as `Symbol` values.
public sealed interface Expression
{
    ...
    /// The condition must be a [BinaryOperation] over a comparison operator. A conditional
    ...
    static Expression substitute(Expression expression, Map<String, Expression> substitutions)
    {
        ...
        return substitute(expression, substitutions, null, 0);
    }
    ...
    private static Expression substitute(Expression expression, Map<String, Expression> substitutions, Set<String> activeVariables, int depth)
    {
        ...
        return switch (expression) {
            case Application application -> {
                Expression head = substitute(application.head(), substitutions, activeVariables, depth + 1);
                List<Expression> arguments = substituteAll(application.arguments(), substitutions, activeVariables, depth + 1);
            ...
            case Row row -> {
                ...
                for (int i = 0; i < row.fields().size(); i++) {
                    ...
                    Expression type = substitute(field.type(), substitutions, activeVariables, depth + 1);
            ...
            case Variable variable -> {
                ...
                Expression substituted = substitute(replacement, substitutions, active, depth + 1);
                active.remove(variable.name());
                yield substituted;
            }
            case BinaryOperation binary -> {
                Expression left = substitute(binary.left(), substitutions, activeVariables, depth + 1);
                Expression right = substitute(binary.right(), substitutions, activeVariables, depth + 1);
                yield left == binary.left() && right == binary.right() ? binary : operation(binary.operator(), left, right);
            }
            case Conditional conditional -> {
                Expression condition = substitute(conditional.condition(), substitutions, activeVariables, depth + 1);
                Expression ifTrue = substitute(conditional.ifTrue(), substitutions, activeVariables, depth + 1);
                Expression ifFalse = substitute(conditional.ifFalse(), substitutions, activeVariables, depth + 1);
            ...
            case FunctionType functionType -> {
                List<Expression> parameters = substituteAll(functionType.parameterTypes(), substitutions, activeVariables, depth + 1);
                Optional<Expression> variadic = functionType.variadicParameterType().map(parameter -> substitute(parameter, substitutions, activeVariables, depth + 1));
                Expression returnType = substitute(functionType.returnType(), substitutions, activeVariables, depth + 1);
    ...
    private static List<Expression> substituteAll(List<Expression> expressions, Map<String, Expression> substitutions, Set<String> activeVariables, int depth)
    {
        List<Expression> result = null;
        for (int i = 0; i < expressions.size(); i++) {
            Expression original = expressions.get(i);
            Expression substituted = substitute(original, substitutions, activeVariables, depth);
    ...
    static boolean isGround(Expression expression)
    {
        return switch (expression) {
            ...
            case Application(Expression head, List<Expression> arguments) -> isGround(head) && areGround(arguments);
            case Row(List<RowField> fields) -> {
                for (RowField field : fields) {
                    if (!isGround(field.type())) {
                        yield false;
                    }
                }
                yield true;
            ...
            case FunctionType functionType -> areGround(functionType.parameterTypes())
                    && (functionType.variadicParameterType().isEmpty() || isGround(functionType.variadicParameterType().orElseThrow()))
                    && isGround(functionType.returnType());
        };
    }

    private static boolean areGround(List<Expression> expressions)
    {
        for (Expression expression : expressions) {
            if (!isGround(expression)) {
                return false;
            }
        }
        return true;
