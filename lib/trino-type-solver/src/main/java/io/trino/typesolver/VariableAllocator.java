...
/// Monotonic source of internal fresh variable names (`$v1`, `$v2`, ...).
///
...
/// solve consumes constraints whose variables were minted by a *different* allocator
/// (which also started at `$v1`), the consumer must [reserve][#reserveThrough(int)]
...
public class VariableAllocator
{
    ...
    /// than `id`. Seeds this allocator past variables produced elsewhere so the names
    ...
    /// Parse the numeric id of an allocator-minted name, e.g. `"$v7"` → `7`.
    /// Returns `0` for any name not of that form.
    ///
    /// @param name the variable name
    /// @return the numeric id, or `0` when `name` was not minted by this allocator
