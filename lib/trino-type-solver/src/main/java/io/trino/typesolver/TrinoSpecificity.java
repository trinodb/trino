...
import static io.trino.typesolver.Specificity.Order.EQUIVALENT;
import static io.trino.typesolver.Specificity.Order.INCOMPARABLE;
import static io.trino.typesolver.Specificity.Order.LESS_SPECIFIC;
import static io.trino.typesolver.Specificity.Order.MORE_SPECIFIC;

/// Trino-faithful overload specificity.
///
/// Ports the algorithm from Trino's `FunctionSpecificityComparator`
/// ([trino#28791](https://github.com/trinodb/trino/pull/28791)). Two-phase:
///
/// 1. **Bidirectional bound-signature forwardability.** Try to bind each candidate's bound
...
///    fall through to phase 2. If neither forwards, the candidates are [INCOMPARABLE][Order#INCOMPARABLE].
/// 2. **Structural declared-signature comparison.** Walk the two declared [CompiledSignature]s
///    positionally. Concrete types beat type variables. Two concretes compare via coercion
///    direction ([TypeSystem#coercionPlan]) — the side that coerces TO the other is
///    narrower. Parametric types recurse. Equality patterns (`(T, T)` vs
///    `(T, U)`) are compared by subset. Fixed arity beats variadic when all else ties.
///
/// Intended composition: `Specificity.BY_COERCION_COUNT.then(new TrinoSpecificity(typeSystem))`.
        ...
        implements Specificity
{
    ...
    {
        boolean leftForwardsToRight = forwards(left, right.compiledSignature());
        boolean rightForwardsToLeft = forwards(right, left.compiledSignature());
        ...
        return compareDeclared(left.compiledSignature(), right.compiledSignature(), arity);
    }

    /// Phase 1 primitive: can `from`'s bound-signature argument types be bound by
    /// `targetScheme`'s declared form (allowing coercion)?
    ///
    /// A bound formal can be [Expression.AnyRow] if the declared position was a row family
    ...
    private boolean forwards(FunctionResolver.Resolution from, CompiledSignature targetScheme)
    {
        ...
        return targetScheme.matchFunctionCallOutcome(boundFormals, typeSystem) instanceof CompiledSignature.Satisfied;
    }

    /// Phase 2: structural specificity on the declared schemes.
    private Order compareDeclared(CompiledSignature leftScheme, CompiledSignature rightScheme, int arity)
    {
    ...
    /// If neither coerces (or both do equivalently) → [Order#INCOMPARABLE].
    ...
    /// Anything else returns [Order#INCOMPARABLE].
    ...
    /// variables (calculated-literal params like `decimal(p, s)`), or types (concrete or
    ...
    ///
    /// - If the left set is a strict superset of the right's, left imposes strictly more
    ///   equalities and is narrower ([Order#MORE_SPECIFIC]).
    /// - Strict subset → [Order#LESS_SPECIFIC].
    /// - Equal sets → [Order#EQUIVALENT].
    /// - Otherwise → [Order#INCOMPARABLE].
    ///
    /// Example: `(T, T)` has one equality pair {(arg0, arg1)}; `(T, U)` has none.
    /// So `(T, T)` is narrower.
    ...
    private static Set<String> quantifiedVariables(CompiledSignature scheme)
    {
