...
public interface CoercionRule
{
    ...
    /// @return [Optional#empty()] if this rule does not apply, otherwise a [Match]
    ///         whose `constraints` must be satisfied for the coercion to hold and whose
    ///         optional `plan` gives a planner-facing description of the runtime conversion
    Optional<Match> matches(VariableAllocator allocator, Expression from, Expression to);

    /// Stable identifier used when classifying an assembled [CoercionPlan] (e.g. to mark
    ...
    /// Whether this rule equates two representations of one engine type. A conditional
    /// representation bridge can bind an open target parameter during function matching, but it
    /// does not establish a general subtype relation for every value of that parameter.
    default boolean isRepresentationBridge()
    {
        return false;
    }

    /// Result of a successful [#matches] call.
    ///
