...
///
/// - **Bounds** — lower and upper types the variable must lie between. Added as
///   [Subtype] constraints flow through the solver.
/// - **Domain** — the set of concrete [Alternative] witnesses still feasible;
///   narrowed by intersection whenever new coercion candidates arrive.
/// - **Binding** — a concrete expression once the variable is committed. Once set,
///   all subsequent bounds/candidates must be consistent or the solver throws
///   [UnsatisfiableException].
///
/// A family restriction (`mustBeRow`) exists to handle [Expression.AnyRow] —
/// "the variable must be *some* row type." This is the only family flag in the model.
        ...
        implements VariableState
{
    ...
    TypeVariableState copy()
    {
        TypeVariableState copy = new TypeVariableState();
        copy.binding = binding;
        copy.lowerBounds = lowerBounds.map(HashSet::new);
        copy.upperBounds = upperBounds.map(HashSet::new);
        copy.mustBeRow = mustBeRow;
        if (domain.isRestricted()) {
            copy.domain.replace(domain.alternatives());
        }
        return copy;
    }

    /// Commit the variable to `expression`. If already bound, fails unless the new
    /// expression is identical. Preserve existing bounds, including row field names,
    /// and initialize absent bounds from the binding.
    public void bind(Expression expression)
    {
        ...
        if (lowerBounds.isEmpty()) {
            lowerBounds = Optional.of(new HashSet<>(Set.of(expression)));
        }
        if (upperBounds.isEmpty()) {
            upperBounds = Optional.of(new HashSet<>(Set.of(expression)));
        }
    }

    void recordLowerBound(Expression expression)
    {
