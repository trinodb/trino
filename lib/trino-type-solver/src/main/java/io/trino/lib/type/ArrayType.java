...
public record ArrayType(Type elementType)
        implements Type
{
    public static final TypeConstructor CONSTRUCTOR = new ArrayTypeConstructor();

    static class ArrayTypeConstructor
            extends ParametricTypeConstructor
    {
        public ArrayTypeConstructor()
        {
            super("array", List.of("@e"), List.of(new RequireKind("@e", TYPE)));
        }

        // An array is comparable/orderable iff its element type is.
        @Override
        ...
        {
            return new ArrayType(((TypeArgument) arguments.getFirst()).type());
        }
    }
}
