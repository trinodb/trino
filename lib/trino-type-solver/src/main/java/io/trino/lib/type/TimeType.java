...
public record TimeType(int precision)
        implements Type
{
    public static final TypeConstructor CONSTRUCTOR = new TimeTypeConstructor();

    static class TimeTypeConstructor
            extends ParametricTypeConstructor
    {
        public TimeTypeConstructor()
        {
            super("time", List.of("@p"), List.of(new RequireKind("@p", NUMBER)));
        }

        @Override
        public Type newInstance(List<Argument> arguments)
        {
            return new TimeType((int) ((NumericArgument) arguments.getFirst()).value());
        }
    }
}
