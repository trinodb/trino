...
public record TimestampWithTimeZoneType(int precision)
        implements Type
{
    public static final TypeConstructor CONSTRUCTOR = new TimestampWithTimeZoneTypeConstructor();

    static class TimestampWithTimeZoneTypeConstructor
            extends ParametricTypeConstructor
    {
        public TimestampWithTimeZoneTypeConstructor()
        {
            super("timestamp_with_time_zone", List.of("@p"), List.of(new RequireKind("@p", NUMBER)));
        }

        @Override
        public Type newInstance(List<Argument> arguments)
        {
            return new TimestampWithTimeZoneType((int) ((NumericArgument) arguments.getFirst()).value());
        }
    }
}
