...
public record TimeWithTimeZoneType(int precision)
        implements Type
{
    public static final TypeConstructor CONSTRUCTOR = new TimeWithTimeZoneTypeConstructor();

    static class TimeWithTimeZoneTypeConstructor
            extends ParametricTypeConstructor
    {
        public TimeWithTimeZoneTypeConstructor()
        {
            super("time_with_time_zone", List.of("@p"), List.of(new RequireKind("@p", NUMBER)));
        }

        @Override
        public Type newInstance(List<Argument> arguments)
        {
            return new TimeWithTimeZoneType((int) ((NumericArgument) arguments.getFirst()).value());
        }
    }
}
