...
public record TimestampType(int precision)
        implements Type
{
    public static final TypeConstructor CONSTRUCTOR = new TimestampTypeConstructor();

    static class TimestampTypeConstructor
            extends ParametricTypeConstructor
    {
        public TimestampTypeConstructor()
        {
            super("timestamp", List.of("@p"), List.of(new RequireKind("@p", NUMBER)));
        }

        @Override
        public Type newInstance(List<Argument> arguments)
        {
            return new TimestampType((int) ((NumericArgument) arguments.getFirst()).value());
        }
    }
}
