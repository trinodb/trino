...
import io.grpc.netty.UdsNameResolverProvider;
...
public class AlluxioUtils
{
    ...
    public static AlluxioURI convertToAlluxioURIBak(Location location)
    {
    ...
    {
        Optional<String> schema = location.scheme();
        if (schema.isPresent()) {
            if (!schema.get().equals("alluxio")) {
    ...
    public static AlluxioURI convertToAlluxioURI(URIStatus fileStatus)
    {
        //TODO: check if this way builds a correct AlluxioURI
        return new AlluxioURI(fileStatus.getPath());
    }

    public static String getUdsDefaultSchema()
    {
        UdsNameResolverProvider provider = new UdsNameResolverProvider();
        return provider.getDefaultScheme();
    }
}
