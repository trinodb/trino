...
public class TimestampWithValueTimezoneType extends ArrowType.ExtensionType {
    ...
    public FieldVector getNewVector(String name, FieldType fieldType, BufferAllocator allocator) {
        ...
        TimeStampHolder timestampType = switch (precision) {
            ...
            case 3 -> new TimeStampHolder(Types.MinorType.TIMESTAMPMILLI, TimeStampMilliVector.class);
            case 6 -> new TimeStampHolder(Types.MinorType.TIMESTAMPMICRO, TimeStampMicroVector.class);
            case 9, 12 -> new TimeStampHolder(Types.MinorType.TIMESTAMPNANO, TimeStampNanoVector.class);
        ...
        structVector.addOrGet("timestamp", fieldType, timestampType.clazz );
        if (precision == 12){
            FieldType picoAdjustmentField = new FieldType(false, new Int(4, false), null);
            structVector.addOrGet("pico_adjustment", picoAdjustmentField, IntVector.class);
        }
        structVector.addOrGet("zone_id", new FieldType(false, new ArrowType.Int(32, true), null), IntVector.class); //TODO do we need a 4 byte int?
        return structVector;
