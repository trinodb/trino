        ...
        implements Closeable
{
            ...
            throws AvroTypeException, IOException
    {
        this(
                ...
                AvroTypeUtils.typeFromAvro(schema, avroTypeManager).getTypeParameters());
    }
    ...
    {
        outputStream = new OutputStreamSliceOutput(rawOutput);
        verify(compressionKind.isSupportedLocally(), "compression kind must be supported locally");
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : requireNonNull(fileMetadata, "metadata is null").entrySet()) {
                fileWriter.setMeta(entry.getKey(), entry.getValue());
            }
            pagePositionFileWriter = fileWriter.create(schema, outputStream);
        }
    ...
    public long getRetainedSize()
    {
        ...
        return INSTANCE_SIZE + outputStream.getRetainedSize() + 8192;
    }
