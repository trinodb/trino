...
import static io.trino.hive.formats.esri.OGCType.ST_LINESTRING;
import static io.trino.hive.formats.esri.OGCType.ST_MULTILINESTRING;
import static io.trino.hive.formats.esri.OGCType.ST_MULTIPOINT;
import static io.trino.hive.formats.esri.OGCType.ST_MULTIPOLYGON;
import static io.trino.hive.formats.esri.OGCType.ST_POINT;
import static io.trino.hive.formats.esri.OGCType.ST_POLYGON;
import static io.trino.hive.formats.esri.OGCType.UNKNOWN;
...
{
    protected static final VarHandle INT_HANDLE_BIG_ENDIAN = MethodHandles.byteArrayViewVarHandle(int[].class, ByteOrder.BIG_ENDIAN);
    protected static final DateTimeFormatter DATE_FORMATTER =
            DateTimeFormatter.ofPattern("yyyy-M-d").withZone(UTC);
    protected static final List<DateTimeFormatter> TIMESTAMP_FORMATTERS = List.of(
    ...
    protected final int geometryColumn;
    protected final List<Column> columns;
    protected final Map<String, Integer> columnIndex;
    protected final List<Type> types;
    protected final boolean[] fieldWritten;

    public AbstractDeserializer(List<Column> columns)
    {
    ...
    protected void writeGeometry(PageBuilder pageBuilder, OGCGeometry ogcGeometry)
    {
    ...
    protected BlockBuilder getBlockBuilderForWrite(PageBuilder pageBuilder, int fieldIndex)
    {
    ...
    protected void parseAttributes(JsonParser parser, PageBuilder pageBuilder, String attributesName)
            throws IOException
    {
        if (parser.currentToken() != START_OBJECT) {
            throw invalidJson(attributesName + " is not an object");
        }
