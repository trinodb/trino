        ...
        implements DatumWriter<Integer>
{
    ...
    {
        requireNonNull(page, "page must be set before beginning to write positions");
    }

    private abstract static class BlockPositionEncoder
    {
        protected Block block;
        private final Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx;

        public BlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx)
        {
            this.nullIdx = requireNonNull(nullIdx, "nullIdx is null");
        }
                ...
                throws IOException
        {
            ...
            if (isNull && nullIdx.isEmpty()) {
                throw new IOException("Can not write null value for non-nullable schema");
            }
            if (nullIdx.isPresent()) {
                encoder.writeIndex(isNull ? nullIdx.get().idx : 1 ^ nullIdx.get().idx);
            }
            if (isNull) {
                encoder.writeNull();
            }
    ...
    private static BlockPositionEncoder createBlockPositionEncoder(Schema schema, AvroTypeManager avroTypeManager, Type type, Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx)
            throws AvroTypeException
    {
        Optional<BiFunction<Block, Integer, Object>> overrideToAvroGenericObject = requireNonNull(avroTypeManager, "avroTypeManager is null").overrideBlockToAvroObject(schema, type);
        ...
        switch (schema.getType()) {
            case NULL -> {
                return new NullBlockPositionEncoder(nullIdx);
            }
            case BOOLEAN -> {
    ...
    private static class NullBlockPositionEncoder
            extends BlockPositionEncoder
    {
        public NullBlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx)
        {
            super(nullIdx);
            verify(nullIdx.isEmpty(), "Null Block Position Encoder is only for nulls outside of unions");
        }
                ...
                throws IOException
        {
            if (!block.isNull(position)) {
                throw new IOException("Non null value provided to be written to null schema");
            }
            encoder.writeNull();
        }
    }
            ...
            extends BlockPositionEncoder
    {
        ...
        public ArrayBlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx, Schema schema, AvroTypeManager avroTypeManager, ArrayType type)
                throws AvroTypeException
        {
            super(nullIdx);
            ...
            extends BlockPositionEncoder
    {
        ...
        public MapBlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx, Schema schema, AvroTypeManager avroTypeManager, MapType type)
                throws AvroTypeException
        {
            super(nullIdx);
            ...
            extends BlockPositionEncoder
    {
        ...
        public RecordBlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx, Schema schema, AvroTypeManager avroTypeManager, RowType rowType)
                throws AvroTypeException
        {
            this(nullIdx, schema, avroTypeManager, rowType.getFields().stream().map(RowType.Field::getName).map(optName -> optName.orElseThrow(() -> new IllegalArgumentException("Unable to use nested anonymous row type for avro writing"))).collect(toImmutableList()), rowType.getFields().stream().map(RowType.Field::getType).collect(toImmutableList()));
        }
        ...
        private RecordBlockPositionEncoder(Optional<AvroTypeUtils.SimpleUnionNullIndex> nullIdx, Schema schema, AvroTypeManager avroTypeManager, List<String> channelNames, List<Type> channelTypes)
                throws AvroTypeException
        {
            super(nullIdx);
