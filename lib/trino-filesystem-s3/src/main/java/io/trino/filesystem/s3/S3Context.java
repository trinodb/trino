        ...
        boolean exclusiveWriteSupported)
{
    ...
    public S3Context withCustomerKey(String sseCustomerKey)
    {
        return new S3Context(partSize, requesterPays, S3SseContext.withCustomerKey(sseCustomerKey), credentialsProviderOverride, cannedAcl, exclusiveWriteSupported);
    }
    ...
    record S3SseContext(S3SseType sseType, Optional<String> sseKmsKeyId, Optional<S3SseCustomerKey> sseCustomerKey)
    {
        ...
        public static S3SseContext withCustomerKey(String sseCustomerKey)
        {
            return new S3SseContext(CUSTOMER, Optional.empty(), Optional.ofNullable(sseCustomerKey).map(S3SseCustomerKey::onAes256));
        }
    }
}
