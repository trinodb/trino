            ...
            if (objectAlreadyExists) {
                ...
                if (exclusiveCreate && firstNonNull(putObjectException.numAttempts(), 0) > 1) {
                    ...
                    try {
                        ...
                        // The object may be gone. In such case we have no way to determine whether it was created by us.
                        if (getObjectTaggingException instanceof NoSuchKeyException
                                // Getting object tags failed. In such case we can't determine whether it was created by us.
                                || getObjectTaggingException instanceof S3Exception s3Exception && firstNonNull(s3Exception.numAttempts(), 0) > 1) {
                ...
                switch (createdByUs) {
                    ...
                    case MAYBE -> throw new IOException("Put failed for bucket [%s] key [%s] but provenance could not be verified".formatted(location.bucket(), location.key()), putObjectException);
                }
            }
