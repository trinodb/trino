...
public class TestDictionaryBuilder
{
    ...
    public void testSkipReservedSlots()
    {
        ...
        for (int i = 0; i < 64; i++) {
            positions.add(dictionaryBuilder.putIfAbsent(new VariableWidthBlock(1, wrappedBuffer(new byte[] {1}), new int[] {0, 1}, Optional.empty()), 0));
            positions.add(dictionaryBuilder.putIfAbsent(new VariableWidthBlock(1, wrappedBuffer(new byte[] {2}), new int[] {0, 1}, Optional.empty()), 0));
        }
