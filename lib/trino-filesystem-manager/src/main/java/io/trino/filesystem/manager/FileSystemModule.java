        ...
        extends AbstractConfigurationAwareModule
{
    ...
    protected void setup(Binder binder)
    {
        ...
        if (config.isCacheEnabled()) {
            fileSystemCache = Optional.of(context.getCacheFactory().createFileSystemCache(DEFAULT_CACHE_TTL, CacheTier.DISK));
        }
        else if (coordinatorFileCaching && isCoordinator) {
            fileSystemCache = Optional.of(context.getCacheFactory().createFileSystemCache(DEFAULT_CACHE_TTL, CacheTier.MEMORY));
        }

        binder.bind(new TypeLiteral<Optional<ConnectorFileSystemCache>>() {}).toInstance(fileSystemCache);
    }
