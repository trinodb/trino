        ...
        implements HiveMetastore
{
    ...
    public Metrics getMetrics()
    {
        ...
        allApiCallsStats.put(metrics, "metastore.all");
        for (Map.Entry<String, MetastoreApiCallStats> callStats : apiCallStats.entrySet()) {
            callStats.getValue().put(metrics, "metastore.%s".formatted(callStats.getKey()));
        }
    ...
    private <T> T wrap(String callName, Callable<T> call)
    {
        ...
        try {
            ...
            callStats.addFailure();
            allApiCallsStats.addFailure();
            Throwables.throwIfUnchecked(e);
            throw new RuntimeException(e);
        }
        finally {
            long timeNanos = ticker.read() - start;
            callStats.addTime(timeNanos);
            allApiCallsStats.addTime(timeNanos);
        }
    }
    ...
    static class MetastoreApiCallStats
    {
        ...
        {
            metrics.put(prefix + ".time.distribution", new DistributionSnapshot(new TDigestHistogram(timeNanosDistribution)));
