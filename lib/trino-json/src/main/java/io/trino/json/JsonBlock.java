        ...
        implements ValueBlock
{
    ...
    public JsonBlock getSingleValueBlock(int position)
    {
        checkReadable(position);
        if (isNull(position)) {
            return new JsonBlock(0, 1, new boolean[] {true}, new int[] {0, 0}, EMPTY_SLICE);
        }
        int index = position + arrayOffset;
    ...
    public JsonBlock copyRegion(int positionOffset, int length)
    {
        ...
        boolean[] newValueIsNull = compactIsNull(valueIsNull, index, length);

        if (newSlice == slice && newOffsets == offsets && newValueIsNull == valueIsNull) {
            return this;
        }
        return new JsonBlock(0, length, newValueIsNull, newOffsets, newSlice);
    }
