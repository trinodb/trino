...
import static io.trino.parquet.crypto.AesCipher.createModuleAAD;
            ...
            columnIndexStore = loadIndexes(dataSource, columnIndexReferences, (inputStream, columnMetadata) -> {
                try {
                    ...
                    byte[] aad = columnContext
                            .map(context -> createModuleAAD(context.getFileAad(), ModuleType.ColumnIndex, columnMetadata.getRowGroupOrdinal(), columnMetadata.getColumnOrdinal(), -1))
                            .orElse(null);
                    BlockCipher.Decryptor thriftDecryptor = columnContext
                            .map(ColumnDecryptionContext::getMetadataDecryptor)
                            .orElse(null);
        ...
        if (offsetIndexStore == null) {
            offsetIndexStore = loadIndexes(dataSource, offsetIndexReferences, (inputStream, columnMetadata) -> {
                try {
                    Optional<ColumnDecryptionContext> columnContext = decryptionContext.flatMap(context -> context.getColumnDecryptionContext(columnMetadata.getPath()));
                    byte[] aad = columnContext
                            .map(context -> createModuleAAD(context.getFileAad(), ModuleType.OffsetIndex, columnMetadata.getRowGroupOrdinal(), columnMetadata.getColumnOrdinal(), -1))
                            .orElse(null);
                    BlockCipher.Decryptor thriftDecryptor = columnContext
                            .map(ColumnDecryptionContext::getMetadataDecryptor)
                            .orElse(null);
