    ...
    public Optional<BloomFilter> getBloomFilter(ColumnPath columnPath)
    {
        ...
        try {
            ...
            if (columnContext.isPresent()) {
                ...
                int encryptedSize = BytesUtils.readIntLittleEndian(dataSource.readFully(columnBloomFilterOffset, AesCipher.SIZE_LENGTH).getBytes(), 0);
                Slice module = dataSource.readFully(columnBloomFilterOffset, AesCipher.SIZE_LENGTH + encryptedSize);
                ...
                byte[] aad = AesCipher.createModuleAAD(columnContext.get().getFileAad(), ModuleType.BloomFilterHeader, chunk.getRowGroupOrdinal(), chunk.getColumnOrdinal(), -1);
                bloomFilterHeader = Util.readBloomFilterHeader(in, columnContext.get().getMetadataDecryptor(), aad);
        ...
        try {
            ...
            if (columnContext.isPresent()) {
                ...
                int encryptedSize = BytesUtils.readIntLittleEndian(dataSource.readFully(bloomFilterDataOffset, AesCipher.SIZE_LENGTH).getBytes(), 0);
                Slice module = dataSource.readFully(bloomFilterDataOffset, AesCipher.SIZE_LENGTH + encryptedSize);
                ColumnChunkMetadata chunk = requireNonNull(columnChunks.get(columnPath), "missing chunk metadata");
                byte[] aad = AesCipher.createModuleAAD(columnContext.get().getFileAad(), ModuleType.BloomFilterBitset, chunk.getRowGroupOrdinal(), chunk.getColumnOrdinal(), -1);
                byte[] plain = columnContext.get().getMetadataDecryptor().decrypt(module.getBytes(), aad);
