...
public class Int32DecimalBatchReader
        extends IntFlatBatchReader
{
    private final DecimalType parquetDecimalType;

    public Int32DecimalBatchReader(ColumnDescriptor descriptor, DecimalType parquetDecimalType)
    {
        super(descriptor);
    ...
    protected ColumnChunk makeColumnChunk(Field field, int totalNonNullCount, int batchSize, boolean[] isNull)
    {
        ...
        if (trinoType instanceof DecimalType) {
            ...
            if (trinoType.equals(INTEGER)) {
                ...
                long[] longValues = new long[batchSize];
                copyIntToLong(values, longValues);
                block = new LongArrayBlock(batchSize, nullArr, longValues);
            }
