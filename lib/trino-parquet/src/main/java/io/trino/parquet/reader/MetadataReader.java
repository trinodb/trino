...
import static io.trino.parquet.crypto.AesCipher.GCM_TAG_LENGTH;
import static io.trino.parquet.crypto.AesCipher.NONCE_LENGTH;
...
public final class MetadataReader
{
    ...
    public static ParquetMetadata readFooter(ParquetDataSource dataSource, Optional<ParquetWriteValidation> parquetWriteValidation, Optional<FileDecryptionProperties> fileDecryptionProperties)
            throws IOException
    {
        // Parquet File Layout:
        //
        ...
        if (encryptedFooterMode) {
            ...
            aad = AesCipher.createFooterAAD(decryptionContext.get().getFileAad());
        }
    ...
    private static void verifyFooterIntegrity(ParquetDataSource dataSource, SliceInput metadataStream, FileDecryptionContext decryptionContext, int metadataLength)
    {
        ...
        byte[] signedFooterAAD = AesCipher.createFooterAAD(decryptionContext.getFileAad());
