...
public final class PageReader
{
            ...
            int columnOrdinal)
    {
        ...
        if (blockDecryptor.isPresent()) {
            dataPageAad = AesCipher.createModuleAAD(decryptionContext.get().getFileAad(), ModuleType.DataPage, rowGroupOrdinal, columnOrdinal, 0);
            dictionaryPageAad = AesCipher.createModuleAAD(decryptionContext.get().getFileAad(), ModuleType.DictionaryPage, rowGroupOrdinal, columnOrdinal, -1);
        }
    }
