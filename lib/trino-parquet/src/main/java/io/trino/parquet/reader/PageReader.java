...
public final class PageReader
{
            ...
            int rowGroupOrdinal)
    {
        ...
        this.blockDecryptor = decryptionContext.map(ColumnDecryptionContext::getDataDecryptor);
        if (blockDecryptor.isPresent()) {
            dataPageAad = AesCipher.createModuleAAD(decryptionContext.get().getFileAad(), ModuleType.DataPage, rowGroupOrdinal, decryptionContext.get().getOrdinal(), 0);
            dictionaryPageAad = AesCipher.createModuleAAD(decryptionContext.get().getFileAad(), ModuleType.DictionaryPage, rowGroupOrdinal, decryptionContext.get().getOrdinal(), -1);
        }
    }
    ...
    public DataPage readPage()
    {
        ...
        try {
            if (blockDecryptor.isPresent()) {
                AesCipher.quickUpdatePageAAD(dataPageAad, ((DataPage) compressedPage).getPageIndex());
            }
