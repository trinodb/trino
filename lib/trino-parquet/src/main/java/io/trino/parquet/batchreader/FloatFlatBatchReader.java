...
public class FloatFlatBatchReader
        extends AbstractBatchPrimitiveColumnReader
{
    protected int[] values;

    public FloatFlatBatchReader(ColumnDescriptor descriptor)
    {
    ...
    protected ColumnChunk makeColumnChunk(Field field, int totalNonNullCount, int batchSize, boolean[] isNull)
    {
        ...
        Block block = new IntArrayBlock(batchSize, nullArr, values);
    ...
    {
        values = new int[nextBatchSize];
    }

    float[] tmpArr;

    // Not efficient to convert floats to ints here. Would be nice to have an actual FloatArrayBlock or the ability
    // to read floats as ints.
    @Override
    protected void readValues(int valuePosition, int chunkSize)
    {
        // Retain a temporary array to hold the floats. It's probably faster to convert smaller
        ...
        if (tmpArr == null || tmpArr.length < chunkSize) {
            tmpArr = new float[chunkSize];
        }
        valuesReader.readFloats(tmpArr, 0, chunkSize);
        for (int i = 0; i < chunkSize; i++) {
            values[valuePosition++] = Float.floatToRawIntBits(tmpArr[i]);
        }
    }
}
