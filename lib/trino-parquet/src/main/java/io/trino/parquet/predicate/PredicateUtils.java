...
public final class PredicateUtils
{
    ...
    private static Optional<PageHeaderWithData> readPageHeaderWithData(SliceInput inputStream, ColumnChunkMetadata columnMetaData, Optional<FileDecryptionContext> decryptionContext)
    {
        ...
        if (columnContext.isPresent()) {
            decryptor = columnContext.map(ColumnDecryptionContext::getMetadataDecryptor).orElse(null);
            byte[] fileAad = decryptionContext.get().getFileAad();
            headerAad = AesCipher.createModuleAAD(fileAad, ModuleType.DictionaryPageHeader, columnMetaData.getRowGroupOrdinal(), columnMetaData.getColumnOrdinal(), -1);
        }

        final PageHeader pageHeader;
        try {
            pageHeader = org.apache.parquet.format.Util.readPageHeader(inputStream, decryptor, headerAad);
        }
        catch (IOException e) {
    ...
    private static DictionaryPage decodeDictionaryPage(ParquetDataSourceId dataSourceId, PageHeaderWithData pageHeaderWithData, ColumnChunkMetadata chunkMetaData, Optional<FileDecryptionContext> decryptionContext)
    {
        ...
        if (columnContext.isPresent()) {
            byte[] aad = AesCipher.createModuleAAD(
                    columnContext.get().getFileAad(),
                    ModuleType.DictionaryPage,
                    chunkMetaData.getRowGroupOrdinal(),   // set in ParquetMetadata
                    chunkMetaData.getColumnOrdinal(),     // set in ParquetMetadata
                    -1);
            byte[] plain = columnContext.get().getDataDecryptor().decrypt(compressedData.getBytes(), aad);
