        ...
        implements Iterator<Page>
{
    ...
    public Page next()
    {
        ...
        try {
            ...
            if (decryptionContext.isPresent()) {
                if (dataPageCount == 0 && !dictionaryWasRead && metadata.hasDictionaryPage()) {
            ...
            PageHeader pageHeader = readPageHeader(decryptionContext.map(ColumnDecryptionContext::getMetadataDecryptor).orElse(null), pageHeaderAAD);
    ...
    private byte[] getPageHeaderAAD()
    {
        ...
        dataPageHeaderAad = AesCipher.createModuleAAD(
                decryptionContext.get().getFileAad(),
                ModuleType.DataPageHeader,
                metadata.getRowGroupOrdinal(),
                decryptionContext.get().getOrdinal(),
                dataPageCount);
        return dataPageHeaderAad;
    }
    ...
    {
        checkState(decryptionContext.isPresent());
        return AesCipher.createModuleAAD(
                decryptionContext.get().getFileAad(),
