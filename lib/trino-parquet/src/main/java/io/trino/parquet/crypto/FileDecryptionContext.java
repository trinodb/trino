...
public class FileDecryptionContext
{
    ...
    public FileDecryptionContext(ParquetDataSourceId dataSourceId, FileDecryptionProperties fileDecryptionProperties, EncryptionAlgorithm algorithm, Optional<byte[]> footerKeyMetadata)
    {
        ...
        fileAad = aadPrefix.map(bytes -> AesCipher.concatByteArrays(bytes, aadFileUnique)).orElse(aadFileUnique);
    ...
    public void initPlaintextColumn(ColumnPath path)
    {
        ...
        Optional<ColumnDecryptionContext> context = columnDecryptionContext.get(path);
        checkArgument(context == null, "Column %s already exists in decryption context", path);
        columnDecryptionContext.put(path, Optional.empty());
    }
    ...
    public Optional<ColumnDecryptionContext> initializeColumnCryptoMetadata(ColumnPath path, boolean encryptedWithFooterKey, Optional<byte[]> columnKeyMetadata, int columnOrdinal)
    {
        ...
        if (encryptedWithFooterKey) {
            ...
            context = Optional.of(new ColumnDecryptionContext(getDataModuleDecryptor(Optional.empty()), getThriftModuleDecryptor(Optional.empty()), fileAad, columnOrdinal));
        }
        else {
            ...
            context = Optional.of(new ColumnDecryptionContext(getDataModuleDecryptor(columnKeyBytes), getThriftModuleDecryptor(columnKeyBytes), fileAad, columnOrdinal));
        }
