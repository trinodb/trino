        ...
        implements ParquetDataSource
{
    ...
    public Slice readTail(int length)
    {
        ...
        try {
            ...
            MockInputStreamTail fileTail = MockInputStreamTail.readTail(getId().toString(), getEstimatedSize(), inputStream, length);
            tailSlice = fileTail.getTailSlice();
        }
        catch (IOException e) {
            throw new RuntimeException("Error reading tail from %s with length");
        }
                ...
                if (mergedRange.contains(diskRange)) {
                    ...
                    slices.put(diskRangeEntry.getKey(), new ChunkReader()
                    {
                        ...
                        public Slice read()
                        {
                            ...
                            return mergedRangeLoader.read().slice(offset, Long.valueOf(diskRange.getLength()).intValue());
                        }
            ...
            implements ChunkReader
    {
        ...
        public Slice read()
        {
            ...
            if (data == null) {
                byte[] buffer = new byte[Long.valueOf(range.getLength()).intValue()];
