...
public class BenchmarkParquetSelectedPositions
{
    ...
    private long selectedDataPageCount;
    private long totalDataPageCount;
    ...
    @Param("0")
    public int preloadedColumnCount;
    ...
    @Param("1")
    public int dictionaryRunLength;
            ...
            throws IOException
    {
        ...
        if (dictionaryRunLength < 1) {
            throw new IllegalArgumentException("dictionaryRunLength must be positive");
        }
        ...
        if (preloadedColumnCount < 0 || preloadedColumnCount > columnCount) {
            throw new IllegalArgumentException("preloadedColumnCount must be between zero and columnCount");
        }
        if (pageValueCount < 1) {
        ...
        selectedDataPageCount = countSelectedPages(dataPageEndPositions, selectedFilePositions);
        totalDataPageCount = Arrays.stream(dataPageEndPositions)
                .mapToLong(pageEndPositions -> pageEndPositions.length)
                .sum();
    ...
    {
        public long unselectedDataPages;
        ...
        public long fullDecodeOutputRetainedBytes;
        ...
        public long selectedPushdownOutputBytes;
        public long selectedPushdownFirstOutputRetainedBytes;
            ...
            throws IOException
    {
        ...
        try (ParquetReader reader = createParquetReader(
            ...
            for (SourcePage page = reader.nextPage(); page != null; page = reader.nextPage()) {
                ...
                for (int channel = columnCount - preloadedColumnCount; channel < columnCount; channel++) {
                ...
                for (int channel = 0; channel < columnCount; channel++) {
                    ...
                    if (pushdown) {
                        counters.selectedPushdownOutputBytes += block.getSizeInBytes();
                        counters.selectedPushdownOutputRetainedBytes += block.getRetainedSizeInBytes();
                        if (channel == 0) {
                            counters.selectedPushdownFirstOutputRetainedBytes += block.getRetainedSizeInBytes();
                        }
                    }
                    else {
                        counters.fullDecodeOutputBytes += block.getSizeInBytes();
                        counters.fullDecodeOutputRetainedBytes += block.getRetainedSizeInBytes();
                        if (channel == 0) {
                            counters.fullDecodeFirstOutputRetainedBytes += block.getRetainedSizeInBytes();
                        }
                    }
        ...
        counters.unselectedDataPages += totalDataPageCount - selectedDataPageCount;
        counters.rejectedRows += rowCount - selectedRows;
        counters.readOperations++;
        counters.selectedDataPages += selectedDataPageCount;
        if (pushdown) {
        ...
        counters.totalDataPages += totalDataPageCount;
        long checksum = 0;
    ...
    private int[][] readDataPageEndPositions()
            throws IOException
    {
        List<List<Integer>> pageEndPositions = new ArrayList<>();
        int[] rowOffsets = new int[columnCount];
        for (int channel = 0; channel < columnCount; channel++) {
            pageEndPositions.add(new ArrayList<>());
        }
        AggregatedMemoryContext memoryContext = newSimpleAggregatedMemoryContext();
        try (TestingParquetDataSource dataSource = new TestingParquetDataSource(parquetFile, readerOptions)) {
            for (var block : parquetMetadata.getBlocks()) {
                for (int channel = 0; channel < columnCount; channel++) {
                    var columnMetadata = block.columns().get(channel);
                    DiskRange diskRange = new DiskRange(columnMetadata.getStartingPos(), columnMetadata.getTotalSize());
                    ChunkedInputStream input = dataSource.planRead(ImmutableListMultimap.of(channel, diskRange), memoryContext).get(channel);
                    try (input) {
                        PageReader pageReader = PageReader.createPageReader(
                                dataSource.getId(),
                                input,
                                columnMetadata,
                                parquetMetadata.getFileMetaData().getSchema().getColumns().get(channel),
                                null,
                        ...
                        while (pageReader.hasNext()) {
                            ...
                            rowOffsets[channel] += page.getValueCount();
                            pageEndPositions.get(channel).add(rowOffsets[channel]);
        ...
        memoryContext.close();
        for (int channel = 0; channel < columnCount; channel++) {
            checkState(rowOffsets[channel] == rowCount, "Column %s data pages contain %s rows, expected %s", channel, rowOffsets[channel], rowCount);
        }
        return pageEndPositions.stream()
    ...
    private int[] createPageSelectedFilePositions(boolean shifted)
    {
        ...
        int pageStart = 0;
        int[] firstColumnPageEndPositions = dataPageEndPositions[0];
        for (int pageIndex = 0; pageIndex < firstColumnPageEndPositions.length; pageIndex++) {
            int pageEnd = firstColumnPageEndPositions[pageIndex];
    ...
    private static long countSelectedPages(int[][] dataPageEndPositions, int[] selectedPositions)
    {
        long count = 0;
        for (int[] columnPageEndPositions : dataPageEndPositions) {
            boolean[] selectedPages = new boolean[columnPageEndPositions.length];
            for (int selectedPosition : selectedPositions) {
                selectedPages[lowerBound(columnPageEndPositions, selectedPosition + 1)] = true;
            }
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            for (int channel = 0; channel < columnCount; channel++) {
                ...
                if (nullShape.isNull(row, rowCount, columnNullPercentages[channel], nullRunCount, valueId)) {
                    ...
                    if (dictionaryCardinality > 0) {
                        valueId = ((row / dictionaryRunLength) * columnCount + channel) % dictionaryCardinality;
                    }
    ...
    public enum SelectionShape
    {
        ...
        SKEWED_RUNS {
            ...
            int[] select(int globalOffset, int positionCount, double selectivity, int requestedRunCount, int pageValueCount)
            {
                ...
                int runs = min(max(1, requestedRunCount), min(selectedCount, positionCount - selectedCount + 1));
                int[] positions = new int[selectedCount];
                int position = positionCount - selectedCount - (runs - 1);
                int longRunLength = selectedCount - (runs - 1);
                int outputIndex = 0;
                for (int index = 0; index < longRunLength; index++) {
                    positions[outputIndex++] = position++;
                }
