        ...
        implements DataDefinitionTask<RenameColumn>
{
    ...
    public ListenableFuture<?> execute(RenameColumn statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        ...
        String source = metadata.getNameCanonicalizer(session, tableHandle.getCatalogName().getCatalogName()).canonicalize(statement.getSource().getValue(), statement.getSource().isDelimited());
        String target = metadata.getNameCanonicalizer(session, tableHandle.getCatalogName().getCatalogName()).canonicalize(statement.getTarget().getValue(), statement.getTarget().isDelimited());
