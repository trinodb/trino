...
public class OrExpressionNormalizer
{
            ...
            extends ExpressionRewriter<Optional<Context>>
    {
        ...
        {
            InPredicate rewrittenExpression = treeRewriter.defaultRewrite(inPredicate, Optional.empty());
            if (rewrittenExpression.getValueList() instanceof InListExpression && context.isPresent()) {
                context.get().addMapping(
                        inPredicate.getValue(),
                        ((InListExpression) rewrittenExpression.getValueList()).getValues());
                return FALSE_LITERAL;
            }
            return inPredicate;
        }
