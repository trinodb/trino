...
public class ExtractDayOfWeekWithWeekStart
{
    ...
    public static long extract(@SqlType("timestamp(p) with time zone") long packedEpochMillis, @SqlType(StandardTypes.BIGINT) long weekStart)
    {
        ...
        return floorMod(ExtractDayOfWeek.extract(packedEpochMillis) - weekStart, DAYS_IN_WEEK) + 1;
    }

    @LiteralParameters("p")
    @SqlType(StandardTypes.BIGINT)
    public static long extract(@SqlType("timestamp(p) with time zone") LongTimestampWithTimeZone timestamp, @SqlType(StandardTypes.BIGINT) long weekStart)
    {
        checkCondition(weekStart > 0 && weekStart <= 7, INVALID_FUNCTION_ARGUMENT, format("Invalid start day of the week : %s, valid values are [1-7]", weekStart));
