...
public class TestJdbcClient
{
    ...
    public void testMetadataWithSchemaPattern()
    {
        ...
        assertEquals(jdbcClient.getColumns(session, table.get()), ImmutableList.of(
                new JdbcColumnHandle("TE_T", JDBC_VARCHAR, VARCHAR, true, false),
                new JdbcColumnHandle("VA%UE", JDBC_BIGINT, BIGINT, true, false)));
    }
    ...
    public void testMetadataWithFloatAndDoubleCol()
    {
        ...
        assertTrue(table.isPresent(), "table is missing");
        assertEquals(jdbcClient.getColumns(session, table.get()), ImmutableList.of(
                new JdbcColumnHandle("COL1", JDBC_BIGINT, BIGINT, true, false),
                new JdbcColumnHandle("COL2", JDBC_DOUBLE, DOUBLE, true, false),
                new JdbcColumnHandle("COL3", JDBC_DOUBLE, DOUBLE, true, false),
                new JdbcColumnHandle("COL4", JDBC_REAL, REAL, true, false)));
    }
}
