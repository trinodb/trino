...
public class DeleteDeltaLocations
{
    ...
    {
        this.partitionLocation = requireNonNull(partitionLocation, "Partition Location is null");
        this.deleteDeltas = ImmutableList.copyOf(requireNonNull(deleteDeltas, "delete detlas is null"));
    }
    ...
    public boolean hadDeletedRows()
    {
        return !deleteDeltas.isEmpty();
    }
    ...
    {
        private final String partitionLocation;
        private final ImmutableList.Builder deleteDetlaInfoBuilder;

        public Builder(Path partitionPath)
        {
            partitionLocation = partitionPath.toString();
            deleteDetlaInfoBuilder = new ImmutableList.Builder();
        }
        ...
        public void addDeleteDelta(Path deleteDeltaPath, long minWriteId, long maxWriteId, int statementId)
        {
            ...
            deleteDetlaInfoBuilder.add(new DeleteDeltaInfo(minWriteId, maxWriteId, statementId));
        }

        public DeleteDeltaLocations build()
        {
            return new DeleteDeltaLocations(partitionLocation, deleteDetlaInfoBuilder.build());
        }
    }
}
