        ...
        extends AbstractIterator<LocatedFileStatus>
{
    ...
    private Iterator<LocatedFileStatus> getLocatedFileStatusRemoteIterator(Path path, PathFilter pathFilter)
    {
        try (TimeStat.BlockTimer ignored = namenodeStats.getListLocatedStatus().time()) {
            ...
            return Iterators.filter(new FileStatusIterator(table, path, fileSystem, directoryLister, namenodeStats, pathFilter), input -> pathFilter.accept(input.getPath()));
        }
    }
            ...
            implements Iterator<LocatedFileStatus>
    {
        ...
        private FileStatusIterator(Table table, Path path, FileSystem fileSystem, DirectoryLister directoryLister, NamenodeStats namenodeStats, PathFilter pathFilter)
        {
            this.path = path;
            this.namenodeStats = namenodeStats;
            try {
                this.fileStatusIterator = directoryLister.list(fileSystem, table, path, pathFilter);
            }
            catch (IOException e) {
