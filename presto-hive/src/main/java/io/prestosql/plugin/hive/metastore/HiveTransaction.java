...
public class HiveTransaction
{
    ...
    private final Map<HiveTableHandle, ValidTxnWriteIdList> validHiveTransactionsForTable = new ConcurrentHashMap<>();
    ...
    public ValidTxnWriteIdList getValidWriteIds(HiveMetastore metastore, HiveTableHandle tableHandle)
    {
        ...
        return validHiveTransactionsForTable.computeIfAbsent(tableHandle, handle -> new ValidTxnWriteIdList(
                metastore.getValidWriteIds(
                        identity,
                        ImmutableList.of(handle.getSchemaTableName()),
                        transactionId)));
    }
}
