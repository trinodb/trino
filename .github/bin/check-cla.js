...
function commentRequestsClaCheck(comment) {
    return /(?:@cla-bot(\[bot\])?\s+check|\/cla-check)\b/i.test(comment);
}
...
function findMissingClaContributors(commits, contributors) {
    const unidentified = sortedUnique(
        commits
            ...
            .map(commit => commit.commit?.author?.name || commit.sha)
    );
...
async function main() {
    ...
    try {
        ...
        await setStatus(target.statusSha, 'error', 'CLA is missing for one or more commit authors');
