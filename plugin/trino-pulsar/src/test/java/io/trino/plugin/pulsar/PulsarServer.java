        ...
        implements Closeable
{
    ...
    public PulsarServer(String pulsarImage) throws IOException
    {
        ...
        pulsar = new GenericContainer<>(pulsarImage)
                ...
                .waitingFor(new HttpWaitStrategy()
                            ...
                            .withStartupTimeout(java.time.Duration.of(300, SECONDS)));
    ...
    public void copyAndIngestTpchData(MaterializedResult rows, String datasource, Class clazz)
            throws IOException, InterruptedException
    {
        String tsvFileLocation = format("%s/%s.tsv", hostWorkingDirectory, datasource);
        writeTpchDataAsTsv(rows, tsvFileLocation);
        ingestData(tsvFileLocation, datasource, clazz);
    }

    public <T> void ingestData(String inputTSV, String source, Class<T> clazz) throws IOException, InterruptedException
    {
        ...
        while ((line = reader.readLine()) != null) {
            if (source.equalsIgnoreCase(CUSTOMER)) {
                sendMsgWithRetry(producer, toCustomer(line), 3, counter);
            }
            else if (source.equalsIgnoreCase(ORDERS)) {
                sendMsgWithRetry(producer, toOrders(line), 3, counter);
            }
            else if (source.equalsIgnoreCase(LINEITEM)) {
                sendMsgWithRetry(producer, toLineItem(line), 3, counter);
            }
            else if (source.equalsIgnoreCase(NATION)) {
                sendMsgWithRetry(producer, toNation(line), 3, counter);
            }
            else if (source.equalsIgnoreCase(REGION)) {
                sendMsgWithRetry(producer, toRegion(line), 3, counter);
            }
        }
        if (source.equalsIgnoreCase(CUSTOMER)) {
            sendMsgWithRetry(producer, new Customer(), 3, counter);
        }
        else if (source.equalsIgnoreCase(ORDERS)) {
            sendMsgWithRetry(producer, new Orders(), 3, counter);
        }
        else if (source.equalsIgnoreCase(LINEITEM)) {
            sendMsgWithRetry(producer, new LineItem(), 3, counter);
        }
        else if (source.equalsIgnoreCase(NATION)) {
            sendMsgWithRetry(producer, new Nation(), 3, counter);
        }
        else if (source.equalsIgnoreCase(REGION)) {
            sendMsgWithRetry(producer, new Region(), 3, counter);
        }
    }

    private static <T> void sendMsgWithRetry(Producer<T> producer, Object data, int retry, LongAdder counter)
    {
        if (retry > 0) {
            try {
                producer.newMessage().value((T) data).send();
                counter.increment();
            }
            catch (PulsarClientException e) {
                sendMsgWithRetry(producer, data, retry - 1, counter);
            }
        }
    }
    ...
    private Orders toOrders(String line)
    {
        ...
        try {
            orders.orderdate = (int) (new SimpleDateFormat("yyyy-MM-dd").parse(fields[4]).getTime() / 86_400_000);
        }
    ...
    private LineItem toLineItem(String line)
    {
        ...
        try {
            lineItem.shipdate = (int) (new SimpleDateFormat("yyyy-MM-dd").parse(fields[10]).getTime() / 86_400_000);
        }
        catch (ParseException e) {
            // ignored
        }
        try {
            lineItem.commitdate = (int) (new SimpleDateFormat("yyyy-MM-dd").parse(fields[11]).getTime() / 86_400_000);
        }
        catch (ParseException e) {
            // ignored
        }
        try {
            lineItem.receiptdate = (int) (new SimpleDateFormat("yyyy-MM-dd").parse(fields[12]).getTime() / 86_400_000);
        }
        catch (ParseException e) {
            // ignored
        }
