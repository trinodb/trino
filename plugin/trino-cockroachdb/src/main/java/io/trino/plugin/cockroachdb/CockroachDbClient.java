...
import io.trino.plugin.cockroachdb.CockroachDbConfig.ArrayMapping;
...
import static io.trino.plugin.cockroachdb.CockroachDbSessionProperties.getArrayMapping;
import static io.trino.plugin.cockroachdb.TypeUtils.arrayDepth;
import static io.trino.plugin.cockroachdb.TypeUtils.getArrayElementPgTypeName;
import static io.trino.plugin.cockroachdb.TypeUtils.getJdbcObjectArray;
import static io.trino.plugin.cockroachdb.TypeUtils.toPgTimestamp;
...
{
    private static final Logger log = Logger.get(CockroachDbClient.class);
    ...
    private static final int COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION = 6;
    ...
    private static final PredicatePushdownController COCKROACHDB_CHARACTER_PUSHDOWN = (session, domain) -> {
        checkArgument(
        ...
        if (domain.isOnlyNull() ||
                // PostgreSQL is case sensitive by default, CockroachDB probably as well.
        ...
        // TODO need to figure out for CockroachDB
        return DISABLE_PUSHDOWN.apply(session, domain);
    };

    @Inject
    public CockroachDbClient(
            ...
            IdentifierMapping identifierMapping)
    {
        ...
        if (cockroachDbConfig.isIncludeSystemTables()) {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter(
                ...
                ImmutableSet.<AggregateFunctionRule>builder()
                        ...
                        .add(new ImplementSum(CockroachDbClient::toTypeHandle))
        ...
        switch (typeHandle.getJdbcType()) {
            ...
            case Types.NUMERIC: {
                ...
                if (getDecimalRounding(session) == ALLOW_OVERFLOW) {
                    if (columnSize == 131089) {
                        ...
                        // 131089 = SELECT LENGTH(pow(10::numeric,131071)::varchar); 131071 = 2^17-1  (org.cockroachdb.jdbc.TypeInfoCache#getDisplaySize)
            ...
            case Types.TIMESTAMP:
                ...
                return Optional.of(ColumnMapping.longMapping(
                        ...
                        CockroachDbClient::shortTimestampWriteFunction));
    ...
    private Optional<ColumnMapping> arrayToPrestoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        if (baseElementTypeHandle.getJdbcType() == Types.BINARY) {
            ...
            // need to check issue
            // need to figure out for CockroachDB uses PostgreSQL JDBC driver
            return Optional.empty();
        }
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type instanceof TimeType) {
            ...
            if (timeType.getPrecision() <= COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                return WriteMapping.longMapping(format("time(%s)", timeType.getPrecision()), timeWriteFunction(timeType.getPrecision()));
            }
            return WriteMapping.longMapping(format("time(%s)", COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), timeWriteFunction(COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION));
        }

        if (type instanceof TimestampType) {
            TimestampType timestampType = (TimestampType) type;
            if (timestampType.getPrecision() <= COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                verify(timestampType.getPrecision() <= TimestampType.MAX_SHORT_PRECISION);
                return WriteMapping.longMapping(format("timestamp(%s)", timestampType.getPrecision()), CockroachDbClient::shortTimestampWriteFunction);
            }
            verify(timestampType.getPrecision() > TimestampType.MAX_SHORT_PRECISION);
            return WriteMapping.objectMapping(format("timestamp(%s)", COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWriteFunction());
        }
        if (type instanceof TimestampWithTimeZoneType) {
            TimestampWithTimeZoneType timestampWithTimeZoneType = (TimestampWithTimeZoneType) type;
            if (timestampWithTimeZoneType.getPrecision() <= COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
            ...
            return WriteMapping.objectMapping(format("timestamptz(%d)", COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWithTimeZoneWriteFunction());
        }
    ...
    private static ColumnMapping charColumnMapping(int charLength)
    {
        ...
        return ColumnMapping.sliceMapping(
                ...
                charWriteFunction(),
                COCKROACHDB_CHARACTER_PUSHDOWN);
    }
    ...
    private static ObjectWriteFunction longTimestampWriteFunction()
    {
        return ObjectWriteFunction.of(LongTimestamp.class, ((statement, index, timestamp) -> {
            ...
            // TODO need to figure out for CockroachDB
            verify(COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION == 6);
    ...
    private static ColumnMapping timestampWithTimeZoneColumnMapping(int precision)
    {
        ...
        // TODO need to figure out for CockroachDB
        checkArgument(precision <= COCKROACHDB_MAX_SUPPORTED_TIMESTAMP_PRECISION, "unsupported precision value %s", precision);
    ...
    private static ColumnMapping moneyColumnMapping()
    {
         ...
         *
         */
        // TODO need to figure out for CockroachDB, above github issue is closed
        return ColumnMapping.sliceMapping(
                VARCHAR,
