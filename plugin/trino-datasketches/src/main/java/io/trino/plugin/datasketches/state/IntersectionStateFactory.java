...
public class IntersectionStateFactory
        implements AccumulatorStateFactory<IntersectionState>
{
    @Override
    public IntersectionState createSingleState()
    {
        return new SingleIntersectionState();
    }

    @Override
    public IntersectionState createGroupedState()
    {
            ...
            implements IntersectionState
    {
        ...
        {
            // The seed may not be set on the receiving state when combine runs before any local input.
            if (getSeed() == 0) {
        ...
        {
            ThetaSketch sketchA = ThetaSketch.wrap(MemorySegment.ofArray(a.getBytes()), seed);
            ThetaSketch sketchB = ThetaSketch.wrap(MemorySegment.ofArray(b.getBytes()), seed);
            ThetaIntersection intersection = ThetaSetOperation.builder().setSeed(seed).buildIntersection();
            intersection.intersect(sketchA);
            intersection.intersect(sketchB);
            return Slices.wrappedBuffer(intersection.getResult().toByteArray());
        }
    }
            ...
            extends BaseIntersectionState
    {
        ...
        // Matches sizeOf(byte[]) = array header + length, computed once to avoid per-call allocation.
