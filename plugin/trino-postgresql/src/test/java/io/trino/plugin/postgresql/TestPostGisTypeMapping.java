...
public class TestPostGisTypeMapping
        extends AbstractTestQueryFramework
{
    ...
    protected TestingPostGisServer postgisServer;
    ...
    protected QueryRunner createQueryRunner()
            throws Exception
    {
        postgisServer = closeAfterClass(new TestingPostGisServer());
    ...
    public void setUp()
    {
        ...
        JdbcSqlExecutor executor = new JdbcSqlExecutor(postgisServer.getJdbcUrl(), postgisServer.getProperties());
    ...
    {
        JdbcSqlExecutor jdbcSqlExecutor = new JdbcSqlExecutor(postgisServer.getJdbcUrl(), postgisServer.getProperties());
    ...
    public void testArray()
    {
        ...
        arrayUnicodeDataTypeTest(TestPostGisTypeMapping::arrayDataType, DataType::charDataType)
                .execute(getQueryRunner(), session, trinoCreateAsSelect(session, "test_array_parameterized_char_unicode"));
        arrayUnicodeDataTypeTest(TestPostGisTypeMapping::postgresArrayDataType, DataType::charDataType)
                .execute(getQueryRunner(), session, postgresCreateAndInsert("test_array_parameterized_char_unicode"));
        arrayVarcharUnicodeDataTypeTest(TestPostGisTypeMapping::arrayDataType)
                .execute(getQueryRunner(), session, trinoCreateAsSelect(session, "test_array_parameterized_varchar_unicode"));
        arrayVarcharUnicodeDataTypeTest(TestPostGisTypeMapping::postgresArrayDataType)
    ...
    private static DataType<String> hstoreArrayAsJsonDataType()
    {
        return dataType(
                ...
                json -> HSTORE_CODEC.fromJson(json).stream()
                        .map(TestPostGisTypeMapping::hstoreLiteral)
    ...
    {
        try (TestTable testTable = new TestTable(
                new JdbcSqlExecutor(postgisServer.getJdbcUrl(), postgisServer.getProperties()),
                "test_time_24",
...
//                    .isNotFullyPushedDown(AggregationNode.class);
    ...
    private DataType<Map<String, String>> hstoreDataType()
    {
        return dataType(
                ...
                TestPostGisTypeMapping::hstoreLiteral);
    }
    ...
    {
        return new CreateAndInsertDataSetup(new JdbcSqlExecutor(postgisServer.getJdbcUrl(), postgisServer.getProperties()), tableNamePrefix);
    }

    private DataSetup postgresCreateTrinoInsert(String tableNamePrefix)
    {
        return new CreateAndTrinoInsertDataSetup(new JdbcSqlExecutor(postgisServer.getJdbcUrl(), postgisServer.getProperties()), new TrinoSqlExecutor(getQueryRunner()), tableNamePrefix);
    }
