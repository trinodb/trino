        ...
        extends BaseJdbcConnectionCreationTest
{
    ...
    private void testJdbcMergeConnectionCreations()
    {
        ...
        assertJdbcConnections("DELETE FROM copy_of_customer_transactional WHERE abs(custkey) = 1", 24, Optional.empty());
        assertJdbcConnections("UPDATE copy_of_customer_transactional SET name = 'POLAND' WHERE abs(custkey) = 1", 32, Optional.empty());
        assertJdbcConnections("MERGE INTO copy_of_customer_transactional n USING customer r ON r.custkey = n.custkey WHEN MATCHED THEN DELETE", 28, Optional.empty());
        ...
        assertJdbcConnections(nonTransactionalMergeSession, "DELETE FROM copy_of_customer_non_transactional WHERE abs(custkey) = 1", 24, Optional.empty());
        assertJdbcConnections(nonTransactionalMergeSession, "UPDATE copy_of_customer_non_transactional SET name = 'POLAND' WHERE abs(custkey) = 1", 32, Optional.empty());
        assertJdbcConnections(nonTransactionalMergeSession, "MERGE INTO copy_of_customer_non_transactional c USING customer r ON r.custkey = c.custkey WHEN MATCHED THEN DELETE", 28, Optional.empty());
    }
