        ...
        extends BaseJdbcConnectorTest
{
    ...
    public void testNumericAggregationPushdown()
    {
        ...
        try (TestTable testTable = createTableWithNumericTypes(schemaName + ".test_aggregation_pushdown",
                ImmutableList.of("100.000, 100000000.000000000, 100000000, 100.000", "123.321, 123456789.987654321, 123456789, 123.321"))) {
            assertThat(query("SELECT min(short_decimal), min(long_decimal), min(big_int), min(t_double) FROM " + testTable.getName())).isFullyPushedDown();
            assertThat(query("SELECT max(short_decimal), max(long_decimal), max(big_int), max(t_double) FROM " + testTable.getName())).isFullyPushedDown();
            assertThat(query("SELECT sum(short_decimal), sum(long_decimal), sum(big_int), sum(t_double) FROM " + testTable.getName())).isFullyPushedDown();
            assertThat(query("SELECT avg(short_decimal), avg(long_decimal), avg(big_int), avg(t_double) FROM " + testTable.getName())).isFullyPushedDown();
        }
    }

    @Override
    protected TestTable createTableWithNumericTypes(String name, List<String> rows)
    {
        return new TestTable(onRemoteDatabase(), name, "(short_decimal Decimal(9, 3), long_decimal Decimal(30, 10), a_bigint Int64, t_double Float64) Engine=Log", rows);
    }
