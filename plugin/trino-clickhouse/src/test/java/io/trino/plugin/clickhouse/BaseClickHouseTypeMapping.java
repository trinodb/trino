        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testClickHouseDateTimeWithTimeZone()
    {
        for (ZoneId sessionZone : timezones()) {
            ...
            SqlDataTypeTest clickhouseCreateAndInsertTrinoTests = SqlDataTypeTest.create()
                    .addRoundTrip("Nullable(DateTime('UTC'))", "NULL", TIMESTAMP_TZ_SECONDS, "CAST(NULL AS TIMESTAMP(0) WITH TIME ZONE)");
            addClickhouseCreateAndTrinoInsertTestCases(clickhouseCreateAndInsertTrinoTests, "DateTime('%s')"::formatted, 0);
            IntStream.rangeClosed(0, CLICKHOUSE_MAX_SUPPORTED_TIMESTAMP_PRECISION).forEach(precision ->
                    addClickhouseCreateAndTrinoInsertTestCases(clickhouseCreateAndInsertTrinoTests, zone -> "DateTime64(%s, '%s')".formatted(precision, zone), precision));
            clickhouseCreateAndInsertTrinoTests.execute(getQueryRunner(), session, clickhouseCreateAndTrinoInsert("tpch.test_timestamp_with_time_zone"));

            SqlDataTypeTest clickhouseCreateAndInsertTests = SqlDataTypeTest.create()
                    .addRoundTrip("Nullable(DateTime('UTC'))", "NULL", TIMESTAMP_TZ_SECONDS, "CAST(NULL AS TIMESTAMP(0) WITH TIME ZONE)");
            addClickhouseCreateAndInsertTestCases(clickhouseCreateAndInsertTests, "DateTime('%s')"::formatted, 0);
            IntStream.rangeClosed(0, CLICKHOUSE_MAX_SUPPORTED_TIMESTAMP_PRECISION).forEach(precision ->
                    addClickhouseCreateAndInsertTestCases(clickhouseCreateAndInsertTests, zone -> "DateTime64(%s, '%s')".formatted(precision, zone), precision));
            clickhouseCreateAndInsertTests.execute(getQueryRunner(), session, clickhouseCreateAndInsert("tpch.datetime_tz"));
        }
    }

    private void addClickhouseCreateAndTrinoInsertTestCases(SqlDataTypeTest tests, Function<String, String> inputTypeFactory, int precision)
    {
    ...
    private void addClickhouseCreateAndInsertTestCases(SqlDataTypeTest tests, Function<ZoneId, String> inputTypeFactory, int precision)
    {
        TimestampWithTimeZoneType expectedType = createTimestampWithTimeZoneType(precision);
