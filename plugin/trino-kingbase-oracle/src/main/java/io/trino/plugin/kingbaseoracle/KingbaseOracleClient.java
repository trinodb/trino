...
import static io.trino.plugin.kingbaseoracle.KingbaseOracleSessionProperties.getNumberDefaultScale;
import static io.trino.plugin.kingbaseoracle.KingbaseOracleSessionProperties.getNumberRoundingMode;
        ...
        extends BaseJdbcClient
{
    ...
    @Inject
    public KingbaseOracleClient(
            BaseJdbcConfig config,
            KingbaseOracleConfig kingbaseoracleConfig,
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.synonymsEnabled = kingbaseoracleConfig.isSynonymsEnabled();
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, ParameterizedExpression>>builder()
                        ...
                        .add(new ImplementSum(KingbaseOracleClient::toTypeHandle))
        ...
        this.fetchSize = kingbaseoracleConfig.getFetchSize();
    }
        ...
        switch (typeHandle.jdbcType()) {
            ...
            case Types.NCHAR:
                ...
                return Optional.of(ColumnMapping.sliceMapping(
                        charType,
                        charReadFunction(charType),
                        charWriteFunction(), // 标准 write function
                        FULL_PUSHDOWN));
...
//    }
    ...
    public static LongWriteFunction trinoDateToOracleDateWriteFunction()
    {
        return new LongWriteFunction()
        {
            ...
            public void set(PreparedStatement statement, int index, long value)
                    throws SQLException
            {
                // value is already the number of days since 1970-01-01 (epoch day)
        ...
        return (resultSet, columnIndex) -> {
            ...
            try {
                ...
                if (dateStr != null) {
                    ...
                    // Try parsing with timestamp format first (e.g., "2025-01-01 00:00:00")
                    try {
                        localDateTime = LocalDateTime.parse(dateStr, TIMESTAMP_SECONDS_FORMATTER);
                    }
                    catch (Exception e1) {
                        // Try parsing with date format only (e.g., "2025-01-01")
                        try {
                            localDateTime = LocalDate.parse(dateStr, DATE_FORMATTER).atStartOfDay();
                        }
                        catch (Exception e2) {
            ...
            if (localDateTime == null) {
                try {
                    ...
                    if (date != null) {
                        localDateTime = date.toLocalDate().atStartOfDay();
                    }
                }
                catch (SQLException e) {
                    lastException = e;
            ...
            // Method 3: Try getTimestamp() as final fallback
            if (localDateTime == null) {
                try {
            ...
            if (localDateTime == null) {
                throw new SQLException("Failed to read date value", lastException);
            }

            return toTrinoTimestamp(TIMESTAMP_SECONDS, localDateTime);
        };
    }
    ...
    private static LongWriteFunction trinoTimestampToOracleDateWriteFunction()
    {
        return new LongWriteFunction()
        {
            ...
            public String getBindExpression()
            {
                // Oracle DATE 在 KingbaseES 中不需要符号年份，使用普通 YYYY
...
//    }
    ...
    public static ColumnMapping oracleTimestampWithTimeZoneColumnMapping()
    {
        return ColumnMapping.longMapping(
                TIMESTAMP_TZ_MILLIS,
                (resultSet, columnIndex) -> {
                    // KingbaseES JDBC driver doesn't support direct conversion to ZonedDateTime
                    // Use getString() and parse manually
                    String timestampStr = resultSet.getString(columnIndex);
                    if (timestampStr == null) {
                        throw new SQLException("NULL timestamp with time zone");
                    }

                    // Parse format like "2025-01-01 12:34:56.123456 +08:00" or "2025-01-01 12:34:56.123456 -08:00"
                    // Try multiple formats
                    ZonedDateTime zonedDateTime;
                    try {
                        // Try with timezone offset format: "2025-01-01 12:34:56.123456 +08:00"
                        DateTimeFormatter formatter = new DateTimeFormatterBuilder()
                                .appendPattern("yyyy-MM-dd HH:mm:ss")
                                .appendFraction(java.time.temporal.ChronoField.NANO_OF_SECOND, 0, 9, true)
                                .appendPattern(" XXX")
                                .toFormatter();
                        zonedDateTime = ZonedDateTime.parse(timestampStr, formatter);
                    }
                    catch (Exception e1) {
                        try {
                            // Try with Timestamp and assume UTC (fallback)
                            Timestamp timestamp = resultSet.getTimestamp(columnIndex);
                            zonedDateTime = timestamp.toInstant().atZone(ZoneId.of("UTC"));
                        }
                        catch (Exception e2) {
                            throw new SQLException("Failed to parse timestamp with time zone: " + timestampStr, e1);
                        }
                    }
...
//    }
    ...
    public static LongWriteFunction oracleTimestampWithTimeZoneWriteFunction()
    {
        return new LongWriteFunction()
        {
            ...
            public String getBindExpression()
            {
                // Use TO_TIMESTAMP_TZ with format string to handle timezone properly
                // TZH:TZM format accepts +08:00 or +0800 format
                return "TO_TIMESTAMP_TZ(?, 'YYYY-MM-DD HH24:MI:SS.FF TZH:TZM')";
            }

            @Override
            public void set(PreparedStatement statement, int index, long encodedTimeWithZone)
                    throws SQLException
            {
                Instant time = Instant.ofEpochMilli(unpackMillisUtc(encodedTimeWithZone));
                ZoneId zone = unpackZoneKey(encodedTimeWithZone).getZoneId();
                ...
                // Format as: "2025-01-01 12:34:56.123456 +08:00"
                // Use XXX pattern which generates +08:00 format (with colon) compatible with TZH:TZM
                DateTimeFormatter formatter = new DateTimeFormatterBuilder()
                        .appendPattern("yyyy-MM-dd HH:mm:ss")
                        .appendFraction(java.time.temporal.ChronoField.NANO_OF_SECOND, 0, 9, true)
                        .appendPattern(" XXX")
...
//    }
    ...
    private SliceWriteFunction oracleCharWriteFunction()
    {
        return SliceWriteFunction.of(Types.CHAR, (statement, index, value) ->
                statement.setObject(index, value.toStringUtf8(), Types.CHAR));
    }
