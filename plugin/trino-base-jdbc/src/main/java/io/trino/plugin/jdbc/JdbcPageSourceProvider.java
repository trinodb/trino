        ...
        implements ConnectorPageSourceProvider
{
            ...
            DynamicFilter dynamicFilter)
    {
        ...
        if (table instanceof JdbcProcedureHandle procedureHandle) {
            Map<JdbcColumnHandle, Integer> columnIndexMap = IntStream.range(0, jdbcColumns.size())
                    .boxed()
                    .collect(toImmutableMap(jdbcColumns::get, identity()));

            return new MappedPageSource(
                    createPageSource(session, jdbcSplit, procedureHandle, jdbcColumns),
                    columns.stream()
                            .map(JdbcColumnHandle.class::cast)
