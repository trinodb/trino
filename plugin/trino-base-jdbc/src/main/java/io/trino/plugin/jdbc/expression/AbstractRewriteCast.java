        ...
        implements ProjectFunctionRule<JdbcExpression, ParameterizedExpression>
{
    ...
    protected abstract boolean pushdownSupported(JdbcTypeHandle sourceType, Type targetType);
    ...
    @Override
    public Optional<JdbcExpression> rewrite(ConnectorExpression castExpression, Captures captures, RewriteContext<ParameterizedExpression> context)
    {
        ...
        if (!pushdownSupported(sourceTypeJdbcHandle, targetType)) {
        ...
        Optional<JdbcTypeHandle> targetTypeJdbcHandle = toJdbcTypeHandle(targetType);
        if (targetTypeJdbcHandle.isEmpty()) {
            return Optional.empty();
        }
