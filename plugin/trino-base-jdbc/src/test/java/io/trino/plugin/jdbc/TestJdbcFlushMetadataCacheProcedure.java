        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testFlushMetadataCacheProcedureFlushMetadata()
    {
        ...
        assertUpdate("CREATE TABLE cached.cached AS SELECT * FROM \"nation\"", 25);
        ...
        getQueryRunner().execute("CALL \"system\".\"flush_metadata_cache\"()");
        assertQuery(showSchemasSql, "VALUES ('RENAMED'), ('information_schema'), ('PUBLIC'), ('tpch')");
    }
    ...
    public void testFlushMetadataCacheProcedureFlushIdentifierMapping()
    {
        ...
        // Should succeed. H2 will cache undelimited identifier mapping to uppercase
        assertQuerySucceeds("SELECT \"name\", \"regionkey\" FROM \"CACHED_NAME\"");
        assertQuerySucceeds("SELECT \"name\", \"regionkey\" FROM cached_name");

        // H2 read unquoted identifier as uppercase. So this query should fail
        assertThatThrownBy(() -> h2SqlExecutor.execute("SELECT * FROM \"cached_name\""))
                .hasRootCauseMessage("Table \"cached_name\" not found (candidates are: \"CACHED_NAME\"); SQL statement:\n" +
                        "SELECT * FROM \"cached_name\" [42103-240]");
        // H2 stores unquoted names as uppercase. So this query should succeed
        h2SqlExecutor.execute("SELECT * FROM cached_name");

        String showTablesSql = "SHOW TABLES FROM public";
        assertQuery(showTablesSql, "VALUES ('nation'), ('CACHED_NAME')");

        // Rename to lowercase name outside Trino
        h2SqlExecutor.execute("ALTER TABLE cached_name RENAME TO renamed_name");

        // Should still return old tables from cache
        assertQuery(showTablesSql, "VALUES ('nation'), ('CACHED_NAME')");

        // Should succeed after flushing Trino cache
        getQueryRunner().execute(getSession(), "CALL \"system\".\"flush_metadata_cache\"()");

        assertQuery(showTablesSql, "VALUES ('nation'), ('RENAMED_NAME')");
    }
}
