        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        for (int i = 0; i < nameVariants.length; i++) {
            for (int j = i + 1; j < nameVariants.length; j++) {
                try (AutoCloseable ignore1 = withTable( schema + "." + nameVariants[i], "(c varchar(5))");
                        AutoCloseable ignore2 = withTable( schema + "." + nameVariants[j], "(d varchar(5))");
                        AutoCloseable ignore3 = withTable(schema + "." + "some_table", "(d varchar(5))")) {
            ...
            throws Exception
    {
        ...
        for (int i = 0; i < nameVariants.length; i++) {
            for (int j = i + 1; j < nameVariants.length; j++) {
                ...
                try (AutoCloseable ignore1 = withTable(schema + "." + remoteTable, "(c varchar(5))");
