        ...
        extends TrinoSeekableDataInputStream
{
    ...
    public InlineSeekableDataInputStream(TrinoInputStream stream, long startOffset, long length)
            throws IOException
    {
        super(bounded(stream, startOffset, length));
        this.startOffset = startOffset;
        this.length = length;
        stream.seek(startOffset);
    }

    private static TrinoInputStream bounded(TrinoInputStream stream, long startOffset, long length)
    {
        if (startOffset < 0) {
        ...
        return new BoundedTrinoInputStream(stream, startOffset, length);
    }
    ...
    /**
     * Wraps a {@link TrinoInputStream} and enforces a read/skip boundary at {@code startOffset + length}.
     * This is necessary because {@link java.io.DataInputStream#read(byte[], int, int)} is {@code final}
     * and delegates directly to the wrapped stream, so bounds enforcement must happen at the stream level.
     */
    private static final class BoundedTrinoInputStream
            extends TrinoInputStream
    {
        private final TrinoInputStream delegate;
        private final long end;

        BoundedTrinoInputStream(TrinoInputStream delegate, long startOffset, long length)
        {
        ...
        public int read(byte[] b, int off, int len)
                throws IOException
        {
            long remaining = end - delegate.getPosition();
            if (remaining <= 0) {
                return -1;
            }
            return delegate.read(b, off, (int) Math.min(len, remaining));
        }
        ...
        {
            long remaining = end - delegate.getPosition();
            return delegate.skip(Math.min(n, remaining));
        }
