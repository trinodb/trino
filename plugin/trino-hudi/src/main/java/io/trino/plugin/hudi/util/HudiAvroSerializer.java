...
public class HudiAvroSerializer
{
    ...
    /**
     * Maps each channel index to its Avro schema field index, or -1 if the column is hidden
     * (hidden columns are excluded from the schema and must be skipped during serialization).
     */
    private final int[] channelToSchemaFieldIndex;
    ...
    public HudiAvroSerializer(List<HiveColumnHandle> columnHandles)
    {
        ...
        // Build schema only from non-hidden columns. constructSchema fails on an empty list,
        // so keep schema null when every column is hidden.
        ...
        // Pre-compute channel to schema field index mapping so serialize() uses the correct index.
        int[] mapping = new int[columnHandles.size()];
        int fieldIdx = 0;
        for (int i = 0; i < columnHandles.size(); i++) {
            mapping[i] = columnHandles.get(i).isHidden() ? -1 : fieldIdx++;
        }
        this.channelToSchemaFieldIndex = mapping;
    }
    ...
    public IndexedRecord serialize(SourcePage sourcePage, int position)
    {
        if (schema == null) {
            throw new TrinoException(GENERIC_INTERNAL_ERROR,
                    "Cannot serialize: no non-hidden columns available to build an Avro schema");
        }
        IndexedRecord record = new GenericData.Record(schema);
        for (int i = 0; i < columnTypes.size(); i++) {
            int fieldIdx = channelToSchemaFieldIndex[i];
            if (fieldIdx < 0) {
                continue; // hidden column, not part of the schema
            }
            record.put(fieldIdx, getValue(sourcePage, i, position));
        }
        return record;
    }
        ...
        switch (type) {
            case VarcharType _ -> {
                if (value instanceof Utf8 utf8) {
                    type.writeSlice(output, utf8Slice(utf8.toString()));
                }
                else if (value instanceof String str) {
                    type.writeSlice(output, utf8Slice(str));
                }
                else {
            ...
            case VarbinaryType _ -> {
                if (value instanceof ByteBuffer byteBuffer) {
                    type.writeSlice(output, Slices.wrappedHeapBuffer(byteBuffer));
                }
