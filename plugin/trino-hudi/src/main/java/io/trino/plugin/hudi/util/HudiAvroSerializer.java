...
public class HudiAvroSerializer
{
    ...
    public HudiAvroSerializer(List<HiveColumnHandle> columnHandles, SynthesizedColumnHandler synthesizedColumnHandler)
    {
        ...
        this.schema = constructSchema(
                columnHandles.stream().filter(column -> !column.isHidden()).map(HiveColumnHandle::getName).toList(),
                columnHandles.stream().filter(column -> !column.isHidden()).map(HiveColumnHandle::getHiveType).toList(),
                false);
    ...
    public void appendTo(Type type, Object value, BlockBuilder output)
    {
        ...
        try {
            if (javaType == boolean.class) {
                ...
                writeRow((RowBlockBuilder) output, (GenericRecord) value, rowType);
            }
            else if (type instanceof MapType mapType) {
                writeMap((MapBlockBuilder) output, (Map<?, ?>) value, mapType);
            }
            else {
                throw new TrinoException(GENERIC_INTERNAL_ERROR, format("Unhandled type for %s: %s", javaType.getSimpleName(), type));
            }
    ...
    private void writeRow(RowBlockBuilder output, GenericRecord record, RowType rowType)
    {
    ...
    private void writeMap(MapBlockBuilder output, Map<?, ?> value, MapType mapType)
    {
