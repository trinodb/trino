...
{
    private static final Logger log = Logger.get(HudiPartitionStatsIndexSupport.class);
        ...
        Stream<String> prunedPartitions = allPartitions.stream()
                .filter(partition -> {
                    ...
                    if (partitionDomainsWithStats == null) {
                        ...
                        // i.e. to not prune
                        return TupleDomainUtils.evaluateStatisticPredicate(filteredRegularPredicates, partitionDomainsWithStats, regularColumns);
                    }
                });
