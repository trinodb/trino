...
/**
 * Defines the stock_ticks_mor test table - a partitioned Merge-on-Read table with stock ticker data.
 */
public class StockTicksMorTableDefinition
        extends HudiTableDefinition
{
    private static final String TABLE_NAME = "stock_ticks_mor";
    private static final String RECORD_KEY_FIELD = "key";
    private static final String PRE_COMBINE_FIELD = "ts";
    private static final int RECORD_COUNT = 99;

    public StockTicksMorTableDefinition()
    {
        super(
                TABLE_NAME,
                MERGE_ON_READ,
    ...
    public void executeCommits(HoodieJavaWriteClient<HoodieAvroPayload> client)
    {
        ...
        // For MOR tables, use bulkInsert to create base parquet files directly
        ...
        for (int i = 0; i < RECORD_COUNT; i++) {
            ...
            updateRecords.add(recordFactory.createRecord(
                    ...
                    "2018/08/31",
                    fieldValues()
                            .longField("volume", 2000L + (i * 100L))  // Updated volume
