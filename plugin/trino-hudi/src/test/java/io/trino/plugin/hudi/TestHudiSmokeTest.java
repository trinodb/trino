        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testReadPartitionedMORTables()
    {
        getQueryRunner().execute(getSession(), "SET SESSION hudi.metadata_enabled=true");
        assertQuery(
                "SELECT count(*), count(DISTINCT symbol), sum(volume) FROM " + HUDI_STOCK_TICKS_MOR,
                "VALUES (99, 99, 825295)");
        assertQuery(
                "SELECT key, volume, close FROM " + HUDI_STOCK_TICKS_MOR + " WHERE symbol IN ('AAPL', 'AMD')",
                "VALUES ('AAPL_2018-08-31 10', 38710, 228.04), ('AMD_2018-08-31 10', 155370, 25.045)");
    }
