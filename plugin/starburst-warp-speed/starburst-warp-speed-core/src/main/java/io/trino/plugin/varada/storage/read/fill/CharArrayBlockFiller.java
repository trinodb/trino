...
public class CharArrayBlockFiller
        extends ArraySliceFiller
{
    public CharArrayBlockFiller(StorageEngineConstants storageEngineConstants)
    {
        super(storageEngineConstants, BlockFillerType.ARRAY_CHAR, CharType.createCharType(10));
    }
    ...
    protected Block getBlock(Slice slice, int offset, int length)
    {
        ...
        for (int i = 0; i < numberOfElements; i++) {
            if (!valueIsNull[i]) {
                byte[] bytes = sliceConverter.apply(slice.slice(nativePositions[i], valueLengths[i])).getBytes();
    ...
    protected Block getBlock(ByteBufferInputStream byteBufferInputStream, int offset, int length)
    {
        ...
        for (int i = 0; i < numberOfElements; i++) {
            if (!valueIsNull[i]) {
                byte[] bytes = sliceConverter.apply(Slices.wrappedBuffer(byteBufferInputStream.readBytes(nativePositions[i], valueLengths[i]))).getBytes();
                outputSlice.setBytes(outputOffsets[i], bytes);
            }
        }
