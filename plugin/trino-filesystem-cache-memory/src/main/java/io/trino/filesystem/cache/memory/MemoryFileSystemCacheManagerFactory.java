...
public class MemoryFileSystemCacheManagerFactory
        implements FileSystemCacheManagerFactory
{
    ...
    public FileSystemCacheManager create(Map<String, String> config, CacheManagerContext context)
    {
        Module module = (Binder binder) -> {
            configBinder(binder).bindConfig(MemoryFileSystemCacheConfig.class);
            binder.bind(MemoryFileSystemCache.class).in(Scopes.SINGLETON);
            binder.bind(MemoryFileSystemCacheManager.class).in(Scopes.SINGLETON);
        };
        ...
        return injector.getInstance(MemoryFileSystemCacheManager.class);
    }
}
