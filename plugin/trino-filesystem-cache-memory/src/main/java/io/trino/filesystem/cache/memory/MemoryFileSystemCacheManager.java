 ...
 */
package io.trino.filesystem.cache.memory;
...
public class MemoryFileSystemCacheManager
        implements FileSystemCacheManager
{
    private final MemoryFileSystemCache sharedCache;
    private final Set<CatalogName> catalogs = ConcurrentHashMap.newKeySet();

    @Inject
    public MemoryFileSystemCacheManager(MemoryFileSystemCache sharedCache)
    {
    ...
    public ConnectorFileSystemCache createFileSystemCache(CatalogName catalog, Duration ttl)
    {
        ...
        return new CatalogScopedMemoryFileSystemCache(sharedCache, catalog);
    }

    @Override
    public void invalidate(CatalogName catalog)
    {
        sharedCache.expireCatalog(catalog);
    }
    ...
    private record CatalogScopedMemoryFileSystemCache(MemoryFileSystemCache delegate, CatalogName catalogName)
            implements ConnectorFileSystemCache
    {
        ...
        public TrinoInput cacheInput(TrinoInputFile file, String key)
                throws IOException
        {
            return delegate.cacheInput(file, catalogName.toString() + "/" + key);
        }
        ...
        public TrinoInputStream cacheStream(TrinoInputFile file, String key)
                throws IOException
        {
            return delegate.cacheStream(file, catalogName.toString() + "/" + key);
        }
        ...
        public long cacheLength(TrinoInputFile file, String key)
                throws IOException
        {
            return delegate.cacheLength(file, catalogName.toString() + "/" + key);
        }
