...
public class TestBARBRecordSet
{
    private BARBHttpServer barbHttpServer;
    private String dataUri;

    @Test
    public void testGetColumnTypes()
    {
        RecordSet recordSet = new BARBRecordSet(new BARBSplit(dataUri), ImmutableList.of(
                new BARBColumnHandle("text", createUnboundedVarcharType(), 0),
                new BARBColumnHandle("value", BIGINT, 1)));
        assertEquals(recordSet.getColumnTypes(), ImmutableList.of(createUnboundedVarcharType(), BIGINT));

        recordSet = new BARBRecordSet(new BARBSplit(dataUri), ImmutableList.of(
                new BARBColumnHandle("value", BIGINT, 1),
                new BARBColumnHandle("text", createUnboundedVarcharType(), 0)));
    ...
    public void setUp()
    {
        barbHttpServer = new BARBHttpServer();
        dataUri = barbHttpServer.resolve("/example-data/numbers-2.csv").toString();
    }

    @AfterClass(alwaysRun = true)
    public void tearDown()
    {
        if (barbHttpServer != null) {
            barbHttpServer.stop();
        }
    }
}
