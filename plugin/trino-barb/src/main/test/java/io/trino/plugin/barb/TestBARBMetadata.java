...
public class TestBARBMetadata
{
    private static final BARBTableHandle NUMBERS_TABLE_HANDLE = new BARBTableHandle("example", "numbers");
    private BARBMetadata metadata;
            ...
            throws Exception
    {
        ...
        BARBClient client = new BARBClient(new BARBConfig().setMetadata(metadataUrl.toURI()), CATALOG_CODEC);
        metadata = new BARBMetadata(client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new BARBColumnHandle("text", createUnboundedVarcharType(), 0),
                "value", new BARBColumnHandle("value", BIGINT, 1)));

        // unknown table
        assertThatThrownBy(() -> metadata.getColumnHandles(SESSION, new BARBTableHandle("unknown", "unknown")))
                .isInstanceOf(TableNotFoundException.class)
                .hasMessage("Table 'unknown.unknown' not found");
        assertThatThrownBy(() -> metadata.getColumnHandles(SESSION, new BARBTableHandle("example", "unknown")))
                .isInstanceOf(TableNotFoundException.class)
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new BARBTableHandle("unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new BARBTableHandle("example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new BARBTableHandle("unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new BARBColumnHandle("text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
