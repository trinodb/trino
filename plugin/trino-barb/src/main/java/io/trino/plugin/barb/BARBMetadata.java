...
public class BARBMetadata
        implements ConnectorMetadata
{
    private final BARBClient exampleClient;

    @Inject
    public BARBMetadata(BARBClient exampleClient)
    {
    ...
    public BARBTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName)
    {
        ...
        BARBTable table = exampleClient.getTable(tableName.getSchemaName(), tableName.getTableName());
        if (table == null) {
            return null;
        }
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        BARBTable table = exampleClient.getTable(exampleTableHandle.getSchemaName(), exampleTableHandle.getTableName());
        if (table == null) {
        ...
        for (ColumnMetadata column : table.getColumnsMetadata()) {
            columnHandles.put(column.getName(), new BARBColumnHandle(column.getName(), column.getType(), index));
            index++;
        }
    ...
    {
        return ((BARBColumnHandle) columnHandle).getColumnMetadata();
    }
}
