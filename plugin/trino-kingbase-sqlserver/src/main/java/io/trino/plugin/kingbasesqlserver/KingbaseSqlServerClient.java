...
import static io.trino.plugin.kingbasesqlserver.KingbaseSqlServerSessionProperties.isBulkCopyForWrite;
import static io.trino.plugin.kingbasesqlserver.KingbaseSqlServerSessionProperties.isBulkCopyForWriteLockDestinationTable;
import static io.trino.plugin.kingbasesqlserver.KingbaseSqlServerTableProperties.getDataCompression;
...
public class KingbaseSqlServerClient
        extends BaseJdbcClient
{
    private static final Logger log = Logger.get(KingbaseSqlServerClient.class);
...
//    private static final DateTimeOffset GREGORIAN_SWITCH_DATETIMEOFFSET = DateTimeOffset.valueOf(new Timestamp(GREGORIAN_SWITCH_INSTANT.toEpochMilli()), 0);
    ...
    @Inject
    public KingbaseSqlServerClient(
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, ParameterizedExpression>>builder()
                        ...
                        .add(new ImplementSum(KingbaseSqlServerClient::toTypeHandle))
            ...
            throws SQLException
    {
        ...
        // Build full table name: handle null catalog
        String fullTableFromName;
        if (catalogName != null) {
            fullTableFromName = DOT_JOINER.join(
                    quoted(catalogName),
        ...
        try (PreparedStatement preparedStatement = queryBuilder.prepareStatement(this, session, connection, preparedQuery, Optional.empty())) {
            ...
            if (message != null && (
                    message.contains("does not exist") ||
                            ...
                            message.contains("relation does not exist") ||
    ...
    public Optional<ColumnMapping> toColumnMapping(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        switch (jdbcTypeName) {
            ...
            case "ntext":
            {
                boolean caseSensitive = typeHandle.caseSensitivity()
                        .map(sensitivity -> sensitivity == CaseSensitivity.CASE_SENSITIVE)
                        .orElse(false);
                return Optional.of(varcharColumnMapping(1_073_741_823, caseSensitive));
            }
        }
        ...
        try (Connection connection = connectionFactory.openConnection(session);
            ...
            for (JdbcColumnHandle column : JdbcMetadata.getColumns(session, this, table)) {
                ...
                try (CallableStatement showStatistics = handle.getConnection().prepareCall("DBCC SHOW_STATISTICS (?, ?)")) {
                    // Build table name for DBCC SHOW_STATISTICS: handle null catalog/schema
                    String tableIdentifier;
                    if (catalog != null && schema != null) {
                        tableIdentifier = format("%s.%s.%s", catalog, schema, tableName);
                    }
                    else if (schema != null) {
                        tableIdentifier = format("%s.%s", schema, tableName);
                    }
                    else {
                        // If both catalog and schema are null, use table name only
                        tableIdentifier = tableName;
                    }
                    showStatistics.setString(1, tableIdentifier);
    ...
    private static LongReadFunction shortTimestampWithTimeZoneReadFunction()
    {
        return (resultSet, columnIndex) -> {
            ...
            return packDateTimeWithZone(instant.toEpochMilli(), offsetDateTime.getOffset().getId());
        };
    }
    ...
    {
        // 直接返回空，不查 sys.partition_schemes 等 Kingbase 不存在的系统表
    ...
    public Connection getConnection(ConnectorSession session, JdbcOutputTableHandle handle)
            throws SQLException
    {
        return super.getConnection(session, handle);
    }
    ...
    public static <T> T retryOnDeadlock(CheckedSupplier<T> supplier, String attemptLogMessage)
    {
        ...
        RetryPolicy<T> retryPolicy = RetryPolicy.<T>builder()
                ...
                .onFailedAttempt(event -> log.warn(event.getLastException(),
                        "Attempt %d of %d: %s", event.getAttemptCount(), maxAttemptCount, attemptLogMessage))
                .build();
    ...
    private static class StatisticsDao
    {
        ...
        {
            // Build table name for object_id(): handle null catalog/schema
            String tableIdentifier;
            if (catalog != null && schema != null) {
                tableIdentifier = format("%s.%s.%s", catalog, schema, tableName);
            }
                ...
                // If both catalog and schema are null, use table name only
                // This may not work in all cases, but matches the original behavior
                tableIdentifier = tableName;
            }
            return handle.createQuery("SELECT object_id(:table)")
                    .bind("table", tableIdentifier)
                    .mapTo(Long.class)
                    .one();
        }
