        ...
        extends AbstractTestQueryFramework
{
    ...
    {
        try (TestTable table = createTable("comment", "(id INTEGER, name VARCHAR)")) {
    ...
    {
        try (TestTable table = createTable("cannot_comment", "(id INTEGER, name VARCHAR)")) {
    ...
    {
        try (TestTable table = createTable("single_column", "(rnd_bigint bigint NOT NULL)")) {
    ...
    {
        try (TestTable table = createTable("default_table_limit", "(rnd_bigint bigint NOT NULL) WITH (default_limit = 100)")) {
    ...
    {
        try (TestTable table = createTable("only_nulls", "(rnd_bigint bigint) WITH (null_probability = 1.0)")) {
    ...
    {
        try (TestTable table = createTable("generators",
                """
                (
