...
public class TestDatabaseShardManager
{
    ...
    public void testReplaceShardUuids()
    {
        ...
        assertTrinoExceptionThrownBy(() -> {
            long finalTransactionId = shardManager.beginTransaction();
            shardManager.replaceShardUuids(finalTransactionId, tableId, columns, replacedUuids, finalNewShards, OptionalLong.of(0));
        })
                .hasErrorCode(TRANSACTION_CONFLICT)
    ...
    public void testExternalBatches()
    {
        ...
        assertTrinoExceptionThrownBy(() -> {
            long finalTransactionId = shardManager.beginTransaction();
            shardManager.commitShards(finalTransactionId, tableId, columns, finalShards, externalBatchId, 0);
        })
