        ...
        implements DbSpecsProvider
{
    ...
    @Inject
    public RefreshingDbSpecsProvider(DbSessionPropertyManagerConfig config, SessionPropertiesDao dao, FlywayMigration flywayMigration)
    {
        this.dao = requireNonNull(dao, "dao is null");
        this.flywayMigration = requireNonNull(flywayMigration, "flywayMigration is null");
    ...
    public void initialize()
    {
        if (!started.getAndSet(true)) {
            // Migrate before the first load so the schema exists, then load once so the configuration is available
            // before any query is served, and only then start the periodic refresh.
            flywayMigration.migrate();
            refresh();
            executor.scheduleWithFixedDelay(this::refresh, refreshIntervalMillis, refreshIntervalMillis, TimeUnit.MILLISECONDS);
        }
    }
    ...
    void refresh()
    {
        try {
            ...
            // suppresses all future scheduled invocations. Staleness is enforced in get() by comparing the age of
            // the last successful load against maxRefreshInterval, so it holds even if a refresh hangs here.
    ...
    public List<SessionMatchSpec> get()
    {
        ...
        // last == 0 means no successful load yet; otherwise fail once the cached config is older than
        // maxRefreshInterval. Checking here (rather than only in refresh()) enforces staleness even if a refresh
        // hangs and never throws.
        if (last == 0 || succinctNanos(System.nanoTime() - last).compareTo(maxRefreshInterval) > 0) {
