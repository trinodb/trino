...
public interface SessionPropertiesDao
{
     ...
     * <p>
     * The three queries run in a single repeatable-read transaction so they observe one consistent snapshot; without
     * it a concurrent write could, for example, surface a spec before its client-tag rows and apply a rule without
     * the tags meant to constrain it.
     */
    @Transaction(value = TransactionIsolationLevel.REPEATABLE_READ, readOnly = true)
