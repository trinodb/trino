...
import static io.trino.plugin.databend.DatabendClient.DEFAULT_DOMAIN_COMPACTION_THRESHOLD;
import static io.trino.plugin.jdbc.JdbcModule.bindTablePropertiesProvider;

public class DatabendClientModule
        implements Module
{
    @Override
    public void configure(Binder binder)
    {
        ...
        configBinder(binder).bindConfig(DatabendJdbcConfig.class);
    ...
    @Singleton
    @ForBaseJdbc
    public static ConnectionFactory createConnectionFactory(BaseJdbcConfig config, CredentialProvider credentialProvider, DatabendConfig databendConfig, OpenTelemetry openTelemetry)
            throws SQLException
    {
        return DriverConnectionFactory.builder(new DatabendDriver(), config.getConnectionUrl(), credentialProvider)
                .setConnectionProperties(getConnectionProperties(databendConfig))
                .setOpenTelemetry(openTelemetry)
                .build();
    }

    // custom databend config can be added here
    private static Properties getConnectionProperties(DatabendConfig databendConfig)
    {
