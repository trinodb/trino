...
public final class HsqlDbQueryRunner
{
    ...
    // FIXME: If we want to pass the TestHsqlDbCaseInsensitiveMapping test,
    // FIXME: then it is imperative to specify a schema in uppercase characters.
    private static final String TEST_SCHEMA = "public";

    public static Builder builder(TestingHsqlDbServer hsqlDbServer)
    {
        return new Builder(hsqlDbServer)
    ...
    {
        private final TestingHsqlDbServer server;
        private final Map<String, String> connectorProperties = new HashMap<>();
        private List<TpchTable<?>> initialTables = ImmutableList.of();

        private Builder(TestingHsqlDbServer server)
        {
                ...
                throws Exception
        {
            ...
            try {
                ...
                copyTpchTables(server, queryRunner, "tpch", TINY_SCHEMA_NAME, initialTables);
            ...
            throws SQLException
    {
        try (Connection connection = server.getConnection();
            ...
            for (TpchTable<?> table : initialTables) {
                ...
                String tpchTableName = format("%s.%s.%s", sourceCatalog, sourceSchema, tableName);
                ...
                assertThat(queryRunner.execute(format("SELECT count(*) FROM %s.%s.%s", CATALOG, TEST_SCHEMA, tableName)).getOnlyValue())
