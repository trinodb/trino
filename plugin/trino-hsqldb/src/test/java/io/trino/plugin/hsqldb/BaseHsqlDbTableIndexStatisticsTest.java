...
public abstract class BaseHsqlDbTableIndexStatisticsTest
        extends BaseHsqlDbTableStatisticsTest
{
    protected BaseHsqlDbTableIndexStatisticsTest(String dockerImageName)
    {
        super(
                dockerImageName,
    ...
    protected void gatherStats(String tableName)
    {
        for (MaterializedRow row : computeActual("SHOW COLUMNS FROM " + tableName)) {
            ...
            executeInHsqlDb(format("CREATE INDEX %2$s ON %1$s (%2$s)", tableName, columnName));
        }
        executeInHsqlDb("ANALYZE TABLE " + tableName);
    }
