        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        return HsqlDbQueryRunner.builder(hsqldbServer).build();
    }
    ...
    void testBoolean()
    {
        // FIXME: It appears that HsqlDB does not support the value "UNKNOWN" and only the value "NULL".
    ...
    void testIntegerMappings()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("numeric", "0", createDecimalType(38, 0), "CAST(0 AS DECIMAL(38, 0))")
                .addRoundTrip("dec", "0", createDecimalType(38, 0), "CAST(0 AS DECIMAL(38, 0))")
                .addRoundTrip("bigint", "0", BIGINT, "CAST(0 AS BIGINT)")
                .execute(getQueryRunner(), hsqldbCreateAndInsert("integers"));
    }
    ...
    private void testNumberMappingsNativeQuery(String hsqldbType, String expectedType)
    {
        // FIXME: HsqlDB does not support SELECT queries without a FROM clause.
        // FIXME: The table INFORMATION_SCHEMA.SYSTEM_USERS is used as a dummy table (ie: DUAL table).
        assertQuery("SELECT * FROM TABLE(system.query(query => 'SELECT CAST(1 AS " + hsqldbType + ") col FROM INFORMATION_SCHEMA.SYSTEM_USERS'))",
    ...
    void testDouble()
    {
        SqlDataTypeTest.create()
                ...
                .execute(getQueryRunner(), hsqldbCreateAndInsert("hsqldb_test_double"));
    }
    ...
    void testChar()
    {
        SqlDataTypeTest.create()
                ...
                .execute(getQueryRunner(), hsqldbCreateAndInsert("test_char"));
    }
    ...
    void testVarchar()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("varchar(2)", "'😂'", createVarcharType(2), "CAST('😂' AS varchar(2))") // varchar with a length of 2 is the minimum here
                .addRoundTrip("varchar(77)", "'Ну, погоди!'", createVarcharType(77), "CAST('Ну, погоди!' AS varchar(77))")
                .addRoundTrip("varchar(32768)", "'text_f'", createVarcharType(32768), "CAST('text_f' AS varchar(32768))") // max varchar length in HsqlDB
    ...
    void testDate()
    {
        testDate(UTC);
        testDate(jvmZone);
        // using two non-JVM zones so that we don't need to worry what HsqlDB system zone is
        testDate(vilnius);
        testDate(kathmandu);
    ...
    private void testDate(ZoneId sessionZone)
    {
        ...
        SqlDataTypeTest.create()
                ...
                // .addRoundTrip("date", "DATE '0001-01-01'", DATE, "DATE '0001-01-01'") // min value in HsqlDB
                ...
                // FIXME: Cant run this test with HsqlDB
                ...
                .addRoundTrip("date", "DATE '9999-12-31'", DATE, "DATE '9999-12-31'") // max value in HsqlDB
    ...
    void testUnsupportedType()
    {
        ...
        assertThatThrownBy(this::setUnsupportedType)
                ...
                .hasMessageMatching("java.sql.SQLSyntaxErrorException: type not found or user lacks privilege: GEOMETRY");
    }
    ...
    {
        return new CreateAndInsertDataSetup(hsqldbServer.getSqlExecutor(), tableNamePrefix, this::canonicalize);
    }
