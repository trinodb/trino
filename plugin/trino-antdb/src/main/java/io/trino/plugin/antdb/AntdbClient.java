...
import io.trino.plugin.antdb.AntdbConfig.ArrayMapping;
...
import static io.trino.plugin.antdb.AntdbConfig.ArrayMapping.AS_ARRAY;
import static io.trino.plugin.antdb.AntdbConfig.ArrayMapping.AS_JSON;
import static io.trino.plugin.antdb.AntdbConfig.ArrayMapping.DISABLED;
import static io.trino.plugin.antdb.AntdbSessionProperties.getArrayMapping;
import static io.trino.plugin.antdb.AntdbSessionProperties.isEnableStringPushdownWithCollate;
import static io.trino.plugin.antdb.TypeUtils.arrayDepth;
import static io.trino.plugin.antdb.TypeUtils.getArrayElementPgTypeName;
import static io.trino.plugin.antdb.TypeUtils.getJdbcObjectArray;
import static io.trino.plugin.antdb.TypeUtils.toPgTimestamp;
...
public class AntdbClient
        extends BaseJdbcClient
{
    private static final Logger log = Logger.get(AntdbClient.class);
    ...
    private static final int ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION = 6;
    private static final int PRECISION_OF_UNSPECIFIED_DECIMAL = 0;

    private static final DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("HH:mm:ss.SSSSSS");

    private static final PredicatePushdownController ANTDB_STRING_COLLATION_AWARE_PUSHDOWN = (session, domain) -> {
    ...
    private static final Map<Integer, Integer> DATA_TYPE_MAP = ImmutableMap.<Integer, Integer>builder()
            .put(2001, 91)
            .buildOrThrow();

    private static final Map<String, String> TYPE_NAME_MAP = ImmutableMap.<String, String>builder()
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        if (antdbConfig.isIncludeSystemTables()) {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, String>>builder()
                        ...
                        .add(new ImplementSum(AntdbClient::toTypeHandle))
            ...
            try (ResultSet resultSet = getColumns(tableHandle, connection.getMetaData())) {
                ...
                while (resultSet.next()) {
                    ...
                    if ("varchar2".equals(typeName)) {
                        typeName = TYPE_NAME_MAP.get(typeName);
                    }
                    int jdbcType = getInteger(resultSet, "DATA_TYPE").orElseThrow(() -> new IllegalStateException("DATA_TYPE is null"));
                    String columnName = resultSet.getString("COLUMN_NAME");
                    String[] typeNames = typeName.split("\\.");

                    if (typeNames.length > 1) {
                        typeName = typeNames[1];
                        jdbcType = DATA_TYPE_MAP.getOrDefault(jdbcType, jdbcType);
                    }
        ...
        switch (typeHandle.getJdbcType()) {
            ...
            case Types.TIMESTAMP:
                ...
                return Optional.of(ColumnMapping.longMapping(
                        ...
                        AntdbClient::shortTimestampWriteFunction));
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type instanceof TimeType timeType) {
            if (timeType.getPrecision() <= ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                return WriteMapping.longMapping(format("time(%s)", timeType.getPrecision()), timeWriteFunction(timeType.getPrecision()));
            }
            return WriteMapping.longMapping(format("time(%s)", ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), timeWriteFunction(ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION));
        }

        if (type instanceof TimestampType timestampType) {
            if (timestampType.getPrecision() <= ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                verify(timestampType.getPrecision() <= TimestampType.MAX_SHORT_PRECISION);
                return WriteMapping.longMapping(format("timestamp(%s)", timestampType.getPrecision()), AntdbClient::shortTimestampWriteFunction);
            }
            verify(timestampType.getPrecision() > TimestampType.MAX_SHORT_PRECISION);
            return WriteMapping.objectMapping(format("timestamp(%s)", ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWriteFunction());
        }
        if (type instanceof TimestampWithTimeZoneType timestampWithTimeZoneType) {
            if (timestampWithTimeZoneType.getPrecision() <= ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
            ...
            return WriteMapping.objectMapping(format("timestamptz(%d)", ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWithTimeZoneWriteFunction());
        }
    ...
    private static ColumnMapping charColumnMapping(int charLength)
    {
        ...
        return ColumnMapping.sliceMapping(
                ...
                charWriteFunction(),
                ANTDB_STRING_COLLATION_AWARE_PUSHDOWN);
    }
    ...
    private static ObjectWriteFunction longTimestampWriteFunction()
    {
        return ObjectWriteFunction.of(LongTimestamp.class, ((statement, index, timestamp) -> {
            ...
            verify(ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION == 6);
    ...
    private static ColumnMapping timestampWithTimeZoneColumnMapping(int precision)
    {
        ...
        checkArgument(precision <= ANTDB_MAX_SUPPORTED_TIMESTAMP_PRECISION, "unsupported precision value %s", precision);
