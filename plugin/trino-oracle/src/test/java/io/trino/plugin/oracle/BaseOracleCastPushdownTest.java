        ...
        extends BaseJdbcCastPushdownTest
{
    ...
    public void testJoinPushdownWithNestedCast()
    {
        ...
        assertJoinFullyPushedDown(left.tableName(), right.tableName(), "JOIN", "CAST(CAST(l.%s AS %s) AS VARCHAR(50)) = r.%s".formatted(testCase.sourceColumn(), testCase.castType(), testCase.targetColumn().orElseThrow()));
    }

    @Test
    public void testAllJoinPushdownWithCast()
    {
        String leftTableName = left.tableName();
        String rightTableName = right.tableName();

        CastTestCase testCase = new CastTestCase("c_varchar_10", "varchar(50)", Optional.of("c_varchar_50"));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "LEFT JOIN", "CAST(l.%s AS %s) = r.%s".formatted(testCase.sourceColumn(), testCase.castType(), testCase.targetColumn().orElseThrow()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "RIGHT JOIN", "CAST(l.%s AS %s) = r.%s".formatted(testCase.sourceColumn(), testCase.castType(), testCase.targetColumn().orElseThrow()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "INNER JOIN", "CAST(l.%s AS %s) = r.%s".formatted(testCase.sourceColumn(), testCase.castType(), testCase.targetColumn().orElseThrow()));

        testCase = new CastTestCase("c_varchar_10", "varchar(10)", Optional.of("c_varchar_50"));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "LEFT JOIN", "l.%s = CAST(r.%s AS %s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "RIGHT JOIN", "l.%s = CAST(r.%s AS %s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "INNER JOIN", "l.%s = CAST(r.%s AS %s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));

        testCase = new CastTestCase("c_varchar_10", "varchar(200)", Optional.of("c_varchar_50"));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "LEFT JOIN", "CAST(l.%1$s AS %3$s) = CAST(r.%2$s AS %3$s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "RIGHT JOIN", "CAST(l.%1$s AS %3$s) = CAST(r.%2$s AS %3$s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));
        assertJoinFullyPushedDown(leftTableName, rightTableName, "INNER JOIN", "CAST(l.%1$s AS %3$s) = CAST(r.%2$s AS %3$s)".formatted(testCase.sourceColumn(), testCase.targetColumn().orElseThrow(), testCase.castType()));
    }
    ...
    public void testCastPushdownDisabled()
    {
        Session sessionWithoutComplexExpressionPushdown = Session.builder(getSession())
                ...
                .build();
        assertThat(query(sessionWithoutComplexExpressionPushdown, "SELECT CAST (c_varchar_10 AS VARCHAR(100)) FROM %s".formatted(left.tableName())))
    ...
    public void testCastPushdownWithForcedTypedToVarchar()
    {
        ...
        assertThat(query("SELECT CAST(c_interval_ym AS VARCHAR(100)) FROM %s".formatted(left.tableName())))
                .isNotFullyPushedDown(ProjectNode.class);
        assertThat(query("SELECT CAST(c_timestamp_ltz AS VARCHAR(100)) FROM %s".formatted(left.tableName())))
                .isNotFullyPushedDown(ProjectNode.class);
    }
