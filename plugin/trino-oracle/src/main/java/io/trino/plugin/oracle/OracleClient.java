        ...
        extends BaseJdbcClient
{
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.connectorExpressionRewriter = JdbcConnectorExpressionRewriterBuilder.newBuilder()
                ...
                .withTypeClass("exact_numeric_type", ImmutableSet.of("tinyint", "smallint", "integer", "bigint", "decimal"))
                .withTypeClass("approx_numeric_type", ImmutableSet.of("real", "double"))
                .map("$equal(left: exact_numeric_type, right: exact_numeric_type)").to("left = right")
                .map("$not_equal(left: exact_numeric_type, right: exact_numeric_type)").to("left <> right")
                .map("$equal(left: approx_numeric_type, right: approx_numeric_type)").to("left = right AND left <> 'NaN' AND right <> 'NaN'")
                .map("$not_equal(left: approx_numeric_type, right: approx_numeric_type)").to("left <> right OR (left = 'NaN' AND right = 'NaN')")
                .map("$less_than(left: exact_numeric_type, right: exact_numeric_type)").to("left < right")
                ...
                .map("$less_than_or_equal(left: approx_numeric_type, right: approx_numeric_type)").to("left <= right AND right <> 'NaN'")
                .map("$greater_than(left: approx_numeric_type, right: approx_numeric_type)").to("left > right AND left <> 'NaN'")
                .map("$greater_than_or_equal(left: approx_numeric_type, right: approx_numeric_type)").to("left >= right AND left <> 'NaN'")
