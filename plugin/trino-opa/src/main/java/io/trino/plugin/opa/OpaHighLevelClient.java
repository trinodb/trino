...
public class OpaHighLevelClient
{
    ...
    private static Map<String, String> loadAdditionalContextFromFile(Optional<File> additionalContextFile)
    {
        if (additionalContextFile.isEmpty()) {
            return ImmutableMap.of();
        }

        try (InputStream inputStream = new FileInputStream(additionalContextFile.get())) {
            Properties properties = new Properties();
            properties.load(inputStream);
            return properties.entrySet().stream()
                    .collect(toImmutableMap(p -> String.valueOf(p.getKey()), p -> String.valueOf(p.getValue())));
        }
        catch (IOException e) {
