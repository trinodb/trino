        ...
        extends AbstractTestQueryFramework
{
    ...
    private void testTimestampWithTimeZone(ZoneId sessionZone)
    {
        ...
        SqlDataTypeTest test = SqlDataTypeTest.create()
                ...
                //test cases for timestamp with zero precision and with non-zero seconds
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '2017-07-01 00:00:01'", createTimestampWithTimeZoneType(0), "TIMESTAMP '2017-07-01 05:00:01 UTC'") // summer on northern hemisphere (possible DST)
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '2017-01-01 00:00:02'", createTimestampWithTimeZoneType(0), "TIMESTAMP '2017-01-01 06:00:02 UTC'") // winter on northern hemisphere (possible DST on southern hemisphere)
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '1970-01-01 00:00:03'", createTimestampWithTimeZoneType(0), "TIMESTAMP '1970-01-01 07:00:03 UTC'") // change forward at midnight in JVM
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '1983-04-01 00:00:04'", createTimestampWithTimeZoneType(0), "TIMESTAMP '1983-04-01 07:00:04 UTC'") // change forward at midnight in Vilnius
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '1983-10-01 00:00:05'", createTimestampWithTimeZoneType(0), "TIMESTAMP '1983-10-01 07:00:05 UTC'") // change backward at midnight in Vilnius
                .addRoundTrip("timestamp(0) with local time zone", "TIMESTAMP '9999-12-31 00:00:59'", createTimestampWithTimeZoneType(0), "TIMESTAMP '9999-12-31 06:00:59 UTC'"); // max value in Exasol
    ...
    void testUnsupportedTimestampValues()
    {
        ...
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp", "TIMESTAMP '10000-01-01 00:00:00.000000'", createTimestampType(3), "TIMESTAMP '10000-01-01 00:00:00.000000'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp")))
                .cause().hasMessageStartingWith("data exception - invalid character value for cast; Value: '10000-01-01 00:00:00.000000'");

        // Above maximum supported TIMESTAMP value (must be <= 9999-12-31)
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp", "TIMESTAMP '0000-12-31 23:59:59.999999'", createTimestampType(3), "TIMESTAMP '0000-12-31 23:59:59.999999'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp")))
                .cause().hasMessageStartingWith("data exception - invalid date value; Value: '0000-12-31 23:59:59.999999'");

        // Exceeds TIMESTAMP maximum supported fractional seconds precision (9 digits)
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp", "TIMESTAMP '2024-01-01 12:34:56.1234567890'", createTimestampType(3), "TIMESTAMP '2024-01-01 12:34:56.1234567890'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp")))
                .cause().hasMessageStartingWith("data exception - invalid character value for cast; Value: '2024-01-01 12:34:56.1234567890'");

        // Negative precisions are not supported
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp(-1)", "TIMESTAMP '2024-01-01 12:34:56.1234567890'", createTimestampType(3), "TIMESTAMP '2024-01-01 12:34:56.1234567890'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp")))
                .cause().hasMessageStartingWith("syntax error, unexpected '-', expecting UNSIGNED_INTEGER");
    }
    ...
    void testUnsupportedTimestampWithLocalTimeZoneValues()
    {
        ...
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp with local time zone", "TIMESTAMP '10000-01-01 00:00:00.000000'", createTimestampWithTimeZoneType(3), "TIMESTAMP '10000-01-01 00:00:00.000000'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp_with_local_time_zone")))
                .cause().hasMessageStartingWith("data exception - invalid character value for cast; Value: '10000-01-01 00:00:00.000000'");

        // Above maximum supported TIMESTAMP WITH LOCAL TIME ZONE value (must be <= 9999-12-31)
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp with local time zone", "TIMESTAMP '0000-12-31 23:59:59.999999'", createTimestampWithTimeZoneType(3), "TIMESTAMP '0000-12-31 23:59:59.999999'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp_with_local_time_zone")))
                .cause().hasMessageStartingWith("data exception - invalid date value; Value: '0000-12-31 23:59:59.999999'");
        ...
        assertThatThrownBy(() ->
                SqlDataTypeTest.create()
                        .addRoundTrip("timestamp with local time zone", "TIMESTAMP '2024-01-01 12:34:56.1234567890'", createTimestampWithTimeZoneType(3), "TIMESTAMP '2024-01-01 12:34:56.1234567890'")
                        .execute(getQueryRunner(), exasolCreateAndInsert(TEST_SCHEMA + "." + "test_unsupported_timestamp_with_local_time_zone")))
                .cause().hasMessageStartingWith("data exception - invalid character value for cast; Value: '2024-01-01 12:34:56.1234567890'");
