        ...
        extends AbstractTestingTrinoClient<Void>
{
            ...
            String dataFormat)
    {
        ...
        this.redisClient = requireNonNull(redisClient, "redisClient is null");
            ...
            for (List<Object> fields : rows) {
                ...
                switch (dataFormat) {
                    case "string":
                        ...
                        redisClient.set(redisKey, jsonEncoder.toString(builder.buildOrThrow()));
                        break;
                    case "hash":
                        // add keys to zset
                        String redisZset = "keyset:" + tableName;
                        redisClient.zadd(redisZset, count.get(), redisKey);

                        // add values to Hash
                        for (int i = 0; i < fields.size(); i++) {
                            redisClient.hset(redisKey, columns.get(i).getName(), fields.get(i).toString());
                        }
                        break;
                    default:
