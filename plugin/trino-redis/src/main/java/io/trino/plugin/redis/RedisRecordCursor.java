        ...
        implements RecordCursor
{
            ...
            RedisClientManager redisClientManager)
    {
        ...
        this.redisClient = redisClientManager.getClient(split.getNodes().get(0));
        this.redisKeyDelimiter = redisClientManager.getRedisKeyDelimiter();
        this.isKeyPrefixSchemaTable = redisClientManager.isKeyPrefixSchemaTable();
        this.redisScanCount = redisClientManager.getRedisScanCount();
        this.scanParams = setScanParams();
        this.maxKeysPerFetch = redisClientManager.getRedisMaxKeysPerFetch();
    ...
    private void fetchKeys()
    {
        switch (split.getKeyDataType()) {
            case STRING: {
                ...
                redisCursor = redisClient.scan(cursor, scanParams);
                keys = new ArrayList<>(redisCursor.getResult());
            }
            break;
            case ZSET:
                keys = new ArrayList<>(redisClient.zrange(split.getKeyName(), split.getStart(), split.getEnd()));
                break;
            default:
    ...
    private void fetchData(List<String> currentKeys)
    {
        ...
        switch (split.getValueDataType()) {
            case STRING:
                stringValues = redisClient.mget(currentKeys.toArray(new String[0]));
                break;
            case HASH:
