...
import static io.trino.plugin.druid.DruidSessionProperties.isUseDefaultValueForNull;
        ...
        extends BaseJdbcClient
{
    ...
    {
        // Druid clusters running with druid.generic.useDefaultValueForNull=true return legacy default
        // values (for example 0) instead of NULL, which diverges from Trino's aggregation semantics.
        // Skip aggregation pushdown in that mode so results remain correct.
        if (isUseDefaultValueForNull(session)) {
            return Optional.empty();
        }
