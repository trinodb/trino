        ...
        implements ConnectorPageSource
{
            ...
            EntriesTableSplit split)
    {
        ...
        try {
            ...
            this.readableMetricsType = typeManager.getType(new TypeDescriptor(JSON));

            ManifestReader<? extends ContentFile<?>> manifestReader = closer.register(
                    readerForManifest(split.manifestFile(), fileIO, idToPartitionSpecMapping));
            ...
            this.columnNameToIndex = mapWithIndex(
                    ...
                    (columnName, position) -> entry(columnName, Long.valueOf(position).intValue()))
    ...
    private Type columnType(String columnName)
    {
        return switch (columnName) {
            ...
            case READABLE_METRICS_COLUMN_NAME -> readableMetricsType;
