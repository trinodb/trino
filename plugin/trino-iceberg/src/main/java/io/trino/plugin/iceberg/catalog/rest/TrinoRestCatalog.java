...
import static io.trino.plugin.iceberg.catalog.AbstractTrinoCatalog.ICEBERG_VIEW_OWNER;
import static io.trino.plugin.iceberg.catalog.AbstractTrinoCatalog.ICEBERG_VIEW_RUN_AS_INVOKER;
import static io.trino.plugin.iceberg.catalog.AbstractTrinoCatalog.ICEBERG_VIEW_SQL_DIALECT;
        ...
        implements TrinoCatalog
{
    ...
    public void createView(ConnectorSession session, SchemaTableName schemaViewName, ConnectorViewDefinition definition, boolean replace)
    {
        ...
        Namespace defaultNamespace = definition.getSchema().map(Namespace::of).orElse(Namespace.empty());
        ImmutableMap.Builder<String, String> properties = ImmutableMap.builder();
        properties.put(ICEBERG_VIEW_RUN_AS_INVOKER, Boolean.toString(definition.isRunAsInvoker()));
        String tableLocation = defaultTableLocation(session, schemaViewName);
        definition.getOwner().ifPresent(owner -> properties.put(ICEBERG_VIEW_OWNER, owner));
        definition.getComment().ifPresent(comment -> properties.put(ViewProperties.COMMENT, comment));
        ...
        viewBuilder = viewBuilder.withSchema(schema)
                .withQuery(ICEBERG_VIEW_SQL_DIALECT, definition.getOriginalSql())
    ...
    public Map<SchemaTableName, ConnectorViewDefinition> getViews(ConnectorSession session, Optional<String> namespace)
    {
        ...
        for (Namespace restNamespace : namespaces) {
            List<TableIdentifier> restViews = restSessionCatalog.listViews(sessionContext, restNamespace);
    ...
    {
        Optional<View> icebergView = getIcebergView(session, viewName);
        return icebergView.flatMap(view -> {
            SQLViewRepresentation sqlView = view.sqlFor(ICEBERG_VIEW_SQL_DIALECT);
            if (!sqlView.dialect().equalsIgnoreCase(ICEBERG_VIEW_SQL_DIALECT)) {
                throw new UnsupportedViewDialectException(viewName, sqlView.dialect());
            }

            Optional<String> comment = Optional.ofNullable(view.properties().get(ViewProperties.COMMENT));
            ...
            if (!currentVersion.defaultNamespace().isEmpty()) {
                schema = Optional.of(currentVersion.defaultNamespace().toString());
            }

            boolean runAsInvoker = Boolean.parseBoolean(view.properties().get(ICEBERG_VIEW_RUN_AS_INVOKER));
            Optional<String> owner = Optional.ofNullable(view.properties().get(ICEBERG_VIEW_OWNER));
            return Optional.of(new ConnectorViewDefinition(sqlView.sql(), catalog, schema, viewColumns, comment, owner, runAsInvoker, null));
        });
    }
    ...
    {
        Optional<View> view = getIcebergView(session, schemaViewName);
        if (view.isEmpty()) {
            throw new ViewNotFoundException(schemaViewName);
        }

        UpdateViewProperties updateViewProperties = view.get().updateProperties();
        if (comment.isEmpty()) {
            updateViewProperties.remove(ViewProperties.COMMENT).commit();
        }
        else {
            updateViewProperties.set(ViewProperties.COMMENT, comment.get()).commit();
        }
    }
