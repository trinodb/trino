        ...
        implements ConnectorMetadata
{
    ...
    public ConnectorTableProperties getTableProperties(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        if (!partitionSourceIds.isEmpty()) {
            ...
            Iterable<TupleDomain<ColumnHandle>> discreteTupleDomain = Iterables.transform(files, fileScan -> {
                ...
                Map<Integer, Optional<String>> partitionColumnValueStrings = getSerializedPartitionKeys(fileScan);
    ...
    public void finishUpdate(ConnectorSession session, ConnectorTableHandle tableHandle, Collection<Slice> fragments)
    {
        IcebergTableHandle table = (IcebergTableHandle) tableHandle;
        Table icebergTable = transaction.table();
        Optional<Snapshot> snapshot = table.getSnapshotId().map(icebergTable::snapshot);
