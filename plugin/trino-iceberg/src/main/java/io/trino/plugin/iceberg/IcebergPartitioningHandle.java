        ...
        implements ConnectorPartitioningHandle
{
    ...
    public OptionalInt getBucketCount()
    {
        if (partitionFunctions.size() == 1 && partitionFunctions.getFirst().transform() == BUCKET) {
            return partitionFunctions.getFirst().size();
        }
        return OptionalInt.empty();
    }

    public int estimateNumberOfBuckets()
    {
        int estimate = 1;
        for (IcebergPartitionFunction partitionFunction : partitionFunctions) {
            if (partitionFunction.transform() == BUCKET) {
                estimate *= partitionFunction.size().orElse(1);
            }
        }
        return estimate;
    }
