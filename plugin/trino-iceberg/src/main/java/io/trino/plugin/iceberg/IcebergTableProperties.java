    ...
    public static void validateCompression(IcebergFileFormat fileFormat, Optional<HiveCompressionCodec> compressionCodec, OptionalInt compressionLevel)
    {
        ...
        if (compressionLevel.isPresent()) {
            if (compressionCodec.isEmpty()) {
                ...
                if (compressionCodec.get().equals(HiveCompressionCodec.GZIP)) {
                    if (!((compressionLevel.getAsInt() >= GZIP_MIN_COMPRESSION_LEVEL) && (compressionLevel.getAsInt() <= GZIP_MAX_COMPRESSION_LEVEL))) {
                        throw new TrinoException(NOT_SUPPORTED,
                                format("File format %s with compression codec %s and compression level %d not supported", fileFormat, compressionCodec, compressionLevel.getAsInt()));
                    }
                }
                else if (compressionCodec.get().equals(HiveCompressionCodec.ZSTD)) {
                    if (!((compressionLevel.getAsInt() >= ZSTD_MIN_COMPRESSION_LEVEL) && (compressionLevel.getAsInt() <= ZSTD_MAX_COMPRESSION_LEVEL))) {
                        throw new TrinoException(NOT_SUPPORTED,
                                format("File format %s with compression codec %s and compression level %d not supported", fileFormat, compressionCodec, compressionLevel.getAsInt()));
                    }
                }
