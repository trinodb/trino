        ...
        implements TableFunctionSplitProcessor
{
            ...
            implements TableFunctionSplitProcessor
    {
        ...
        private final Map<TrinoRow, AtomicInteger> rows = new ConcurrentHashMap<>();
                ...
                ExecutorService executor)
        {
            ...
            this.channels = IntStream.range(0, splits.size())
                    .filter(channel -> channel != dataChangeTypeChannel)
                    .toArray();
            this.types = dataTypeChannels(functionHandle.columns());
        ...
        private static Type[] dataTypeChannels(List<IcebergColumnHandle> columns)
        {
        ...
        private void processAndRemoveCarryoverRows(TableFunctionSplitProcessor processor)
        {
            ...
            for (int position = 0; position < result.getPositionCount(); position++) {
                TrinoRow row = new TrinoRow(types, SourcePage.create(result.getColumns(channels)), position);
                AtomicInteger count = rows.computeIfAbsent(row, _ -> new AtomicInteger());
                int delta = computeDelta(dataChangeTypeBlock, position);
        ...
        public TableFunctionProcessorState process()
        {
            ...
            for (Map.Entry<TrinoRow, AtomicInteger> entry : rows.entrySet()) {
                TrinoRow row = entry.getKey();
                ...
                for (int channel = 0; channel < types.length; channel++) {
                    ...
                    pageBuilder.getBlockBuilder(channel)
                            .appendBlockRange(RunLengthEncodedBlock.create(type, row.get(channel, type.getJavaType()), positionCount), 0, positionCount);
                }
