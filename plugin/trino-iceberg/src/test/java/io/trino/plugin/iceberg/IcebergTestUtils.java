...
public final class IcebergTestUtils
{
    ...
    private static boolean checkOrcFileSorting(Supplier<OrcDataSource> dataSourceSupplier, String sortColumnName)
    {
        ...
        try (OrcDataSource dataSource = dataSourceSupplier.get()) {
            ...
            // Build a nested RowType wrapping the leaf from the inside out, same as the Parquet path
    ...
    @SuppressWarnings("unchecked")
    private static Comparable<Object> drillToLeaf(Block block, int position, String[] pathParts, Type leafType)
    {
        for (int depth = 1; depth < pathParts.length; depth++) {
