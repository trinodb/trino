        ...
        extends BaseRuleTest
{
    ...
    public void testProjectionPushdown()
    {
        ...
        tester().assertThat(pushProjectionIntoTableScan)
                .on(p ->
                        p.project(
                                Assignments.of(
                                        p.symbol("expr_deref", BIGINT), new SubscriptExpression(p.symbol("struct_of_int", baseType).toSymbolReference(), GenericLiteral.constant(INTEGER, 1L))),
                                p.tableScan(
                                        table,
                                        ImmutableList.of(p.symbol("struct_of_int", baseType)),
                                        ImmutableMap.of(p.symbol("struct_of_int", baseType), fullColumn))))
    ...
    public void testPredicatePushdown()
    {
        ...
        tester().assertThat(pushPredicateIntoTableScan)
                .on(p ->
                        p.filter(
                                new ComparisonExpression(EQUAL, new SymbolReference("a"), GenericLiteral.constant(INTEGER, 5L)),
                                p.tableScan(
                                        table,
    ...
    public void testPushdownWithDuplicateExpressions()
    {
        ...
        tester().assertThat(pushProjectionIntoTableScan)
                .on(p -> {
                    SubscriptExpression subscript = new SubscriptExpression(p.symbol("struct_of_bigint", ROW_TYPE).toSymbolReference(), GenericLiteral.constant(INTEGER, 1L));
                    Expression sum = new ArithmeticBinaryExpression(ADD, subscript, GenericLiteral.constant(INTEGER, 2L));
                    return p.project(
                            Assignments.of(
                ...
                .matches(project(
                        ImmutableMap.of(
                                ...
                                "expr_deref_2", expression(new ArithmeticBinaryExpression(ADD, new SymbolReference("struct_of_bigint#a"), GenericLiteral.constant(INTEGER, 2L)))),
                        tableScan(
                                icebergTable.withProjectedColumns(ImmutableSet.of(partialColumn))::equals,
                                TupleDomain.all(),
