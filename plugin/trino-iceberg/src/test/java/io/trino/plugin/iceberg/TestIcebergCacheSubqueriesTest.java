        ...
        extends BaseCacheSubqueriesTest
{
            ...
            throws Exception
    {
        return IcebergQueryRunner.builder()
                ...
                .addIcebergProperty("iceberg.dynamic-filtering.wait-timeout", "20s")
                .build();
    }
    ...
    public void testChangeWhenSchemaEvolved()
    {
        try (TestTable testTable = new TestTable(
            ...
            // cache data with first query
            @Language("SQL") String selectQuery = "SELECT name FROM %s WHERE year = 2001".formatted(testTable.getName());
            MaterializedResultWithPlan result = executeWithPlan(withCacheEnabled(), selectQuery);
            ...
            // partitioning is changed and a new files are added with new partitioning
            ...
            // data for split with old partitioning should still be read from cache
            result = executeWithPlan(withCacheEnabled(), selectQuery);
            assertThat(result.result().getRowCount()).isEqualTo(3);
