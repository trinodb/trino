 ...
 * (format version 2, Parquet). This is a compatibility guard: it verifies that
 ...
 */
final class TestIcebergCopyOnWriteParquetConnectorTest
        extends BaseIcebergParquetConnectorTest
{
    TestIcebergCopyOnWriteParquetConnectorTest()
    {
        super(2);
    }
    ...
    static void enableCopyOnWrite(QueryRunner queryRunner)
    {
        for (TpchTable<?> table : REQUIRED_TPCH_TABLES) {
            queryRunner.execute("ALTER TABLE " + table.getTableName() +
