        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testAddNestedPartitioning()
    {
        ...
        assertEquals(initialPartitionedFiles.stream().mapToLong(row -> (long) row.getField(1)).sum(), 4L);

        assertThat(laterPartitionedFiles).hasSize(3);
        assertEquals(laterPartitionedFiles.stream().mapToLong(row -> (long) row.getField(1)).sum(), 4L);
    ...
    public void testRemoveNestedPartitioning()
    {
        ...
        assertEquals(partitionedFiles.stream().mapToLong(row -> (long) row.getField(1)).sum(), 4L);

        assertThat(unpartitionedFiles).hasSize(1);
        assertEquals((long) unpartitionedFiles.get(0).getField(1), 4);
