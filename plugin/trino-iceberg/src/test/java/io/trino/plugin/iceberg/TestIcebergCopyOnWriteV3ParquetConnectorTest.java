 ...
 * (format version 3, Parquet), exercising the suite against v3 tables whose
 * delete artifacts are deletion vectors. This is a compatibility guard: it
 * verifies that enabling copy-on-write table properties does not break reads,
 * statistics, metadata, or schema operations. It does not by itself add
 * row-level DML copy-on-write coverage, because the inherited DML tests create
 * their own tables that default to merge-on-read. Dedicated copy-on-write DML
 * coverage lives in {@link TestIcebergCopyOnWrite}.
 */
final class TestIcebergCopyOnWriteV3ParquetConnectorTest
        extends BaseIcebergParquetConnectorTest
{
    TestIcebergCopyOnWriteV3ParquetConnectorTest()
    {
        super(3);
    }
