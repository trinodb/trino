 ...
 * does NOT set {@code s3.endpoint} in the static Trino catalog config. It sets
 * {@code s3.region} only as a baseline default so the S3 client can initialize
 * in environments without {@code AWS_REGION} (for example, CI); the endpoint,
 * cross-region access, and path-style access used to reach storage still come
 * from the vended properties via {@link VendedCredentialsS3SecurityMappingProvider}.
 * If vended properties are not consumed, all S3 operations will fail because the
 * S3 client won't know which endpoint to connect to.
 */
final class TestIcebergS3VendingRestCatalogWithVendedConfigConnectorSmokeTest
        extends BaseIcebergConnectorSmokeTest
{
            ...
            throws Exception
    {
        ...
        String flociAddress = floci.endpoint().toString();

        backendCatalog = closeAfterClass(createBackendCatalog(flociAddress));

        Credentials stsCredentials;
        try (StsClient stsClient = StsClient.builder().applyMutation(floci::updateClient).build()) {
            stsCredentials = stsClient.assumeRole(AssumeRoleRequest.builder()
        ...
        Map<String, String> vendedS3Properties = ImmutableMap.<String, String>builder()
                ...
                .put(S3FileIOProperties.ENDPOINT, flociAddress)
        ...
        HttpConfig httpConfig = new HttpConfig()
                .setHttpPort(0);
        HttpServerInfo httpServerInfo = new HttpServerInfo(serverConfig, Optional.of(httpConfig), Optional.empty(), nodeInfo);
        restServer = new TestingHttpServer("s3-vended-config-smoke", httpServerInfo, nodeInfo, serverConfig, servlet, ServerFeature.builder()
    ...
    private JdbcCatalog createBackendCatalog(String flociAddress)
            throws IOException
    {
        ...
        properties.put(S3FileIOProperties.ACCESS_KEY_ID, FLOCI_ACCESS_KEY);
        properties.put(S3FileIOProperties.SECRET_ACCESS_KEY, FLOCI_SECRET_KEY);
        properties.put(S3FileIOProperties.ENDPOINT, flociAddress);
        properties.put(S3FileIOProperties.PATH_STYLE_ACCESS, "true");
        properties.put(AwsClientProperties.CLIENT_REGION, FLOCI_REGION);
