        ...
        extends AbstractTestQueryFramework
{
    ...
    void testIntegerMappings()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("int", "0", BIGINT, "CAST(0 AS BIGINT)")
    ...
    void testDate()
    {
        // FIXME: SQLite doesn't support the DATE type, so we're using the date() function on a literal here.
        // FIXME: As a result, this test loses some of its usefulness.
        testDate(UTC);
        testDate(jvmZone);
    ...
    {
        // FIXME: SQLite is not strictly typed: it converts any unknown type to varchar,
        // FIXME: but for Trino's purposes, only recognized types will be supported.
    ...
    private void testUnsupportedType(String dataTypeName, String value)
    {
        try (TestTable table = new TestTable(onRemoteDatabase(), "unsupported_type", format("(col %s)", dataTypeName))) {
            ...
            //assertQuery("SELECT * FROM " + table.getName(), "VALUES (" + value + ")");
            //assertQuery(format("SELECT 1 FROM %s WHERE col = %s", table.getName(), value));
        }
    }

    private DataSetup sqliteCreateAndInsert(String tableNamePrefix)
    {
        return new CreateAndInsertDataSetup(sqliteServer.getSqlExecutor(), tableNamePrefix);
    }
