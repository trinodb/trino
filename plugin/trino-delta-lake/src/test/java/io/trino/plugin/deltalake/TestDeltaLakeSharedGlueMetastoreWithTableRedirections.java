        ...
        extends BaseDeltaLakeSharedMetastoreWithTableRedirectionsTest
{
            ...
            throws Exception
    {
        ...
        queryRunner.execute("CREATE TABLE hive_with_redirections." + schema + ".hive_table (a_integer, b_timestamp) WITH (format='PARQUET') AS " +
                "VALUES (1, cast('2024-01-01' AS timestamp(3))), (2, cast('2024-01-01' AS timestamp(3))), (3, cast('2024-01-01' AS timestamp(3)))");
        queryRunner.execute("CREATE TABLE delta_with_redirections." + schema + ".delta_table (a_varchar, b_timestamp) AS " +
                "VALUES ('a', cast('2024-01-01' AS timestamp(6))), ('b', cast('2024-01-01' AS timestamp(6))), ('c', cast('2024-01-01' AS timestamp(6)))");
    ...
    public void testUnsupportedHiveTypeRedirect()
    {
        ...
        TableInput tableInput = TableInput.builder()
                ...
                .parameters(Map.of(
                        ...
                        "delta.maxReaderVersion", "7",
                        "spark.sql.sources.schema", "{\"type\":\"struct\",\"fields\":[{\"name\":\"last_hour_load\",\"type\":\"timestamp_ntz\",\"nullable\":true,\"metadata\":{}}]}",
                        "spark.sql.partitionProvider", "catalog",
                        "delta.feature.timestampNtz", "supported",
                        "spark.sql.sources.provider", "delta",
                        "delta.lastCommitTimestamp", "1720682605000",
                        "spark.sql.create.version", "3.4.1",
        ...
        try (GlueClient glueClient = GlueClient.create()) {
            ...
            // The table is incomplete, so it is expected to fail after reading the metadata from Glue
            String tableDefinition = (String) computeActual("SHOW CREATE TABLE hive_with_redirections." + schema + "." + tableName).getOnlyValue();
            String expected = """
                    CREATE TABLE delta_with_redirections.%s.%s (
                       a_varchar varchar,
                       b_timestamp timestamp(6)
                    )
                    WITH (
                       location = '%s'
                    )""";
