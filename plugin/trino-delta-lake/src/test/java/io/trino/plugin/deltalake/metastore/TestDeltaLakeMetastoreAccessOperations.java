        ...
        extends AbstractTestQueryFramework
{
    ...
    {
        Session session = sessionWithStoreTableMetadata(storeTableMetadata);
        try (TestTable table = createTable("test_cache_metastore", "(col int)")) {
    ...
    {
        Session session = sessionWithStoreTableMetadata(storeTableMetadata);
        try (TestTable table = createTable("test_cache_metastore", "(col int COMMENT 'test comment')")) {
    ...
    private void testStoreMetastoreAlterColumn(boolean storeTableMetadata)
    {
        ...
        try (TestTable table = createTable("test_cache_metastore", "(col int NOT NULL) WITH (column_mapping_mode = 'name')")) {
    ...
    private void testStoreMetastoreVacuum(boolean storeTableMetadata)
    {
        ...
        try (TestTable table = createTable("test_cache_metastore", "AS SELECT 1 a")) {
    ...
    {
        Session session = sessionWithStoreTableMetadata(storeTableMetadata);
        try (TestTable table = createTable("test_cache_metastore", "AS SELECT 1 col")) {
