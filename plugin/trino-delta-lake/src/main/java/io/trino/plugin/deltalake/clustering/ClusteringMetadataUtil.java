...
import static io.trino.plugin.deltalake.DeltaLakeMetadata.getTemporalTimeTravelLinearSearchMaxSize;
...
public final class ClusteringMetadataUtil
{
    ...
    static long getRestoreVersion(CommitInfoEntry commitInfoEntry, TrinoFileSystem fileSystem, TableSnapshot tableSnapshot)
    {
        ...
        if (!Strings.isNullOrEmpty(restoredVersion)) {
            ...
            try {
                version = findLatestVersionUsingTemporal(fileSystem, tableLocation, epochMillis, restoredInfoExecutor, getTemporalTimeTravelLinearSearchMaxSize());
            }
            catch (IOException e) {
