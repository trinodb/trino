        ...
        implements ConnectorPageSource
{
    ...
    private final List<DeltaLakeColumnHandle> notSynthesizedColumn;
    private final Set<Integer> missingIndexes;
            ...
            boolean cdfEnabled)
    {
        ...
        this.delegateIndexes = new int[columnsToRead.size()];
        ...
        for (DeltaLakeColumnHandle column : columnsToRead) {
        ...
        missingIndexes = IntStream.range(0, columnsToRead.size())
                .filter(i -> !relevantColumnNames.contains(columnsToRead.get(i).getName()))
                .boxed()
                .collect(toImmutableSet());
        this.rowIdIndex = rowIdIndex;
        this.pathBlock = pathBlock;
        this.partitionsBlock = partitionsBlock;
        this.notSynthesizedColumn = columnsToRead.stream().filter(column -> !column.getColumnType().equals(SYNTHESIZED)).collect(toImmutableList());
    ...
    private Block createRowIdBlock(Block rowIndexBlock, Page page)
    {
        ...
        if (cdfEnabled) {
            blocks.add(getCurrentColumnsBlock(page));
        }
