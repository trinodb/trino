        ...
        extends AbstractIterator<DeltaLakeTransactionLogEntry>
{
    ...
    private final TupleDomain<DeltaLakeColumnHandle> domains;
            ...
            Optional<Predicate<String>> addStatsMinMaxColumnFilter)
    {
        ...
        this.domains = requireNonNull(domains, "domains is null");
    ...
    private TupleDomain<HiveColumnHandle> buildTupleDomainColumnHandle(EntryType entryType, HiveColumnHandle column)
    {
        ...
        if (entryType == ADD) {
            this.domains.getDomains().orElseThrow().forEach((key, value) -> domains.put(toPartitionValuesParsedField(column, key), value));
        }
            ...
            implements CheckpointFieldExtractor
    {
        ...
        public DeltaLakeTransactionLogEntry getEntry(ConnectorSession session, int pagePosition, Block addBlock)
        {
            ...
            if (!domains.isAll() && !partitionMatchesPredicate(canonicalPartitionValues, domains.getDomains().orElseThrow())) {
                return null;
            }
            ...
            if (!domains.overlaps(statisticsPredicate)) {
                return null;
            }
