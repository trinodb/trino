...
public class TableSnapshot
{
    ...
    private List<String> getCheckpointPartPaths(LastCheckpoint checkpoint)
    {
        ...
        if (checkpoint.getParts().isEmpty()) {
            paths.add(stringPathOf(transactionLogDir, format("%020d.checkpoint.parquet", checkpoint.getVersion())));
        }
        else {
            int partsCount = checkpoint.getParts().get();
            for (int i = 1; i <= partsCount; i++) {
                paths.add(stringPathOf(transactionLogDir, format("%020d.checkpoint.%010d.%010d.parquet", checkpoint.getVersion(), i, partsCount)));
            }
        }
        return paths.build();
    }
