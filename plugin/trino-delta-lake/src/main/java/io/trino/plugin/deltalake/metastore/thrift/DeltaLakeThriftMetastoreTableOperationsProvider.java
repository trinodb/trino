        ...
        implements DeltaLakeTableOperationsProvider
{
    ...
    @Override
    public DeltaLakeTableOperations createTableOperations(ConnectorSession session, SchemaTableName schemaTableName)
    {
        Optional<ConnectorIdentity> identity = Optional.of(session.getIdentity());
        return new DeltaLakeThriftMetastoreTableOperations(session, hiveMetastoreFactory.createMetastore(identity), thriftMetastoreFactory.createMetastore(identity), schemaTableName);
    }
}
