        ...
        implements ConnectorPageSourceProvider
{
            ...
            DynamicFilter dynamicFilter)
    {
        ...
        boolean cdfEnabled = changeDataFeedEnabled(metadataEntry);

        List<DeltaLakeColumnHandle> relevantColumns = columns.stream()
                .map(DeltaLakeColumnHandle.class::cast)
                .collect(toImmutableList());
        List<DeltaLakeColumnHandle> columnsToRead = relevantColumns;
        if (cdfEnabled) {
            Stream<DeltaLakeColumnHandle> allColumns = extractSchema(metadataEntry, typeManager).stream()
            ...
            Stream<DeltaLakeColumnHandle> synthesizedColumns = relevantColumns.stream()
                    .filter(column -> column.getColumnType().equals(SYNTHESIZED));
            columnsToRead = concat(allColumns, synthesizedColumns).collect(toImmutableList());
        }
