    ...
    private static class TrinoToPaimonTypeVisitor
    {
        ...
        private DataType visit(Type trinoType)
        {
            switch (trinoType) {
                ...
                case io.trino.spi.type.TimeType type -> {
                    if (type.getPrecision() > 9) {
                        throw new UnsupportedOperationException("Unsupported time type precision: " + type.getPrecision());
                    }
                    // ERROR here, trino time type is long, but paimon time type is int
                    return new TimeType(type.getPrecision());
                }
                ...
                case TimestampWithTimeZoneType timestampType -> {
                    return DataTypes.TIMESTAMP_WITH_LOCAL_TIME_ZONE(timestampType.getPrecision());
                }
