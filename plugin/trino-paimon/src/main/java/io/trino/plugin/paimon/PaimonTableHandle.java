        ...
        implements ConnectorTableHandle, ConnectorInsertTableHandle, ConnectorOutputTableHandle
{
    ...
    public Table table(ConnectorSession session, TrinoCatalog catalog)
    {
        if (table != null) {
            return table;
        }
        table = catalog.loadTable(session, SchemaTableName.schemaTableName(schemaName, tableName)).copy(dynamicOptions);
        return table;
    }

    public ConnectorTableMetadata tableMetadata(ConnectorSession session, TrinoCatalog catalog)
    {
        return new ConnectorTableMetadata(
                SchemaTableName.schemaTableName(schemaName, tableName),
                columnMetadatas(session, catalog),
                Collections.emptyMap(),
                table(session, catalog).comment());
    }

    public List<ColumnMetadata> columnMetadatas(ConnectorSession session, TrinoCatalog catalog)
    {
    ...
    public PaimonColumnHandle columnHandle(ConnectorSession session, TrinoCatalog catalog, String field)
    {
        Table paimonTable = table(session, catalog);
