...
import static io.trino.plugin.kingbasemysql.KingbaseMySqlTableProperties.PRIMARY_KEY_PROPERTY;
...
public class KingbaseMySqlClient
        extends BaseJdbcClient
{
    private static final Logger log = Logger.get(KingbaseMySqlClient.class);
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, ParameterizedExpression>>builder()
                        ...
                        .add(new ImplementSum(KingbaseMySqlClient::toTypeHandle))
...
//    }
    ...
    protected Map<String, CaseSensitivity> getCaseSensitivityForColumns(ConnectorSession session, Connection connection, SchemaTableName schemaTableName, RemoteTableName remoteTableName)
    {
        String sql = "SELECT * FROM " + quoted(remoteTableName) + " WHERE 1=0";
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            try (ResultSet rs = statement.executeQuery()) {
                ...
                for (int i = 1; i <= columnCount; i++) {
                    ...
                    CaseSensitivity caseSensitivity = metadata.isCaseSensitive(i)
                            ? CASE_SENSITIVE
                            : CASE_INSENSITIVE;
                    columns.put(name, caseSensitivity);
                }
                return columns.buildOrThrow();
            }
        }
            ...
            throw new TrinoException(JDBC_ERROR,
                    "Failed to get case sensitivity for columns. " + firstNonNull(e.getMessage(), e), e);
        }
    }
...
//    }
    ...
    protected List<String> createTableSqls(RemoteTableName remoteTableName, List<String> columns, ConnectorTableMetadata tableMetadata)
    {
        ...
        List<String> primaryKeys = KingbaseMySqlTableProperties.getPrimaryKey(tableMetadata.getProperties());
        ...
        switch (jdbcTypeName.toLowerCase(ENGLISH)) {
            ...
            case "money":
                return Optional.of(ColumnMapping.doubleMapping(
                        DOUBLE,
                        moneyDoubleReadFunction(),
                        moneyWriteFunction()));
        }

        switch (typeHandle.jdbcType()) {
            case Types.BIT:
                // KingbaseES BIT type returns hex string like "0x01", need custom read function
                return Optional.of(ColumnMapping.booleanMapping(
                        BOOLEAN,
                        bitBooleanReadFunction(),
    ...
    private static BooleanReadFunction bitBooleanReadFunction()
    {
        return new BooleanReadFunction()
        {
            @Override
            public boolean readBoolean(ResultSet resultSet, int columnIndex)
                    throws SQLException
            {
                try {
                    ...
                    if (strValue.startsWith("0x") || strValue.startsWith("0X")) {
                        try {
                            int hexValue = Integer.parseInt(strValue.substring(2), 16);
                            return hexValue != 0;
                        }
    ...
    private static DoubleReadFunction moneyDoubleReadFunction()
    {
        return new DoubleReadFunction()
        {
            @Override
            public double readDouble(ResultSet resultSet, int columnIndex)
                    throws SQLException
            {
                ...
                String cleanedValue = value.replaceAll("[$,€£¥]", "").replace(",", "").trim();
    ...
    private static DoubleWriteFunction moneyWriteFunction()
    {
        return new DoubleWriteFunction()
        {
            ...
            public void set(PreparedStatement statement, int index, double value)
                    throws SQLException
            {
                // Use setString to avoid automatic ::double precision casting by JDBC driver
    ...
    public void createTable(ConnectorSession session, ConnectorTableMetadata tableMetadata)
    {
        try {
            ...
            String sqlState = e.getSQLState();
            // 使用标准 SQLState 判断“表已存在”
            if ("42S01".equals(sqlState)) {
                throw new TrinoException(ALREADY_EXISTS, "Table already exists: " + tableMetadata.getTable(), e);
            }
            throw new TrinoException(JDBC_ERROR, "Failed to create table: " + tableMetadata.getTable(), e);
        }
    }
    ...
    {
        // KingbaseES 在 MySQL 兼容模式下支持 schema 概念，这里按 catalog/schema/table 三段名生成 RENAME 语句
...
//    }
    ...
    public List<JdbcColumnHandle> getPrimaryKeys(ConnectorSession session, RemoteTableName remoteTableName)
    {
        String schemaName = remoteTableName.getSchemaName().orElse("public"); // 提供默认值
        String tableName = remoteTableName.getTableName();
        SchemaTableName schemaTableName = new SchemaTableName(schemaName, tableName);
        Map<String, JdbcColumnHandle> columns = getColumns(session, schemaTableName, remoteTableName).stream()
                .collect(toImmutableMap(JdbcColumnHandle::getColumnName, identity()));
        try (Connection connection = connectionFactory.openConnection(session)) {
            DatabaseMetaData metaData = connection.getMetaData();

            ResultSet primaryKeysResult = metaData.getPrimaryKeys(null, schemaName, tableName);
            ...
            throw new TrinoException(JDBC_ERROR, "Failed to get primary keys for " + remoteTableName + ": " + e.getMessage(), e);
        }
    }
    ...
    {
        // KingbaseES does not support GTID in MySQL style, and we don't use it
        return false;
    }
    ...
    private static class StatisticsDao
    {
        ...
        Long getRowCount(JdbcTableHandle table)
        {
            ...
            // KingbaseES MySQL 模式下，TABLE_SCHEMA 对应 JDBC 的 schemaName
        ...
        {
            // ===== 第一步：探测 COLUMN_STATISTICS 表是否存在 =====
            try {
                ...
                if (cause instanceof SQLException sqlException) {
                    ...
                    if ("42S02".equals(sqlState) ||
                        ...
                        log.debug("INFORMATION_SCHEMA.COLUMN_STATISTICS table is not available or not supported: %s", e.toString());
                        return ImmutableMap.of();
                    }
                }
                // 如果不是“表不存在”类错误，重新抛出（可能是语法错误、权限问题等）
                throw e;
            }
