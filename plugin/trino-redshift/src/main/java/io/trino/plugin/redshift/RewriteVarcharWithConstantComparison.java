...
public class RewriteVarcharWithConstantComparison
        implements ConnectorExpressionRule<Call, ParameterizedExpression>
{
    private static final Capture<Variable> LEFT = newCapture();
    private static final Capture<Constant> RIGHT = newCapture();

    private final Pattern<Call> pattern;

    public RewriteVarcharWithConstantComparison(Set<ComparisonOperator> enabledOperators)
    {
        ...
        pattern = call()
                ...
                .with(argument(1).matching(constant().capturedAs(RIGHT).with(type().matching(type -> type instanceof VarcharType))));
    }
    ...
    public Optional<ParameterizedExpression> rewrite(Call call, Captures captures, RewriteContext<ParameterizedExpression> context)
    {
        ...
        return Optional.of(new ParameterizedExpression(
                format("COLLATE(%s, 'case_sensitive') %s (%s)", left.get().expression(), operator.getOperator(), right.get().expression()),
