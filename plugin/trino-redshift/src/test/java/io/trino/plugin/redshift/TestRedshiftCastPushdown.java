        ...
        extends BaseJdbcCastPushdownTest
{
    ...
    @Test
    public void testCastPushdownWithCharConvertedToVarchar()
    {
        String table = "t_char_" + randomNameSuffix();
        // char type with precision `REDSHIFT_MAX_CHAR` + 1 is converted to `varchar(4097)` in Redshift
        assertUpdate("CREATE TABLE " + table + " (a char(4097))");
        assertUpdate("INSERT INTO " + table + " (a) VALUES ('hello')", 1);
        assertThat(query("SELECT cast(a AS varchar(50)) FROM " + table))
                .isFullyPushedDown();
        assertThat(query("SELECT cast(a AS char(50)) FROM " + table))
                .isNotFullyPushedDown(ProjectNode.class);
        executeInRedshift("DROP TABLE IF EXISTS " + table);
    }
