        ...
        extends AbstractTestQueries
{
    ...
    {
        List<String> redshiftUnloadSplitPaths = redshiftUnloadSplits.stream().map(RedshiftUnloadSplit::path).collect(toImmutableList());
        assertThat(redshiftUnloadSplitPaths).allMatch(path -> path.startsWith(S3_UNLOAD_ROOT));
        try (S3Client s3 = S3Client.builder()
                .credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create(AWS_ACCESS_KEY, AWS_SECRET_KEY)))
                .region(Region.of(AWS_REGION))
                .build()) {
            URI unloadSplitPath = URI.create(redshiftUnloadSplitPaths.getFirst());
            String bucket = unloadSplitPath.getHost();
            String prefix = unloadSplitPath.getPath().substring(1, unloadSplitPath.getPath().lastIndexOf("/"));

            List<String> expectedKeys = ImmutableList.<String>builder()
                    .addAll(redshiftUnloadSplitPaths)
                    .add("s3://%s/%s".formatted(bucket, prefix + "/manifest"))
                    .build();
            assertThat(
                    s3.listObjectsV2(request -> request.bucket(bucket).prefix(prefix)).contents().stream()
                            .map(key -> "s3://%s/%s".formatted(bucket, key.key()))
    ...
    private static void removeS3UnloadedFiles(List<RedshiftUnloadSplit> redshiftUnloadSplits)
    {
        ...
        try (S3Client s3 = S3Client.builder()
                ...
                .region(Region.of(AWS_REGION))
                .build()) {
            redshiftUnloadSplitPaths.forEach(
                    path -> {
                        URI s3Path = URI.create(path);
                        s3.deleteObject(request -> request.bucket(s3Path.getHost()).key(s3Path.getPath().substring(1)));
                    });
            URI s3Path = URI.create(redshiftUnloadSplitPaths.getFirst());
            s3.deleteObject(request -> request.bucket(s3Path.getHost()).key(s3Path.getPath().substring(1, s3Path.getPath().lastIndexOf("/")) + "/manifest"));
        }
    }
}
