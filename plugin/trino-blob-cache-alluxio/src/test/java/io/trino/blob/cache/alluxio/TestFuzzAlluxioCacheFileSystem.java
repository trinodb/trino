                ...
                throws IOException
        {
            ...
            BlobCache blobCache = new BlobCache()
            {
                ...
                public Blob get(CacheKey key, BlobSource source)
                        throws IOException
                {
                    return alluxioCache.get(key, source, Tracing.noopTracer(), stats);
                }

                @Override
                public void invalidate(CacheKey key)
                {
                    alluxioCache.invalidate(key);
                }

                @Override
                public void invalidate(Collection<CacheKey> keys)
                {
                    alluxioCache.invalidate(keys);
                }
            };
            return new CacheFileSystem(new IncompleteStreamMemoryFileSystem(), blobCache, new TestingCacheKeyProvider());
        }
