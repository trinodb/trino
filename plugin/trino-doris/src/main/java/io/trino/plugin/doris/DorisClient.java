        ...
        extends BaseJdbcClient
{
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, ParameterizedExpression>>builder()
                        ...
                        .add(new ImplementSum(DorisClient::toTypeHandle))
    ...
    private String buildCreateTableProperties(DorisEngineType engineType, Map<String, Object> tableProperties)
    {
        String tablePropertiesFormat = "\n PROPERTIES (%s)";
        ImmutableList.Builder<String> tablePropertiesBuilder = ImmutableList.builder();
        if (engineType == DorisEngineType.OLAP) {
            int replicationNum = DorisTableProperties.getReplicationNum(tableProperties);
        ...
        switch (jdbcTypeName.toLowerCase(ENGLISH)) {
            ...
            case "datetime":
                ...
                return Optional.of(ColumnMapping.longMapping(
                        timestampType,
                        starRocksDateTimeReadFunctionUsingLocalDate(timestampType),
    ...
    private static LongReadFunction starRocksDateTimeReadFunctionUsingLocalDate(TimestampType timestampType)
    {
        return new LongReadFunction()
        {
    ...
    private static void checkDateValue(long epochDay)
    {
        if (epochDay < MIN_SUPPORTED_DATE.toEpochDay() || epochDay > MAX_SUPPORTED_DATE.toEpochDay()) {
            throw new TrinoException(INVALID_ARGUMENTS, "Date must be between %s and %s in Doris: %s".formatted(MIN_SUPPORTED_DATE, MAX_SUPPORTED_DATE, LocalDate.ofEpochDay(epochDay)));
        }
    }
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type == DATE) {
            return WriteMapping.longMapping("datev2", starRocksDateWriteFunctionUsingLocalDate());
        }
        ...
        if (type instanceof TimestampType timestampType) {
            return WriteMapping.longMapping("datetimev2", timestampWriteFunction(timestampType));
        }
        ...
        try (Connection connection = connectionFactory.openConnection(session);
            ...
            return handle.createQuery("SELECT TABLE_ENGINE, DISTRIBUTE_KEY, PROPERTIES FROM information_schema.tables_config " +
                    ...
                    .map((rs, ctx) -> {
                        ...
                        tablePropertiesBuilder.put("engine", DorisEngineType.valueOf(engine));
