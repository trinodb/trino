...
import static io.trino.plugin.weaviate.WeaviateUtil.GEO_COORDINATES;
import static io.trino.plugin.weaviate.WeaviateUtil.PHONE_NUMBER;
...
public final class Decoder
{
    ...
    public static void decode(BlockBuilder blockBuilder, Type type, Object raw)
    {
        if (raw == null) {
        ...
        switch (type) {
            case ArrayType arrayType -> decodeArray(blockBuilder, arrayType, raw);
            case RowType rowType -> decodeRow(blockBuilder, rowType, raw);
    ...
    {
        switch (type) {
            case VarcharType varcharType -> varcharType.writeString(blockBuilder, (String) raw);
            case BooleanType booleanType -> booleanType.writeBoolean(blockBuilder, (Boolean) raw);
            case DoubleType doubleType -> doubleType.writeDouble(blockBuilder, ((Number) raw).doubleValue());
            case IntegerType integerType -> integerType.writeLong(blockBuilder, ((Number) raw).longValue());
    ...
    private static void decodeArray(BlockBuilder parentBlockBuilder, ArrayType arrayType, Object raw)
    {
        List<?> list = (List<?>) raw;
        ...
        for (Object element : list) {
            decode(blockBuilder, elementType, element);
        }
    ...
    private static void decodeRow(BlockBuilder blockBuilder, RowType rowType, Object raw)
    {
    ...
    private static void buildRow(RowType type, Map<String, Object> row, List<BlockBuilder> fieldBuilders)
    {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            decode(fieldBuilders.get(i), field.getType(), row.get(fieldName));
        }
    }
