...
@Isolated
final class TestFirebirdTypeMapping
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        return FirebirdQueryRunner.builder(firebirdServer).build();
    }
    ...
    void testBoolean()
    {
        SqlDataTypeTest.create()
                ...
                .execute(getQueryRunner(), firebirdCreateAndInsert("test_boolean"));
    }
    ...
    void testIntegerMappings()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("int", "0", INTEGER, "CAST(0 AS INT)")
                .addRoundTrip("numeric", "0", createDecimalType(9, 0), "CAST(0 AS DECIMAL(9, 0))")
                .addRoundTrip("dec", "0", createDecimalType(9, 0), "CAST(0 AS DECIMAL(9, 0))")
                .addRoundTrip("bigint", "0", BIGINT, "CAST(0 AS BIGINT)")
                .execute(getQueryRunner(), firebirdCreateAndInsert("integers"));
    }
    ...
    void testNumberMappingsNativeQuery()
    {
        ...
        testNumberMappingsNativeQuery("DOUBLE PRECISION", "DOUBLE");
    }

    private void testNumberMappingsNativeQuery(String firebirdType, String expectedType)
    {
        assertQuery("SELECT * FROM TABLE(system.query(query => 'SELECT CAST(1 AS " + firebirdType + ") col FROM RDB$DATABASE'))",
    ...
    void testDouble()
    {
        SqlDataTypeTest.create()
                ...
                .execute(getQueryRunner(), firebirdCreateAndInsert("firebird_test_double"));
    }
    ...
    void testDecimal()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("decimal(36, 36)", "CAST('-0.27182818284590452353602874713526624977' AS decimal(36, 36))", createDecimalType(36, 36), "CAST('-0.27182818284590452353602874713526624977' AS decimal(36, 36))")
                .execute(getQueryRunner(), firebirdCreateAndInsert("test_decimal"));
    }
    ...
    void testChar()
    {
        SqlDataTypeTest.create()
                ...
                .execute(getQueryRunner(), firebirdCreateAndInsert("test_char"));
    }
    ...
    void testVarchar()
    {
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("varchar(32765)", "'text_f'", createVarcharType(32765), "CAST('text_f' AS varchar(32765))") // max varchar length in Firebird
                .execute(getQueryRunner(), firebirdCreateAndInsert("test_varchar"));
    }
    ...
    void testDate()
    {
        testDate(UTC);
        testDate(jvmZone);
        // using two non-JVM zones so that we don't need to worry what Firebird system zone is
        testDate(vilnius);
        testDate(kathmandu);
    ...
    private void testDate(ZoneId sessionZone)
    {
        ...
        SqlDataTypeTest.create()
                ...
                .addRoundTrip("date", "DATE '0001-01-01'", DATE, "DATE '0001-01-01'") // min value in Firebird
                ...
                // FIXME: Cant run this test with Firebird
                ...
                // .addRoundTrip("date", "DATE '1582-10-14'", DATE, "DATE '1582-10-24'")
                ...
                .addRoundTrip("date", "DATE '9999-12-31'", DATE, "DATE '9999-12-31'") // max value in Firebird
                .execute(getQueryRunner(), session, firebirdCreateAndInsert("test_date"));
    }

    @Test
    void testUnsupportedType()
    {
        // FIXME: I can't get this test to work because I get a JDBC driver error immediately when trying to create the table.
        assertThatThrownBy(this::setUnsupportedType)
                .isInstanceOf(RuntimeException.class)
                .hasMessageMatching("java.sql.SQLSyntaxErrorException: unsuccessful metadata update; CREATE TABLE .* failed;.*");
    }

    private void setUnsupportedType()
    {
        try (TestTable table = new TestTable(onRemoteDatabase(), "unsupported_type", "(col GEOMETRY)")) {
            onRemoteDatabase().execute(format("INSERT INTO %s (col) VALUES ('POINT(1 2)')", table.getName()));
            assertQueryFails("SELECT * FROM " + table.getName(), "Table '.*' has no supported columns.*");
            assertQueryFails(format("SELECT 1 FROM %s WHERE col = 'POINT(1 2)'", table.getName()), "Table '.*' has no supported columns.*");
        }
    }

    private DataSetup firebirdCreateAndInsert(String tableNamePrefix)
    {
