        ...
        extends AbstractTestIntegrationSmokeTest
{
    ...
    public void testObjectIds()
    {
        ...
        String inlineTable = format("(%s) AS t(i, object_id_one, object_id_two)", values);

        assertUpdate("DROP TABLE IF EXISTS tmp_objectid");
        assertUpdate("CREATE TABLE tmp_objectid AS SELECT * FROM " + inlineTable, 6);

        // IS NULL
        assertQuery("SELECT i FROM " + inlineTable + " WHERE object_id_one IS NULL", "VALUES 10, 15");
        assertQuery("SELECT i FROM tmp_objectid WHERE object_id_one IS NULL", "SELECT 0 WHERE false"); // NULL gets replaced with new unique ObjectId in MongoPageSink, this affects other test cases

        // CAST AS varchar
        assertQuery(
                "SELECT i, CAST(object_id_one AS varchar) FROM " + inlineTable + " WHERE i <= 13",
                "VALUES (10, NULL), (11, 'ffffffffffffffffffffffff'), (12, 'ffffffffffffffffffffffff'), (13, '000000000000000000000000')");

        // EQUAL
        assertQuery("SELECT i FROM tmp_objectid WHERE object_id_one = object_id_two", "VALUES 11, 13");
        assertQuery("SELECT i FROM tmp_objectid WHERE object_id_one = ObjectId('ffffffffffffffffffffffff')", "VALUES 11, 12, 14");

        // IS DISTINCT FROM
        assertQuery("SELECT i FROM " + inlineTable + " WHERE object_id_one IS DISTINCT FROM object_id_two", "VALUES 12, 14, 15");
        assertQuery("SELECT i FROM " + inlineTable + " WHERE object_id_one IS NOT DISTINCT FROM object_id_two", "VALUES 10, 11, 13");

        assertQuery("SELECT i FROM tmp_objectid WHERE object_id_one IS DISTINCT FROM object_id_two", "VALUES 10, 12, 14, 15");
        assertQuery("SELECT i FROM tmp_objectid WHERE object_id_one IS NOT DISTINCT FROM object_id_two", "VALUES 11, 13");

        // Join on ObjectId
        assertQuery(
                format("SELECT l.i, r.i FROM (%1$s) AS l(i, object_id_one, object_id_two) JOIN (%1$s) AS r(i, object_id_one, object_id_two) ON l.object_id_one = r.object_id_two", values),
                "VALUES (11, 11), (14, 11), (11, 15), (12, 15), (12, 11), (14, 15), (13, 13)");

        // Group by ObjectId (IS DISTINCT FROM)
        assertQuery("SELECT array_agg(i ORDER BY i) FROM " + inlineTable + " GROUP BY object_id_one", "VALUES ((10, 15)), ((11, 12, 14)), ((13))");
        assertQuery("SELECT i FROM " + inlineTable + " GROUP BY object_id_one, i", "VALUES 10, 11, 12, 13, 14, 15");

        // Group by Row(ObjectId) (ID DISTINCT FROM in @OperatorDependency)
        assertQuery(
                "SELECT r.i, count(*) FROM (SELECT CAST(row(object_id_one, i) AS row(object_id_one ObjectId, i bigint)) r FROM " + inlineTable + ") GROUP BY r",
                "VALUES (10, 1), (11, 1), (12, 1), (13, 1), (14, 1), (15, 1)");
        assertQuery(
                "SELECT r.x, CAST(r.object_id_one AS varchar), count(*) FROM (SELECT CAST(row(object_id_one, i / 3 * 3) AS row(object_id_one ObjectId, x bigint)) r FROM " + inlineTable + ") GROUP BY r",
