        ...
        implements Blob
{
    ...
    @Override
    public void readTail(byte[] buffer, int offset, int length)
            throws IOException
    {
        ...
        if (length > data.length()) {
            throw new EOFException("Cannot read tail of %s bytes. Blob size is %s".formatted(length, data.length()));
        }
        data.getBytes(data.length() - length, buffer, offset, length);
    }
