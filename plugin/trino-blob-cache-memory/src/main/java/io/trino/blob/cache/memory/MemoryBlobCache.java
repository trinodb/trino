        ...
        implements BlobCache
{
            ...
            throws IOException
    {
        ...
        // the file, matching the prior MemoryFileSystemCache behavior. The read is short when the
        // storage object holds fewer bytes than the reported length.
        byte[] buffer = new byte[toIntExact(length)];
        int read = source.readTail(buffer, 0, buffer.length);
        return Optional.of(Slices.wrappedBuffer(buffer, 0, read));
    }
}
