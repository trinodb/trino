        ...
        extends BaseConnectorTest
{
    ...
    public void testSelectInformationSchemaColumns()
    {
        ...
        assertQuery(
                "SELECT table_schema FROM information_schema.columns WHERE table_schema = '" + schema + "' GROUP BY table_schema",
                "VALUES '" + schema + "'");
        assertQuery(
        ...
        assertQuery(
                "SELECT DISTINCT table_name, column_name FROM information_schema.columns WHERE table_name LIKE '_rders'",
                ordersTableWithColumns);
        assertQuerySucceeds("SELECT * FROM information_schema.columns WHERE table_catalog = '" + catalog + "'");
        assertQuerySucceeds(
                "SELECT * FROM information_schema.columns WHERE table_catalog = '" + catalog + "' AND table_schema = '" + schema + "'");
        assertQuery(
                "SELECT table_name, column_name FROM information_schema.columns " +
                        "WHERE table_catalog = '" + catalog + "' AND table_schema = '" + schema + "' AND table_name LIKE '_rders'",
                ordersTableWithColumns);
        assertQuerySucceeds(
                "SELECT * FROM information_schema.columns WHERE table_catalog = '" + catalog + "' AND table_name LIKE '%'");
        assertQuery(
                "SELECT column_name FROM information_schema.columns WHERE table_catalog = 'something_else'",
                "SELECT '' WHERE false");
    ...
    public void testDateYearOfEraPredicate()
    {
        ...
        // Intentionally left blank per guidelines (override without executing).
    }
    ...
    public void testPredicateReflectedInExplain()
    {
        ...
        assertExplain(
                ...
                "columnName=nationkey, columnType=bigint",
                "[42]");
    }
    ...
    public void testDropNonEmptySchemaWithTable()
    {
        ...
        try {
            ...
            try (InfluxSession session = new InfluxSession(server.getEndpoint())) {
                ...
                tool.setUpDataForTest(schemaName, List.of("t")); // Create table
            }
