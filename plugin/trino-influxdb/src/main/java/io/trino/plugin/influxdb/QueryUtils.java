...
public class QueryUtils
{
    ...
    public static Query buildQueryCommand(InfluxTableHandle tableHandle, List<InfluxColumnHandle> columnHandles)
    {
        ...
        SelectQueryImpl selectQuery = selectionQuery.from(tableHandle.schemaName(), wrapQuota(tableHandle.tableName()));
        appendWhereClause(selectQuery, tableHandle);
        tableHandle.limit().ifPresent(selectQuery::limit);
        return selectQuery;
    }
    ...
    {
        List<String> columns = buildProjection(tableHandle.projections());
        if (!columns.isEmpty()) {
            columns.stream()
            ...
            columnHandles.stream()
                    .map(col -> wrapQuota(col.columnName()))
                    .forEach(query::column);
        }
    }
    ...
    private static List<String> buildProjection(List<ColumnHandle> assignments)
    {
        ...
        assignments.stream()
                ...
                .map(InfluxColumnHandle::columnName)
                .forEach(columns::add);
    ...
    {
        String columnName = wrapQuota(columnHandle.columnName());
