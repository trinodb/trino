        ...
        implements RecordCursor
{
    ...
    public InfluxRecordCursor(List<InfluxColumnHandle> columnHandles, InfluxRecord sourceData)
    {
        ...
        List<String> columns = sourceData.columns();
        List<List<Object>> recordSet = sourceData.values();

        // even column not contains 'time', it still returns as the first field in influx-client query result
        boolean noTimeCol = columnHandles.stream().noneMatch(columnHandle -> columnHandle.columnName().equals(TIME.getName()));
        if (noTimeCol && sourceData.columns().contains(TIME.getName())) {
            columns = sourceData.columns().subList(1, sourceData.columns().size());
            recordSet = sourceData.values().stream().map(record -> record.subList(1, record.size())).collect(toImmutableList());
        }
        ...
        List<String> names = columnHandles.stream().map(InfluxColumnHandle::columnName).toList();
    ...
    public boolean getBoolean(int field)
    {
        ...
        throw unexpectedFieldValue(field, value, "boolean or numeric");
    }
    ...
    public long getLong(int field)
    {
        ...
        throw unexpectedFieldValue(field, value, "numeric");
    }
    ...
    private static IllegalStateException unexpectedFieldValue(int field, Object value, String expected)
    {
        String message = "Expected field %s to contain %s value but was %s"
                .formatted(field, expected, valueTypeName(value));
