        ...
        implements RecordSet
{
    ...
    public InfluxRecordSet(InfluxTableHandle tableHandle, List<InfluxColumnHandle> columnHandles, NativeInfluxClient client)
    {
        requireNonNull(tableHandle, "tableHandle is null");

        this.columnHandles = ImmutableList.copyOf(columnHandles);
        this.columnTypes = columnHandles.stream()
                .map(InfluxColumnHandle::columnType)
                .collect(toImmutableList());
        this.sourceData = querySourceData(client, tableHandle, columnHandles);
    }

    private static InfluxRecord querySourceData(NativeInfluxClient client, InfluxTableHandle tableHandle, List<InfluxColumnHandle> columnHandles)
    {
