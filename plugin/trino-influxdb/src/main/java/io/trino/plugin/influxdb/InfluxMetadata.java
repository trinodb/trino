        ...
        implements ConnectorMetadata
{
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        InfluxTableHandle table = client.getTableHandle(influxTableHandle.schemaName(), influxTableHandle.tableName())
    ...
    public ColumnMetadata getColumnMetadata(ConnectorSession session, ConnectorTableHandle tableHandle, ColumnHandle columnHandle)
    {
        ...
        return new ColumnMetadata(influxColumnHandle.name(), influxColumnHandle.type());
    }
    ...
    {
        InfluxTableHandle tableHandle = (InfluxTableHandle) handle;
        if (client.getTableHandle(tableHandle.schemaName(), tableHandle.tableName()).isEmpty()) {
            ...
            Constraint constraint)
    {
        ...
        if (newDomain.isNone()) {
            ...
            domains.forEach((key, domain) -> {
                if (isPushdownSupportedType(((InfluxColumnHandle) key).type())
            ...
            Map<String, ColumnHandle> assignments)
    {
        ...
        List<Assignment> assignmentsList = assignments.entrySet().stream()
                .map(assignment -> new Assignment(
                        assignment.getKey(),
                        assignment.getValue(),
                        ((InfluxColumnHandle) assignment.getValue()).type()))
                .collect(toImmutableList());
