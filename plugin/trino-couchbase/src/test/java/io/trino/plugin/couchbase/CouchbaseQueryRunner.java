...
public class CouchbaseQueryRunner
{
            ...
            try {
                ...
                try (Cluster cluster = Cluster.connect(server.getConnectionString(), server.getUsername(), server.getPassword())) {
                    for (TpchTable<?> table : initialTables) {
                        ...
                        copyAndIngestTpchData(cluster, rows, table.getTableName());
    ...
    {
        Path mappingFileUri = Path.of(new File(SCHEMA_DIR.toFile(), String.format(
                "%s.%s.%s.json",
                CBBUCKET,
                TEST_SCHEMA,
                tableName)).toURI());
        log.info("Generating mapping file %s", mappingFileUri);
        try (Writer fw = Files.newBufferedWriter(mappingFileUri)) {
            HashMap<String, Object> mappings = new HashMap<>();
            for (int i = 0; i < rows.getColumnNames().size(); i++) {
                JsonObject mapping = JsonObject.create();
                mapping.put("type", rows.getTypes().get(i).getTypeDescriptor().jsonValue());
                mapping.put("order", i);
    ...
    private static void copyAndIngestTpchData(Cluster cluster, MaterializedResult rows, String tableName)
    {
        ...
        for (MaterializedRow row : rows) {
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                document.put(columnName, TrinoToCbType.serialize(types.get(i), row.getField(i)));
            }
