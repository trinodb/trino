...
{
    private final BarbApiClient barbApiClient;

    @Inject
    public ExampleMetadata(BarbApiClient barbApiClient)
    {
        this.barbApiClient = requireNonNull(barbApiClient, "exampleClient is null");
    }
    ...
    {
        return ImmutableList.copyOf(barbApiClient.getSchemaNames());
    }
    ...
    public ExampleTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName)
    {
        ...
        BarbApiTable table = barbApiClient.getTable(tableName.getSchemaName(), tableName.getTableName());
        if (table == null) {
            return null;
        }
    ...
    public List<SchemaTableName> listTables(ConnectorSession session, Optional<String> optionalSchemaName)
    {
        Set<String> schemaNames = optionalSchemaName.map(ImmutableSet::of)
                .orElseGet(() -> ImmutableSet.copyOf(barbApiClient.getSchemaNames()));

        ImmutableList.Builder<SchemaTableName> builder = ImmutableList.builder();
        for (String schemaName : schemaNames) {
            for (String tableName : barbApiClient.getTableNames(schemaName)) {
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        BarbApiTable table = barbApiClient.getTable(exampleTableHandle.getSchemaName(), exampleTableHandle.getTableName());
        if (table == null) {
