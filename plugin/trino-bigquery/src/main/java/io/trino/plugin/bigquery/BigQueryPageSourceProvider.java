        ...
        implements ConnectorPageSourceProvider
{
    ...
    private ConnectorPageSource createStoragePageSource(ConnectorSession session, BigQuerySplit split, List<BigQueryColumnHandle> columnHandles)
    {
        if (arrowSerializationEnabled) {
            return new BigQueryStorageArrowPageSource(bigQueryReadClientFactory.create(session),
                    maxReadRowsRetries,
                    split,
                    columnHandles);
        }
        return new BigQueryStorageAvroPageSource(bigQueryReadClientFactory.create(session),
                maxReadRowsRetries,
                split,
                columnHandles);
    }
