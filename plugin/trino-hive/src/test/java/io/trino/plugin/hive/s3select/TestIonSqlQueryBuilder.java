...
public class TestIonSqlQueryBuilder
{
    ...
    public void testBuildSQL()
    {
        ...
        // CSV
        IonSqlQueryBuilder queryBuilder = new CsvIonSqlQueryBuilder(TESTING_TYPE_MANAGER);
        ...
        // JSON
        queryBuilder = new JsonIonSqlQueryBuilder(TESTING_TYPE_MANAGER);
        assertEquals("SELECT s.n_nationkey, s.n_name, s.n_regionkey FROM S3Object s",
                queryBuilder.buildSql(columns, TupleDomain.all()));
        assertEquals("SELECT s.n_nationkey, s.n_name, s.n_regionkey FROM S3Object s " +
    ...
    public void testDecimalColumns()
    {
        ...
        // CSV
        IonSqlQueryBuilder queryBuilder = new CsvIonSqlQueryBuilder(typeManager);
        ...
        // JSON
        queryBuilder = new JsonIonSqlQueryBuilder(typeManager);
        assertEquals("SELECT s.quantity, s.extendedprice, s.discount FROM S3Object s WHERE ((case s.quantity when '' then null else CAST(s.quantity AS DECIMAL(20,0)) end < 50)) AND " +
    ...
    public void testDateColumn()
    {
        ...
        // JSON
        queryBuilder = new JsonIonSqlQueryBuilder(TESTING_TYPE_MANAGER);
        assertEquals("SELECT s.t1, s.t2 FROM S3Object s WHERE (case s.t2 when '' then null else CAST(s.t2 AS TIMESTAMP) end = `2001-08-22`)", queryBuilder.buildSql(columns, tupleDomain));
    }
    ...
    public void testNotPushDoublePredicates()
    {
        ...
        // JSON
        queryBuilder = new JsonIonSqlQueryBuilder(TESTING_TYPE_MANAGER);
        assertEquals("SELECT s.quantity, s.extendedprice, s.discount FROM S3Object s WHERE ((case s.quantity when '' then null else CAST(s.quantity AS INT) end < 50))",
                queryBuilder.buildSql(columns, tupleDomain));
    }
}
