...
public class ParquetTester
{
    ...
    public <W, R> void testRoundTrip(PrimitiveObjectInspector columnObjectInspector, Iterable<W> writeValues, Function<W, R> readTransform, Type parameterType)
            throws Exception
    {
        testRoundTrip(columnObjectInspector, writeValues, stream(writeValues).map(readTransform).collect(toImmutableList()), parameterType);
    }
            ...
            throws Exception
    {
        ...
        assertRoundTrip(singletonList(objectInspector), new Iterable<?>[] {
                        stream(writeValues).map(constant(null)).collect(toList())},
                new Iterable<?>[] {stream(writeValues).map(constant(null)).collect(toList())}, TEST_COLUMN, singletonList(type), Optional.empty());
        if (objectInspector.getTypeName().contains("map<")) {
            List<TypeInfo> typeInfos = getTypeInfosFromTypeString(objectInspector.getTypeName());
            ...
            // all nulls
            assertRoundTrip(singletonList(objectInspector), new Iterable<?>[] {stream(writeValues).map(constant(null)).collect(toList())},
                    new Iterable<?>[] {stream(writeValues).map(constant(null)).collect(toList())}, TEST_COLUMN, singletonList(type), Optional.of(schema));
        }
    }
    ...
    public static Properties createTableProperties(List<String> columnNames, List<ObjectInspector> objectInspectors)
    {
        ...
        orderTableProperties.setProperty("columns.types", objectInspectors.stream().map(ObjectInspector::getTypeName).collect(joining(",")));
    ...
    {
        return Stream.of(values)
                .map(v -> stream(v).map(constant(null)).collect(toList()))
