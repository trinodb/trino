        ...
        implements HiveFileWriterFactory
{
            ...
            WriterKind writerKind)
    {
        ...
        if (!compressionKind.isSupportedLocally()) {
            throw new TrinoException(HIVE_WRITER_OPEN_ERROR, "Avro Compression codec %s is not supported in the environment".formatted(compressionKind));
        }
        ...
        List<Type> inputColumnTypes = inputColumnNames.stream().map(inputColumnName -> {
            int idx = fileColumnNames.indexOf(inputColumnName);
            if (idx < 0) {
                throw new TrinoException(HIVE_WRITER_OPEN_ERROR, "Input column name [%s] not preset in file columns names %s".formatted(inputColumnName, fileColumnNames));
            }
            return fileColumnTypes.get(idx);
        }).collect(toImmutableList());
