...
public final class PartitionProjection
{
    ...
    public boolean allowWrite()
    {
        if (storageLocationTemplate.isPresent()) {
            ...
            String partitionTemplate = toPartitionLocationTemplate(columns);
            if (!template.endsWith(partitionTemplate)) {
                return false;
            }

            template = template.substring(0, template.length() - partitionTemplate.length());
    ...
    private Partition buildPartitionObject(Table table, String partitionName)
    {
        ...
        for (int i = 0; i < partitionColumns.size(); i++) {
            ...
            String partitionValue = columnProjections.get(partitionColumn).transforming(partitionValues.get(i));
            // Don't escape partition value if storage location template is defined
            partitions.add(storageLocationTemplate.isPresent() ? partitionValue : escapePathName(partitionValue));
        }
    ...
    private static String toPartitionLocationTemplate(List<String> partitionColumns)
    {
