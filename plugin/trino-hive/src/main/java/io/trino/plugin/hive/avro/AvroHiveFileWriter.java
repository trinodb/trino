        ...
        implements FileWriter
{
    ...
    public void appendRows(Page dataPage)
    {
        try {
            ...
            for (int i = 0; i < dataPage.getChannelCount() + typeCorrectNullBlocks.size(); i++) {
                if (i < dataPage.getChannelCount()) {
                    blocks[i] = dataPage.getBlock(i);
                }
                else {
                    blocks[i] = RunLengthEncodedBlock.create(typeCorrectNullBlocks.get(i - dataPage.getChannelCount()), dataPage.getPositionCount());
                }
            }
