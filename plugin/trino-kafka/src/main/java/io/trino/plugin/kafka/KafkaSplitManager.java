            ...
            for (PartitionInfo partitionInfo : partitionInfos) {
                ...
                class RangeToRepartition
                {
...

                    {
                        ...
                        this.limit = kafkaTableHandle.limit().isPresent() ? kafkaTableHandle.limit() : OptionalLong.empty();
                    }
                ...
                if (kafkaTableHandle.limit().isPresent()) {
                    // Handle the logic for calculating splits when a limit is present.
                    long limit = kafkaTableHandle.limit().getAsLong();
                        ...
                        long rowsInReminder = splitList.stream().skip(round).limit(1L)
                                .map(split ->
                                    split.getSplitByRange(split.getMessagesRange()
                                            .slice(split.getMessagesRange().begin(), remainder)))
                                .findFirst()
                                .map(KafkaSplit::getMessagesRange)
                                .map(r -> r.end() - r.begin())
                                .orElse(0L);
                            ...
                            if (finalRows > 0) {
                                ...
                                if (remainder2 > 0) {
                                    splitList.stream().skip(round2).limit(1L)
                                            .map(split ->
                                                    split.getSplitByRange(split.getMessagesRange()
                                                            .slice(split.getMessagesRange().begin(), remainder2)))
                                            .forEach(splits::add);
                                }
                            }
                            break;
                            ...
                            if (finalRows > 0) {
                                first.map(split ->
                                        ...
                                        // since we can fetch what we need
                                                split.getSplitByRange(split.getMessagesRange()
                                                        .slice(split.getMessagesRange().begin(), toIntExact(finalRows))))
                                        .ifPresent(splits::add);
                            }
                            break;
                        }
