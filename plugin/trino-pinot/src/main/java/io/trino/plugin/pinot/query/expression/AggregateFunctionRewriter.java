...
{
    // Extracted from io.trino.plugin.jdbc.expression.AggregateFunctionRewriter
    ...
    public Optional<PinotColumnHandle> rewrite(ConnectorSession session, AggregateFunction aggregateFunction, Map<String, ColumnHandle> assignments, PinotTableHandle tableHandle)
    {
        ...
        for (AggregateFunctionRule rule : rules) {
            ...
            while (matches.hasNext()) {
                ...
                Optional<PinotColumnHandle> rewritten = rule.rewrite(aggregateFunction, match.captures(), context);
