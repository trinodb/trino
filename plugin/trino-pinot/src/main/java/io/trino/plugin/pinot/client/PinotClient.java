...
public class PinotClient
{
    ...
    public PinotStartReplaceSegmentsResponse startReplaceSegments(String tableName, List<String> segmentsToReplace, List<String> newSegments, boolean forceCleanup)
    {
        try {
            ...
            throw new PinotException(PINOT_EXCEPTION, Optional.empty(), format("Could not start replace segments for %s. %s", tableName, e.getMessage()));
        }
    }
    ...
    {
        try {
            EndReplaceSegmentsRequest endReplaceSegmentsRequest = new EndReplaceSegmentsRequest(ImmutableList.of());
            SimpleHttpResponse response = fileUploadDownloadClient.endReplaceSegments(FileUploadDownloadClient.getEndReplaceSegmentsURI(getControllerUrl(), tableName, OFFLINE.name(), pinotStartReplaceSegmentsResponse.getSegmentLineageEntryId()), DEFAULT_SOCKET_TIMEOUT_MS, endReplaceSegmentsRequest, controllerAuthProvider);
