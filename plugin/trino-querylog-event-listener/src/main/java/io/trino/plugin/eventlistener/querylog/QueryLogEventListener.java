 ...
 * <p>
 * The listener logs all configured event types (QUERY_CREATED, QUERY_COMPLETED, QUERY_EXECUTED)
 ...
 */
public class QueryLogEventListener
        implements EventListener
{
    private final Logger log = Logger.get(QueryLogEventListener.class);
    ...
    private final JsonCodec<QueryExecutionEvent> queryExecutionEventJsonCodec;
    ...
    private final boolean logExecuted;
    ...
    public QueryLogEventListener(
            ...
            JsonCodec<QueryExecutionEvent> queryExecutionEventJsonCodec,
            QueryLogEventListenerConfig config)
    {
        ...
        this.queryExecutionEventJsonCodec = requireNonNull(queryExecutionEventJsonCodec, "queryExecutionEventJsonCodec is null");
        ...
        this.logExecuted = config.getLogExecuted();

        log.info("QueryLogEventListener initialized with logCreated=%s, logCompleted=%s, logExecuted=%s, excludedFields=%s, maxFieldSize=%s, truncatedFields=%s, truncationSizeLimit=%s, ignoredQueryStates=%s, ignoredUpdateTypes=%s, ignoredQueryTypes=%s, ignoredFailureTypes=%s",
                logCreated, logCompleted, logExecuted, config.getExcludedFields(), config.getMaxFieldSize(),
    ...
    public void queryCreated(QueryCreatedEvent queryCreatedEvent)
    {
        if (logCreated && filterPolicy.shouldLogQueryCreated(queryCreatedEvent)) {
            try {
                ...
                String filtered = fieldFilter.toJson(json);
    ...
    @Override
    public void queryExecuted(QueryExecutionEvent queryExecutionEvent)
    {
        if (logExecuted && filterPolicy.shouldLogQueryExecuted(queryExecutionEvent)) {
            try {
                String json = queryExecutionEventJsonCodec.toJson(queryExecutionEvent);
                String filtered = fieldFilter.applyFiltering(json);
                log.info("QUERY_EXECUTED: %s", filtered);
            }
            catch (Exception e) {
                log.error(e, "Failed to log query executed event");
            }
        }
    }
}
