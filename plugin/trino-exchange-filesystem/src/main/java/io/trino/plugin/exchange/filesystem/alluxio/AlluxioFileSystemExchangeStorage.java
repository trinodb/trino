        ...
        implements FileSystemExchangeStorage
{
    ...
    {
        try {
            AlluxioURI locationUri = convertToAlluxioURI(dir);
            if (fileSystem.exists(locationUri)) {
                URIStatus status = fileSystem.getStatus(locationUri);
                if (!status.isFolder()) {
                    throw new IOException(
                            "Cannot create a directory for an existing file location " + dir);
                }
            }
        ...
        try {
            ...
            if (!fileSystem.exists(locationUri)) {
                CreateFilePOptions options = CreateFilePOptions.newBuilder()
                        .setOverwrite(false).setRecursive(true).build();
                fileSystem.createFile(locationUri, options);
            }
        }
        catch (Exception e) {
    ...
    public static AlluxioConfiguration loadAlluxioConfiguration(Optional<String> alluxioSiteConfPath)
    {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(alluxioSiteConfPath.get())) {
            AlluxioProperties alluxioProperties = Configuration.copyProperties();
