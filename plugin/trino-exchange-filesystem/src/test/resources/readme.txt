...
./kes identity new --ip "127.0.0.1" localhost  --dns kes --key=kes.key --cert=kes.crt --expiry 87660h
...
The generated TLS private key is stored at: kes.key
The generated TLS certificate is stored at: kes.crt

The identity can be computed again via:

    kes identity of kes:v1:ADmUvSFjVFNtq2yEdSmS1qYMhHV5g9GcKa62QXMd3PuE
    kes identity of kes.crt

4. Copy identity value into a file ../kes/config.yml as a value to property admin.identity
i.e.
<config.yml>  excerpt
...
./kes identity new --key=kes_client.key --cert=kes_client.crt minio --expiry 87660h
...
The generated TLS private key is stored at: kes_client.key
The generated TLS certificate is stored at: kes_client.crt

The identity can be computed again via:

    kes identity of kes:v1:ABNO9w2+ml5m3Co8odj2ztpYydMSBHK8DkvKe/nJ3tYW
    kes identity of kes_client.crt



6. Copy identity value into file ../kes/config.yml as a value for property policy.minio.identities
i.e.
<config.yml>  excerpt
...
cd kes
docker cp kes-server:/kes.crt .
docker cp kes-server:/kes.key .
cd ../minio
docker cp kes-server:/kes_client.crt .
docker cp kes-server:/kes_client.key .
