...
^^^^^^^^^^^^^^^^^^^
    ...
    * **Default value:** ``32MB``
    ...
    another writer is eligible to be added. As this is per writer task, not
    per writer, this value is effectively divided by the number of writer
    tasks, configured via ``task.writer-count``.
