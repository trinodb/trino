...
public class PasswordStore
{
    ...
    public PasswordStore(List<String> lines, int cacheMaxSize)
    {
        ...
        cache = CacheBuilder.newBuilder()
                ...
                .build(CacheLoader.from(this::isValid));
    }
    ...
    private boolean isValid(Credential credential)
    {
        ...
        return (hashed != null) && hashed.isValid(credential.getPassword());
    }
    ...
    private static HashedPassword getHashedPassword(String hashedPassword)
    {
        switch (getHashingAlgorithm(hashedPassword)) {
            case BCRYPT:
                return password -> isBCryptPasswordValid(password, hashedPassword);
            case PBKDF2:
                return password -> isPBKDF2PasswordValid(password, hashedPassword);
        }
