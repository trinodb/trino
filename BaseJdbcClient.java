        ...
        implements JdbcClient
{
    ...
    protected Collection<String> listSchemas(Connection connection)
    {
        ...
        try (ResultSet resultSet = connection.getMetaData().getSchemas(connection.getCatalog(), null)) {
            ...
            try {
              //needed for firebird and mysql
                if (!found && connection.getMetaData().getURL() != null && (connection.getMetaData().getURL().startsWith("jdbc:firebirdsql:") || connection.getMetaData().getURL().startsWith("jdbc:mysql:"))) {
                    schemaNames.add("@noschema@");
                }
            }
            catch (Exception e) {
    ...
    protected ResultSet getColumns(JdbcTableHandle tableHandle, DatabaseMetaData metadata)
            throws SQLException
    {
        if (metadata.getSearchStringEscape() == null || metadata.getSearchStringEscape().isEmpty()) {
            return metadata.getColumns(
                    tableHandle.getRemoteTableName().getCatalogName().orElse(null),
                    tableHandle.getRemoteTableName().getSchemaName().orElse(null),
                    Optional.of(tableHandle.getRemoteTableName().getTableName()).orElse(null),
                    null);
        }
        return metadata.getColumns(
                tableHandle.getRemoteTableName().getCatalogName().orElse(null),
            ...
            throws SQLException
    {
        ...
        try {
            ...
            //Enabling read-only mode not supported at org.apache.hive.jdbc.HiveConnection.setReadOnly(HiveConnection.java:1411)
            //if (connection.getMetaData().getSearchStringEscape() != null) {
            //    connection.close();
            //    throw e;
            //}
        }
            ...
            throws SQLException
    {
        ...
        if (metadata.getSearchStringEscape() == null || metadata.getSearchStringEscape().isEmpty()) {
            return metadata.getTables(
                    ...
                    schemaName.isPresent() ? schemaName.get() : null,
                    tableName.isPresent() ? tableName.get() : null,
    ...
    protected String toRemoteSchemaName(JdbcIdentity identity, Connection connection, String schemaName)
    {
        ...
        try {
            ...
            try {
                ...
                //not all drivers have storesUpperCaseIdentifiers...like SparkThrift
            }
            return schemaName;
        }
        catch (SQLException e) {
    ...
    {
        StringBuilder sb = new StringBuilder();
        //ideally connnection knows that jdbc driver is dremio too, otherwise a dremio catalog in non-dremio db (ie sybase for example) would not work
        if (!isNullOrEmpty(catalog) && !catalog.equals("DREMIO")) {
            ...
            throws SQLException
    {
        return new RemoteTableName(
                Optional.ofNullable((resultSet.getString("TABLE_CAT") == null || resultSet.getString("TABLE_CAT").isEmpty()) ? null : resultSet.getString("TABLE_CAT")),
