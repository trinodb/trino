...
public class AsyncSegmentIterator
        extends AbstractIterator<Segment>
        implements CancellableIterator<Segment>
{
    ...
    private static final ExecutorService executorService = newCachedThreadPool(
            new ThreadFactoryBuilder().setNameFormat("Trino ADBC worker-%s").setDaemon(true).build());

    private final StatementClient client;
    private final BlockingQueue<Segment> segmentQueue;
    ...
    AsyncSegmentIterator(StatementClient client, Consumer<QueryStats> progressCallback, Optional<BlockingQueue<Segment>> queue)
    {
        requireNonNull(progressCallback, "progressCallback is null");

        this.client = client;
        this.segmentQueue = queue.orElseGet(() -> new ArrayBlockingQueue<>(MAX_QUEUED_ROWS));
        ...
        this.future = executorService.submit(() -> {
            try {
                ...
                do {
                    ...
                    EncodedQueryData queryData = (EncodedQueryData) client.currentData();
                    for (Segment segment : queryData.getSegments()) {
                ...
                if (results.getError() != null) {
                    QueryError queryError = results.getError();
                    throw new RuntimeException(new AdbcException(queryError.getMessage(), null, AdbcStatusCode.INTERNAL, queryError.getSqlState(), queryError.getErrorCode()));
                }
            }
