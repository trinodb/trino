...
public class ProxyResource
{
    ...
    private static final JsonMapper JSON_MAPPER = new JsonMapper(jsonFactoryBuilder().disable(CANONICALIZE_PROPERTY_NAMES).build());
    ...
    {
        try {
            JsonParser parser = JSON_MAPPER.createParser(input);
            ByteArrayOutputStream out = new ByteArrayOutputStream(input.length * 2);
            JsonGenerator generator = JSON_MAPPER.createGenerator(out);
