...
//        execute("CREATE EXTENSION file_fdw");
    ...
    public void testSystemTable()
    {
        assertThat(computeActual("SHOW TABLES FROM system").getOnlyColumnAsSet())
                .contains("databases", "settings", "tables", "quotas");
        // SYSTEM TABLE
        assertThat(computeActual("SELECT name FROM system.databases").getOnlyColumnAsSet())
                .contains("string");
        // SYSTEM VIEW
        assertThat(computeActual("SELECT name FROM system.tables WHERE database = 'tpch'").getOnlyColumn())
                .contains("orders");
    }
    ...
    private AutoCloseable withSchema(String schema)
            throws Exception
    {
        execute(format("CREATE DATABASE %s", schema));
        return () -> {
            try {
                execute(format("DROP DATABASE %s", schema));
            }
