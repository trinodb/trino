...
import io.prestosql.decoder.avro.AvroBytesDecoder;
import io.prestosql.decoder.avro.AvroDataDecoderFactory;
import io.prestosql.decoder.avro.AvroFileDecoder;
import io.prestosql.decoder.avro.AvroReaderSupplierFactory;
import io.prestosql.decoder.avro.AvroRowDecoderFactory;
import io.prestosql.decoder.avro.FixedSchemaAvroReaderSupplier;
...
import static io.prestosql.decoder.avro.AvroRowDecoderFactory.AVRO_READER;
import static io.prestosql.decoder.avro.AvroRowDecoderFactory.DATA_ENCODING;
...
{
    public static final Map<String, String> CONFLUENT_DECODER_PARAMS = ImmutableMap.<String, String>builder()
            .put(DATA_ENCODING, AvroBytesDecoder.NAME)
            .put(AVRO_READER, ConfluentAvroReaderSupplier.NAME)
            .build();
    ...
    private RowDecoder getRowDecoder(SchemaRegistryClient schemaRegistryClient, Set<DecoderColumnHandle> columnHandles, Schema schema)
    {
        ImmutableMap<String, String> decoderParams = ImmutableMap.<String, String>builder()
                .putAll(CONFLUENT_DECODER_PARAMS)
    ...
    {
        return new AvroRowDecoderFactory(
                ImmutableMap.<String, AvroReaderSupplierFactory>builder()
                        .put(FixedSchemaAvroReaderSupplier.NAME, new FixedSchemaAvroReaderSupplier.Factory())
                        .put(ConfluentAvroReaderSupplier.NAME, new ConfluentAvroReaderSupplier.Factory(schemaRegistryClient))
                        .build(),
                ImmutableMap.<String, AvroDataDecoderFactory>builder()
                        .put(AvroBytesDecoder.NAME, new AvroBytesDecoder.Factory())
                        .put(AvroFileDecoder.NAME, new AvroFileDecoder.Factory())
                        .build());
    }
