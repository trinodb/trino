...
import static io.prestosql.plugin.kafka.schema.confluent.ConfluentInternalFields.MESSAGE_SCHEMA_ID;
import static io.prestosql.spi.type.BigintType.BIGINT;
import static java.util.Objects.requireNonNull;

public class InternalMessageFieldDecoder
        implements RowDecoder
{
    private final DecoderColumnHandle messageSchemaIdHandle;

    public InternalMessageFieldDecoder(DecoderColumnHandle messageSchemaIdHandle)
    {
        this.messageSchemaIdHandle = requireNonNull(messageSchemaIdHandle, "messageSchemaIdHandle is null");
    }
    ...
    public Optional<Map<DecoderColumnHandle, FieldValueProvider>> decodeRow(byte[] data, Map<String, String> dataMap)
    {
        ...
        return Optional.of(ImmutableMap.<DecoderColumnHandle, FieldValueProvider>builder()
                .put(messageSchemaIdHandle, new AvroColumnDecoder.ObjectValueProvider(schemaId, BIGINT, messageSchemaIdHandle.getName()))
                .build());
    }
            ...
            implements RowDecoderFactory
    {
        ...
        public RowDecoder create(Map<String, String> decoderParams, Set<DecoderColumnHandle> columns)
        {
            ...
            Optional<DecoderColumnHandle> keySchemaIdHandle = columns.stream().filter(handle -> handle.getName().equals(MESSAGE_SCHEMA_ID)).findFirst();
            checkState(keySchemaIdHandle.isPresent(), "Internal field '%s' not found", MESSAGE_SCHEMA_ID);
            return new InternalMessageFieldDecoder(keySchemaIdHandle.get());
        }
    }
}
