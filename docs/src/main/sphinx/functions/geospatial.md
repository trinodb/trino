...
Returns the WKB representation of the geometry. WKB output does not carry SRID
...
geometries are ignored, and a repeated coordinate at the boundary of two inputs
is included only once. Array elements must not be `NULL`. The returned geometry
preserves input Z coordinates and uses the non-zero SRID from the input
geometries. Inputs with different non-zero SRIDs fail.
:::
...
Transforms the X and Y coordinates of the input geometry from its source SRID to
the target EPSG SRID. The input geometry must have a non-zero SRID; use
...
supported. Z coordinates, when present, are preserved unchanged. The returned
geometry has the target SRID.
:::
