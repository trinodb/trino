...
```{try-sql}
SELECT bitwise_left_shift(1, 2)
```

Shifting `5` (binary: `0101`) by two bits results in `20` (binary: `010100`):

```{try-sql}
SELECT bitwise_left_shift(5, 2)
```

Shifting a `value` by `0` always results in the original `value`:

```{try-sql}
SELECT bitwise_left_shift(20, 0),
       bitwise_left_shift(42, 0)
```

Shifting `0` by a `shift` always results in `0`:

```{try-sql}
SELECT bitwise_left_shift(0, 1),
       bitwise_left_shift(0, 2)
```
:::
...
```{try-sql}
SELECT bitwise_right_shift(9, 1)
```

Shifting a `value` by `0` always results in the original `value`:

```{try-sql}
SELECT bitwise_right_shift(20, 0),
       bitwise_right_shift(42, 0)
```

Shifting a `value` by `64` or more bits results in `0`:

```{try-sql}
SELECT bitwise_right_shift( 12, 64),
       bitwise_right_shift(-45, 64)
```

Shifting `0` by a `shift` always results in `0`:

```{try-sql}
SELECT bitwise_right_shift(0, 1),
       bitwise_right_shift(0, 2)
```
:::
...
```{try-sql}
SELECT bitwise_right_shift_arithmetic( 12, 64),
       bitwise_right_shift_arithmetic(-45, 64)
```
:::
