...
perform its storage. At the storage level, SQLite use 4 data types: `NULL`, `INTEGER`, `REAL`, `TEXT`, `BLOB`.

However, SQLite offers the option, when declaring a table, to assign a named type to each column.
This named type will be used by Trino to provide you with better SQL compatibility.
This allows Trino to recognize the following Java SQL types:
...
By default, the aliases `TEXT` and `INT` will point to the Trino SQL types `VARCHAR` and `BIGINT`, respectively.
...
If no named type is used in a column's declaration when creating a table, yes SQLite allows this, then Trino will consider that column to be `BLOB`.

If the named type given to a column does not correspond to any of the SQL types recognized by Trino, nor to the default aliases
or aliases defined in the `catalog.properties` file, then Trino will not support that column.
