...
be called in a SQL statement. The following code snippet displays how to
execute the `system.bucket` function:
...
This function exposes the Iceberg bucket transform so that users can determine
what bucket a particular value would fall into.
...
The function takes two arguments: the partition value and the number of buckets.
...
This function could also be used in the `optimize` function to only optimize a
subset of buckets:
...
Note that pushdown of this function is not yet supported.
