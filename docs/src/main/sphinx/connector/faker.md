...
* - `generator`
  - Name of the Faker library generator used to generate data for the column.
    ...
    provider.
* - `min`
  - Minimum constraint for generated values, inclusive. Cannot be set for
    character-based type columns.
* - `max`
  - Maximum constraint for generated values, inclusive. Cannot be set for
    character-based type columns.
* - `options`
  - Values to choose from. Cannot be set together with the `min`, or `max`
    properties.
:::
