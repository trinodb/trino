...
In order to use a file-based metastore, you must configure the metastore with:

.. code-block:: text

    hive.metastore=file
    hive.metastore.catalog.dir={S3 or HDFS file}

.. warning::

  File-based metastore is an experimental feature used for development. It
  should not be used as a replacement for Thrift or Glue metastores. You
  should not rely on any of the metadata files generated by the file-based
  metastore. It should only be used to export data.
