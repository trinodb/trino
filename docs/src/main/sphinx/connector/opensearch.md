...
OpenSearch scores every matching document by relevance and exposes the score
in the `_score` builtin column. The score decides which documents are
retrieved when a `LIMIT` is pushed down. Use an `ORDER BY` clause to order the
rows:

```sql
SELECT orderkey, "_score"
FROM TABLE(
    example.system.search(
        schema => 'sales',
        index => 'orders',
