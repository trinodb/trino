...
The InfluxDB Connector allows access to
[InfluxDB](https://www.influxdata.com/) data from Trino.
This document describes how to setup the InfluxDB Connector to run SQL queries
against InfluxDB.
...
* InfluxDB v1.8.0 or higher. v2.x is not currently supported.
...
* - `influx.password`
  ...
  -
* - `influx.insecure-allowed`
  - Allow credentials to be sent to an HTTP endpoint instead of requiring HTTPS.
  - `false`
...
The connector maps InfluxDB types to the corresponding Trino types according
...
* - `TIMESTAMP`
  - `TIMESTAMP`
  - The `time` key is a Unix nanosecond timestamp in InfluxDB. See the
    [InfluxDB data types].
* - `BOOLEAN`
  - `BOOLEAN`
  -
* - `FLOAT`
  - `DOUBLE`
  - `FLOAT` is a 64-bit floating-point number in InfluxDB. See the
    [InfluxDB data types].
* - `INTEGER`
  - `BIGINT`
  - `INTEGER` is a 64-bit integer in InfluxDB. See the
    [InfluxDB data types].
* - `STRING`
  - `VARCHAR`
  -
...
Creation and deletion of schemas are supported. As InfluxDB does not support
creation of empty tables, creation of tables is not supported but deletion of
tables is supported.
...
Predicate pushdown of keys of `STRING`, `BOOLEAN`, `INTEGER` and `FLOAT` types
are supported:

For `TIMESTAMP` key, supports equality predicates `=`, range predicates, such
as `>`, `<`.

For keys of `STRING` or `BOOLEAN` type (both tag set and field keys in
InfluxDB), supports equality predicates `=` only.

For keys of `INTEGER` or `FLOAT` type (field keys in InfluxDB), supports
equality predicates `=`, inequality predicates `!=`, and range predicates,
such as `>`, `<`, or `BETWEEN`.

```{note}
Decimal integer literals `int_lit = ("1"…"9"){digit}` pushdown works.
Hexadecimal and octal literals are not currently supported.
Floating-point literals `float_lit = int_lit"."int_lit` pushdown works.
...
WHERE time >= timestamp '2020-01-01 00:00:00'
  AND time <= timestamp '2020-01-01 23:59:59';
```

[InfluxDB data types]: https://docs.influxdata.com/influxdb/v1.8/write_protocols/line_protocol_reference/#data-types
