...
The connector supports [TopN push down](topn-pushdown) to improve query
performance by pushing `ORDER BY ... LIMIT` clauses to Elasticsearch for
processing. TopN push down is supported for columns that support predicate push
down, as listed in the preceding table.

### Aggregation push down

The connector supports [aggregation push down](aggregation-pushdown) for the
...
Aggregation push down is only supported for queries with a single grouping set.
Multiple grouping sets, such as those generated by `GROUPING SETS`, `CUBE`, or
`ROLLUP` clauses, are not supported and are processed by Trino.
