...
The HsqlDB connector supports displaying HsqlDB system tables.

The functionality is turned off by default, and can be enabled
using the configuration entry: `hsqldb.include-system-tables=true`.

### Default table type

By default, when creating table with the HsqlDB connector, the table type was `CACHED`.

This setting can be changed by using for example the configuration entry:
`hsqldb.default-table-type=MEMORY`.
