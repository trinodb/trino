...
* Expose `coordinatorId` with the `/v1/info` endpoint. ({issue}`23910`)
...
* Remove `raw_input_bytes` and `raw_input_rows` columns from 
...
* Improve performance of simple queries in clusters with small number of 
  nodes. ({issue}`26525`)
* Improve cluster stability when rolling back writes to slow data sources. ({issue}`26602`)
...
* Fix incompatibility issue with Hive OpenCSV to allow reading from tables with
  quoting and escaping disabled. ({issue}`26619`)
...
* Improve performance of writes on iceberg tables when task retries are 
  enabled. ({issue}`26620`)
...
* Fix incorrect results when reading `decimal` numbers from Parquet files and the declared precision
  differs from the precision described in the Parquet metadata. ({issue}`26203`)
* Fix coordinator crashes from running `OPTIMIZE_MANIFESTS` on partitioned 
  tables. ({issue}`26323`)
...
* Allow configuring job name with `openlineage-event-listener.job.name-format` 
