...
* {{breaking}} Require catalogs to be deployed in all nodes. ({issue}`26063`)
...
* Expose `coordinatorId` via the `/v1/info` endpoint. ({issue}`23910`)
...
* Fix writing malformed checkpoint files when deletion vectors are enabled. ({issue}`26145`)
...
* Fix incorrect results when a table uses deletion vectors and its partition path 
  contains special characters. ({issue}`26299`)
...
* Fix incorrect results when reading from Parquet files produced by old versions
  of PyArrow. ({issue}`26058`)
* Improve resource utilization when using Alluxio. ({issue}`26121`)
* Add support for `SIGV4` as an independent authentication scheme. It can be enabled by setting the 
  `iceberg.rest-catalog.security` config property to `SIGV4`. The `iceberg.rest-catalog.sigv4-enabled`
   config property is no longer supported. ({issue}`26218`)
