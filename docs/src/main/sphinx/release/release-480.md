...
* Improve query performance when exchange manager is configured but query does not use task level retries. ({issue}`28698`)
* Allow query parameters within table `[VERSION | TIMESTAMP] AS OF` clause. ({issue}`28681`)
...
* Fix query failures and increased latency when using Azure exchange manager. ({issue}`28058`)
...
* Return the correct class name for `map`, `row`, `time with time zone`, `timestamp with time zone`,
  `varbinary` and `null` values when calling `ResultSetMetaData.getColumnClassName`. ({issue}`28314`)
...
* Fix potential table corruption when executing `CREATE OR REPLACE` with table definition
  changes. ({issue}`27805`)
* Fix query failures and increased latency when using Azure file system. ({issue}`28058`)
...
* Fix `INSERT` into and `ANALYZE` on tables with `timestamp` columns when using Hive metastore version 4. ({issue}`26214`, {issue}`28330`)
...
* Add support for Iceberg v3 tables in `optimize`, `expire_snapshots` and `remove_orphan_files` table procedures. ({issue}`27836`)
* Add support for Iceberg v3 [row lineage](https://iceberg.apache.org/spec/#row-lineage). ({issue}`27836`)
...
* Add support for creating materialized views with columns of `number` type. ({issue}`28399`)
...
* Add support for temporary GCS credentials provided by REST catalog. ({issue}`24518`)
...
* Fix failure when querying `$files` table after `CREATE OR REPLACE TABLE` with a different partitioning schema. ({issue}`25339`)
...
* {{breaking}} Remove incorrect support for reading MySQL `BIT(n)` columns when `n > 1`. ({issue}`28744`)
* Fix failure when creating a table if a prior `CREATE TABLE ... AS SELECT`
  operation for the same table failed. ({issue}`27702`)
...
* Add support for reading all Oracle `NUMBER` columns. ({issue}`28747`, {issue}`28401`)
* Fix failure when creating a table if a prior `CREATE TABLE ... AS SELECT`
  operation for the same table failed. ({issue}`27702`)
...
* Add support for reading all PostgreSQL `NUMERIC` and `DECIMAL` columns. ({issue}`28141`)
* Fix failure when creating a table if a prior `CREATE TABLE ... AS SELECT`
  operation for the same table failed. ({issue}`27702`)
