...
The example above is executed as if it were written as::

   SELECT
    CASE
      WHEN present AND s.address = 'Berkeley' THEN
          -- Null values for delete; present=true; operation DELETE=2
          row(null, null, null, 0, 2)
      WHEN present AND s.customer = 'Joe Shmoe' THEN
          -- Update column values; present=true; operation UPDATE=3
          row(t.customer, t.purchases + 100.0, t.address, true, 3)
      WHEN present THEN
          -- Update column values; present=true; operation UPDATE=3
          row(t.customer, s.purchases + t.purchases, s.address, true, 3)
      WHEN (present IS NULL) THEN
          -- Insert column values; present=false; operation INSERT=1
          row(s.customer, s.purchases, s.address, false, 1)
      ELSE
          -- Null values for no case matched; present=false; operation=-1
          row(null, null, null, false, -1)
    END
...
``ConnectorMergeSink.storeMergedRows(...)`` method.
...
* A block containing the ``MERGE`` case operation, encoded as
...
examines successive rows produced by the ``CASE`` expression,
excluding ``INSERT``ed rows, and checks that the
``writeRedistributionColumns`` and the ``rowId`` column are the
identical in successive rows. If so, it raises the
``MERGE_TARGET_ROW_MULTIPLE_MATCHES`` exception.
