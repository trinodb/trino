...
features, such as a connector for another data source, to Trino.
...
Because Trino plugins use the `trino-plugin` packaging type, building a plugin
creates a ZIP file in the `target` directory. The file contains the plugin JAR
and all its dependencies as JAR files, and is suitable for [plugin
