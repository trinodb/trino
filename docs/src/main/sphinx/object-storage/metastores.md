...
authenticate with the REST catalog itself, and are independent from `gcs.json-key` and
`gcs.json-key-file-path`, which authenticate GCS access for table data. All four properties
are optional. When the REST catalog ones are omitted, or when the GCS ones are omitted,
[Application Default Credentials](https://cloud.google.com/docs/authentication/application-default-credentials)
(ADC) are used for the respective purpose, which supports GKE Workload Identity and other
