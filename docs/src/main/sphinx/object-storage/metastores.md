...
* - Property name
  - Description
  - Default
* - `iceberg.hive-catalog.lock-enabled`
  - Commit to tables without using Hive locks.
  - `true`
:::

:::{warning}
Setting `iceberg.hive-catalog.lock-enabled=false` will cause the catalog to
...
* [HIVE-28121](https://issues.apache.org/jira/browse/HIVE-28121) is available on
  the Hive metastore server, if it is backed by MySQL or MariaDB
* All other catalogs committing to tables that this catalogs commits to are also
  on Iceberg 1.3 or later
* All other catalogs committing to tables that this catalogs commits to have also
  disabled Hive locks on commit.
:::
