...
* - `iceberg.rest-catalog.vended-credentials-enabled`
  - Use credentials and configuration provided by the REST backend for file system access.
    ...
    `s3.secret-access-key`, `s3.session-token`) together with related configuration
    properties (`client.region`, `s3.endpoint`, `s3.cross-region-access-enabled`,
    `s3.path-style-access`). These S3 overrides are applied when the REST catalog
    vends S3 session credentials. Vended per-table properties take precedence over
    static catalog configuration, which
    serves as a fallback when the REST catalog does not provide a value.
    Defaults to `false`.
