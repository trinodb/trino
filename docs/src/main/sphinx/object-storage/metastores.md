...
* - `iceberg.snowflake-catalog.password`
  - Snowflake password. Required, unless `iceberg.snowflake-catalog.private-key`
    or `iceberg.snowflake-catalog.private-key-file` is set. Deprecated: Snowflake
    is phasing out password authentication for non-human accounts; use
...
iceberg.snowflake-catalog.private-key=base64-encoded-key
