...
authentication of the nodes in the cluster. Communication between Trino nodes
can be secured with :ref:`TLS <glossTLS>`.
...
In typical deployments you want to enable :ref:`TLS directly on the coordinator
...
2. Enable automatic certificate creation and trust setup in
   ...
   on the nodes. If you upgrade from this setup, you need to ensure that
   ``internal-communication.https.keystore.path`` and
   ``internal-communication.https.truststore.path`` are not set.
