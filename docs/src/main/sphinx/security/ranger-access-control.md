...
* Users must have permission to impersonate themselves in Trino. Without a
  ...
  * To allow this, create a policy in Apache Ranger for a `trinouser` resource
    ...
    * External clients might use the `system.runtime.kill_query()` procedure to 
      kill running queries by id. Add a policy with Schema `system`, Database
      `runtime` and Procedure `kill_query` with `execute` permission on user
      `{USER}`.
