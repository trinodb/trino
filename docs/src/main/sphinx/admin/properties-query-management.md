...
## `query.max-write-physical-bytes`

- **Type:** {ref}`prop-type-data-size`
- **Session property:** `query_max_write_physical_bytes`

The maximum number of bytes that can be written by a query during its execution.
When this limit is reached, query processing is terminated to prevent excessive
resource usage.
