...
Node groups do not apply to the coordinator, which stays available to every query so that
coordinator-only stages keep working. Set `node-scheduler.include-coordinator=false`, the
default for multi-node clusters, or the coordinator also takes worker splits from every node
group and the groups are not isolated from one another on it.

A query waits for `query.required-workers` workers in its own node group, not in the cluster,
before it starts. A value larger than the smallest node group leaves that group's queries
waiting for `query.required-workers-max-wait` and then failing, so keep it at or below the
size of the smallest group, or raise it per resource group with a
{doc}`session property manager </admin/session-property-managers>`.
