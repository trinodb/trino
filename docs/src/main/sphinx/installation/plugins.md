...
the binary packages - specifically the `tar.gz` archive and the Docker
container.
...
* Additional types, functions, and table functions
* Block encodings
* Resource group configuration managers
* Session property configuration managers
* Exchange managers

All plugins are optional for your use of Trino because they support a specific
...
many plugins as part of the binary packages.
...
:::{note}
Every Trino plugin must be in a separate directory. Do not put JAR
files directly into the `plugin` directory. Each plugin directory can only
contain JAR files. Any subdirectories and other files are ignored. 
:::
...
container](/installation/containers) uses the path `/usr/lib/trino/plugins`.
...
More advanced deployments can also use rolling deletion and restarts of cluster
nodes.
...
your installation by creating a custom `tar.gz` archive or a custom container
image.
