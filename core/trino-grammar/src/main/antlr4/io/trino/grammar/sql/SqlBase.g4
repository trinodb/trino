...
statement
    ...
    | ALTER TABLE tableName=qualifiedName
        CREATE (OR REPLACE)? TAG (IF NOT EXISTS)? tagName=identifier
        (FOR VERSION AS OF snapshotId=expression)?
        (RETAIN retentionDays=expression DAYS)?                        #createTag
    | ALTER TABLE tableName=qualifiedName
        REPLACE TAG tagName=identifier
        (FOR VERSION AS OF snapshotId=expression)?
        (RETAIN retentionDays=expression DAYS)?                        #replaceTag
    | ALTER TABLE tableName=qualifiedName
        DROP TAG tagName=identifier                                    #dropTag
...
nonReserved
    ...
    | DATA | DATE | DAY | DAYS | DECLARE | DEFAULT | DEFINE | DEFINER | DENY | DESC | DESCRIPTOR | DETERMINISTIC | DISTRIBUTED | DO | DOUBLE
    ...
    | QUOTES
    | RANGE | READ | REFRESH | RENAME | REPEAT  | REPEATABLE | REPLACE | RESET | RESPECT | RESTRICT | RETAIN | RETURN | RETURNING | RETURNS | REVOKE | ROLE | ROLES | ROLLBACK | ROW | ROWS | RUNNING
    | SCALAR | SCHEMA | SCHEMAS | SECOND | SECURITY | SEEK | SERIALIZABLE | SESSION | SET | SETS
    | SHOW | SOME | START | STATS | SUBSET | SUBSTRING | SYSTEM
    | TABLES | TABLESAMPLE | TAG | TEXT | TEXT_STRING | TIES | TIME | TIMESTAMP | TO | TRAILING | TRANSACTION | TRUNCATE | TRY_CAST | TYPE
