...
public final class ExpressionTreeRewriter<C>
{
            ...
            extends AstVisitor<Expression, ExpressionTreeRewriter.Context<C>>
    {
        ...
        @Override
        protected Expression visitMeasure(Measure node, Context<C> context)
        {
            if (!context.isDefaultRewrite()) {
                Expression result = rewriter.rewriteMeasure(node, context.get(), ExpressionTreeRewriter.this);
                if (result != null) {
                    return result;
                }
            }
            ...
            if (name != node.getName() || window != node.getWindow()) {
                return new Measure(name, window);
            }
