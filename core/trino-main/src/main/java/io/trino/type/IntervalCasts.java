...
/// casting to a different fractional precision may re-encode the value. Narrowing the trailing field
/// truncates the fields the target cannot represent.
public final class IntervalCasts
{
    ...
    public static final SqlScalarFunction INTERVAL_DAY_TIME_TO_VARCHAR_CAST = new PolymorphicScalarFunctionBuilder(CAST, IntervalCasts.class)
            ...
            .choice(choice -> choice.implementation(methodsGroup -> methodsGroup
                    ...
                    .withExtraParameters(context -> ImmutableList.of(varcharLength(context.getReturnType()), context.getParameterTypes().getFirst()))))
            .build();
    ...
    private static SqlScalarFunction numericToIntervalCast(String numericType, String base, IntervalField field, String method, long fieldFactor)
    {
        return new PolymorphicScalarFunctionBuilder(CAST, IntervalCasts.class)
                .signature(Signature.builder()
                        ...
                        .returnType(singleFieldInterval(base, field, numericVariable("precision"), numericVariable("fractional")))
                        .build())
                .deterministic(true)
                .choice(choice -> choice.implementation(methodsGroup -> methodsGroup
                        .methods(base.equals(INTERVAL_DAY_TO_SECOND) ? new String[] {method, method + "Long"} : new String[] {method})
                        .withExtraParameters(context -> ImmutableList.of(fieldFactor, context.getReturnType()))))
                .build();
    }
    ...
    // The target qualifier and precision govern truncation of trailing fields and leading-field overflow.
    private static List<Object> targetLeadingField(io.trino.spi.type.Type target)
    {
        if (target instanceof IntervalDayTimeType type) {
            return ImmutableList.of(dayTimeFieldMicros(type.getStartField()), (long) type.getLeadingPrecision(), type.getStartField(), (long) type.getFractionalPrecision(), type.getEndField());
        }
        IntervalYearMonthType type = (IntervalYearMonthType) target;
        return ImmutableList.of(yearMonthFieldMonths(type.getStartField()), (long) type.getLeadingPrecision(), type.getStartField(), type.getEndField());
    }

    @UsedByGeneratedCode
    public static long dayTimeToDayTimeShortToShort(long micros, long fieldMicros, long precision, IntervalField startField, long fractionalPrecision, IntervalField endField)
    {
        // narrow the value to the target's fractional-seconds precision, then enforce the leading precision
        long rounded = endField == SECOND
                ? roundMicros(micros, (int) fractionalPrecision)
                : truncateToField(micros, 0, endField);
        checkLeadingFieldPrecision(rounded, 0, fieldMicros, precision, startField);
        return rounded;
    }

    @UsedByGeneratedCode
    public static LongInterval dayTimeToDayTimeShortToLong(long micros, long fieldMicros, long precision, IntervalField startField, long fractionalPrecision, IntervalField endField)
    {
        // a short value has no sub-microsecond fraction, so widening to a long precision only adds zero picoseconds
        if (endField != SECOND) {
            micros = truncateToField(micros, 0, endField);
        }
    ...
    @UsedByGeneratedCode
    public static long dayTimeToDayTimeLongToShort(LongInterval value, long fieldMicros, long precision, IntervalField startField, long fractionalPrecision, IntervalField endField)
    {
        long micros = value.getMicros();
        long rounded;
        if (endField != SECOND) {
            rounded = truncateToField(micros, value.getPicosOfMicro(), endField);
        }
        else if ((int) fractionalPrecision < MAX_SHORT_FRACTIONAL_PRECISION) {
            // rounding below microsecond resolution drops the sub-microsecond fraction without affecting the result
            rounded = roundMicros(micros, (int) fractionalPrecision);
        }
        else {
    ...
    @UsedByGeneratedCode
    public static LongInterval dayTimeToDayTimeLongToLong(LongInterval value, long fieldMicros, long precision, IntervalField startField, long fractionalPrecision, IntervalField endField)
    {
        LongInterval rounded = endField == SECOND
                ? roundToLongInterval(value.getMicros(), value.getPicosOfMicro(), (int) fractionalPrecision)
                : new LongInterval(truncateToField(value.getMicros(), value.getPicosOfMicro(), endField), 0);
        checkLeadingFieldPrecision(rounded.getMicros(), rounded.getPicosOfMicro(), fieldMicros, precision, startField);
        return rounded;
    }

    @UsedByGeneratedCode
    public static long yearMonthToYearMonth(long months, long fieldMonths, long precision, IntervalField startField, IntervalField endField)
    {
        months = months / yearMonthFieldMonths(endField) * yearMonthFieldMonths(endField);
        checkLeadingFieldPrecision(months, 0, fieldMonths, precision, startField);
        return months;
    }

    /// Drops fields below the requested day-time field, truncating the complete value toward zero.
    public static long truncateToField(long micros, long picosOfMicro, IntervalField field)
    {
        long unit = dayTimeFieldMicros(field);
        return leadingFieldValue(micros, picosOfMicro, unit) * unit;
    }
    ...
    @UsedByGeneratedCode
    public static Slice dayTimeToVarchar(long value, long length, IntervalDayTimeType type)
    {
        return toVarchar(formatDayTime(value, 0, type), length);
    }

    @UsedByGeneratedCode
    public static Slice dayTimeToVarcharLong(LongInterval value, long length, IntervalDayTimeType type)
    {
        return toVarchar(formatDayTime(value.getMicros(), value.getPicosOfMicro(), type), length);
    }

    @UsedByGeneratedCode
    public static Slice yearMonthToVarchar(long value, long length, IntervalYearMonthType type)
    {
        String sign = value < 0 ? "-" : "";
        long months = abs(value);
        String result = switch (type.getStartField()) {
            case MONTH -> sign + months;
            case YEAR -> type.getEndField() == YEAR
                    ? sign + months / 12
                    : format("%s%d-%d", sign, months / 12, months % 12);
            default -> throw new IllegalArgumentException("Not a year-month field: " + type.getStartField());
        };
        return toVarchar(result, length);
    }

    private static String formatDayTime(long micros, int picosOfMicro, IntervalDayTimeType type)
    {
        // Split before taking absolute values so Long.MIN_VALUE is representable too.
        long seconds = micros / 1_000_000;
        long fraction = (micros % 1_000_000) * PICOSECONDS_PER_MICROSECOND + picosOfMicro;
        if (micros < 0 && fraction > 0) {
            seconds++;
            fraction -= 1_000_000_000_000L;
        }
        seconds = abs(seconds);
        ...
        for (IntervalField field : new IntervalField[] {DAY, HOUR, MINUTE, SECOND}) {
            if (field.code() < type.getStartField().code() || field.code() > type.getEndField().code()) {
                continue;
            }
            long unitSeconds = dayTimeFieldMicros(field) / 1_000_000;
            long value = seconds / unitSeconds;
            seconds %= unitSeconds;
            if (field != type.getStartField()) {
                result.append(field == HOUR ? ' ' : ':');
                if (value < 10) {
                    result.append('0');
                }
            }
        ...
        if (type.getFractionalPrecision() > 0) {
            result.append('.').append(format("%012d", fraction), 0, type.getFractionalPrecision());
        }
    ...
    {
        long micros = roundMicros(parseDayTimeInterval(value.toStringUtf8(), startField, endField), (int) fractionalPrecision);
    ...
    @UsedByGeneratedCode
    public static long bigintToDayTime(long value, long fieldMicros, IntervalDayTimeType type)
    {
        try {
            long micros = multiplyExact(value, fieldMicros);
            checkLeadingFieldPrecision(micros, type);
            return micros;
        }
        catch (ArithmeticException e) {
    ...
    @UsedByGeneratedCode
    public static LongInterval bigintToDayTimeLong(long value, long fieldMicros, IntervalDayTimeType type)
    {
        return new LongInterval(bigintToDayTime(value, fieldMicros, type), 0);
    }
    ...
    @UsedByGeneratedCode
    public static long bigintToYearMonth(long value, long fieldMonths, IntervalYearMonthType type)
    {
        try {
            long months = toIntExact(multiplyExact(value, fieldMonths));
            checkLeadingFieldPrecision(months, type);
            return months;
        }
        catch (ArithmeticException e) {
            throw new TrinoException(NUMERIC_VALUE_OUT_OF_RANGE, "Value out of range for an interval: " + value, e);
