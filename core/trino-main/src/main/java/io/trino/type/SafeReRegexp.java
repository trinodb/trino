...
public final class SafeReRegexp
{
    ...
    private final Pattern patternWithoutDotStarPrefix;
    private final String rawPattern;

    public static SafeReRegexp safeReRegexp(Slice pattern)
    {
        try {
    ...
    {
        this.rawPattern = pattern.toStringUtf8();
        this.pattern = Pattern.compile(rawPattern);

        String withoutDotStarPrefix = removeDotStarPrefix(rawPattern);
        this.patternWithoutDotStarPrefix = withoutDotStarPrefix.equals(rawPattern) ? this.pattern : Pattern.compile(withoutDotStarPrefix);
    }
    ...
    {
        return pattern.matcher(utf8Input(source));
    }

    public boolean matches(Slice source)
    {
        // Only the boolean outcome is used here, so the match position the prefix would have shifted does not matter.
        return patternWithoutDotStarPrefix.find(utf8Input(source));
    }
