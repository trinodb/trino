...
public final class IntervalYearMonthOperators
{
    ...
    private static SqlScalarFunction intervalScalar(OperatorType operator, String numericType, String method)
    {
        return new PolymorphicScalarFunctionBuilder(operator, IntervalYearMonthOperators.class)
                ...
                .choice(choice -> choice.implementation(methodsGroup -> methodsGroup
                        .methods(method)
                        .withExtraParameters(context -> numericType.equals(DOUBLE)
                                ? ImmutableList.of(((IntervalYearMonthType) context.getReturnType()).getEndField())
                                : ImmutableList.of())))
                .build();
    }
    ...
    @UsedByGeneratedCode
    public static long multiplyByDouble(long left, double right, IntervalField endField)
    {
        if (Double.isNaN(right)) {
            throw new TrinoException(INVALID_FUNCTION_ARGUMENT, "Cannot multiply by double NaN");
        }
        return normalizeMonths((long) (left * right), endField);
    }
    ...
    @UsedByGeneratedCode
    public static long doubleMultiply(double left, long right, IntervalField endField)
    {
    ...
    @UsedByGeneratedCode
    public static long divideByDouble(long left, double right, IntervalField endField)
    {
    ...
    private static long normalizeMonths(long months, IntervalField endField)
    {
        if (endField == YEAR) {
            return months / 12 * 12;
        }
