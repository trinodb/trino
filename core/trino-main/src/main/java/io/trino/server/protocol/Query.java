...
class Query
{
    ...
    private synchronized void setQueryOutputInfo(QueryExecution.QueryOutputInfo outputInfo)
    {
        ...
        if (columns == null) {
            ...
            checkArgument(columnInfos.size() == columnTypes.size(), "Column metadata and types size mismatch");

            ImmutableList.Builder<Column> list = ImmutableList.builder();
            for (int i = 0; i < columnInfos.size(); i++) {
                list.add(createColumn(columnInfos.get(i), columnTypes.get(i), supportsParametricDateTime, supportsNumberType, supportsVariant, supportsVariantBinary));
            }
            columns = list.build();
            types = columnTypes;
