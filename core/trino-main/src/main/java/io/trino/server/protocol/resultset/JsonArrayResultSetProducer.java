...
import static io.trino.server.protocol.resultset.JsonArrayResultSetProducer.QueryResultsJsonArrayIterator.toIterableList;
...
/**
 * This ResultSetProducer represents existing v1 protocol semantics that streams partial result as JSON-serialized list of objects
 */
public class JsonArrayResultSetProducer
        implements ResultSetProducer
{
    @Override
    public QueryResults handleNextRows(Session session, QueryResults emptyResultSet, QueryResultRows rows, Consumer<Throwable> exceptionHandler)
    {
        if (rows.isEmpty()) {
            return emptyResultSet;
        }
        return new JsonQueryResults(emptyResultSet, toIterableList(session, rows, exceptionHandler));
    }

    @Override
    public Class<? extends QueryResults> produces()
    {
        return JsonQueryResults.class;
    }

    static class QueryResultsJsonArrayIterator
            ...
            implements Iterable<List<Object>>
    {
        ...
        public QueryResultsJsonArrayIterator(ConnectorSession session, List<Page> pages, List<QueryResultRows.ColumnAndType> columns, boolean supportsParametricDateTime, Consumer<Throwable> exceptionConsumer)
        {
        ...
        {
            return new QueryResultsJsonArrayIterator(
        ...
        public Iterator<List<Object>> iterator()
        {
            ...
            return new QueryResultsJsonArrayIterator(session, pages, columns, supportsParametricDateTime, exceptionConsumer);
        }
    }
}
