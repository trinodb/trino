...
final class XQuerySqlToJoniTranslator
{
    ...
    public static Regex translate(RegularExpression regularExpression, boolean dotAllMode, boolean multiLineMode, String pattern)
    {
        ...
        return new Regex(joniPatternBytes, 0, joniPatternBytes.length, Option.SINGLELINE, XQuerySqlEncoding.INSTANCE, SYNTAX);
    }
            ...
            extends RegexTreeVisitor<Void>
    {
        ...
        protected Void visitWildcardEscape(WildcardEscape node)
        {
            ...
            if (dotAllMode) {
                ...
                translated.append("(?:\\x{D}\\x{A}|(?!\\x{D}\\x{A})[\\x{D}\\x{A}]|[^\\x{D}\\x{A}])");
            }
            else {
        ...
        protected Void visitEndMetaCharacter(EndMetaCharacter node)
        {
            ...
            if (multiLineMode) {
                // The following regex consists of five alternatives:
                // 1. \z - matches the end of the string.
                // 2. \x{D}\x{A} - lookahead matching the CRLF sequence.
                // 3. \x{D}(?!\x{A}) - lookahead matching CR that is not followed by LF.
                // 4. (?<!\x{D})\x{A} - lookahead matching LF that is not preceded by CR.
                // 5. [\x{B}\x{C}\x{85}\x{2028}\x{2029}] - lookahead matching other newline characters.
                //
                // Alternatives 3 and 4 ensure that LF and CR within the CRLF sequence are not matched during backtracking.
                // Without these checks, the regex '^$' would incorrectly match the empty string between CR and LF in the
                // CRLF sequence, violating the rules defined in Unicode Technical Standard #18.
                translated.append("(?:\\z|(?=\\x{D}\\x{A}|\\x{D}(?!\\x{A})|(?<!\\x{D})\\x{A}|[\\x{B}\\x{C}\\x{85}\\x{2028}\\x{2029}]))");
            }
            else {
                // The following regex is a lookahead that matches the end of the string (\z), optionally preceded by a newline.
                // The newline matching is performed similarly to how it is handled in multiline mode.
                translated.append("(?=(?:\\x{D}\\x{A}|\\x{D}(?!\\x{A})|(?<!\\x{D})\\x{A}|[\\x{B}\\x{C}\\x{85}\\x{2028}\\x{2029}])?\\z)");
            }
            return null;
        }
        ...
        protected Void visitStartMetaCharacter(StartMetaCharacter node)
        {
            ...
            if (multiLineMode) {
                // The following regex consists of five alternatives:
                // 1. \A - matches the start of the string.
                // 2. (?<=\x{D}\x{A}) - lookbehind matching the CRLF sequence.
                // 3. (?<!\x{D})(?<=\x{A}) - lookbehind matching LF that is not preceded by CR.
                // 4. (?<=\x{D})(?!\x{A}) - lookbehind matching CR that is not followed by LF.
                // 5. (?<=[\x{B}\x{C}\x{85}\x{2028}\x{2029}]) - lookbehind matching other newline characters.
                //
                // Alternatives 3 and 4 ensure that LF and CR within the CRLF sequence are not matched during backtracking.
                // Without these checks, the regex '^$' would incorrectly match the empty string between CR and LF in the
                // CRLF sequence, which violates the rules defined in Unicode Technical Standard #18.
                translated.append("(?:\\A|(?<=\\x{D}\\x{A})|(?<!\\x{D})(?<=\\x{A})|(?<=\\x{D})(?!\\x{A})|(?<=[\\x{B}\\x{C}\\x{85}\\x{2028}\\x{2029}]))");
            }
            else {
