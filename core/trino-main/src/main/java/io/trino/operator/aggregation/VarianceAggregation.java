...
public final class VarianceAggregation
{
    ...
    // the intermediate row layout must match the generated VarianceState serializer, which orders
    // state fields alphabetically: (count bigint, m2 double, mean double)
    @InputFunction(hidden = true)
    public static void intermediateInput(
            @AggregationState VarianceState state,
            @BlockPosition @SqlType("row(count bigint, m2 double, mean double)") ValueBlock block,
    ...
    public static void intermediateOutput(@AggregationState VarianceState state, BlockBuilder out)
    {
        ((RowBlockBuilder) out).buildEntry(fieldBuilders -> {
            ...
            DOUBLE.writeDouble(fieldBuilders.get(1), state.getM2());
            DOUBLE.writeDouble(fieldBuilders.get(2), state.getMean());
        });
    }
