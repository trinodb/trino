        ...
        implements GroupByHash
{
    ...
    public long getRawHash(int groupId)
    {
        ...
        int position = groupId & OUTPUT_PAGE_MASK;
    ...
    private int addNewGroup(int hashPosition, int position, Page page, long rawHash)
    {
        ...
        if (currentPageBuilder.getPositionCount() == OUTPUT_PAGE_SIZE) {
            startNewPage();
        }
    ...
    {
        int blockIndex = groupId >> OUTPUT_PAGE_BITS;
        int blockPosition = groupId & OUTPUT_PAGE_MASK;
        if (precomputedHashChannel.isPresent()) {
