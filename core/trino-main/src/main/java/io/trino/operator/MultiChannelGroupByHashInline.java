...
public class MultiChannelGroupByHashInline
        implements GroupByHash
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(MultiChannelGroupByHashInline.class).instanceSize();
    ...
    public MultiChannelGroupByHashInline(
            ...
            UpdateMemory updateMemory)
    {
        ...
        mask = hashCapacity - 1;
        hashTableMask = 2 * hashCapacity - 1;
        hashTable = new long[hashCapacity * 2];
    ...
    public boolean contains(int position, Page page, int[] hashChannels, long rawHash)
    {
        ...
        while (hashTable[hashPosition] != -1) {
            if (positionNotDistinctFromCurrentRow(hashTable[hashPosition], (int) (hashTable[hashPosition + 1] >> 32), position, page, rawHash, hashChannels)) {
                // found an existing slot for this key
                return true;
            }
            // increment position and mask to handle wrap around
            hashPosition = (hashPosition + 2) & hashTableMask;
        }
    ...
    private int putIfAbsent(int position, Page page, long rawHash)
    {
        ...
        return findSlot(position, page, rawHash, hashPosition);
    }

    private int findSlot(int position, Page page, long rawHash, int hashPosition)
    {
        // look for an empty slot or a slot containing this key
        int groupId = -1;
        while (hashTable[hashPosition] != -1) {
            long groupIdAndHash = hashTable[hashPosition + 1];
            if (positionNotDistinctFromCurrentRow(hashTable[hashPosition], (int) (groupIdAndHash >> 32), position, page, rawHash, channels)) {
                // found an existing slot for this key
                groupId = (int) groupIdAndHash;
    ...
    private int addNewGroup(int hashPosition, int position, Page page, long rawHash)
    {
        ...
        hashTable[hashPosition] = address;
        hashTable[hashPosition + 1] = groupId | (rawHash << 32);
    ...
    private boolean tryRehash()
    {
        ...
        int newHashTableMask = 2 * newCapacity - 1;
        long[] newKey = new long[newCapacity * 2];
        Arrays.fill(newKey, -1);

        for (int oldIndex = 0; oldIndex < hashTable.length; oldIndex += 2) {
            // seek to the next used slot
            if (hashTable[oldIndex] == -1) {
                continue;
            }

            // get the address for this slot
            long address = hashTable[oldIndex];

            long rawHash = hashPosition(address);
            // find an empty slot for the address
            int pos = (int) getHashPosition(rawHash, newMask);
            while (newKey[pos] != -1) {
                pos = (pos + 2) & newHashTableMask;
                hashCollisions++;
            }

            // record the mapping
            newKey[pos] = address;
            newKey[pos + 1] = hashTable[oldIndex + 1];
        }

        this.mask = newMask;
        this.hashTableMask = newHashTableMask;
        this.hashCapacity = newCapacity;
        this.maxFill = calculateMaxFill(newCapacity);
        this.hashTable = newKey;
        groupAddressByGroupId.ensureCapacity(maxFill);
        return true;
    }
    ...
    private boolean positionNotDistinctFromCurrentRow(long address, int hash, int position, Page page, long rawHash, int[] hashChannels)
    {
        if (hash != (int) rawHash) {
            return false;
        }
    ...
    {
        return 2 * (HashCommon.murmurHash3(rawHash) & mask);
    }
            ...
            implements GroupCursor
    {
        ...
        {
            MultiChannelGroupByHashInline.this.appendValuesTo(currentGroupId, pageBuilder, outputChannelOffset);
        }
