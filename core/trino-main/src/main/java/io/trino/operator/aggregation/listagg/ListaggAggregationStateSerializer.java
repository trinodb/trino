        ...
        implements AccumulatorStateSerializer<ListaggAggregationState>
{
    ...
    public void serialize(ListaggAggregationState state, BlockBuilder out)
    {
        if (state.isEmpty()) {
            ...
            VARCHAR.writeSlice(rowBlockBuilder, state.getOverflowTruncationFiller());
            BOOLEAN.writeBoolean(rowBlockBuilder, state.isOverflowTruncationCountIndication());

            BlockBuilder stateElementsBlockBuilder = rowBlockBuilder.beginBlockEntry();
            state.forEach((block, position) -> elementType.appendTo(block, position, stateElementsBlockBuilder));
    ...
    public void deserialize(Block block, int index, ListaggAggregationState state)
    {
        ...
        Slice overflowTruncationFiller = VARCHAR.getSlice(columnarRow.getField(2), index);
        boolean overflowTruncationCountIndication = BOOLEAN.getBoolean(columnarRow.getField(3), index);
