        ...
        extends ValueWindowFunction
{
    ...
    public void processRow(BlockBuilder output, int frameStart, int frameEnd, int currentPosition, int excludedStart, int excludedEnd, int keptRow)
    {
        ...
        if (excludedStart <= excludedEnd) {
            // the frame has a hole punched out by an EXCLUDE clause; scan for the first surviving position,
            // skipping nulls when IGNORE NULLS is specified. The recorded-position optimization below assumes
            // a contiguous frame, so it is bypassed here.
