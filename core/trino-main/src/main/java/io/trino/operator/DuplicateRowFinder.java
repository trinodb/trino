...
/**
 * This class throws an exception if two adjacent rows have the same identity.
 * It compares both the rowId column as well as the writeRedistributionColumns,
 * because it is called before the page is separated by partition, and
 * different partitions can have identical rowId columns.
 */
public class DuplicateRowFinder
{
     ...
     * A sequential duplicate signals that a single target table row matched more than one source row, which
     * violates the SQL MERGE spec.
     * @param page The page
