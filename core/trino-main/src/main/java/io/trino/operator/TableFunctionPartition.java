...
public class TableFunctionPartition
{
    ...
    public WorkProcessor<Page> toOutputPages()
    {
        return WorkProcessor.create(new WorkProcessor.Process<>()
        {
            ...
            {
                TableFunctionProcessState state = tableFunction.process(inputPages);
                if (state.type() == FINISHED) {
                    return WorkProcessor.ProcessState.finished();
                }
                if (state.type() == BLOCKED) {
                    return WorkProcessor.ProcessState.blocked(toListenableFuture(state.blockedFuture()));
                }
                if (state.usedData()) {
                    inputPages = prepareInputPages();
                }
                if (state.result() != null) {
                    return WorkProcessor.ProcessState.ofResult(appendPassThroughColumns(state.result()));
                }
     ...
     */
    private Page appendPassThroughColumns(TableFunctionResult result)
    {
            ...
            for (int position = 0; position < page.getPositionCount(); position++) {
                if (indexes.isNull(position)) {
                    ...
                    if (index < 0 || index >= endOfData[inputChannel] || index >= processedPositions) {
                        ...
                        String message;
                        if (end >= 0) {
                            message = format("Index of a pass-through row: %s out of processed portion of partition [0, %s]", index, end);
                        }
                        else {
                            message = "Index of a pass-through row must be null when no input data from the partition was processed. Actual: " + index;
                        }
                        throw new TrinoException(FUNCTION_IMPLEMENTATION_ERROR, message);
                    }
                    // index in PagesIndex
