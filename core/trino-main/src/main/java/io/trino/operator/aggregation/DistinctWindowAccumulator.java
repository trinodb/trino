        ...
        implements WindowAccumulator
{
    ...
    private void indexCurrentPage(Page page)
    {
        ...
        checkArgument(positionCount == page.getPositionCount(), "Page position count dies not match distinct mask position count");
        ...
        if (distinctMask instanceof RunLengthEncodedBlock) {
            ...
            for (int position = 0; position < positionCount; position++) {
                ...
                for (int channel = 0; channel < argumentChannels.size(); channel++) {
                    BlockBuilder filteredBlockBuilder = filteredPageBuilder.getBlockBuilder(channel);
                    Block block = page.getBlock(channel);
                    if (block instanceof ValueBlock valueBlock) {
                        filteredBlockBuilder.append(valueBlock, position);
                    }
                    else {
                        block.getUnderlyingValueBlock();
                        ValueBlock singleValueBlock = block.getSingleValueBlock(position);
                        filteredBlockBuilder.append(singleValueBlock, 0);
                    }
                }
