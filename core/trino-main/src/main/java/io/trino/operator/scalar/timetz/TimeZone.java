...
public final class TimeZone
{
    ...
    public static Slice extractTimeZoneFromTime(
            @SqlType("time(p) with time zone") final long timeWithTimeZone)
    {
        long totalOffsetMinutes = unpackOffsetMinutes(timeWithTimeZone);

        long hours = Math.abs(totalOffsetMinutes) / MINUTES_PER_HOUR;
        long minutes = Math.abs(totalOffsetMinutes) % MINUTES_PER_HOUR;

        String sign = totalOffsetMinutes < 0 ? "-" : "+";

        return utf8Slice(String.format("%s%02d:%02d", sign, hours, minutes));
    }
    ...
    @LiteralParameters("p")
    @SqlType(StandardTypes.VARCHAR)
    public static Slice extractTimeZoneFromTimestamp(
            @SqlType("time(p) with time zone")
            final LongTimeWithTimeZone timeWithTimeZone)
    {
