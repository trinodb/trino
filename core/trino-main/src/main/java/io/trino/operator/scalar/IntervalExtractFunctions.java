...
public final class IntervalExtractFunctions
{
    ...
    /// Truncate toward zero before extracting fields from the floored microsecond representation.
    @UsedByGeneratedCode
    public static long extractLong(LongInterval total, long fieldUnit, long parentUnit)
    {
        long micros = total.getMicros();
        if (micros < 0 && total.getPicosOfMicro() > 0) {
            micros++;
        }
        return extract(micros, fieldUnit, parentUnit);
    }
}
