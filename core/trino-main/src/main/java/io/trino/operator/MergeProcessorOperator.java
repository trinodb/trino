 ...
 * creates the {@link RowChangeProcessor}.
 */
        ...
        implements WorkProcessorOperator
{
    public static OperatorFactory createOperatorFactory(
            ...
            MergeElements merge)
    {
        RowChangeProcessor rowChangeProcessor = createRowChangeProcessor(merge);
        return createAdapterOperatorFactory(new Factory(operatorId, planNodeId, rowChangeProcessor));
    }

    private static RowChangeProcessor createRowChangeProcessor(MergeElements merge)
    {
        switch (merge.getParadigm()) {
            case DELETE_ROW_AND_INSERT_ROW:
                return new DeleteAndInsertMergeProcessor(merge.getColumnHandles(), merge.getColumnTypes(), merge.getWriteRedistributionColumns(), merge.getRowIdType());
            case CHANGE_ONLY_UPDATED_COLUMNS:
                return new ChangeOnlyUpdatedColumnsMergeProcessor(merge.getColumnHandles(), merge.getColumnTypes(), merge.getWriteRedistributionColumns(), merge.getRowIdType());
            default:
                throw new IllegalArgumentException("Unsupported RowChangeParadigm " + merge.getParadigm());
        }
    }
