...
public class MultiChannelGroupByHashBatch
        implements GroupByHash
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(MultiChannelGroupByHashBatch.class).instanceSize();
    ...
    private final long[] rawHash;

    public MultiChannelGroupByHashBatch(
            ...
            int batchSize)
    {
        ...
        this.rawHash = new long[batchSize];
    ...
    private void putIfAbsent(int[] positionsToProcess, int positionToProcessCount, Page page, int[] outGroupIds)
    {
        getHashPositions(positionsToProcess, positionToProcessCount, page);

        do {
            positionToProcessCount = processIteration(outGroupIds, positionsToProcess, positionToProcessCount, page);
        }
        while (positionToProcessCount > 0);
    }

    private void getHashPositions(int[] positionsToProcess, int positionToProcessCount, Page page)
    {
        for (int positionIndex = 0; positionIndex < positionToProcessCount; positionIndex++) {
            long rawHash = hashGenerator.hashPosition(positionsToProcess[positionIndex], page);
            this.rawHash[positionIndex] = rawHash;
            hashPositions[positionIndex] = (int) getHashPosition(rawHash, mask);
        }
    }

    private int processIteration(int[] outGroupIds, int[] positionsToProcess, int positionToProcessCount, Page page)
    {
        long[] currentGroupAddresses = new long[batchSize];
        for (int i = 0; i < positionToProcessCount; i++) {
            currentGroupAddresses[i] = groupAddressByHash[hashPositions[i]];
        }
        ...
        for (int i = 0; i < positionToProcessCount; i++) {
            ...
            if (currentGroupAddresses[i] == -1) {
                if (groupAddressByHash[hashPosition] == -1) {
                    // empty slot found
                    int groupId = addNewGroup(hashPosition, currentPosition, page, rawHash[i]);

                    outGroupIds[currentPosition] = groupId;
                    continue;
                }
                // mid-batch conflict
                currentGroupAddresses[i] = groupAddressByHash[hashPosition];
            }
            if (positionNotDistinctFromCurrentRow(currentGroupAddresses[i], hashPosition, currentPosition, page, (byte) rawHash[i], channels)) {
                outGroupIds[currentPosition] = groupIdsByHash[hashPosition];
                continue;
            }
            ...
            rawHash[remainingToProcess] = rawHash[i];
            remainingToProcess++;
            hashCollisions++;
        }
    ...
    {
        return nextGroupId + newGroups >= maxFill;
    }
            ...
            implements Work<Void>
    {
        ...
        public boolean process()
        {
            ...
            for (int batchStart = lastPosition; batchStart < positionCount; batchStart += batchSize) {
                ...
                putIfAbsent(positions, currentBatchSize, page, groupIds);
            ...
            implements GroupCursor
    {
        ...
        {
            MultiChannelGroupByHashBatch.this.appendValuesTo(currentGroupId, pageBuilder, outputChannelOffset);
        }
