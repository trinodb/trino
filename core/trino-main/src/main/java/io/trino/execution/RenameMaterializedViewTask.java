        ...
        implements DataDefinitionTask<RenameMaterializedView>
{
            ...
            WarningCollector warningCollector)
    {
        ...
        if (!metadata.isMaterializedView(session, materializedViewName)) {
            if (metadata.isView(session, materializedViewName)) {
                throw semanticException(
                        TABLE_NOT_FOUND,
                        statement,
                        "Materialized View '%s' does not exist, but a view with that name exists. Did you mean ALTER VIEW %s RENAME TO ...?",
            ...
            if (metadata.getTableHandle(session, materializedViewName).isPresent()) {
                throw semanticException(
                        TABLE_NOT_FOUND,
                        statement,
                        "Materialized View '%s' does not exist, but a table with that name exists. Did you mean ALTER TABLE %s RENAME TO ...?",
                        materializedViewName,
                        materializedViewName);
            }
        ...
        Resolver resolver = metadata.getResolverManager().getResolver(session, materializedViewName.catalogName());
        QualifiedObjectName target = createTargetQualifiedObjectName(materializedViewName, statement.getTarget(), resolver);
