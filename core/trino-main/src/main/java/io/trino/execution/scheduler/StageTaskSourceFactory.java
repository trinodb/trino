        ...
        implements TaskSourceFactory
{
            ...
            implements TaskSource
    {
                ...
                Optional<CatalogName> catalogRequirement)
        {
            ...
            checkArgument(bucketToPartitionMap.isPresent() || partitionedExchangeSourceHandles.isEmpty() || splitSources.isEmpty(), "bucket to partition map is expected to be set when partitioned inputs are present");
                ...
                for (Map.Entry<PlanNodeId, SplitSource> entry : splitSources.entrySet()) {
                    ...
                    while (!splitSource.isFinished()) {
                        ...
                        for (Split split : splitBatch.getSplits()) {
                            ...
                            int partition = bucketToPartitionMap.map(map -> map[bucket]).orElse(bucket);
