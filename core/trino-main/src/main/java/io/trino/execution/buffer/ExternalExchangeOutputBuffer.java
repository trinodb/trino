...
public class ExternalExchangeOutputBuffer
        implements OutputBuffer
{
    ...
    public ExternalExchangeOutputBuffer(
            ...
            Supplier<LocalMemoryContext> systemMemoryContextSupplier)
    {
        ...
        checkArgument(outputBuffers.getType() == EXTERNAL, "Expected an EXTERNAL output buffer");
    ...
    public OutputBufferInfo getInfo()
    {
        ...
        return new OutputBufferInfo(
                ...
                false,
                state.canAddPages(),
                exchangeSink.getSystemMemoryUsage(),
                totalPagesAdded.get(),
                totalRowsAdded.get(),
                totalPagesAdded.get(),
                ImmutableList.of());
    ...
    public void enqueue(int partition, List<SerializedPage> pages)
    {
        ...
        for (SerializedPage page : pages) {
            ...
            Slice slice = Slices.allocate(page.getSizeInBytes() + SERIALIZED_PAGE_HEADER_SIZE);
            writeSerializedPage(slice.getOutput(), page);
            exchangeSink.add(partition, slice);
            totalRowsAdded.addAndGet(page.getPositionCount());
        }
        updateMemoryUsage(exchangeSink.getSystemMemoryUsage());
