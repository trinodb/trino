...
public class QueryStateMachine
{
    ...
    {
        Map<QualifiedObjectName, Map<String, ColumnMetadata>> tableColumnMetadata = isQuery ? getTableColumnMetadata(columnFields) : null;
        ...
        for (String columnName : columnNames) {
            ...
            if (isQuery) {
                ...
                if (tableName.isPresent() && tableColumnMetadata.containsKey(tableName.get()) && tableColumnMetadata.get(tableName.get()).containsKey(columnName)) {
                    columnInfo.setMetadata(tableColumnMetadata.get(tableName.get()).get(columnName));
                }
            }
    ...
    private Map<QualifiedObjectName, Map<String, ColumnMetadata>> getTableColumnMetadata(List<Field> columnFields)
    {
        Map<QualifiedObjectName, Map<String, ColumnMetadata>> tableColumnMetadata = new HashMap<>();
        for (Field field : columnFields) {
            if (field.getOriginTable().isPresent() && !tableColumnMetadata.containsKey(field.getOriginTable().get())) {
                QualifiedObjectName tableName = field.getOriginTable().get();
                // FIXME: Some connectors throw an exception when accessing Metadata::getTableMetadata with a view. See: https://github.com/trinodb/trino/issues/30736
                if (!metadata.isView(session, tableName)) {
                    Optional<TableHandle> tableHandle = metadata.getTableHandle(session, tableName);
                    if (tableHandle.isPresent()) {
                        TableMetadata tableMetadata = metadata.getTableMetadata(session, tableHandle.get());
                        tableColumnMetadata.put(tableName, tableMetadata.columns().stream().collect(Collectors.toMap(ColumnMetadata::getName, identity())));
                    }
                }
            }
        }
