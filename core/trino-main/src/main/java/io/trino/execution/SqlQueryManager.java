        ...
        implements QueryManager
{
    ...
    private final Optional<DataSize> maxQueryWritePhysicalBytes;
    ...
    public SqlQueryManager(ClusterMemoryManager memoryManager, Tracer tracer, QueryManagerConfig queryManagerConfig)
    {
        ...
        this.maxQueryWritePhysicalBytes = queryManagerConfig.getQueryMaxWritePhysicalBytes();
    ...
    private void enforceWriteLimits()
    {
        for (QueryExecution query : queryTracker.getAllQueries()) {
            ...
            Optional<DataSize> limitOpt = getQueryMaxWritePhysicalBytes(query.getSession());
            if (maxQueryWritePhysicalBytes.isPresent()) {
                limitOpt = limitOpt
                        .flatMap(sessionLimit -> maxQueryWritePhysicalBytes.map(serverLimit -> Ordering.natural().min(serverLimit, sessionLimit)))
                        .or(() -> maxQueryWritePhysicalBytes);
            }
