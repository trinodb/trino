        ...
        implements DataDefinitionTask<SetAuthorizationStatement>
{
            ...
            WarningCollector warningCollector)
    {
        ...
        switch (statement.getOwnedEntityKind().toUpperCase(Locale.ENGLISH)) {
            case "SCHEMA" -> setSchemaAuthorization(session, statement, principal);
            case "TABLE" -> setTableAuthorization(session, statement, principal);
            case "VIEW" -> setViewAuthorization(session, statement, principal);
            default -> setEntityAuthorization(session, statement, principal);
        }

        return immediateVoidFuture();
    }

    private void setSchemaAuthorization(Session session, SetAuthorizationStatement statement, TrinoPrincipal principal)
    {
        ...
        setEntityAuthorization(session, statement, principal);
    }

    private void setTableAuthorization(Session session, SetAuthorizationStatement statement, TrinoPrincipal principal)
    {
    ...
    private void setViewAuthorization(Session session, SetAuthorizationStatement statement, TrinoPrincipal principal)
    {
    ...
    private void setEntityAuthorization(Session session, SetAuthorizationStatement statement, TrinoPrincipal principal)
    {
        ...
        accessControl.checkCanSetEntityAuthorization(session.toSecurityContext(), statement.getOwnedEntityKind(), entityName, principal);

        metadata.setEntityAuthorization(session, statement.getOwnedEntityKind(), entityName, principal);
    }
}
