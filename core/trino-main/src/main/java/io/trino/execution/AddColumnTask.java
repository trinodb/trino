        ...
        implements DataDefinitionTask<AddColumn>
{
            ...
            WarningCollector warningCollector)
    {
        ...
        if (element.getName().getParts().size() == 1) {
            ...
            if (position instanceof ColumnPosition.After after && getCandidates(currentType, after.column().getValue()).isEmpty()) {
                throw semanticException(COLUMN_NOT_FOUND, statement, "Field '%s' does not exist within %s", after.column().getValue().toLowerCase(ENGLISH), currentType);
            }
