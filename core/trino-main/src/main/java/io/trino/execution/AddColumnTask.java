        ...
        implements DataDefinitionTask<AddColumn>
{
            ...
            WarningCollector warningCollector)
    {
        ...
        String columnName = canonicalizer.canonicalizeColumn(element.getName().getOriginalParts().getFirst());
        ...
        if (element.getName().getParts().size() == 1) {
            ...
            if (position instanceof ColumnPosition.After(Identifier after)) {
                String afterColumn = canonicalizer.canonicalizeColumn(after);
                if (!columns.containsKey(afterColumn)) {
            ...
            List<String> fieldPath = statement.getColumn().getName().getOriginalParts().subList(1, statement.getColumn().getName().getOriginalParts().size()).stream()
                    .map(canonicalizer::canonicalizeColumn)
                    .collect(toImmutableList());
            ...
            Identifier fieldIdentifier = statement.getColumn().getName().getOriginalParts().getLast();
            String fieldName = canonicalizer.canonicalizeColumn(fieldIdentifier);
            List<RowType.Field> candidates = getCandidates(currentType, fieldName);
