        ...
        implements Rule<ExceptNode>
{
    ...
    public Result apply(ExceptNode node, Captures captures, Context context)
    {
        ...
        predicatesBuilder.add(comparison(metadata, getTypeResolutionPolicy(context.getSession()), GREATER_THAN_OR_EQUAL, result.countSymbols().get(0).toSymbolReference(), new Constant(BIGINT, 1L)));
        for (int i = 1; i < node.getSources().size(); i++) {
            predicatesBuilder.add(comparison(metadata, getTypeResolutionPolicy(context.getSession()), EQUAL, result.countSymbols().get(i).toSymbolReference(), new Constant(BIGINT, 0L)));
        }
        return Result.ofPlanNode(
                new ProjectNode(
                        context.getIdAllocator().getNextId(),
                        new FilterNode(context.getIdAllocator().getNextId(), result.planNode(), and(predicatesBuilder.build())),
