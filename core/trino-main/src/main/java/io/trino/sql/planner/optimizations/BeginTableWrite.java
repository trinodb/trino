            ...
            extends SimplePlanRewriter<Optional<WriterTarget>>
    {
        ...
        private PlanNode rewriteModifyTableScan(PlanNode node, TableHandle handle)
        {
            ...
            PlanNode rewrittenNode = SimplePlanRewriter.rewriteWith(
                    new SimplePlanRewriter<>()
                    {
                        @Override
                        public PlanNode visitTableScan(TableScanNode scan, RewriteContext<AtomicInteger> context)
                        {
            ...
            verify(modifyCount.get() == 1, "Expected to find exactly one update target TableScanNode but found " + modifyCount.get());
