...
{
    private NonNullDerivation() {}

    public static Set<Symbol> deriveNonNullSymbols(PlannerContext plannerContext, Session session, PlanNode node)
    {
        return deriveNonNullSymbols(plannerContext, session, node, identity());
    }

    public static Set<Symbol> deriveNonNullSymbols(PlannerContext plannerContext, Session session, PlanNode node, Function<PlanNode, PlanNode> lookup)
    {
        Visitor visitor = new Visitor(plannerContext, session, lookup);
        return Sets.intersection(visitor.derive(node), ImmutableSet.copyOf(node.getOutputSymbols())).immutableCopy();
    }
            ...
            extends PlanVisitor<Set<Symbol>, Void>
    {
        ...
        // With the iterative optimizer, plan nodes are replaced with GroupReference nodes,
        // which must be resolved through Lookup before visiting.
        private final Function<PlanNode, PlanNode> lookup;
        private final IrExpressionOptimizer evaluator;

        private Visitor(PlannerContext plannerContext, Session session, Function<PlanNode, PlanNode> lookup)
        {
        ...
        public Set<Symbol> derive(PlanNode node)
        {
            return lookup.apply(node).accept(this, null);
        }
