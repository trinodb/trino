        ...
        implements IrOptimizerRule
{
    ...
    public Optional<Expression> apply(Expression expression, Session session, SymbolAllocator symbolAllocator, Map<Symbol, Expression> bindings)
    {
        if (expression instanceof Cast(Call call, Type type, _) &&
            ...
            return switch (type) {
                case ArrayType arrayType -> Optional.of(new Call(metadata.getCoercion(getTypeResolutionPolicy(session), builtinFunctionName(JSON_STRING_TO_ARRAY_NAME), string.type(), arrayType), call.arguments()));
                case MapType mapType -> Optional.of(new Call(metadata.getCoercion(getTypeResolutionPolicy(session), builtinFunctionName(JSON_STRING_TO_MAP_NAME), string.type(), mapType), call.arguments()));
                case RowType rowType -> Optional.of(new Call(metadata.getCoercion(getTypeResolutionPolicy(session), builtinFunctionName(JSON_STRING_TO_ROW_NAME), string.type(), rowType), call.arguments()));
