...
public class SetOperationNodeTranslator
{
    ...
    public SetOperationNodeTranslator(Session session, Metadata metadata, SymbolAllocator symbolAllocator, PlanNodeIdAllocator idAllocator)
    {
        ...
        this.countFunction = metadata.resolveBuiltinFunction(getTypeResolutionPolicy(session), "count", ImmutableList.of(BOOLEAN));
        this.rowNumberFunction = metadata.resolveBuiltinFunction(getTypeResolutionPolicy(session), "row_number", ImmutableList.of());
    }
