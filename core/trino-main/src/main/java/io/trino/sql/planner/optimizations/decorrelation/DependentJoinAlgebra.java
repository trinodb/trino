...
public final class DependentJoinAlgebra
{
    ...
    public DependentJoinAlgebra(Metadata metadata, PlanNodeIdAllocator idAllocator, SymbolAllocator symbolAllocator, Lookup lookup, Node originSubquery)
    {
    ...
    public Expression correlationIdentical(List<Symbol> correlation, List<Symbol> clonedCorrelation)
    {
        ...
        for (int i = 0; i < correlation.size(); i++) {
            conjuncts.add(comparison(metadata, IDENTICAL, correlation.get(i).toSymbolReference(), clonedCorrelation.get(i).toSymbolReference()));
        }
