...
import static io.trino.sql.dialect.ir.Dialect.IR;
import static io.trino.sql.dialect.ir.Dialect.TERMINAL;
        ...
        implements SourceNode
{
    ...
    public String print(int version, int indentLevel)
    {
        ...
        if (!parameters().isEmpty()) {
            builder.append(parameters().stream()
                    .map(parameter -> parameter.name() + " : " + formatType(version, parameter.type()))
                    .collect(joining(", ", " (", ")")));
        }

        builder.append(operations().stream()
                .map(operation -> operation.print(version, indentLevel + 1))
                .collect(joining("\n", "\n", "")));
