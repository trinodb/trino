...
public class PageFunctionCompiler
{
    ...
    // stripped: structurally identical expressions with different literals share one
    // compiled template
    private final ClassTemplateCache<PageProjectionWork> projectionTemplates;
    private final ClassTemplateCache<PageFilter> filterTemplates;
    ...
    public PageFunctionCompiler(FunctionManager functionManager, Metadata metadata, TypeManager typeManager, int expressionCacheSize)
    {
        ...
        projectionTemplates = new ClassTemplateCache<>(PageProjectionWork.class, expressionCacheSize);
        filterTemplates = new ClassTemplateCache<>(PageFilter.class, expressionCacheSize);
    ...
    {
        return projectionTemplates.getStats();
    }
    ...
    private CompiledProjection compileProjectionClass(Expression projection, Map<Symbol, Integer> layout, Optional<String> classNameSuffix)
    {
        ...
        try {
            pageProjectionWorkClass = projectionTemplates.defineClass(
                    projection,
                    ImmutableList.of(),
                    callSiteBinder -> definePageProjectWorkClass(projection, result.compactLayout(), callSiteBinder, classNameSuffix));
        }
    ...
    private Class<? extends PageFilter> compileFilterClass(Expression filter, Map<Symbol, Integer> layout, Optional<String> classNameSuffix)
    {
        ...
        try {
            return filterTemplates.defineClass(
                    filter,
                    ImmutableList.of(),
                    callSiteBinder -> defineFilterClass(filter, result.compactLayout(), callSiteBinder, classNameSuffix));
        }
        catch (TrinoException e) {
            throw e;
        }
