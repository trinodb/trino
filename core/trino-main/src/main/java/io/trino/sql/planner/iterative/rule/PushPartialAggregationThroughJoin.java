...
public class PushPartialAggregationThroughJoin
{
            ...
            implements Rule<AggregationNode>
    {
        ...
        {
            return applyPushdown(node, captures.get(JOIN_NODE), context);
        }
    }
            ...
            implements Rule<AggregationNode>
    {
        ...
        private static final Pattern<AggregationNode> PATTERN_WITH_PROJECTION = aggregation()
                .matching(PushPartialAggregationThroughJoin::isSupportedAggregationNode)
                .with(source().matching(project().capturedAs(PROJECT_NODE).with(source().matching(join()))));
        ...
        public Result apply(AggregationNode node, Captures captures, Context context)
        {
            ...
            return applyPushdown((AggregationNode) node.replaceChildren(ImmutableList.of(joinNodeOptional.get())), (JoinNode) joinNodeOptional.get(), context);
        }
    }
