            ...
            extends PlanVisitor<RewriteResult, Void>
    {
        ...
        public RewriteResult visitEnforceSingleRow(EnforceSingleRowNode node, Void context)
        {
            ...
            Expression predicate = ifExpression(
                    ...
                    new Cast(
                            failFunction(metadata, getTypeResolutionPolicy(session), SUBQUERY_MULTIPLE_ROWS, "Scalar sub-query has returned multiple rows"),
                            BOOLEAN),
                    TRUE);
        ...
        public RewriteResult visitLimit(LimitNode node, Void context)
        {
            ...
            return new RewriteResult(
                    new FilterNode(
                            idAllocator.getNextId(),
                            sourceNode,
                            comparison(metadata, getTypeResolutionPolicy(session), LESS_THAN_OR_EQUAL, rowNumberSymbol.toSymbolReference(), new Constant(BIGINT, node.getCount()))),
        ...
        public RewriteResult visitTopN(TopNNode node, Void context)
        {
            ...
            WindowNode.Function rowNumberFunction = new WindowNode.Function(
                    metadata.resolveBuiltinFunction(getTypeResolutionPolicy(session), "row_number", ImmutableList.of()),
                    ImmutableList.of(),
                    Optional.empty(),
                    DEFAULT_FRAME,
                    false,
