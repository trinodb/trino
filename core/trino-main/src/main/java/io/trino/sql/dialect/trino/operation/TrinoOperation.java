        ...
        // TODO sealed?
{
     ...
     * <p>
     * Currently, it is only implemented for scalar operations, and for the Query operation, and used in CTE reuse to rewrite nested blocks.
     ...
     * <p>
     * Currently, it is only implemented for certain operations:
     * - for relational operations -- to build the new query program with CTE reuse
     * - for FieldReference operation -- to manipulate boolean expressions
     * When we add more optimizations for scalar operations, we will need to implement argument substitution for them.
     */
    public Operation withArgument(Value newArgument, int index)
    {
        validateArgument(newArgument, index);

        if (newArgument.equals(arguments().get(index))) {
            return this;
        }
