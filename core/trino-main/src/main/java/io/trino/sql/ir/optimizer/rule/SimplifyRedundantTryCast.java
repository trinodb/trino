        ...
        implements IrOptimizerRule
{
    ...
    public Optional<Expression> apply(Expression expression, Session session, SymbolAllocator symbolAllocator, Map<Symbol, Expression> bindings)
    {
        ...
        ResolvedFunction coercion = metadata.getCoercion(getTypeResolutionPolicy(session), value.type(), targetType);
        if (!coercion.neverFails()) {
            return Optional.empty();
        }

        return Optional.of(cast(typeManager, getTypeResolutionPolicy(session), value, targetType));
    }
}
