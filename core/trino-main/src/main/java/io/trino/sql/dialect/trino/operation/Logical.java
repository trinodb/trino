        ...
        implements Operation
{
    ...
    public Logical(String resultName, List<Value> terms, LogicalOperator logicalOperator, List<Map<String, Object>> sourceAttributes)
    {
        ...
        terms.stream()
                .forEach(term -> {
                    if (!term.type().equals(BOOLEAN)) {
                        throw new TrinoException(IR_ERROR, "all terms of a logical operation must be of boolean type. found: " + term.type().getDisplayName());
                    }
                });
