        ...
        extends IrVisitor<Void, C>
{
    ...
    {
        for (io.trino.sql.ir.Expression operand : node.getOperands()) {
    ...
    {
        for (io.trino.sql.ir.Expression expression : node.getValues()) {
    ...
    {
        for (io.trino.sql.ir.Expression argument : node.getArguments()) {
    ...
    {
        for (io.trino.sql.ir.WithQuery query : node.getQueries()) {
    ...
    @Override
    protected Void visitWhenClause(io.trino.sql.ir.WhenClause node, C context)
    {
    ...
    protected Void visitFunctionCall(FunctionCall node, C context)
    {
        ...
        if (node.getWindow().isPresent()) {
            process((io.trino.sql.ir.Node) node.getWindow().get(), context);
        }
    ...
    protected Void visitWindowOperation(WindowOperation node, C context)
    {
        ...
        process((io.trino.sql.ir.Node) node.getWindow(), context);
    ...
    {
        for (io.trino.sql.ir.Expression columnArgument : node.getGroupingColumns()) {
    ...
    public Void visitWindowSpecification(WindowSpecification node, C context)
    {
        ...
        for (io.trino.sql.ir.Expression expression : node.getPartitionBy()) {
            process(expression, context);
        }
    ...
    @Override
    protected Void visitWindowDefinition(io.trino.sql.ir.WindowDefinition node, C context)
    {
    ...
    public Void visitWindowFrame(WindowFrame node, C context)
    {
        ...
        for (io.trino.sql.ir.MeasureDefinition measureDefinition : node.getMeasures()) {
            process(measureDefinition, context);
        }
        for (io.trino.sql.ir.VariableDefinition variableDefinition : node.getVariableDefinitions()) {
    ...
    {
        process(node.getOperand(), context);
        for (io.trino.sql.ir.WhenClause clause : node.getWhenClauses()) {
    ...
    {
        for (io.trino.sql.ir.Expression value : node.getValues()) {
    ...
    {
        for (io.trino.sql.ir.SortItem sortItem : node.getSortItems()) {
    ...
    {
        for (io.trino.sql.ir.Expression expression : node.getItems()) {
            process(expression, context);
        }
        return null;
    }
    ...
    protected Void visitJoin(Join node, C context)
    {
        ...
        node.getCriteria()
                .filter(criteria -> criteria instanceof io.trino.sql.ir.JoinOn)
    ...
    {
        for (io.trino.sql.ir.RowPattern rowPattern : node.getPatterns()) {
