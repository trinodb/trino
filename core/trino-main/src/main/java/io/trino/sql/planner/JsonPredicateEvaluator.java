...
import io.trino.sql.planner.JsonPathEvaluator.Context;
...
import static io.trino.sql.planner.JsonPathEvaluator.unwrapArrays;
...
public class JsonPredicateEvaluator
{
            ...
            extends IrJsonPathVisitor<Boolean, Context>
    {
        ...
        private Boolean compare(IrComparisonPredicate.Operator comparisonOperator, TypedValue left, TypedValue right)
        {
            ...
            if (!signature.getArgumentTypes().get(0).equals(firstType) && !pathEvaluator.isTypeOnlyCoercion(firstType, signature.getArgumentTypes().get(0))) {
                ResolvedFunction firstCast;
                try {
                    firstCast = pathEvaluator.getCoercion(firstType, signature.getArgumentTypes().get(0));
                }
                catch (OperatorNotFoundException e) {
                    return null;
                }
            ...
            if (!signature.getArgumentTypes().get(1).equals(secondType) && !pathEvaluator.isTypeOnlyCoercion(secondType, signature.getArgumentTypes().get(1))) {
                ResolvedFunction secondCast;
                try {
                    secondCast = pathEvaluator.getCoercion(secondType, signature.getArgumentTypes().get(1));
                }
                catch (OperatorNotFoundException e) {
                    return null;
                }
