...
public final class DomainCoercer
{
    ...
    private static class ImplicitCoercer
    {
                ...
                Type coercedValueType)
        {
            ...
            try {
                this.saturatedFloorCastOperator = metadata.getCoercion(getTypeResolutionPolicy(session), SATURATED_FLOOR_CAST, originalValueType, coercedValueType);
            }
            catch (OperatorNotFoundException e) {
                throw new IllegalStateException(
                        format("Saturated floor cast operator not found for coercion from %s to %s", originalValueType, coercedValueType));
            }
            this.castToOriginalTypeOperator = metadata.getCoercion(getTypeResolutionPolicy(session), coercedValueType, originalValueType);
