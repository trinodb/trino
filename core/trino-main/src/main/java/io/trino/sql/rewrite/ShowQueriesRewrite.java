        ...
        implements StatementRewrite.Rewrite
{
            ...
            extends AstVisitor<Node, Void>
    {
        ...
        @Override
        protected Node visitShowBranches(ShowBranches showColumns, Void context)
        {
            QualifiedObjectName tableName = createQualifiedObjectName(session, showColumns, showColumns.table());
            getRequiredCatalogHandle(metadata, session, showColumns, tableName.catalogName());
            if (!metadata.schemaExists(session, new CatalogSchemaName(tableName.catalogName(), tableName.schemaName()))) {
                throw semanticException(SCHEMA_NOT_FOUND, showColumns, "Schema '%s' does not exist", tableName.schemaName());
            }
            if (metadata.isMaterializedView(session, tableName)) {
                throw semanticException(NOT_SUPPORTED, showColumns, "Relation '%s' is a materialized view, not a table", tableName);
            }
            if (metadata.isView(session, tableName)) {
                throw semanticException(NOT_SUPPORTED, showColumns, "Relation '%s' is a view, not a table", tableName);
            }
            ...
            for (String branch : metadata.listBranches(session, tableHandle.get())) {
