        ...
        extends PlanVisitor<OperationAndMapping, Context>
{
    ...
    public OperationAndMapping visitCorrelatedJoin(CorrelatedJoinNode node, Context context)
    {
        ...
        Type inputRowType = relationRowType(input.operation().result().type());
        ...
        Map<Symbol, String> subqueryMapping = deriveOutputMapping(relationRowType(subqueryBuilder.recentOperation().result().type()), node.getSubquery().getOutputSymbols());
        Map<String, Object> subqueryAttributes = subqueryBuilder.recentOperation().attributes();
        ...
        Block.Parameter secondFilterParameter = new Block.Parameter(
                nameAllocator.newName(),
                relationRowType(subquery.getReturnedType()));
        ...
        Map<Symbol, String> outputMapping = deriveOutputMapping(relationRowType(correlatedJoin.result().type()), node.getOutputSymbols());
    ...
    public OperationAndMapping visitFilter(FilterNode node, Context context)
    {
        ...
        Block.Parameter predicateParameter = new Block.Parameter(
                nameAllocator.newName(),
                relationRowType(input.operation().result().type()));
        ...
        Map<Symbol, String> outputMapping = deriveOutputMapping(relationRowType(filter.result().type()), node.getOutputSymbols());
    ...
    public OperationAndMapping visitValues(ValuesNode node, Context context)
    {
        ...
        Map<Symbol, String> outputMapping = deriveOutputMapping(relationRowType(values.result().type()), node.getOutputSymbols());
