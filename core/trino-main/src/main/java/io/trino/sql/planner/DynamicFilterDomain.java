...
public class DynamicFilterDomain
{
    ...
    public DynamicFilterDomain(BloomFilterWithRange.LongBloomFilter bloomFilter, ValueSet ranges, Type type, boolean nullAllowed)
    {
    ...
    public DynamicFilterDomain simplifyDomains(int threshold)
    {
        if (domain.isEmpty()) {
            ...
            return fromBloomFilter(new BloomFilterWithRange(
                    ...
                    ValueSet.ofRanges(filterWithRange.getSpan()),
    ...
    public DynamicFilterDomain union(DynamicFilterDomain other)
    {
        ...
        if (bloomfilterWithRange.isPresent() && other.bloomfilterWithRange.isPresent()) {
            ...
            return new DynamicFilterDomain(
                    newBloomfilter,
                    bloomfilterWithRange.get().ranges().union(other.bloomfilterWithRange.get().ranges()),
    ...
    private static DynamicFilterDomain intersect(BloomFilterWithRange filterWithRange, Domain domain)
    {
        ...
        if (domain.isNullableDiscreteSet()) {
            ...
            return new DynamicFilterDomain(
                    bloomFilter,
                    domain.intersect(Domain.create(filterWithRange.ranges(), filterWithRange.nullAllowed())).getValues(),
                    filterWithRange.type(),
                    domain.isNullAllowed() && filterWithRange.nullAllowed());
        }
        Domain bitmapSpan = toDomain(filterWithRange);
        return fromDomain(bitmapSpan.intersect(domain));
    }
    ...
    private static Optional<BloomFilterWithRange> toBloomFilter(Domain domain)
    {
        ...
        return Optional.of(new BloomFilterWithRange(bloomFilter, domain.getValues(), domain.getType(), domain.isNullAllowed()));
    }
}
