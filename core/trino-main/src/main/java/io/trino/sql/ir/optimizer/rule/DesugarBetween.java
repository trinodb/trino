    ...
    public Optional<Expression> apply(Expression expression, Session session, Map<Symbol, Expression> bindings)
    {
        return switch (expression) {
            case Between(Constant value, Expression min, Expression max) -> Optional.of(new Logical(AND, ImmutableList.of(
                    comparison(metadata, LESS_THAN_OR_EQUAL, min, value),
            ...
            case Between(Expression value, Constant min, Constant max) -> {
                if (min.value() == null && max.value() == null) {
                    ...
                    yield Optional.of(ifExpression(comparison(metadata, GREATER_THAN, value, max), FALSE, NULL_BOOLEAN));
                }
                else if (max.value() == null) {
                    yield Optional.of(ifExpression(comparison(metadata, LESS_THAN, value, min), FALSE, NULL_BOOLEAN));
                }
            ...
            case Between(Expression value, Constant min, Expression max) -> min.value() == null ?
                    Optional.of(ifExpression(comparison(metadata, GREATER_THAN, value, max), FALSE, NULL_BOOLEAN)) :
                    Optional.empty();
            case Between(Expression value, Expression min, Constant max) -> max.value() == null ?
                    Optional.of(ifExpression(comparison(metadata, LESS_THAN, value, min), FALSE, NULL_BOOLEAN)) :
                    Optional.empty();
