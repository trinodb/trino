                ...
                extends PlanVisitor<Optional<PlanNode>, Void>
        {
            ...
            public Optional<PlanNode> visitLimit(LimitNode node, Void context)
            {
                ...
                return Optional.of(new FilterNode(
                        idAllocator.getNextId(),
                        numbered,
                        comparison(metadata, LESS_THAN_OR_EQUAL, counter.toSymbolReference(), new Constant(BIGINT, node.getCount()))));
            }
            ...
            public Optional<PlanNode> visitTopN(TopNNode node, Void context)
            {
                ...
                return Optional.of(new FilterNode(
                        idAllocator.getNextId(),
                        window,
                        comparison(metadata, LESS_THAN_OR_EQUAL, rowNumber.toSymbolReference(), new Constant(BIGINT, node.getCount()))));
            }
            ...
            private Expression joinConditionOf(JoinNode node)
            {
                ...
                for (JoinNode.EquiJoinClause clause : node.getCriteria()) {
                    conjuncts.add(comparison(metadata, EQUAL, clause.getLeft().toSymbolReference(), clause.getRight().toSymbolReference()));
                }
