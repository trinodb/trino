        ...
        extends DependentJoinFrameworkRule
{
    ...
    private static final class Rewriter
    {
        ...
        private Rewriter(PlannerContext plannerContext, Context context, DependentJoinAlgebra algebra)
        {
            ...
            this.charVarcharCoercion = getCharVarcharCoercion(context.getSession());
                ...
                extends PlanVisitor<Optional<PlanNode>, Void>
        {
            ...
            private PushDownVisitor(Rewriter rewriter, PlanNode input, Set<Symbol> correlation)
            {
                ...
                this.charVarcharCoercion = rewriter.charVarcharCoercion;
            ...
            public Optional<PlanNode> visitLimit(LimitNode node, Void context)
            {
                ...
                return Optional.of(new FilterNode(
                        idAllocator.getNextId(),
                        numbered,
                        comparison(metadata, charVarcharCoercion, LESS_THAN_OR_EQUAL, counter.toSymbolReference(), new Constant(BIGINT, node.getCount()))));
            }
            ...
            private WindowNode rankWindow(PlanNode source, List<Symbol> partition, Symbol rank, Optional<OrderingScheme> ordering)
            {
                WindowNode.Function rankFunction = new WindowNode.Function(
                        metadata.resolveBuiltinFunction(charVarcharCoercion, "rank", ImmutableList.of()),
                        ImmutableList.of(),
                        Optional.empty(),
                        DEFAULT_FRAME,
                        false,
            ...
            public Optional<PlanNode> visitTopN(TopNNode node, Void context)
            {
                ...
                WindowNode.Function rowNumberFunction = new WindowNode.Function(
                        metadata.resolveBuiltinFunction(charVarcharCoercion, "row_number", ImmutableList.of()),
                        ImmutableList.of(),
                        Optional.empty(),
                        DEFAULT_FRAME,
                        false,
                ...
                return Optional.of(new FilterNode(
                        idAllocator.getNextId(),
                        window,
                        comparison(metadata, charVarcharCoercion, LESS_THAN_OR_EQUAL, rowNumber.toSymbolReference(), new Constant(BIGINT, node.getCount()))));
            }
            ...
            private Expression joinConditionOf(JoinNode node)
            {
                ...
                for (JoinNode.EquiJoinClause clause : node.getCriteria()) {
                    conjuncts.add(comparison(metadata, charVarcharCoercion, EQUAL, clause.getLeft().toSymbolReference(), clause.getRight().toSymbolReference()));
                }
