...
public class TranslationMap
{
    ...
    private io.trino.sql.ir.Expression translateOverlaps(io.trino.sql.ir.Expression left, OverlapsPredicate predicate)
    {
        ...
        // Order each period's endpoints per the spec (§8.14 GR 4-6) into a lower bound s and an
        // upper bound t, binding each so the operands are evaluated once. The ordering is
        // null-asymmetric: when one endpoint is null, s is the *known* endpoint and only t can be
        // null — unlike least/greatest, which return null on any null argument. GR 7's formula
        // relies on that asymmetry to still yield a definite result when a single bound is null.
        ...
        // GR 7: (s1 > s2 AND NOT (s1 >= t2 AND t1 >= t2))
    ...
    // Lower bound s of a period (§8.14 GR 4-6): the smaller endpoint, or — when the start is null —
    // the known end, so null is pushed onto the upper bound instead of swallowing both.
