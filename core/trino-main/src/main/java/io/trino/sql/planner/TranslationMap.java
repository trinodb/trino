    ...
    private io.trino.sql.ir.Expression translatePartial(io.trino.sql.ir.Expression value, Predicate clause)
    {
        return switch (clause) {
            ...
            case SubmultisetPredicate predicate -> {
                io.trino.sql.ir.Expression right = translateExpression(predicate.getRight());
                io.trino.sql.ir.Expression submultiset = BuiltinFunctionCallBuilder.resolve(plannerContext.getMetadata(), getCharVarcharCoercion(session))
                        .setName("$submultiset")
            ...
            case SetPredicate predicate -> {
                io.trino.sql.ir.Expression isASet = BuiltinFunctionCallBuilder.resolve(plannerContext.getMetadata(), getCharVarcharCoercion(session))
                        ...
                        .addArgument(value.type(), value)
                        .build();
                yield predicate.isNegated() ? not(plannerContext.getMetadata(), getCharVarcharCoercion(session), isASet) : isASet;
            }
        };
    }
