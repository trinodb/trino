...
import static io.trino.SystemSessionProperties.enableUnnestSubfieldPruning;
import static io.trino.SystemSessionProperties.isAllowPushdownIntoConnectors;
import static io.trino.SystemSessionProperties.isLegacyUnnest;
 ...
 * 1. Waiting on OSS's decision to accept Subfield to replace list of dereference names that currently being used
 ...
 * 3. If this is not what OSS want to achieve, then we can fall back to Presto's way to generate Subfields with minimum impact
 * 4. The scope of changes will be very large to support more use cases, and at this point we want to unblock customer issue first
 * 5. The PruneUnnestMappings has been accepted in OSS, which this rule is relying on, there is risk that this need to be rewritten
 *
    ...
    public Result apply(ProjectNode project, Captures captures, Context context)
    {
        ...
        partialTranslations = partialTranslations.entrySet().stream().filter(entry -> {
            ...
            return arrayFieldDereference.getTarget() instanceof Variable
                    && symbolReferenceNamesCount.get(((Variable) arrayFieldDereference.getTarget()).getName()) == 1;
        ...
        return Result.ofPlanNode(
                new ProjectNode(
                        ...
                        project.getProtectedBarrier()));
    }

    /**
     * Extract the sub-expressions of type {@link SymbolReference} from the {@param expression}
     */
    private static List<SymbolReference> getSymbolReferences(Expression expression)
    {
