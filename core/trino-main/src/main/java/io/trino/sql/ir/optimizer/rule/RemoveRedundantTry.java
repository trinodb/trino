        ...
        implements IrOptimizerRule
{
    ...
    public Optional<Expression> apply(Expression expression, Session session, SymbolAllocator symbolAllocator, Map<Symbol, Expression> bindings)
    {
        ...
        return switch (call.arguments().getFirst()) {
            case Lambda lambda -> mayFail(context, getTypeResolutionPolicy(session), lambda.body()) ? Optional.empty() : Optional.of(lambda.body());
            case Bind bind -> {
                Expression body = bind.function().body();
                if (mayFail(context, getTypeResolutionPolicy(session), body)) {
