...
/// evaluating `e` and returns `NULL` instead. When `e` is infallible that handler is dead, so the two are
/// equivalent for every input. Removing the wrapper drops the per-row lambda dispatch and exception handling
/// and, because `$try` is non-deterministic, restores the body's own determinism, re-enabling the
/// optimizations that determinism gates (dictionary processing, constant folding, common-subexpression
/// extraction, ...).
