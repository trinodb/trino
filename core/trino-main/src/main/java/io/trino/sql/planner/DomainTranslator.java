...
public final class DomainTranslator
{
    ...
    public Expression toPredicate(TypeResolutionPolicy typeResolutionPolicy, TupleDomain<Symbol> tupleDomain)
    {
        return IrUtils.combineConjuncts(toPredicateConjuncts(typeResolutionPolicy, tupleDomain));
    }

    private List<Expression> toPredicateConjuncts(TypeResolutionPolicy typeResolutionPolicy, TupleDomain<Symbol> tupleDomain)
    {
        ...
        return domains.entrySet().stream()
                ...
                .map(entry -> toPredicate(typeResolutionPolicy, entry.getValue(), entry.getKey().toSymbolReference()))
                .collect(toImmutableList());
    }

    public Expression toPredicate(TypeResolutionPolicy typeResolutionPolicy, Domain domain, Reference reference)
    {
        ...
        if (domain.getValues().isAll()) {
            return domain.isNullAllowed() ? TRUE : not(metadata, typeResolutionPolicy, new IsNull(reference));
        }
        ...
        disjuncts.addAll(domain.getValues().getValuesProcessor().transform(
                ranges -> extractDisjuncts(typeResolutionPolicy, domain.getType(), ranges, reference),
                discreteValues -> extractDisjuncts(typeResolutionPolicy, domain.getType(), discreteValues, reference),
                _ -> {
    ...
    private Expression processRange(TypeResolutionPolicy typeResolutionPolicy, Type type, Range range, Reference reference)
    {
        if (range.isAll()) {
            return TRUE;
        }
        ...
        if (isBetween(range)) {
            return new Logical(AND, ImmutableList.of(
                    comparison(metadata, typeResolutionPolicy, GREATER_THAN_OR_EQUAL, reference, new Constant(type, range.getLowBoundedValue())),
                    comparison(metadata, typeResolutionPolicy, LESS_THAN_OR_EQUAL, reference, new Constant(type, range.getHighBoundedValue()))));
        }
    ...
    private Expression combineRangeWithExcludedPoints(TypeResolutionPolicy typeResolutionPolicy, Type type, Reference reference, Range range, List<Expression> excludedPoints)
    {
        if (excludedPoints.isEmpty()) {
            return processRange(typeResolutionPolicy, type, range, reference);
        }

        Expression excludedPointsExpression = not(metadata, typeResolutionPolicy, new In(reference, excludedPoints));
        if (excludedPoints.size() == 1) {
            excludedPointsExpression = comparison(metadata, typeResolutionPolicy, NOT_EQUAL, reference, getOnlyElement(excludedPoints));
        }

        return combineConjuncts(processRange(typeResolutionPolicy, type, range, reference), excludedPointsExpression);
    }

    private List<Expression> extractDisjuncts(TypeResolutionPolicy typeResolutionPolicy, Type type, Ranges ranges, Reference reference)
    {
        List<Expression> disjuncts = new ArrayList<>();
        ...
        if (typeHasNaN(type)) {
            ...
            if (!originalRangeIsAll && coalescedRangeIsAll) {
                for (Range range : orderedRanges) {
                    disjuncts.add(processRange(typeResolutionPolicy, type, range, reference));
                }
                return disjuncts;
            }
        }
        ...
        for (Range range : originalUnionSingleValues) {
            ...
            if (!singleValuesInRange.isEmpty()) {
                disjuncts.add(combineRangeWithExcludedPoints(typeResolutionPolicy, type, reference, range, singleValuesInRange));
                continue;
            }
        ...
        if (singleValues.size() == 1) {
            disjuncts.add(comparison(metadata, typeResolutionPolicy, EQUAL, reference, getOnlyElement(singleValues)));
        }
    ...
    private List<Expression> extractDisjuncts(TypeResolutionPolicy typeResolutionPolicy, Type type, DiscreteValues discreteValues, Reference reference)
    {
        ...
        if (values.size() == 1) {
            predicate = comparison(metadata, typeResolutionPolicy, EQUAL, reference, getOnlyElement(values));
        }
        else {
        ...
        if (!discreteValues.isInclusive()) {
            predicate = not(metadata, typeResolutionPolicy, predicate);
        }
            ...
            extends IrVisitor<ExtractionResult, Boolean>
    {
        ...
        {
            return complement ? not(plannerContext.getMetadata(), getTypeResolutionPolicy(session), expression) : expression;
        }
        ...
        protected ExtractionResult visitReference(Reference node, Boolean complement)
        {
            if (node.type().equals(BOOLEAN)) {
                return process(comparison(plannerContext.getMetadata(), getTypeResolutionPolicy(session), EQUAL, node, TRUE), complement);
            }
            ...
            return switch (comparisonOperator) {
                case GREATER_THAN_OR_EQUAL, GREATER_THAN -> {
                    if (coercedValueIsGreaterThanOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), GREATER_THAN_OR_EQUAL, symbolExpression, coercedLiteral);
                    }
                    if (coercedValueIsEqualToOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), comparisonOperator, symbolExpression, coercedLiteral);
                    }
                    if (coercedValueIsLessThanOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), GREATER_THAN, symbolExpression, coercedLiteral);
                    }
                    throw new AssertionError("Unreachable");
                }
                case LESS_THAN_OR_EQUAL, LESS_THAN -> {
                    if (coercedValueIsLessThanOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), LESS_THAN_OR_EQUAL, symbolExpression, coercedLiteral);
                    }
                    if (coercedValueIsEqualToOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), comparisonOperator, symbolExpression, coercedLiteral);
                    }
                    if (coercedValueIsGreaterThanOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), LESS_THAN, symbolExpression, coercedLiteral);
                    }
                    throw new AssertionError("Unreachable");
                }
                case EQUAL -> {
                    if (coercedValueIsEqualToOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), EQUAL, symbolExpression, coercedLiteral);
                    }
                    // Return something that is false for all non-null values
                    yield and(comparison(metadata, getTypeResolutionPolicy(session), GREATER_THAN, symbolExpression, coercedLiteral),
                            comparison(metadata, getTypeResolutionPolicy(session), LESS_THAN, symbolExpression, coercedLiteral));
                }
                case NOT_EQUAL -> {
                    if (coercedValueIsEqualToOriginal) {
                        yield comparison(metadata, getTypeResolutionPolicy(session), comparisonOperator, symbolExpression, coercedLiteral);
                    ...
                    yield or(comparison(metadata, getTypeResolutionPolicy(session), EQUAL, symbolExpression, coercedLiteral),
                            comparison(metadata, getTypeResolutionPolicy(session), NOT_EQUAL, symbolExpression, coercedLiteral));
                }
                ...
                case IDENTICAL -> coercedValueIsEqualToOriginal ?
                        comparison(metadata, getTypeResolutionPolicy(session), comparisonOperator, symbolExpression, coercedLiteral) :
                        FALSE;
            };
        }
        ...
        {
            try {
                return Optional.of(plannerContext.getMetadata().getCoercion(getTypeResolutionPolicy(session), SATURATED_FLOOR_CAST, fromType, toType));
            }
        ...
        private int compareOriginalValueToCoerced(Type originalValueType, Object originalValue, Type coercedValueType, Object coercedValue)
        {
            ...
            ResolvedFunction castToOriginalTypeOperator = plannerContext.getMetadata().getCoercion(getTypeResolutionPolicy(session), coercedValueType, originalValueType);
        ...
        protected ExtractionResult visitIn(In node, Boolean complement)
        {
            ...
            for (Expression expression : node.valueList()) {
                disjuncts.add(comparison(plannerContext.getMetadata(), getTypeResolutionPolicy(session), EQUAL, node.value(), expression));
            }
            ...
            if (extractionResult.tupleDomain().isAll()) {
                ...
                if (complement) {
                    originalPredicate = not(plannerContext.getMetadata(), getTypeResolutionPolicy(session), originalPredicate);
                }
        ...
        private Optional<ExtractionResult> processSimpleInPredicate(In node, Boolean complement)
        {
            ...
            if (excludedExpressions.isEmpty()) {
                ...
                remainingExpression = not(plannerContext.getMetadata(), getTypeResolutionPolicy(session), comparison(plannerContext.getMetadata(), getTypeResolutionPolicy(session), EQUAL, node.value(), getOnlyElement(excludedExpressions)));
            }
            else {
                remainingExpression = not(plannerContext.getMetadata(), getTypeResolutionPolicy(session), new In(node.value(), excludedExpressions));
            }
        ...
        protected ExtractionResult visitCall(Call node, Boolean complement)
        {
            if (matchComparison(node) instanceof Comparison comparison) {
                return processComparison(comparison, node, complement);
            }
