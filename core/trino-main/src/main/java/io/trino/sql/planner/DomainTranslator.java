...
public final class DomainTranslator
{
            ...
            extends IrVisitor<ExtractionResult, Boolean>
    {
        ...
        protected ExtractionResult visitBetween(Between node, Boolean complement)
        {
            ...
            return process(and(
                    comparison(plannerContext.getMetadata(), GREATER_THAN_OR_EQUAL, node.value(), node.min()),
                    comparison(plannerContext.getMetadata(), LESS_THAN_OR_EQUAL, node.value(), node.max())), complement);
        }
