            ...
            extends IrVisitor<ExtractionResult, Boolean>
    {
        ...
        private ExtractionResult processComparison(Comparison node, Expression originalExpression, Boolean complement)
        {
            ...
            if (symbolExpression instanceof Cast castExpression) {
                ...
                if (castSourceType instanceof CharType charType && castTargetType instanceof VarcharType varcharType) {
                    Optional<Expression> rewritten = rewriteCharCastToVarcharComparison(
                            charType,
                            varcharType,
                    ...
                    if (rewritten.isPresent()) {
                        return process(rewritten.get(), complement);
                    }
        ...
        private boolean isOrderPreserving(Cast cast)
        {
            if (cast.expression().type() instanceof CharType && cast.type() instanceof VarcharType) {
                // Char comparison is PAD SPACE while varchar comparison is NO PAD, so a char value ending with a
                // character below the space sorts before its prefix as a char and after it as a varchar:
                // CHAR 'a\0' < CHAR 'a', while 'a\0' > 'a' as varchar. Comparisons of such a cast are handled in
                // processComparison, which does not rely on this method.
                return false;
            }
        ...
        /**
         * Rewrite {@code CAST(char_expression AS varchar) OP varchar_constant} into an equivalent comparison of the
         * char expression with a char constant, so that a domain can be extracted for the char expression.
         */
        private Optional<Expression> rewriteCharCastToVarcharComparison(
                CharType charType,
                ...
                NullableValue value)
        {
            ...
            if (!(charExpression instanceof Reference)) {
                // Calculation is not useful, and the always-false and always-true forms below mention the expression twice
                return Optional.empty();
            }
            // Only the equality family translates to an equivalent comparison of char values. The cast is not
            // order-preserving, so ordering comparisons only yield a wider domain; they are handled in
            // createCharCastToVarcharOrderingExtractionResult.
            if (operator != EQUAL && operator != NOT_EQUAL && operator != IDENTICAL) {
                return Optional.empty();
            }
            // A cast to a varchar shorter than the char length maps distinct char values to the same varchar value,
            // so a comparison of the cast values is not a comparison of the char values. The implicit coercion is
            // char(n) -> varchar(n), so this only rejects explicit narrowing casts.
            ...
            Object literalInChar = functionInvoker.invoke(varcharToChar, session.toConnectorSession(), value.getValue());

            Metadata metadata = plannerContext.getMetadata();
            Constant charLiteral = new Constant(charType, literalInChar);
            if (value.getValue().equals(functionInvoker.invoke(charToVarchar, session.toConnectorSession(), literalInChar))) {
                // The constant is the varchar form of a char value, and the cast is injective, so comparing the cast
                // values is the same as comparing the char values.
                return Optional.of(comparison(metadata, getCharVarcharCoercion(session), operator, charExpression, charLiteral));
            }
            // No char value has the constant as its varchar form: the constant either carries trailing spaces the cast
            // trims (default coercion), or lacks the padding the cast adds (legacy coercion).
            return Optional.of(switch (operator) {
                // Something that is false for all non-null values
                case EQUAL -> and(
                        comparison(metadata, getCharVarcharCoercion(session), GREATER_THAN, charExpression, charLiteral),
                        comparison(metadata, getCharVarcharCoercion(session), LESS_THAN, charExpression, charLiteral));
                // Something that is true for all non-null values
                case NOT_EQUAL -> or(
                        comparison(metadata, getCharVarcharCoercion(session), EQUAL, charExpression, charLiteral),
                        comparison(metadata, getCharVarcharCoercion(session), NOT_EQUAL, charExpression, charLiteral));
                case IDENTICAL -> FALSE;
                case LESS_THAN, LESS_THAN_OR_EQUAL, GREATER_THAN, GREATER_THAN_OR_EQUAL -> throw new IllegalStateException("Unexpected operator: " + operator);
            });
        }

        /**
         * Extract a domain from {@code CAST(char_expression AS varchar) OP varchar_constant} for an ordering
         * {@code OP}. The cast is not order-preserving, so the domain is a superset of the values satisfying the
         * comparison and the comparison must be retained.
         */
        ...
        /**
         * Extract a domain from {@code CAST(varchar_expression AS char) OP char_constant}. The cast truncates and trims
