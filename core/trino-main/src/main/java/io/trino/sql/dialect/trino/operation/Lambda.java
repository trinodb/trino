        ...
        implements Operation
{
    ...
    public Lambda(String resultName, Block lambda)
    {
        ...
        if (lambda.parameters().size() != 1 ||
                !(lambda.parameters().getFirst().type() instanceof RowType ||
                        lambda.parameters().getFirst().type().equals(EMPTY_ROW))) {
            throw new TrinoException(IR_ERROR, format("invalid argument type for lambda: %s. expected RowType or EmptyRowType", lambda.parameters().getFirst().type()));
        }
        this.lambda = singleBlockRegion(lambda);

        FunctionType resultType = new FunctionType(
                lambda.parameters().getFirst().type().getTypeParameters(),
                lambda.getReturnedType());
