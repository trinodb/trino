...
public class CursorProcessorCompiler
{
    ...
    private Class<? extends CursorProcessor> compileProcessor(RowExpression filter, List<RowExpression> projections)
    {
        ...
        for (int projectionIndex = 0; projectionIndex < projections.size(); projectionIndex += MAX_PROJECTIONS_PER_CHUNK) {
            int start = projectionIndex;
            int end = Math.min(projectionIndex + MAX_PROJECTIONS_PER_CHUNK, projections.size());
            int chunkNumber = chunkClasses.size();

            CursorProcessorChunkClass chunkClass = generateCursorProcessorChunk(projections, start, end, callSiteBinder, chunkNumber);
            ...
            int chunkNumber)
    {
        ...
        MethodDefinition project = generateProjectChunk(definition, start, end, callSiteBinder, cachedInstanceBinder, projections);
            ...
            List<RowExpression> projections)
    {
        ...
        for (int projectionIndex = start; projectionIndex < end; projectionIndex++) {
            ...
            Map<LambdaDefinitionExpression, CompiledLambda> projectCompiledLambdaMap = generateMethodsForLambda(
                    ...
                    projections.get(projectionIndex),
                    methodName);
