 ...
 *  - TableFinish
 *    - (optional) Exchange
 *      - TableWriter(TableExecuteTarget)
 *        - (optional) Exchange
 *          - empty Values
 * </pre>
 * into
        ...
        implements Rule<TableFinishNode>
{
    ...
    public Result apply(TableFinishNode finishNode, Captures captures, Context context)
    {
        ...
        if (finishSource.isEmpty() || !(finishSource.get() instanceof TableWriterNode) || !(((TableWriterNode) finishSource.get()).getTarget() instanceof TableWriterNode.TableExecuteTarget)) {
            return Result.empty();
        }

        Optional<PlanNode> tableWriterSource = getSingleSourceSkipExchange(finishSource.get(), context.getLookup());
        if (tableWriterSource.isEmpty() || !(tableWriterSource.get() instanceof ValuesNode)) {
            return Result.empty();
        }
        ValuesNode valuesNode = (ValuesNode) tableWriterSource.get();

        verify(valuesNode.getRowCount() == 0, "Unexpected non-empty Values as source of TableWriterNode for Table Execute");
