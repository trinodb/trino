...
public final class IrExpressionUtils
{
    private IrExpressionUtils() {}

    public static List<Expression> extractConjuncts(Expression expression)
    {
        return extractPredicates(io.trino.sql.tree.LogicalExpression.Operator.AND, expression);
    }

    public static List<Expression> extractDisjuncts(Expression expression)
    {
        return extractPredicates(io.trino.sql.tree.LogicalExpression.Operator.OR, expression);
    }
    ...
    public static List<Expression> extractPredicates(io.trino.sql.tree.LogicalExpression.Operator operator, Expression expression)
    {
    ...
    private static void extractPredicates(io.trino.sql.tree.LogicalExpression.Operator operator, Expression expression, ImmutableList.Builder<Expression> resultBuilder)
    {
    ...
    {
        return logicalExpression(io.trino.sql.tree.LogicalExpression.Operator.AND, expressions);
    }
    ...
    {
        return logicalExpression(io.trino.sql.tree.LogicalExpression.Operator.OR, expressions);
    }

    public static Expression logicalExpression(io.trino.sql.tree.LogicalExpression.Operator operator, Collection<Expression> expressions)
    {
    ...
    {
        if (operator == io.trino.sql.tree.LogicalExpression.Operator.AND) {
    ...
    public static Expression combineConjuncts(Metadata metadata, Collection<Expression> expressions)
    {
        ...
        List<Expression> conjuncts = expressions.stream()
                .flatMap(e -> IrExpressionUtils.extractConjuncts(e).stream())
    ...
    public static Expression combineDisjunctsWithDefault(Metadata metadata, Collection<Expression> expressions, Expression emptyDefault)
    {
        ...
        List<Expression> disjuncts = expressions.stream()
                .flatMap(e -> IrExpressionUtils.extractDisjuncts(e).stream())
    ...
    private static boolean constantExpressionEvaluatesSuccessfully(PlannerContext plannerContext, Session session, Expression constantExpression)
    {
        ...
        Object literalValue = interpreter.optimize(IrNoOpSymbolResolver.INSTANCE);
