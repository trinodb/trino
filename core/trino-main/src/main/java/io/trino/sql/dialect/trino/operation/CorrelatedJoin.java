        ...
        implements Operation
{
    ...
    public CorrelatedJoin(
            ...
            Map<String, Object> subqueryAttributes)
    {
        ...
        if (!IS_RELATION.test(input.type()) || !IS_RELATION.test(subquery.getReturnedType())) {
        ...
        List<Type> outputTypes = ImmutableList.<Type>builder()
                .addAll((relationRowType(input.type())).getTypeParameters())
                .addAll((relationRowType(subquery.getReturnedType())).getTypeParameters())
                .build();

        if (outputTypes.isEmpty()) {
            this.result = new Result(resultName, new MultisetType(EMPTY_ROW));
        }
        else {
            this.result = new Result(resultName, new MultisetType(assignRelationRowTypeFieldNames(RowType.anonymous(outputTypes))));
        }

        this.input = input;

        if (correlation.parameters().size() != 1 ||
                !correlation.parameters().getFirst().type().equals(relationRowType(input.type())) ||
                !(correlation.getReturnedType() instanceof RowType || correlation.getReturnedType().equals(EMPTY_ROW))) {
        ...
        if (subquery.parameters().size() != 1 ||
                !subquery.parameters().getFirst().type().equals(relationRowType(input.type())) ||
                !IS_RELATION.test(subquery.getReturnedType())) {
        ...
        if (filter.parameters().size() != 2 ||
                !filter.parameters().get(0).type().equals(relationRowType(input.type())) ||
                !filter.parameters().get(1).type().equals(relationRowType(subquery.getReturnedType())) ||
                !(filter.getReturnedType().equals(BOOLEAN))) {
