...
public final class GroupConstantAnalysis
{
    ...
    public static Set<Symbol> groupConstantSymbols(Expression condition, Set<Symbol> rightSymbols, TypeCoercion typeCoercion)
    {
        ...
        for (Expression conjunct : extractConjuncts(condition)) {
            if (conjunct instanceof Comparison comparison && comparison.operator() == EQUAL) {
                equatedRightSymbol(comparison.left(), comparison.right(), rightSymbols, typeCoercion).ifPresent(constants::add);
                equatedRightSymbol(comparison.right(), comparison.left(), rightSymbols, typeCoercion).ifPresent(constants::add);
            }
        }
