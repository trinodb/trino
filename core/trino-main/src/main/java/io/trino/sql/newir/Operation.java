...
import static io.trino.sql.newir.FormatOptions.formatType;
        ...
        extends SourceNode
{
    ...
    record AttributeKey(String dialect, String name)
    {
    }
    ...
    default String print(int version, int indentLevel)
    {
        ...
        builder.append(indent)
                ...
                .append(arguments().stream()
                        .map(Value::type)
                        .map(type -> formatType(version, type))
                        .collect(joining(", ", "(", ")")))
                .append(" -> ")
                .append(formatType(version, result().type()))
                .append(regions().stream()
                        .map(region -> region.print(version, indentLevel + 1))
                        .collect(joining(", ", " (", ")")));
        ...
        if (!attributes().isEmpty()) {
            builder.append("\n")
                    ...
                    .append(attributes().entrySet().stream()
                            .map(entry -> formatName(version, entry.getKey()) + " = " + entry.getValue().toString())
                            .collect(joining(", ", "{", "}")));
        }
