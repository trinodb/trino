 ...
 * <p>
 * {@code real} and {@code double} have two zeros, and only one of them is an identity for a given
 * operation: {@code x + -0.0} and {@code x - 0.0} are {@code x} for every {@code x}, while
 * {@code -0.0 + 0.0} and {@code -0.0 - -0.0} are both {@code 0.0}. Multiplying or dividing by one keeps
 * the sign of a zero, so the multiplicative identities need no such distinction. They are in turn
 * limited to numbers, because scaling an interval goes through {@code double}, which is lossy:
        ...
        implements IrOptimizerRule
{
    ...
    public Optional<Expression> apply(Expression expression, Session session, SymbolAllocator symbolAllocator, Map<Symbol, Expression> bindings)
    {
        ...
        if (name.equals(builtinFunctionName(ADD))) {
            if (isZero(right, -1) && left.type().equals(type)) {
                return Optional.of(left);
            }
            if (isZero(left, -1) && right.type().equals(type)) {
        ...
        else if (name.equals(builtinFunctionName(MULTIPLY)) && isNumericType(type)) {
        ...
        else if (name.equals(builtinFunctionName(DIVIDE)) && isNumericType(type)) {
            if (isOne(right) && left.type().equals(type)) {
                return Optional.of(left);
            }
        }
    ...
    /**
     * Whether the expression is a constant zero, with {@code floatingZeroExpectedSign} selecting which
     * of the two floating point zeros is the identity of the operation the constant is an operand of:
     * negative for addition, positive for subtraction.
     */
    private static boolean isZero(Expression expression, int floatingZeroExpectedSign)
    {
        ...
        return switch (type) {
            ...
            case NumberType _ -> ((TrinoNumber) value).toBigDecimal() instanceof BigDecimalValue(BigDecimal number) && number.compareTo(BigDecimal.ZERO) == 0;
            default -> false;
        };
    }
