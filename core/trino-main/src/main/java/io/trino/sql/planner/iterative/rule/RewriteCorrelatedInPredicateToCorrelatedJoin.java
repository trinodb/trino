...
/// - `countNullMatches` = count of rows where `x = b` is `NULL`, including comparisons
///         involving NULL fields or elements inside rows and arrays.
///
...
///                                           countNullMatches <- count() filter ((x = b) IS NULL)
        ...
        implements Rule<ApplyNode>
{
    ...
    public Result apply(ApplyNode apply, Captures captures, Context context)
    {
        ...
        Expression match = comparison(metadata, charVarcharCoercion, EQUAL, probe.toSymbolReference(), reference.toSymbolReference());
        ProjectNode conditions = new ProjectNode(
                context.getIdAllocator().getNextId(),
                apply.getSubquery(),
                Assignments.builder()
                        .put(matchCondition, match)
                        .put(nullMatchCondition, new IsNull(match))
                        .build());
        ...
        AggregationNode counts = singleAggregation(
                ...
                conditions,
                ImmutableMap.of(
                        countMatches, countWithFilter(charVarcharCoercion, matchCondition),
                        countNullMatches, countWithFilter(charVarcharCoercion, nullMatchCondition)),
        ...
        Expression inExpression = new Case(
                ImmutableList.of(
                        new WhenClause(comparison(metadata, charVarcharCoercion, GREATER_THAN, countMatches.toSymbolReference(), new Constant(BIGINT, 0L)), TRUE),
                        new WhenClause(comparison(metadata, charVarcharCoercion, GREATER_THAN, countNullMatches.toSymbolReference(), new Constant(BIGINT, 0L)), new Constant(BOOLEAN, null))),
                FALSE);
    ...
    private AggregationNode.Aggregation countWithFilter(CharVarcharCoercion charVarcharCoercion, Symbol filter)
    {
        return new AggregationNode.Aggregation(
                metadata.resolveBuiltinFunction(charVarcharCoercion, "count", ImmutableList.of()),
                ImmutableList.of(),
                false,
                Optional.of(filter),
                Optional.empty(),
