...
public class BloomColumnarFilter
{
    // Generate a BloomColumnarFilter class per Type to avoid mega-morphic call site when reading a position from input block
    private static final NonEvictableCache<Type, MethodHandle> COLUMNAR_BLOOM_FILTER_CACHE = buildNonEvictableCache(
            CacheBuilder.newBuilder()
                    .maximumSize(100)
                    .expireAfterWrite(2, TimeUnit.HOURS));

    private BloomColumnarFilter() {}
    ...
    private static Supplier<ColumnarFilter> createColumnarBloomFilter(Type type, int inputChannel, BloomFilterWithRange bloomFilterWithRange)
    {
        ...
        return () -> {
            try {
                ...
                return (ColumnarFilter) filterConstructor.invoke(bloomFilterWithRange, inputChannels);
            }
            catch (Throwable e) {
    ...
    private static MethodHandle generateColumnarBloomFilterClass(Type type)
    {
        ClassDefinition classDefinition = new ClassDefinition(
                a(PUBLIC, FINAL),
                makeClassName(ColumnarFilter.class.getSimpleName() + "_bitmap_filter_" + type, Optional.empty()),
                type(Object.class),
                type(ColumnarFilter.class));

        FieldDefinition filterField = classDefinition.declareField(a(PRIVATE, FINAL), "filter", BloomFilterWithRange.class);
        FieldDefinition inputChannelsField = classDefinition.declareField(a(PRIVATE, FINAL), "inputChannels", InputChannels.class);
        Parameter filterParameter = arg("filter", BloomFilterWithRange.class);
        ...
        return constructorMethodHandle(filterClass, BloomFilterWithRange.class, InputChannels.class);
    }
    ...
    {
        BytecodeExpression filter = scope.getThis().getField("filter", BloomFilterWithRange.class);
        // boolean result = this.filter.test(block, position)
        return new BytecodeBlock()
                .append(result.set(filter.invoke("test", boolean.class, block, position)));
    }
}
