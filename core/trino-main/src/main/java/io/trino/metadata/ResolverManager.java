...
public class ResolverManager
{
    ...
    private static final Resolver DEFAULT_RESOLVER = getResolver("DEFAULT_RESOLVER", getUpperCaseCanonicalizer());
    private static final Resolver WITH_RESOLVER = getResolver("WITH_RESOLVER", getUpperCaseCanonicalizer());
    private static final Resolver IDENTITY_RESOLVER = getResolver("PARTITION_RESOLVER", getIdentityCanonicalizer());
    ...
    public Resolver getResolver(Session session, String catalog)
    {
        if (!hasResolver(catalog)) {
            ...
            if (factory.isEmpty()) {
                System.out.println("ResolverManager.getResolver() No Factory for catalog: " + catalog);
            }
    ...
    {
        if (hasResolver(session) || scope.getResolver().isEmpty()) {
            return;
        }
        queries.put(session.getQueryId().id(), scope.getResolver().get().getCatalog());
    }

    public void setQueryResolver(Session session, String catalog)
    {
        if (hasResolver(catalog)) {
            queries.put(session.getQueryId().id(), catalog);
        }
    }
    ...
    public Resolver getWithResolver(Session session)
    {
        ...
        if (hasResolver(session)) {
            return getResolver(session);
        }
        return WITH_RESOLVER;
    }

    public boolean hasResolver(Session session)
    {
        return queries.containsKey(session.getQueryId().id());
    }

    public Resolver getResolver(Session session)
    {
        return getQueryResolver(session.getQueryId().id());
    }
    ...
    private Resolver getQueryResolver(String queryId)
    {
        ...
        // FIXME: If not query resolver has been set then default canonicalizer will be used
