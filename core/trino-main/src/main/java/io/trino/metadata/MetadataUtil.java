...
public final class MetadataUtil
{
    ...
    {
        return createQualifiedObjectName(session, metadata, getQualifiedObjectIdentifiers(session, node, name, metadata));
    }
    ...
    public static QualifiedObjectName createTargetQualifiedObjectName(QualifiedObjectName source, QualifiedName target, Resolver resolver)
    {
        ...
        List<Identifier> originalParts = target.getOriginalParts().reversed();
        String objectName = resolver.canonicalize(originalParts.getFirst());
        String schemaName = (originalParts.size() > 1) ? resolver.canonicalize(originalParts.get(1)) : source.schemaName();
        String catalogName = (originalParts.size() > 2) ? originalParts.get(2).getValue() : source.catalogName();
