...
final class TestTaskCountEstimator
{
    ...
    @Test
    void testCoordinatorCountsInANodeGroupWhenItCanBeScheduledOn()
    {
        // the node selectors never exclude the coordinator, so the estimate must count it too
        // CURRENT_NODE is the coordinator and declares no node group
        TestingInternalNodeManager nodeManager = TestingInternalNodeManager.createDefault(ETL_NODE_1, ADHOC_NODE);

        assertThat(new TaskCountEstimator(new NodeSchedulerConfig().setIncludeCoordinator(true), nodeManager)
                .estimateSourceDistributedTaskCount(sessionWithNodeGroup("etl")))
                .isEqualTo(2);
        assertThat(new TaskCountEstimator(new NodeSchedulerConfig().setIncludeCoordinator(false), nodeManager)
                .estimateSourceDistributedTaskCount(sessionWithNodeGroup("etl")))
                .isEqualTo(1);
    }
