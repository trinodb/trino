...
{
    public static <T> TrinoJsonResponseHandler<T> createTrinoJsonResponseHandler(JsonCodec<T> jsonCodec)
    {
        return new TrinoJsonResponseHandler<>(jsonCodec);
    }

    public static <T> TrinoJsonResponseHandler<T> createTrinoJsonResponseHandler(JsonCodec<T> jsonCodec, int firstSuccessfulResponseCode, int... otherSuccessfulResponseCodes)
    {
    ...
    private TrinoJsonResponseHandler(JsonCodec<T> jsonCodec)
    {
        this(jsonCodec, 200, 201, 202, 203, 204, 205, 206);
    }

    private TrinoJsonResponseHandler(JsonCodec<T> jsonCodec, int firstSuccessfulResponseCode, int... otherSuccessfulResponseCodes)
    {
    ...
    public T handle(Request request, Response response)
    {
        ...
        if (!isJsonUtf8Content(response)) {
            throw new UnexpectedResponseException("Expected application/json response from server but got " + response.getHeader(CONTENT_TYPE).orElse(null), request, response);
        }
