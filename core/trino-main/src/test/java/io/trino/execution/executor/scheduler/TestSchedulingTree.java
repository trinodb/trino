...
public class TestSchedulingTree
{
    ...
    public void testFinishNestedGroup(State state)
    {
        SchedulingNode<String> tree = new SchedulingNode<>();
        tree.startGroup(path("task"));
        tree.startGroup(path("task", "pipeline"));
        tree.startGroup(path("task", "pipeline", "group"));
        tree.enqueue(path("task", "pipeline", "group", "split"), 0);
        if (state != State.RUNNABLE) {
            assertThat(tree.dequeue(QUANTUM)).isEqualTo("split");
        }
        if (state == State.BLOCKED) {
            tree.block(path("task", "pipeline", "group", "split"), QUANTUM);
        }

        assertThat(tree.finishGroup(path("task", "pipeline", "group"))).containsExactly("split");
        assertThat(tree.containsGroup(path("task", "pipeline", "group"))).isFalse();
        assertThat(tree.state(path("task", "pipeline"))).isEqualTo(State.BLOCKED);
        assertThat(tree.state(path("task"))).isEqualTo(State.BLOCKED);
        assertThat(tree.getRunnableCount()).isZero();
        assertThat(tree.dequeue(QUANTUM)).isNull();

        tree.startGroup(path("task", "pipeline", "group"));
        tree.enqueue(path("task", "pipeline", "group", "replacement"), 0);
        assertThat(tree.dequeue(QUANTUM)).isEqualTo("replacement");
        assertThat(tree.dequeue(QUANTUM)).isNull();
    }

    @Test
    public void testFinishNestedGroupWithRunningSibling()
    {
        SchedulingNode<String> tree = new SchedulingNode<>();
        tree.startGroup(path("task"));
        tree.startGroup(path("task", "first"));
        tree.startGroup(path("task", "second"));
        tree.enqueue(path("task", "first", "a"), 0);
        assertThat(tree.dequeue(QUANTUM)).isEqualTo("a");
        tree.enqueue(path("task", "second", "b"), 0);

        assertThat(tree.finishGroup(path("task", "second"))).containsExactly("b");
        assertThat(tree.state(path("task"))).isEqualTo(State.RUNNING);
        assertThat(tree.dequeue(QUANTUM)).isNull();

        tree.enqueue(path("task", "first", "a"), QUANTUM);
        assertThat(tree.dequeue(QUANTUM)).isEqualTo("a");
    }
