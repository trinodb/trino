        ...
        extends BaseDataDefinitionTaskTest
{
    ...
    void testSetDefaultValue()
    {
        ...
        getFutureValue(executeSetDefaultValue(asQualifiedName(tableName), identifier("b"), "123", false));
        assertThat(metadata.getTableMetadata(testSession, table).columns())
                .containsExactly(column("a", null), column("b", "123"));

        getFutureValue(executeSetDefaultValue(asQualifiedName(tableName), identifier("b"), "456", false));
        assertThat(metadata.getTableMetadata(testSession, table).columns())
    ...
    void testSetDefaultValueNotExistingTable()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeSetDefaultValue(asQualifiedName(tableName), identifier("b"), "123", false)))
    ...
    void testSetDefaultValueNotExistingTableIfExists()
    {
        ...
        getFutureValue(executeSetDefaultValue(tableName, identifier("b"), "123", true));
        // no exception
    }
    ...
    void testSetDefaultValueMissingColumn()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeSetDefaultValue(asQualifiedName(tableName), identifier("missing_column"), "123", false)))
    ...
    void testSetDefaultValueOnView()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeSetDefaultValue(asQualifiedName(viewName), identifier("test"), "123", false)))
                .hasErrorCode(TABLE_NOT_FOUND)
    ...
    void testSetDefaultValueOnMaterializedView()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeSetDefaultValue(asQualifiedName(materializedViewName), identifier("test"), "123", false)))
                .hasErrorCode(TABLE_NOT_FOUND)
                .hasMessageContaining("Table '%s' does not exist, but a materialized view with that name exists.", materializedViewName);
    }

    private ListenableFuture<Void> executeSetDefaultValue(QualifiedName table, Identifier column, String defaultValue, boolean tableExists)
    {
        return new SetDefaultValueTask(plannerContext, new AllowAllAccessControl())
                .execute(new SetDefaultValue(
                                ...
                                table,
                                column,
                                new LongLiteral(new NodeLocation(1, 1), defaultValue),
                                tableExists),
                        queryStateMachine,
                        ImmutableList.of(),
