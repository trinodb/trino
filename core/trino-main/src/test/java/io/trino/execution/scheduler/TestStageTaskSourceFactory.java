...
public class TestStageTaskSourceFactory
{
    ...
    public void testArbitraryDistributionTaskSource()
    {
        ...
        Exchange exchange = splittingExchangeManager.create(new ExchangeContext(new QueryId("query"), 0), 3);
        ...
        assertEquals(tasks, ImmutableList.of(new TaskDescriptor(
                ...
                new NodeRequirements(Optional.empty(), Optional.empty()))));

        sources = ImmutableListMultimap.of(FRAGMENT_ID_1, new TestingExchangeSourceHandle(0, 123));
        exchange = nonSplittingExchangeManager.create(new ExchangeContext(new QueryId("query"), 0), 3);
        taskSource = new ArbitraryDistributionTaskSource(
                ImmutableMap.of(FRAGMENT_ID_1, PLAN_NODE_1),
                ImmutableMap.of(FRAGMENT_ID_1, exchange),
                sources,
        ...
        exchange = splittingExchangeManager.create(new ExchangeContext(new QueryId("query"), 0), 3);
        taskSource = new ArbitraryDistributionTaskSource(
                ImmutableMap.of(FRAGMENT_ID_1, PLAN_NODE_1, FRAGMENT_ID_2, PLAN_NODE_2),
                ImmutableMap.of(FRAGMENT_ID_1, exchange, FRAGMENT_ID_2, exchange),
                sources,
    ...
    public void testHashDistributionTaskSource()
    {
        ...
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(
                new TaskDescriptor(0, ImmutableListMultimap.of(), ImmutableListMultimap.of(
                        PLAN_NODE_1, new TestingExchangeSourceHandle(0, 1),
                        PLAN_NODE_2, new TestingExchangeSourceHandle(0, 1),
                        PLAN_NODE_3, new TestingExchangeSourceHandle(0, 1)), new NodeRequirements(Optional.of(CATALOG), Optional.empty())),
    ...
    public void testSourceDistributionTaskSource()
    {
        ...
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(new TaskDescriptor(
                ...
                ImmutableListMultimap.of(),
                new NodeRequirements(Optional.of(CATALOG), Optional.empty()))));
        assertTrue(taskSource.isFinished());
        ...
        taskSource = createSourceDistributionTaskSource(
                ...
                2);
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(
                new TaskDescriptor(0, ImmutableListMultimap.of(PLAN_NODE_1, split1, PLAN_NODE_1, split2), ImmutableListMultimap.of(), new NodeRequirements(Optional.of(CATALOG), Optional.empty()))));
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(
                new TaskDescriptor(1, ImmutableListMultimap.of(PLAN_NODE_1, split3), ImmutableListMultimap.of(), new NodeRequirements(Optional.of(CATALOG), Optional.empty()))));
        assertTrue(taskSource.isFinished());
        ...
        taskSource = createSourceDistributionTaskSource(
                ...
                2);
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(
                new TaskDescriptor(0, ImmutableListMultimap.of(PLAN_NODE_1, split1, PLAN_NODE_1, split2), replicatedSources, new NodeRequirements(Optional.of(CATALOG), Optional.empty()))));
        assertFalse(taskSource.isFinished());
        assertEquals(taskSource.getMoreTasks(), ImmutableList.of(
                new TaskDescriptor(1, ImmutableListMultimap.of(PLAN_NODE_1, split3), replicatedSources, new NodeRequirements(Optional.of(CATALOG), Optional.empty()))));
        assertTrue(taskSource.isFinished());
        ...
        assertEquals(tasks.get(0).getNodeRequirements().getAddresses(), Optional.of(ImmutableSet.of(HostAddress.fromString("host1:8080"))));
        ...
        tasks = taskSource.getMoreTasks();
        assertEquals(tasks.size(), 1);
        assertEquals(tasks.get(0).getNodeRequirements().getAddresses(), Optional.of(ImmutableSet.of(HostAddress.fromString("host2:8080"))));
        assertThat(tasks.get(0).getSplits().get(PLAN_NODE_1)).containsExactlyInAnyOrder(splits.get(1), splits.get(5));
        assertFalse(taskSource.isFinished());

        tasks = taskSource.getMoreTasks();
        assertEquals(tasks.size(), 1);
        assertEquals(tasks.get(0).getNodeRequirements().getAddresses(), Optional.of(ImmutableSet.of(HostAddress.fromString("host3:8080"))));
