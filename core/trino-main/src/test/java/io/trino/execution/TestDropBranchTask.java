        ...
        extends BaseDataDefinitionTaskTest
{
    ...
    void testDropExistingBranch()
    {
        ...
        metadata.createBranch(testSession, table, "dev");
        assertBranches(tableName, "main", "dev");

        getFutureValue(executeDropBranch(asQualifiedName(tableName), "dev"));
        assertBranches(tableName, "main");
    }

    @Test
    void testDropOnlyBranch()
    {
        QualifiedObjectName tableName = qualifiedObjectName("existing_table");
        metadata.createTable(testSession, TEST_CATALOG_NAME, someTable(tableName), FAIL);
        assertBranches(tableName, "main");

        assertTrinoExceptionThrownBy(() -> getFutureValue(executeDropBranch(asQualifiedName(tableName), "main")))
                .hasErrorCode(NOT_SUPPORTED)
                .hasMessage("line 1:1: Cannot drop the only branch in a table");
        assertBranches(tableName, "main");
    }
    ...
    void testDropNotExistingBranch()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeDropBranch(asQualifiedName(tableName), "not_found")))
    ...
    void testDropBranchOnView()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeDropBranch(asQualifiedName(viewName), "main")))
    ...
    void testDropBranchOnMaterializedView()
    {
        ...
        assertTrinoExceptionThrownBy(() -> getFutureValue(executeDropBranch(viewName, "main")))
                .hasErrorCode(GENERIC_USER_ERROR)
                .hasMessage("line 1:1: Dropping branch from materialized view is not supported");
    }

    private ListenableFuture<Void> executeDropBranch(QualifiedName tableName, String branchName)
    {
        return new DropBranchTask(metadata, new AllowAllAccessControl())
                .execute(new DropBranch(new NodeLocation(1, 1), tableName, new Identifier(branchName)), queryStateMachine, ImmutableList.of(), WarningCollector.NOOP);
    }
