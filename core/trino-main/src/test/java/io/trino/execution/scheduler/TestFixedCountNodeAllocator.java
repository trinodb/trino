...
public class TestFixedCountNodeAllocator
{
            ...
            throws Exception
    {
        ...
        try (NodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 1)) {
            ListenableFuture<InternalNode> acquire1 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            assertTrue(acquire1.isDone());
            assertEquals(acquire1.get(), NODE_1);

            ListenableFuture<InternalNode> acquire2 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
        ...
        try (NodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 2)) {
            ...
            ListenableFuture<InternalNode> acquire3 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            assertFalse(acquire3.isDone());

            ListenableFuture<InternalNode> acquire4 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            ...
            throws Exception
    {
        ...
        try (NodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 1)) {
            ...
            ListenableFuture<InternalNode> acquire5 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
        ...
        try (NodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 2)) {
            ...
            ListenableFuture<InternalNode> acquire6 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            ...
            ListenableFuture<InternalNode> acquire7 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            ...
            ListenableFuture<InternalNode> acquire8 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.empty()));
            ...
            throws Exception
    {
        ...
        try (NodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 1)) {
            ListenableFuture<InternalNode> catalog1acquire1 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_1), Optional.empty()));
            assertTrue(catalog1acquire1.isDone());
            assertEquals(catalog1acquire1.get(), NODE_1);

            ListenableFuture<InternalNode> catalog1acquire2 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_1), Optional.empty()));
            assertTrue(catalog1acquire2.isDone());
            assertEquals(catalog1acquire2.get(), NODE_3);

            ListenableFuture<InternalNode> catalog1acquire3 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_1), Optional.empty()));
            assertFalse(catalog1acquire3.isDone());

            ListenableFuture<InternalNode> catalog2acquire1 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_2), Optional.empty()));
            assertTrue(catalog2acquire1.isDone());
            assertEquals(catalog2acquire1.get(), NODE_2);

            ListenableFuture<InternalNode> catalog2acquire2 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_2), Optional.empty()));
            ...
            ListenableFuture<InternalNode> catalog1acquire4 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_1), Optional.empty()));
            assertFalse(catalog1acquire4.isDone());

            ListenableFuture<InternalNode> catalog2acquire4 = nodeAllocator.acquire(new NodeRequirements(Optional.of(CATALOG_2), Optional.empty()));
            ...
            throws Exception
    {
        ...
        try (FixedCountNodeAllocator nodeAllocator = createNodeAllocator(nodeSupplier, 1)) {
            ListenableFuture<InternalNode> acquire1 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.of(ImmutableSet.of(NODE_2_ADDRESS))));
            assertTrue(acquire1.isDone());
            assertEquals(acquire1.get(), NODE_2);

            ListenableFuture<InternalNode> acquire2 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.of(ImmutableSet.of(NODE_2_ADDRESS))));
            assertFalse(acquire2.isDone());
            ...
            ListenableFuture<InternalNode> acquire3 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.of(ImmutableSet.of(NODE_3_ADDRESS))));
            assertTrue(acquire3.isDone());
            ...
            ListenableFuture<InternalNode> acquire4 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.of(ImmutableSet.of(NODE_3_ADDRESS))));
            assertTrue(acquire4.isDone());
            assertEquals(acquire4.get(), NODE_3);

            ListenableFuture<InternalNode> acquire5 = nodeAllocator.acquire(new NodeRequirements(Optional.empty(), Optional.of(ImmutableSet.of(NODE_3_ADDRESS))));
            assertFalse(acquire5.isDone());
