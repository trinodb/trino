...
public class TestFairSchedulerConcurrency
{
    ...
    @Test
    @Timeout(30)
    public void testCancelledAdmissionSchedulesNextTask()
            throws Exception
    {
        ...
        ThreadFactory threadFactory = runnable -> {
            ...
            cancelledThread.compareAndSet(null, thread);
            return thread;
        };
        Ticker ticker = new Ticker()
        {
            ...
            public long read()
            {
                // The first task-thread tick records WAITING; the second records its admission
                // to RUNNING. Cancel after its slot is reserved but before it starts running.
                if (Thread.currentThread() == cancelledThread.get() && ticks.incrementAndGet() == 2) {
                    cancel.get().run();
                }
                return 0;
            }
        ...
        try (FairScheduler scheduler = new FairScheduler(1, threadFactory, ticker)) {
            scheduler.pause();
            Group cancelledGroup = scheduler.createGroup("cancelled");
            Group nextGroup = scheduler.createGroup("next");
            cancel.set(() -> scheduler.removeGroup(cancelledGroup));
            AtomicBoolean cancelledTaskRan = new AtomicBoolean();
            ListenableFuture<Void> cancelled = scheduler.submit(cancelledGroup, 1, _ -> cancelledTaskRan.set(true));
            assertEventually(() -> assertThat(scheduler.getTasks(cancelledGroup)).containsExactly(1));
            ListenableFuture<Void> next = scheduler.submit(nextGroup, 2, _ -> {});
            assertEventually(() -> assertThat(scheduler.getTasks(nextGroup)).containsExactly(2));

            scheduler.resume();
            cancelled.get(10, TimeUnit.SECONDS);
            next.get(10, TimeUnit.SECONDS);
            assertThat(cancelledTaskRan.get()).isFalse();
            assertThat(scheduler.getConcurrencyControlAvailableSlots()).isEqualTo(1);
        }
    }
