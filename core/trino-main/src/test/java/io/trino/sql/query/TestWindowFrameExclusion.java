...
public class TestWindowFrameExclusion
{
    ...
    @Test
    public void testExclusionWithSlidingFrameOverAggregate()
    {
        // A removable aggregate (sum) with BOTH a sliding outer frame and a moving hole exercises the
        // incremental path on every row: restore the previous hole, slide the outer frame, remove the new
        // hole -- rather than the O(frame) rebuild the non-removable array_agg path takes.
        assertThat(assertions.query("SELECT sum(a) OVER(ORDER BY a " +
                "ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING EXCLUDE CURRENT ROW) " +
                "FROM (VALUES 1, 2, 3, 4, 5) t(a)"))
                .matches("VALUES BIGINT '2', BIGINT '4', BIGINT '6', BIGINT '8', BIGINT '4'");

        // Same but EXCLUDE TIES over a peer group split across a sliding frame, so the hole removal carves
        // the kept current row out of the middle of the excluded range while the frame moves.
        assertThat(assertions.query("SELECT sum(a) OVER(ORDER BY a " +
                "RANGE BETWEEN 1 PRECEDING AND 1 FOLLOWING EXCLUDE TIES) " +
                "FROM (VALUES 1, 2, 2, 3) t(a)"))
                .matches("VALUES BIGINT '5', BIGINT '6', BIGINT '6', BIGINT '7'");
    }
    ...
    @Test
    public void testExcludeTiesOverValueFunctions()
    {
        String frame = "RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING EXCLUDE TIES";

        assertThat(assertions.query("SELECT first_value(a) OVER(ORDER BY a " + frame + ") FROM (VALUES 2, 2, 3) t(a)"))
                .matches("VALUES 2, 2, 2");

        assertThat(assertions.query("SELECT last_value(a) OVER(ORDER BY a " + frame + ") FROM (VALUES 1, 2, 2) t(a)"))
                .matches("VALUES 2, 2, 2");

        assertThat(assertions.query("SELECT nth_value(a, 2) OVER(ORDER BY a " + frame + ") FROM (VALUES 2, 2, 3) t(a)"))
                .matches("VALUES 3, 3, 2");
    }
