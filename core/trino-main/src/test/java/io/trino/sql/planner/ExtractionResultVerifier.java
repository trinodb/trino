...
final class ExtractionResultVerifier
{
    ...
    public void verify(Expression predicate, ExtractionResult result)
    {
        ...
        List<Map<String, Object>> generated = rows.rows(symbols, ImmutableList.of(predicate, result.remainingExpression()));
        ...
        List<Object> remainingValues = rows.evaluateAll(result.remainingExpression(), "the remaining expression", symbols, generated);
    ...
    {
        boolean passes = containsRow(result.tupleDomain(), bindings) && TRUE.equals(remainingValue);
