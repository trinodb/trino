...
import io.trino.sql.gen.columnar.BloomFilter;
...
public class TestDynamicPageFilter
{
    ...
    void testVarcharFilter()
    {
        ...
        filterEvaluator = createDynamicFilterEvaluator(
                TupleDomain.withColumnDomains(ImmutableMap.of(
                        columnA,
                        Domain.create(ValueSet.of(VARCHAR, utf8Slice("ab"), utf8Slice("cd")), true),
                        columnB,
                        Domain.create(ValueSet.of(VARCHAR, utf8Slice("de")), true))),
    ...
    void testVarbinaryFilter()
    {
        ...
        FilterEvaluator filterEvaluator = createDynamicFilterEvaluator(
                TupleDomain.withColumnDomains(ImmutableMap.of(
                        column,
                        Domain.create(ValueSet.of(VARBINARY, utf8Slice("ab")), true))),
    ...
    void testCharFilter()
    {
        ...
        FilterEvaluator filterEvaluator = createDynamicFilterEvaluator(
                TupleDomain.withColumnDomains(ImmutableMap.of(
                        column,
                        Domain.create(ValueSet.of(CharType.createCharType(2), utf8Slice("ab")), true))),
                ImmutableMap.of(column, 0));
        verifySelectedPositions(filterPage(page, filterEvaluator), new int[] {0, 2, 4});
    }
    ...
    {
        BloomFilter.SliceBloomFilter filter = new BloomFilter.SliceBloomFilter(
                ImmutableList.of(
                        utf8Slice("Igne"),
                        utf8Slice("natura"),
                        utf8Slice("renovitur"),
                        utf8Slice("integra.")),
                VARCHAR);
        assertThat(contains(filter, utf8Slice("Igne"))).isTrue();
        assertThat(contains(filter, utf8Slice("natura"))).isTrue();
        assertThat(contains(filter, utf8Slice("renovitur"))).isTrue();
        assertThat(contains(filter, utf8Slice("integra."))).isTrue();

        assertThat(contains(filter, utf8Slice("natur"))).isFalse();
        assertThat(contains(filter, utf8Slice("apple"))).isFalse();
        ...
        filter = new BloomFilter.SliceBloomFilter(filterValues, VARCHAR);
        int hits = 0;
        for (int i = 0; i < valuesCount; i++) {
            boolean contains = contains(filter, testValues.get(i));
            if (i % 9 == 0) {
                // No false negatives
    ...
    private static boolean contains(BloomFilter.SliceBloomFilter filter, Slice data)
    {
        return filter.contains(data, 0, data.length());
    }
