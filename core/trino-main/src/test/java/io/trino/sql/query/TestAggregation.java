            ...
            throws Exception
    {
        try (QueryAssertions tpchAssertions = new QueryAssertions()) {
            ...
            assertThat(tpchAssertions.query(session,
                    """
                    SELECT count(orderkey), sum(orderkey), avg(orderkey), round(variance(orderkey), 0), round(var_pop(orderkey), 0), round(geometric_mean(orderkey), 6), round(skewness(orderkey), 6), round(kurtosis(orderkey), 6)
