...
public class TestLocalProperties
{
    ...
    {
        assertThat(stripLeadingConstants(ImmutableList.of())).isEmpty();
        assertThat(extractLeadingConstants(ImmutableList.of())).isEmpty();

        List<LocalProperty<String>> input = ImmutableList.of(grouped("a"));
        assertThat(stripLeadingConstants(input)).containsExactly(grouped("a"));
        assertThat(extractLeadingConstants(input)).isEmpty();

        input = ImmutableList.of(constant("b"), grouped("a"));
        assertThat(stripLeadingConstants(input)).containsExactly(grouped("a"));
        assertThat(extractLeadingConstants(input)).containsExactly("b");

        input = ImmutableList.of(constant("a"), grouped("a"));
        assertThat(stripLeadingConstants(input)).containsExactly(grouped("a"));
        assertThat(extractLeadingConstants(input)).containsExactly("a");

        input = ImmutableList.of(grouped("a"), constant("b"));
        assertThat(stripLeadingConstants(input)).containsExactlyElementsOf(input);
        assertThat(extractLeadingConstants(input)).isEmpty();

        input = ImmutableList.of(constant("a"));
        assertThat(stripLeadingConstants(input)).isEmpty();
        assertThat(extractLeadingConstants(input)).containsExactly("a");

        input = ImmutableList.of(constant("a"), constant("b"));
        assertThat(stripLeadingConstants(input)).isEmpty();
        assertThat(extractLeadingConstants(input)).containsOnly("a", "b");
    }
    ...
    public void testTranslate()
    {
        ...
        assertThat(LocalProperties.translate(input, translateWithMap(map))).isEmpty();
        ...
        input = ImmutableList.of(grouped("a"));
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(grouped("a1"));
        ...
        input = ImmutableList.of(constant("a"), grouped("b"));
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(grouped("b1"));

        map = ImmutableMap.of("a", "a1", "b", "b1");
        input = ImmutableList.of(constant("a"), grouped("b"));
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(constant("a1"), grouped("b1"));

        map = ImmutableMap.of("a", "a1", "b", "b1");
        input = ImmutableList.of(grouped("a", "b"));
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(grouped("a1", "b1"));

        map = ImmutableMap.of("a", "a1", "c", "c1");
        input = ImmutableList.of(constant("a"), grouped("b"), grouped("c"));
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(constant("a1"));
        ...
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(grouped("a1"), grouped("c1"));
        ...
        assertThat(LocalProperties.translate(input, translateWithMap(map))).containsExactly(grouped("a1"), constant("b1"), grouped("c1"));
    }
    ...
    {
        assertThat(LocalProperties.match(actual, wanted)).containsExactlyElementsOf(Arrays.asList(match));
    }

    @SafeVarargs
    private static <T> void assertNormalize(List<LocalProperty<T>> localProperties, Optional<LocalProperty<T>>... normalized)
    {
        assertThat(LocalProperties.normalize(localProperties)).containsExactlyElementsOf(Arrays.asList(normalized));
    }

    @SafeVarargs
    private static <T> void assertNormalizeAndFlatten(List<LocalProperty<T>> localProperties, LocalProperty<T>... normalized)
    {
        assertThat(LocalProperties.normalizeAndPrune(localProperties)).containsExactlyElementsOf(Arrays.asList(normalized));
    }
