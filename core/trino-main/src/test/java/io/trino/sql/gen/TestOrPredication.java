...
public class TestOrPredication
{
    ...
    @Test
    void testTwoTermOrMatchesShortCircuitAcrossThreeValuedLogic()
    {
        ...
        List<Boolean> expected = asList(T, T, T, T, F, N, T, N, N);

        Expression or = new Logical(Logical.Operator.OR, List.of(new Reference(BOOLEAN, "a"), new Reference(BOOLEAN, "b")));
        ...
        Block shortCircuit = project(or, layout, page, false);
        Block predicated = project(or, layout, page, true);

        // predication must not change results, and must match the SQL three-valued OR truth table
        assertBlockEquals(BOOLEAN, predicated, shortCircuit);
        assertBlockEquals(BOOLEAN, predicated, createBooleansBlock(expected));
    }

    @Test
    void testThreeTermOrMatchesShortCircuit()
    {
        List<Boolean> a = asList(F, F, T, N, F, N);
        List<Boolean> b = asList(F, N, F, F, T, N);
        List<Boolean> c = asList(F, F, F, T, N, T);

        Expression or = new Logical(Logical.Operator.OR, List.of(new Reference(BOOLEAN, "a"), new Reference(BOOLEAN, "b"), new Reference(BOOLEAN, "c")));
        ...
        assertBlockEquals(BOOLEAN, project(or, layout, page, true), project(or, layout, page, false));
    }
