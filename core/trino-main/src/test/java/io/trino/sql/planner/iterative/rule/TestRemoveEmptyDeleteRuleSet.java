...
import static io.trino.sql.planner.iterative.rule.test.RuleTester.CATALOG_HANDLE;
    ...
    public void testDoesNotFire(Rule<?> rule)
    {
        tester().assertThat(rule)
                .on(p -> p.tableDelete(
                        ...
                        p.tableScan(
                                new TableHandle(CATALOG_HANDLE, new TpchTableHandle("sf1", "nation", 1.0), TpchTransactionHandle.INSTANCE),
                                ImmutableList.of(),
                                ImmutableMap.of()),
