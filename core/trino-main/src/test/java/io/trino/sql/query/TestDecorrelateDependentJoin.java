...
public class TestDecorrelateDependentJoin
{
    ...
    @Test
    public void testCorrelatedInWithNestedNulls()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, ROW(a, 1) IN (
                    SELECT IF(b = 5, 6, b), c FROM (VALUES (1, NULL), (2, NULL), (2, 1), (3, 2), (5, NULL)) u(b, c) WHERE b = a)
                FROM (VALUES 1, 2, 3, 4, 5) t(a)
                """))
                .matches("VALUES (1, CAST(NULL AS boolean)), (2, true), (3, false), (4, false), (5, false)");

        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, ARRAY[a, 1] NOT IN (
                    SELECT ARRAY[IF(b = 5, 6, b), c] FROM (VALUES (1, NULL), (2, NULL), (2, 1), (3, 2), (5, NULL)) u(b, c) WHERE b = a)
                FROM (VALUES 1, 2, 3, 4, 5) t(a)
                """))
                .matches("VALUES (1, CAST(NULL AS boolean)), (2, false), (3, true), (4, true), (5, true)");
    }

    @Test
    public void testScalarAggregateOverBareCorrelation()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT t.a, s.c, s.m, s.b
                FROM (VALUES (1, true), (2, false), (2, false), (3, true), (NULL, true)) t(a, p)
                CROSS JOIN LATERAL (
                    SELECT count(t.a), min(t.a), bool_or(t.p)
                    FROM (VALUES 1, 1, 2) u(b) WHERE u.b = t.a
                ) s(c, m, b)
                """))
                .matches("VALUES (1, BIGINT '2', 1, true), (2, BIGINT '1', 2, false), (2, BIGINT '1', 2, false), (3, BIGINT '0', NULL, NULL), (NULL, BIGINT '0', NULL, NULL)");
    }
    ...
    @Test
    public void testScalarAggregateArgumentOnEmptyGroup()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT sum(1 / IF(b IS NULL, a - a, 1)) FROM (VALUES 1, 2) u(b) WHERE b > a)
                FROM (VALUES 1, 1, 2) t(a)
                """))
                .matches("VALUES (1, BIGINT '1'), (1, BIGINT '1'), (2, CAST(NULL AS bigint))");
    }

    @Test
    public void testScalarAggregateFilterOnEmptyGroup()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT count(*) FILTER (WHERE 1 / IF(b IS NULL, a - a, 1) = 1)
                           FROM (VALUES 1, 2) u(b) WHERE b > a)
                FROM (VALUES 1, 2) t(a)
                """))
    ...
    @Test
    public void testScalarAggregateArgumentBelowDistinctOnEmptyGroup()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT count(*)
                           FROM (SELECT DISTINCT 1 / IF(b IS NULL, a - a, 1)
                                 FROM (VALUES 1, 2, 2) u(b) WHERE b > a))
                FROM (VALUES 1, 2) t(a)
                """))
                .matches("VALUES (1, BIGINT '1'), (2, BIGINT '0')");
        ...
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT count(DISTINCT 1 / IF(b IS NULL, a - a, 1))
                           FROM (VALUES 1, 2, 2) u(b) WHERE b > a)
                FROM (VALUES 1, 2) t(a)
                """))
                .matches("VALUES (1, BIGINT '1'), (2, BIGINT '0')");
    ...
    @Test
    public void testScalarAggregateArgumentAboveDistinctOnEmptyGroup()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT sum(1 / IF(b IS NULL, a - a, 1))
                           FROM (SELECT DISTINCT b FROM (VALUES 1, 2, 2) u(b) WHERE b > a))
                FROM (VALUES 1, 2) t(a)
                """))
                .matches("VALUES (1, BIGINT '1'), (2, CAST(NULL AS bigint))");
    }

    @Test
    public void testScalarAggregateArgumentFailureOnMatchedRow()
    {
        assertThat(assertions.query(frameworkSession(),
                """
                SELECT a, (SELECT sum(1 / IF(b IS NULL, a - a, 1))
                           FROM (VALUES 1, NULL) u(b) WHERE b IS NULL OR b > a)
                FROM (VALUES 1, 2) t(a)
                """))
    ...
    @Test
    public void testGroupedAggregationInequalityCorrelation()
    {
        // A grouped aggregation whose correlated predicate is an inequality on the grouping key
        // commutes above the aggregation (filtering before vs. after grouping coincides for a
        // group-key column), so it decorrelates clone-free: the aggregation runs correlation-free
        // below a plain LEFT join with the inequality in the ON. An outer row with no matching
        // group null-extends the aggregates to NULL — the correct value for a grouped aggregation
        // over an empty per-outer-row source (which yields no row, unlike a global aggregate).
        //   a=1: no x<1     → (1, NULL, NULL)
        //   a=9: x in {2,3,4} → (9,2,1),(9,3,1),(9,4,1)
        assertThat(assertions.query(frameworkSession(),
                "SELECT t.a, l.k, l.c FROM (VALUES 1, 9) t(a) LEFT JOIN LATERAL (" +
                        "  SELECT x AS k, count(*) c FROM (VALUES 2, 3, 4) v(x) WHERE v.x < t.a GROUP BY x) l(k, c) ON true"))
                .matches("VALUES (1, CAST(NULL AS INTEGER), CAST(NULL AS BIGINT)), (9, 2, BIGINT '1'), (9, 3, BIGINT '1'), (9, 4, BIGINT '1')");
        // count(DISTINCT): the inner dedup's keys include the grouping key, so the predicate
        // commutes above it too. a=9: x=2 has distinct y {7,8}=2, x=3 has {9}=1.
        assertThat(assertions.query(frameworkSession(),
                "SELECT t.a, l.k, l.c FROM (VALUES 1, 9) t(a) LEFT JOIN LATERAL (" +
                        "  SELECT x AS k, count(DISTINCT y) c FROM (VALUES (2,7),(2,7),(2,8),(3,9)) v(x,y) WHERE v.x < t.a GROUP BY x) l(k, c) ON true"))
                .matches("VALUES (1, CAST(NULL AS INTEGER), CAST(NULL AS BIGINT)), (9, 2, BIGINT '2'), (9, 3, BIGINT '1')");
    }

    @Test
    public void testNonDeterministicInputGroupedAggregation()
    {
        // The clone-free grouped form means a non-deterministic input no longer forces the cloning
        // magic-set (whose clone re-evaluates uuid() to diverging values, silently matching
        // nothing): this shape now rides the bypass and answers correctly, matching legacy. The
        // clone-decline guard it used to exercise is pinned by testNonDeterministicInputDeclines.
        ...
        assertThat(assertions.query(frameworkSession(), query)).matches("VALUES BIGINT '6'");
        assertThat(assertions.query(legacySession(), query)).matches("VALUES BIGINT '6'");
    }

    @Test
    public void testNonDeterministicInputDeclines()
    {
        // A HAVING (a filter ABOVE the grouped aggregation) is outside every clone-free grouped
        // form — those walks only accept filters below the aggregation — so this shape falls to the
        // magic-set, whose input clone would re-evaluate the non-deterministic uuid() to diverging
        // values. DependentJoinAlgebra.copyInput must decline that clone rather than silently
        // produce wrong results. Regression pin for the wrong-results-via-clone guard (the clone
        // determinism check), which no other test now exercises.
        String query = "SELECT count(s.m) FROM (SELECT CAST(uuid() AS varchar) u FROM (VALUES 1, 2, 3)) o " +
                "LEFT JOIN LATERAL (SELECT max(x) m FROM (VALUES 'a', 'b') v(x) WHERE length(x) <= length(o.u) GROUP BY x HAVING count(*) > 0) s ON true";
        assertThat(assertions.query(frameworkSession(), query))
                .failure().hasMessageContaining("Given correlated subquery is not supported");
    }

    @Test
    public void testGroupedAggregationChainStaysLinear()
    {
        // The clone-free grouped form keeps a CHAIN of such correlated subqueries linear in plan
        // size: each decorrelates to one LEFT join with no input clone, so N chained laterals give
        // N joins — not the 2^N the cloning magic-set would compound to. Regression guard for the
        // exponential-plan blowup (a chain that cloned would exceed the stage limit / OOM).
        StringBuilder query = new StringBuilder("SELECT t.a");
        for (int i = 0; i < 12; i++) {
            query.append(", l").append(i).append(".c");
        }
        query.append(" FROM (VALUES 1, 2, 3) t(a)");
        for (int i = 0; i < 12; i++) {
            query.append(" LEFT JOIN LATERAL (SELECT x AS k, count(*) c FROM (VALUES 1, 2, 3, 4) v").append(i)
                    .append("(x) WHERE v").append(i).append(".x < t.a GROUP BY x) l").append(i).append("(k, c) ON true");
        }
        // 12 chained correlated subqueries would be 2^13 ≈ 8000 stages if cloned; clone-free it
        // plans (and executes) without exceeding the stage limit.
        assertThat(assertions.query(frameworkSession(), query.toString())).succeeds();
    }
    ...
    @Test
    public void testNonDeterministicCorrelatedLimitDeclinesFastPath()
    {
        // End-to-end check that the fallback decorrelates an outer-only non-deterministic LIMIT
        // shape correctly (stabilized with `* 0` so the answer is assertable). The LIMIT
        // determinism gate itself — that the plain-join LIMIT form declines this shape — is pinned
        // by TestDecorrelateViaPlainJoinForms.tryPlainJoinLimitDeclinesNonDeterministicCondition,
        // since with `* 0` the guarded and unguarded plans return the same rows.
        assertThat(assertions.query(frameworkSession(),
                "SELECT t.a, l.b FROM (VALUES 5, 1) t(a) LEFT JOIN LATERAL (" +
                        "  SELECT u.b FROM (VALUES 10, 20) u(b) WHERE random() * 0 < t.a ORDER BY u.b LIMIT 1) l(b) ON true"))
                .matches("VALUES (5, 10), (1, 10)");
    }

    @Test
    public void testNonDeterministicCorrelatedGroupedAggregationDeclinesFastPath()
    {
        // End-to-end check that the fallback decorrelates a non-deterministic grouped-aggregation
        // shape correctly (stabilized with `* 0` so the answer is assertable). The grouped-form
        // determinism gate itself — that the clone-free grouped LEFT-join form declines this shape —
        // is pinned by
        // TestDecorrelateViaPlainJoinForms.tryGroupedAggregationLeftJoinDeclinesNonDeterministicCondition,
        // since with `* 0` the guarded and unguarded plans return the same rows.
        assertThat(assertions.query(frameworkSession(),
                "SELECT l.k, l.c FROM (VALUES 5) t(a) LEFT JOIN LATERAL (" +
                        "  SELECT v.x AS k, count(*) c FROM (VALUES 10, 20) v(x) WHERE v.x < t.a * 10 + random() * 0 GROUP BY v.x) l(k, c) ON true"))
                .matches("VALUES (10, BIGINT '1'), (20, BIGINT '1')");
    }
