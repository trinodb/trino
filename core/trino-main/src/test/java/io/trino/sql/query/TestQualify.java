...
final class TestQualify
{
    ...
    @Test
    void testQualifyWithAlias()
    {
        // QUALIFY can reference a window function alias defined in SELECT
        assertThat(assertions.query(
                """
                SELECT depname, empno, salary, rank() OVER (PARTITION BY depname ORDER BY salary DESC) AS rnk
                FROM (VALUES
                    ('develop',   1, 4200),
                    ('develop',   2, 6000),
                    ('develop',   3, 4500),
                    ...
                    ('sales',     7, 4800),
                    ('sales',     8, 4800)
                ) AS empsalary(depname, empno, salary)
                QUALIFY rnk = 1
                """))
                .matches(
                        """
                        VALUES
                            (CAST('develop'   AS VARCHAR(9)), 2, 6000, BIGINT '1'),
                            (CAST('personnel' AS VARCHAR(9)), 5, 3900, BIGINT '1'),
                            (CAST('sales'     AS VARCHAR(9)), 6, 5000, BIGINT '1')
                        """);
    }
