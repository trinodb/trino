...
public class TestRemoveRedundantArithmetic
{
     ...
     * {@code -0.0 + 0.0} and {@code -0.0 - -0.0} are both {@code 0.0}.
     */
    @Test
    void testFloatingPointZero()
    {
        Reference value = new Reference(DOUBLE, "x");
        assertThat(optimize(operation(ADD, value, new Constant(DOUBLE, -0.0))))
                .describedAs("double + -0.0")
                .isEqualTo(Optional.of(value));
        assertThat(optimize(operation(ADD, new Constant(DOUBLE, -0.0), value)))
                .describedAs("-0.0 + double")
                .isEqualTo(Optional.of(value));
        assertThat(optimize(operation(SUBTRACT, value, new Constant(DOUBLE, 0.0))))
                .describedAs("double - 0.0")
                .isEqualTo(Optional.of(value));

        assertThat(optimize(operation(ADD, value, new Constant(DOUBLE, 0.0))))
                .describedAs("-0.0 + 0.0 is 0.0")
                .isEmpty();
        assertThat(optimize(operation(ADD, new Constant(DOUBLE, 0.0), value)))
                .describedAs("0.0 + -0.0 is 0.0")
                .isEmpty();
        assertThat(optimize(operation(SUBTRACT, value, new Constant(DOUBLE, -0.0))))
                .describedAs("-0.0 - -0.0 is 0.0")
                .isEmpty();
        assertThat(optimize(operation(SUBTRACT, new Constant(DOUBLE, 0.0), value)))
                .describedAs("0.0 - double negates the value")
                .isEmpty();

        Reference realValue = new Reference(REAL, "x");
        assertThat(optimize(operation(ADD, realValue, new Constant(REAL, toReal(-0.0f)))))
                .describedAs("real + -0.0")
                .isEqualTo(Optional.of(realValue));
        assertThat(optimize(operation(SUBTRACT, realValue, new Constant(REAL, toReal(0)))))
                .describedAs("real - 0.0")
                .isEqualTo(Optional.of(realValue));
        assertThat(optimize(operation(ADD, realValue, new Constant(REAL, toReal(0)))))
                .describedAs("REAL '-0.0' + REAL '0.0' is REAL '0.0'")
                .isEmpty();
