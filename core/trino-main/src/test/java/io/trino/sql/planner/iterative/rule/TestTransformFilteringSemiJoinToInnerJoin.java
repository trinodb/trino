    ...
    public void testTransformSemiJoinToInnerJoin()
    {
        tester().assertThat(new TransformFilteringSemiJoinToInnerJoin())
                .on(p -> {
                    ...
                    return p.filter(
                            new Logical(AND, ImmutableList.of(new Reference(BOOLEAN, "a_in_b"), comparison(GREATER_THAN, new Reference(BIGINT, "a"), new Constant(BIGINT, 5L)))),
                            p.semiJoin(
                                    p.values(a),
                                    p.values(b),
                                    a,
                ...
                .matches(project(
                        ...
                        join(INNER, builder -> builder
                                .equiCriteria("a", "b")
                                .filter(comparison(GREATER_THAN, new Reference(BIGINT, "a"), new Constant(BIGINT, 5L)))
                                .left(values("a"))
                                .right(
                                        aggregation(
                                                singleGroupingSet("b"),
