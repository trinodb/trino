...
public class TestDecorrelateDependentJoinDifferential
{
    ...
    @ValueSource(strings = {
            ...
            // grouped aggregation with an INEQUALITY-correlated predicate on the grouping key — the
            // clone-free pull-predicate-above-the-group form (legacy lifts it the same way); the
            // unmatched outer row null-extends the aggregates to NULL.
            "SELECT t.a, l.k, l.c FROM (VALUES 1, 9) t(a) LEFT JOIN LATERAL (SELECT x AS k, count(*) c FROM (VALUES 2, 3, 4) v(x) WHERE v.x < t.a GROUP BY x) l(k, c) ON true",
            "SELECT t.u, l.k, l.m FROM (VALUES 'aa', 'bbb', 'c') t(u) LEFT JOIN LATERAL (SELECT x AS k, max(x) m FROM (VALUES 'a', 'bb', 'ccc') v(x) WHERE length(x) <= length(t.u) GROUP BY x) l(k, m) ON true",
            "SELECT t.a, l.k, l.c FROM (VALUES 1, 9) t(a) LEFT JOIN LATERAL (SELECT x AS k, count(DISTINCT y) c FROM (VALUES (2,7),(2,7),(2,8),(3,9)) v(x,y) WHERE v.x < t.a GROUP BY x) l(k, c) ON true",
            "SELECT t.a, l.k, l.c FROM (VALUES 3, 3, 5) t(a) LEFT JOIN LATERAL (SELECT x AS k, count(*) c FROM (VALUES 1, 2) v(x) WHERE v.x < t.a GROUP BY x) l(k, c) ON true",
    })
