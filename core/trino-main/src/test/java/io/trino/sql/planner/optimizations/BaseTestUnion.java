        ...
        extends BasePlanTest
{
    ...
    public void testSimpleUnion()
    {
        ...
        assertThat(remotes.size())
                .describedAs("There should be exactly one RemoteExchange")
                .isEqualTo(1);
    ...
    public void testUnionUnderTopN()
    {
        ...
        int numberOfpartialTopN = searchFrom(plan.getRoot())
                .where(planNode -> planNode instanceof TopNNode topNNode && topNNode.getStep() == TopNNode.Step.PARTIAL)
                .count();
    ...
    public void testUnionOverSingleNodeAggregationAndUnion()
    {
        ...
        assertThat(remotes.size())
                .describedAs("There should be exactly two RemoteExchanges")
                .isEqualTo(2);
    ...
    private void assertPlanIsFullyDistributed(Plan plan)
    {
        int numberOfGathers = searchFrom(plan.getRoot())
                .where(BaseTestUnion::isRemoteGatheringExchange)
                .findAll()
                .size();
        ...
        assertThat(searchFrom(plan.getRoot())
                .recurseOnlyWhen(BaseTestUnion::isNotRemoteGatheringExchange)
                .findAll()
                .stream()
                .noneMatch(this::shouldBeDistributed))
                .describedAs("There is a node that should be distributed between output and first REMOTE GATHER ExchangeNode")
                .isTrue();

        assertThat(numberOfGathers)
                .describedAs("Only a single REMOTE GATHER was expected")
                .isEqualTo(1);
    }
    ...
    private boolean shouldBeDistributed(PlanNode planNode)
    {
        ...
        if (planNode instanceof TopNNode topNNode) {
            return topNNode.getStep() == TopNNode.Step.PARTIAL;
        }
        return false;
    }
    ...
    private static void assertAtMostOneAggregationBetweenRemoteExchanges(Plan plan)
    {
        ...
        for (PlanNode fragment : fragments) {
            List<PlanNode> aggregations = searchFrom(fragment)
                    ...
                    .recurseOnlyWhen(BaseTestUnion::isNotRemoteExchange)
                    .findAll();

            assertThat(aggregations.size() > 1)
                    .describedAs("More than a single AggregationNode between remote exchanges")
                    .isFalse();
        }
    }
    ...
    {
        return planNode instanceof ExchangeNode exchangeNode && exchangeNode.getScope() == REMOTE;
    }
}
