        ...
        extends BasePlanTest
{
    ...
    public void testDynamicFilterNotAboveTableScan()
    {
        ...
        assertThatThrownBy(() -> validatePlan(root))
                .isInstanceOf(VerifyException.class)
                .hasMessageMatching("Dynamic filters \\[Descriptor\\{id=DF, input=ORDERS_OK, operator=EQUAL, nullAllowed=false, timeout=null\\}\\] present in filter predicate whose source is not a table scan.");
    }
    ...
    public void testUnsupportedDynamicFilterExpression()
    {
        PlanNode root = builder.output(ImmutableList.of(), ImmutableList.of(),
                builder.join(
                        INNER,
                        builder.filter(
                                createDynamicFilterExpression(metadata, new DynamicFilterId("DF"), BIGINT, new ArithmeticBinaryExpression(ADD, new SymbolReference("LINEITEM_OK"), new Constant(BIGINT, 1L))),
                                lineitemTableScanNode),
                        ordersTableScanNode,
                        ImmutableList.of(new JoinNode.EquiJoinClause(lineitemOrderKeySymbol, ordersOrderKeySymbol)),
