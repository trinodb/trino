    ...
    public void testDoesNotFireWithNonGroupedAggregation()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    return p.filter(
                            comparison(GREATER_THAN, new Reference(INTEGER, "count"), new Constant(INTEGER, 0L)),
    ...
    public void testDoesNotFireWithNoCountAggregation()
    {
        ...
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol avg = p.symbol("avg");
                    return p.filter(
                            comparison(GREATER_THAN, new Reference(INTEGER, "avg"), new Constant(INTEGER, 0L)),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(avg, PlanBuilder.aggregation("avg", ImmutableList.of(new Reference(BIGINT, "g"))), ImmutableList.of(BIGINT), mask)
                                    .source(p.values(g, mask))));
    ...
    public void testFilterPredicateFalse()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            new Logical(AND, ImmutableList.of(comparison(LESS_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)), comparison(GREATER_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)))),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
    ...
    public void testDoesNotFireWhenFilterPredicateSatisfiedByAllCountValues()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            new Logical(AND, ImmutableList.of(new Logical(OR, ImmutableList.of(comparison(LESS_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)), comparison(GREATER_THAN_OR_EQUAL, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)))), comparison(EQUAL, new Reference(BIGINT, "g"), new Constant(BIGINT, 5L)))),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
    ...
    public void testPushDownMaskAndRemoveFilter()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            comparison(GREATER_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
    ...
    public void testPushDownMaskAndSimplifyFilter()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            new Logical(AND, ImmutableList.of(comparison(GREATER_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)), comparison(GREATER_THAN, new Reference(BIGINT, "g"), new Constant(BIGINT, 5L)))),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
        ...
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            new Logical(AND, ImmutableList.of(comparison(GREATER_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 0L)), comparison(EQUAL, new Call(MODULO_BIGINT, ImmutableList.of(new Reference(BIGINT, "count"), new Constant(BIGINT, 2L))), new Constant(BIGINT, 0L)))),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
                ...
                .matches(
                        filter(
                                comparison(EQUAL, new Call(MODULO_BIGINT, ImmutableList.of(new Reference(BIGINT, "count"), new Constant(BIGINT, 2L))), new Constant(BIGINT, 0L)),
                                aggregation(
                                        ImmutableMap.of("count", aggregationFunction("count", ImmutableList.of())),
                                        filter(
                                                new Reference(BOOLEAN, "mask"),
    ...
    public void testPushDownMaskAndRetainFilter()
    {
        tester().assertThat(new PushFilterThroughCountAggregationWithoutProject(tester().getPlannerContext()))
                .on(p -> {
                    ...
                    Symbol count = p.symbol("count");
                    return p.filter(
                            comparison(GREATER_THAN, new Reference(BIGINT, "count"), new Constant(BIGINT, 5L)),
                            p.aggregation(builder -> builder
                                    .singleGroupingSet(g)
                                    .addAggregation(count, PlanBuilder.aggregation("count", ImmutableList.of()), ImmutableList.of(), mask)
                                    .source(p.values(g, mask))));
