        ...
        extends BasePlanTest
{
                ...
                MockConnectorFactory.builder()
                        ...
                        .withApplyFilter((session, tableHandle, constraint) -> {
                            ...
                            if (constraint.getSummary().equals(CONSTRAINT_1)) {
                                return Optional.of(new ConstraintApplicationResult<>(new MockConnectorTableHandle(TABLE_NAME, CONSTRAINT_1, Optional.of(ImmutableList.of(HANDLE_1))), TupleDomain.<ColumnHandle>all(), io.trino.spi.expression.Constant.TRUE, false));
                            }
                            // predicate is rejected
                            else if (constraint.getSummary().equals(CONSTRAINT_2)) {
                                return Optional.of(new ConstraintApplicationResult<>(new MockConnectorTableHandle(TABLE_NAME, TupleDomain.all(), Optional.empty()), CONSTRAINT_2, io.trino.spi.expression.Constant.TRUE, false));
                            }
                            // predicate is subsumed opportunistically
                            else if (constraint.getSummary().equals(CONSTRAINT_3)) {
                                return Optional.of(new ConstraintApplicationResult<>(new MockConnectorTableHandle(TABLE_NAME, CONSTRAINT_3, Optional.empty()), CONSTRAINT_3, io.trino.spi.expression.Constant.TRUE, false));
                            }
    ...
    public void testExtractCommonSubqueries()
    {
        ...
        assertThat(subqueryA.getCommonDynamicFilterDisjuncts()).isEqualTo(Booleans.TRUE);
        assertThat(subqueryB.getCommonDynamicFilterDisjuncts()).isEqualTo(Booleans.TRUE);
    ...
    public void testCommonPredicateWasPushedDownAndDynamicFilter()
    {
        ...
        PlanMatchPattern commonSubplanB = filter(Booleans.TRUE, createDynamicFilterExpression(
                        getPlanTester().getPlannerContext().getMetadata(),
                        new DynamicFilterId("subquery_b_dynamic_id"),
                        BIGINT,
    ...
    private CommonSubqueries extractTpchCommonSubqueries(@Language("SQL") String query, boolean cacheSubqueries, boolean cacheAggregations, boolean cacheProjections, boolean forceSingleNode)
    {
        ...
        return planTester.inTransaction(tpchSession, session -> {
            Plan plan = planTester.createPlan(session, query, planTester.getPlanOptimizers(forceSingleNode), OPTIMIZED_AND_VALIDATED, WarningCollector.NOOP, createPlanOptimizersStatsCollector());
