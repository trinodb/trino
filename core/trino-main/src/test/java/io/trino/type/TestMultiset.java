...
public class TestMultiset
{
    ...
    @Test
    public void testMemberAsCaseFragment()
    {
        assertThat(assertions.expression("CASE 3 WHEN MEMBER OF MULTISET[1, 2] THEN 'a' WHEN MEMBER OF MULTISET[3, 4] THEN 'b' END"))
                .isEqualTo("b");
        // the case operand is evaluated once and shared by every clause, so it must reconcile to a
        // single type with the element type of every clause's multiset, not just the last one
        assertThat(assertions.expression("CASE 1 WHEN MEMBER OF MULTISET[2] THEN 'a' WHEN MEMBER OF MULTISET[1.0, 1.5] THEN 'b' END"))
                .isEqualTo("b");
        // an equality clause widening the operand also widens the multisets it is probed against
        assertThat(assertions.expression("CASE 1 WHEN = 1.5 THEN 'a' WHEN MEMBER OF MULTISET[1, 2] THEN 'b' END"))
                .isEqualTo("b");
    }
