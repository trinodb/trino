...
public class TestScanFilterAndProjectOperator
{
        ...
        for (int i = 1; i <= totalRows * totalColumns; i++) {
            ...
            if (i == totalColumns) {
                ...
                for (int j = 0; j < totalColumns; j++) {
                    assertThat(toValues(BIGINT, page.getBlock(j))).containsExactlyElementsOf(toValues(BIGINT, input.getBlock(0)));
                }
            }
            else {
    ...
    public void testRecordCursorYield()
    {
        ...
        assertThat(toValues(BIGINT, output.getBlock(0))).containsExactlyElementsOf(toValues(BIGINT, input.getBlock(0)));
    }
    ...
    private static List<Page> toPages(Operator operator)
    {
        ...
        while (!operator.isFinished()) {
            ...
            if (outputPage == null) {
                ...
                assertThat(nullPages < 1_000_000).withFailMessage("Too many null pages; infinite loop?").isTrue();
                nullPages++;
            }
            else {
