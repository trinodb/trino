...
public class TestRowFunctions
{
    ...
    {
        assertThat(assertions.expression("data.fields()")
        ...
        assertThat(assertions.expression("element_at(data, 1).fields()")
                .binding("data", "CAST(ARRAY[] AS ARRAY(ROW(greeting VARCHAR, planet VARCHAR)))"))
                .hasType(new ArrayType(VARCHAR))
                .isEqualTo(ImmutableList.of("greeting", "planet"));

        assertThat(assertions.expression("CAST(NULL AS ROW(greeting VARCHAR, planet ROW(color varchar, size bigint))).fields()"))
                .hasType(new ArrayType(VARCHAR))
                .isEqualTo(ImmutableList.of("greeting", "planet"));
    }
}
