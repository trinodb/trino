        ...
        extends AbstractTestBlockBuilder<Variant>
{
    ...
    public void testAppendPositionsUpdatesStatusWithOffsetSource()
    {
        ...
        // appendPositions reads only the three entries starting at offset 1, so the -1 padding is never accessed.
    ...
    {
        VariantBlockBuilder blockBuilder = new VariantBlockBuilder(null, 4);
        blockBuilder.writeEntry(Variant.ofLong(10));
        blockBuilder.writeEntry(Variant.ofLong(20));
        blockBuilder.writeEntry(Variant.ofLong(30));
        blockBuilder.writeEntry(Variant.ofLong(40));

        VariantBlock sliced = blockBuilder.buildValueBlock().getRegion(1, 2);
        VariantBlock appended = sliced.copyWithAppendedNull();

        assertThat(appended.getPositionCount()).isEqualTo(3);
        assertThat(appended.getVariant(0)).isEqualTo(Variant.ofLong(20));
        assertThat(appended.getVariant(1)).isEqualTo(Variant.ofLong(30));
