...
import static io.trino.spi.type.Preconditions.checkState;
...
public class TrinoNumber
{
    ...
    public AsBigDecimal toBigDecimal()
    {
        ...
        if (unscaledBytesLength == 0) {
            return switch (scale) {
                case 0 -> {
                    checkState(!negated, "Invalid negated header with zero byte unscaled value: header=%s, scale=%s", header, scale);
                ...
                default -> throw new IllegalStateException("Invalid negated header with zero byte unscaled value: header=%s, scale=%s".formatted(header, scale));
            };
        }
            ...
            permits NotANumber, Infinity, BigDecimalValue
    {
         ...
         * </ol>
         */
        default int compareToNaNLast(AsBigDecimal other)
        {
            switch (this) {
                case NotANumber() -> {
                    // NaN goes last
                    return other instanceof NotANumber ? 0 : 1;
                }
