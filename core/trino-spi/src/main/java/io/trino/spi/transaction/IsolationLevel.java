...
public enum IsolationLevel
{
    ...
    public static void checkConnectorSupports(IsolationLevel supportedLevel, IsolationLevel requestedLevel)
    {
        if (!supportedLevel.meetsRequirementOf(requestedLevel)) {
            throw new TrinoException(UNSUPPORTED_ISOLATION_LEVEL, "Connector supported isolation level %s does not meet requested isolation level %s".formatted(supportedLevel, requestedLevel));
        }
    }
}
