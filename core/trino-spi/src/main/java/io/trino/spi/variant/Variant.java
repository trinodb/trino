...
import io.trino.spi.variant.VariantDecoder.ObjectLayout;
...
public record Variant(Slice data, Metadata metadata, BasicType basicType, PrimitiveType primitiveType)
{
    ...
    public BigDecimal getDecimal()
    {
        ...
        return switch (primitiveType) {
            ...
            default -> throw new VerifyException("Expected DECIMAL primitive but got " + requireNonNullElse(primitiveType, basicType));
        };
    }
    ...
    public Optional<Variant> getObjectField(int fieldId)
    {
        ...
        // This intentionally avoids decoding the full ObjectLayout. Most production lookups come
        // from dereference and read a single field from a short-lived Variant instance, so paying
        // the full object traversal setup cost up front would be counterproductive here.
        byte header = data.getByte(0);
        ...
        for (int i = 0; i < count; i++) {
            int currentFieldId = readOffset(data, idsStart + i * idSize, idSize);
            if (currentFieldId == fieldId) {
                int startOffset = readOffset(data, offsetsStart + i * offsetSize, offsetSize);
                int start = valuesStart + startOffset;
                return Optional.of(from(metadata, data.slice(start, VariantDecoder.valueSize(data, start))));
            }
        }
    ...
    public Optional<Variant> getObjectField(Slice fieldName)
    {
        ...
        int startOffset = readOffset(data, offsetsStart + fieldIndex * offsetSize, offsetSize);
        int start = valuesStart + startOffset;
        return Optional.of(from(metadata, data.slice(start, VariantDecoder.valueSize(data, start))));
    }
    ...
    {
        verifyType(OBJECT);
        ObjectLayout layout = (ObjectLayout) VariantDecoder.decode(data, 0);

        return IntStream.range(0, layout.count())
                .mapToObj(i -> {
                    int start = layout.valueStart(i);
                    int end = layout.valueEnd(i);
                    return from(metadata, data.slice(start, end - start));
                });
    }
