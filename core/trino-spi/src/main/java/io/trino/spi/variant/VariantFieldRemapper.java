            ...
            case VariantDecoder.ArrayLayout array -> {
                ...
                int written = switch (remapMode) {
                    ...
                    case RESIZE -> VariantEncoder.encodeArrayHeading(
                            ...
                            output,
                            outputOffset);
                    case IDENTITY, NONE -> throw new VerifyException("unexpected remap mode " + remapMode);
                };
                ...
                int expectedSize = (remapMode == RemapMode.SAME_SIZE) ? length : containerSizeCache.get(inputOffset);
                verify(written == expectedSize, "unexpected size mismatch in complete remap");
                yield written;
            }
            case VariantDecoder.ObjectLayout object -> {
