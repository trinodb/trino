...
public class BitArrayBlockBuilder
        implements BlockBuilder
{
    private static final int INSTANCE_SIZE = instanceSize(BitArrayBlockBuilder.class);
    private static final Block NULL_VALUE_BLOCK = new BitArrayBlock(0, 1, new long[] {0}, new long[1]);
    ...
    public BitArrayBlockBuilder(@Nullable BlockBuilderStatus blockBuilderStatus, int expectedEntries)
    {
    ...
    public BlockBuilder writeBoolean(boolean value)
    {
        ensureCapacity(positionCount + 1);

        if (value) {
            set(values, 0, positionCount);
        }
        else {
            clear(values, 0, positionCount);
        }
        ...
        if (blockBuilderStatus != null) {
            blockBuilderStatus.addBytes(BitArrayBlock.SIZE_IN_BYTES_PER_POSITION);
        }
        return this;
    }
    ...
    public void append(ValueBlock block, int position)
    {
        ...
        if (bitArrayBlock.isNull(position)) {
            initializeValidityForFirstNull();
            clear(valueIsValid, 0, positionCount);
            hasNullValue = true;
        }
        else {
            if (bitArrayBlock.getBoolean(position)) {
                set(values, 0, positionCount);
            }
            else {
                clear(values, 0, positionCount);
    ...
    public void appendRepeated(ValueBlock block, int position, int count)
    {
        ...
        if (bitArrayBlock.isNull(position)) {
            ...
            if (bitArrayBlock.getBoolean(position)) {
                setBits(values, 0, positionCount, count);
            }
            else {
                clearBits(values, 0, positionCount, count);
            }
            if (valueIsValid != null) {
        ...
        if (blockBuilderStatus != null) {
            blockBuilderStatus.addBytes(count * BitArrayBlock.SIZE_IN_BYTES_PER_POSITION);
        }
    }
    ...
    public void appendRange(ValueBlock block, int offset, int length)
    {
        ...
        copyBits(bitArrayBlock.getRawValues(), rawOffset + offset, values, positionCount, length);

        long[] rawValueIsValid = bitArrayBlock.getRawValueIsValid();
        ...
        if (blockBuilderStatus != null) {
            blockBuilderStatus.addBytes(length * BitArrayBlock.SIZE_IN_BYTES_PER_POSITION);
        }
    }
    ...
    public void appendPositions(ValueBlock block, int[] positions, int offset, int length)
    {
        ...
        BitArrayBlock bitArrayBlock = (BitArrayBlock) block;
        int rawOffset = bitArrayBlock.getRawValuesOffset();
        copyBits(bitArrayBlock.getRawValues(), rawOffset, positions, offset, values, positionCount, length);
    ...
    @Override
    public BitArrayBlock buildValueBlock()
    {
        return new BitArrayBlock(0, positionCount, hasNullValue ? valueIsValid : null, values);
    }

    @Override
    public BlockBuilder newBlockBuilderLike(int expectedEntries, BlockBuilderStatus blockBuilderStatus)
    {
        return new BitArrayBlockBuilder(blockBuilderStatus, expectedEntries);
    }

    private void ensureCapacity(int capacity)
    {
        if ((long) values.length * Long.SIZE >= capacity) {
            return;
        }
        ...
        values = Bitmap.ensureCapacity(values, newSize);
        updateRetainedSize();
    }
    ...
    private boolean initializeValidityForFirstNull()
    {
        ...
        valueIsValid = Bitmap.allocateWords(values.length * Long.SIZE, false);
    ...
    {
        return BitArrayBlock.SIZE_IN_BYTES_PER_POSITION * (long) positionCount;
    }
    ...
    {
        StringBuilder sb = new StringBuilder("BitArrayBlockBuilder{");
