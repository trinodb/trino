...
/**
 * Stack representation for {@link BigdecimalType}. See there for binary format specification.
 */
public class Bigdecimal
{
    ...
    public Bigdecimal(Slice bytes)
    {
        switch (bytes.length()) {
            case 0, 1 -> throw new IllegalArgumentException("Invalid bytes length for Bigdecimal: " + bytes.length());
            case 2 -> {
                // zero
                // TODO support ±infinity and NaN
                short header = bytes.getShort(0);
                checkArgument(!negatedFromHeader(header) && scaleFromHeader(header) == 0, "Invalid header for Bigdecimal with unscaled zero value: %s", header);
            }
            default -> {
                if (bytes.getByte(2) == 0) {
                    throw new IllegalArgumentException("Bigdecimal has leading zero byte in unscaled value: " +
                            HexFormat.of().withDelimiter(" ").formatHex(bytes.byteArray(), bytes.byteArrayOffset(), bytes.length()));
                }
                checkArgument(bytes.getByte(2) != 0, "Bigdecimal has leading zero byte in unscaled value: %s", bytes);
            }
        }
        this.bytes = bytes;
    }
    ...
    public static Bigdecimal from(BigDecimal bigDecimal)
    {
        ...
        if (negated) {
            // TODO optimize me, avoid this step
