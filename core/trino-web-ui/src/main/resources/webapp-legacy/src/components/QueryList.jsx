...
class DisplayedQueriesList extends React.Component {
    render() {
        const queryNodes = this.props.queries.map(
            function (query) {
                return <QueryListItem key={query.queryId} query={query} />
            }.bind(this)
        )
        return <div>{queryNodes}</div>
    }
}
...
export class QueryList extends React.Component {
    ...
    refreshLoop() {
        ...
        $.get(
            ...
            function (queryList) {
                ...
                newQueries = this.filterQueries(
                    newQueries,
                    this.state.stateFilters,
                    this.state.errorTypeFilters,
                ...
                if (this.state.reorderInterval !== 0 && lastRefresh - lastReorder >= this.state.reorderInterval) {
                    updatedQueries = this.filterQueries(
                        updatedQueries,
                        this.state.stateFilters,
                        this.state.errorTypeFilters,
                        this.state.searchString
    ...
    executeSearch() {
        ...
        const newDisplayedQueries = this.filterQueries(
            this.state.allQueries,
            this.state.stateFilters,
            this.state.errorTypeFilters,
            this.state.searchString
        )
        this.sortAndLimitQueries(
            newDisplayedQueries,
            this.state.currentSortType,
            this.state.currentSortOrder,
            this.state.maxQueries
        )
    ...
    renderMaxQueriesListItem(maxQueries, maxQueriesText) {
        return (
            <li>
                <a
                    href="#"
                    className={this.state.maxQueries === maxQueries ? 'selected' : ''}
    ...
    renderReorderListItem(interval, intervalText) {
        return (
            <li>
                <a
                    href="#"
                    className={this.state.reorderInterval === interval ? 'selected' : ''}
    ...
    renderSortListItem(sortType, sortText) {
        if (this.state.currentSortType === sortType) {
            const directionArrow =
                this.state.currentSortOrder === SORT_ORDER.ASCENDING ? (
