        ...
        implements ConnectorMetadata
{
    ...
    public Map<String, Object> getTableProperties(ConnectorSession session, JdbcTableHandle handle)
    {
        ...
        try (PhoenixConnection connection = phoenixClient.getConnection(JdbcIdentity.from(session));
            ...
            PTable table = getTable(connection, SchemaUtil.getTableName(
                    toPhoenixSchemaName(Optional.ofNullable(handle.getSchemaName())).orElse(null),
                    handle.getTableName()));

            boolean isSalted = table.getBucketNum() != null;
            StringJoiner joiner = new StringJoiner(",");
            List<PColumn> pkColumns = table.getPKColumns();
            for (PColumn pkColumn : pkColumns.subList(isSalted ? 1 : 0, pkColumns.size())) {
